...
It rested on `accept_vs_edit`, which reads `edited` on 35 of 36 rows **as measured that day**
(the corpus is **37** as of 2026-09-03 — LAWNS unchanged at 17, ledger #263); measured
...
`header_amount_edited` is **false on 36 of 36** (that day's population). So the flag measures our
...
ARC: ocr-doc-routing
MAPS-TO: 2.3, 3.4, 3.5
PIECES: invoice_line_read, line_role_classification, history_order_third_door, order_delivery_join, availability_fingerprint, prior_order_guard
...
🔴 **And the second hazard is the mirror of the first: the idempotency key is blind to the orders that were already here.** LAWNS holds **nine history orders transcribed from photographs**, and nothing in that path ever held an Intuit id — so `ON CONFLICT (business_id, qb_invoice_id)` cannot see a single one of them, and a pass keyed only on that column creates **a second order for every sale already captured**: a duplicate in the seller's own revenue, silent and permanent. **This is the Thiry shape at nine times the size** — the delivery ingest hit the identical defect one day earlier with a hand-entered stop — and it is **not theoretical: seven of the eighteen future-dated invoices carry a `TxnDate` of 26 or 27 August, the exact window those nine were captured in, and six of them share one date.** So the guard's primary key is **their own document number** (`orders.source_document_number` holds the same string Intuit returns as `DocNumber` — an identity, not an inference), with customer, date and amount as **corroboration** rather than as the key. That ordering is [[D-47]]'s three-way rule, and its scar is #53: matching on one field the external system permits to collide cross-billed nine real invoices. 🔴 **Only an identity writes anything, and all it writes is the id.** A contradiction, an unanchored match, or more than one candidate all **stop and report** — on the panel above the refusals, because *"do not reconcile silently"* is a requirement about the screen and not about a log.

**WHAT THIS UNLOCKS, and none of it is this story:** the **day sheet's load** (what is actually on the truck), the **material list** (mix, mulch and staking are all computed from CONTAINER SIZE, which lives in the line description this now stores) and the **capacity flag** (one minute per gallon needs gallons, and gallons come from the line). All three were blocked without it; each is its own build. _Grounded: `packages/shared/src/quickbooks/invoiceOrderLines.ts` (+45 probes) · `historyOrderWriter.ts` (+115, incl. the prior-order guard) · `supabase/migrations/20260831c_orders_qb_invoice_uidx.sql` · `docs/owner-tests/qb-order-ingest-full-surface-test.md` · measured against the 2026-08-29 raw invoice capture (1,469 invoices, complete)._

### I captured it — show me it landed, and show me what it became
STATUS: written
SCOPE: vertical:cultivar, platform
BUILD: in-build
ARC: ocr-doc-routing
MAPS-TO: —
PIECES: receipts_read_surface, banked_verdict_readout, capture_outcome_chain, honest_count
NEEDS: ⚠️ **MAPS-TO is `—` because the Receipt Keeper surface has NO id on the 24-capability board** (3.1 is Leakage, 4.2 is `/inventory/reconcile`) — checked this session; the capture half has shipped since 2026-06-11 and has never had a row. **David to mint one or to say it rides another.** 🔴 **And two questions are OPEN and are David's, not defaults to pick in code:** why `accept_vs_edit` reads `edited` on 17 of 17 rows (narrowed this session — `header_amount_edited` is false on all 17, so whatever was changed, it was never the total), and why 6 of 8 captures on 1 September wrote a receipt and no order.
_Written 2026-09-01 alongside the build (§9 story-reconciliation gate: no matching story existed — the nearest, "Snap a document, let TRACE route it", is about ROUTING and is `needs-input`). This is the READ half of a surface whose write half has shipped for eleven weeks._

**Lauren photographs a receipt, presses Save, and the page thanks her — and that is the last she ever sees of it.** `/receipts` could capture and could not show. On 1 September eight documents were captured in one afternoon; **two produced orders and six produced nothing**, and there was no screen on which that difference existed. Seventeen receipts had accumulated at LAWNS, including **two captured twice** — bwi 2026-07-29 $1,283.88 and Bailey Bark 2026-07-07 $2,316.03 — and nobody could have known, because a receipt that landed and a receipt that vanished looked identical: both looked like a thank-you message.

**Three things a row must say, and the third is why this is worth building.** ① **The receipt** — vendor, date, amount, category, as stored. ② **The verdict the platform BANKED at save time** — `reconcile_status`, `reconcile_delta`, `reconcile_overridden_at`, `accept_vs_edit`, `amount_original`, `header_amount_edited`, six columns **write-only since 14 June 2026** and read back here for the first time. ③ **What it became** — `receipt → order → delivery`, and where nothing exists, that nothing exists. **Every problem in the paragraph above is invisible without ③.**

🔴 **THE SCREEN DOES NOT ADJUDICATE, AND THAT IS THE REQUIREMENT RATHER THAN A LIMITATION.** Six of the seventeen produced no order. They read as vendor PURCHASE invoices, which correctly should not become customer orders — **but that is a reading, not an established fact.** So the row says *"No order recorded for this receipt."* and stops: not *missing*, not *orphaned*, not *an error*, and not a blank a reader has to interpret. **Surface, don't decide.** Same for the duplicates: both rows appear, at their own capture times, and the screen never calls them duplicates. **And no document type is derived** — measured live, `receipts` has 21 columns and none of `origin`/`shape`/`source`/`doc_type`/`document_type`/`kind`, so LAWNS's own nine sales invoices sit beside eight supplier invoices with nothing distinguishing them. Retro-classifying from the vendor name works on these seventeen rows and **is not a rule** ([[R-50]]). The fix is capture-side and it is David's 2026-07-07 ruling: *one pipeline, two doors*, the launcher pinning the shape.

🔴 **AND IT DOES NOT RE-EVALUATE.** The verdict on screen is the one the owner was actually shown, not a second verdict computed today over the same row with no way to tell the two apart. `computeReconcile` is not imported and **`line_items` is not even selected**, so re-deriving it is impossible rather than discouraged. The single arithmetic is RECONSTRUCTION: `reconcile_delta` was defined at write time as `lineSum − amount`, so `lineSum = amount + delta` recovers the banked figure exactly and invents nothing.

**The count names its own cap.** *"Showing N of M — capped at 100"*, never a bare N under a count label — that is already a live defect on two other screens and this is not a third. **And a short read against a bigger total reports the gap without blaming a cap it never reached**, because a confident wrong explanation is worse than none.

**WHAT THIS DOES NOT DO, all deliberately:** it repairs no duplicate, backfills no `receipt_id`, re-stamps no reconcile column, and has no per-receipt detail view (`/receipts/:id` — `ProjectCostDrillIn` still degrades to the list, tech-debt #145). _Grounded: ledger #252; `packages/cultivar-os/src/lib/receiptsList.ts` (+87 probes, 14/14 mutants caught against a green control); `packages/cultivar-os/src/components/receipts/ReceiptsList.tsx`; `scripts/measure-receipts-view.mjs` (the live census); `docs/owner-tests/receipts-view-full-surface-test.md`._
...
ARC: delivery
MAPS-TO: 3.4, 3.6
PIECES: load_list_print_view, bill_of_materials, unresolved_is_printed, deer_fence_capture, size_before_trailing_remark
...
✅ **BUILT 2026-09-12 (ledger #315), FOUR OF FIVE PIECES** — all but `deer_fence_capture`, which has nothing to capture from. `loadList.ts` (the pure model — 82 probes, 31/32 mutants) · `/load-list?date=` gated on `deliveries:read` · the print stylesheet. It reads through the existing `readStops`, adds no table, no migration and no api function. Proof: `docs/owner-tests/load-list-full-surface-test.md`.
...
ARC: delivery
MAPS-TO: 3.5, 3.6
PIECES: handoff_order_parity, one_derived_route_url, stop_count_parity, handoff_owner_test, waypoint_cap_decision
NEEDS: 🔴 **WRITTEN BY THUNDER 2026-09-08 (#286) AS THE NO-MATCH CASE, ON DAVID'S RULING, AFTER THE DEFECT WAS REPORTED LIVE BY DAVID AND LAUREN.** The §9 gate found no story covering **what leaves the building**. ⚠️ **AND THE SEARCH ITSELF PRODUCED THE FINDING: a story DOES exist and it is marked OWNER-PROVEN.** The archived *"Route the day's deliveries"* (`:1658`, OWNER-PROVEN 2026-07-03) says the stops are reordered shortest-path *"so **the pins, the on-card list, and the route all agree**"* — **three artefacts, and all three are on Lauren's screen.** The Google Maps link appears in that same story only as a **degradation fallback** (*"degrades to a straight polyline, then to an 'Open in Google Maps' URL card"*), never as the thing a driver is sent. **So the handoff was owner-proven as a fallback and used as the product**, and a board that reads as covering this question has said `covered` on a surface nobody drove. That is [[R-26]] inside the story board, and it is the whole reason fourteen months passed. **What is genuinely OWED by David and is NOT decided here:** 🔴 **the waypoint cap.** Measured 2026-09-08 from LAWNS's own invoice export: stops per delivery day run **1–14, mean 3.6 — 69 of 167 days (41%) exceed 3**, Google's documented **mobile-browser** cap, and **7 of 167 (4%) exceed 9**, the desktop cap; excess waypoints are *"ignored"* silently. **Lauren texts this link to a phone.** That is TRUNCATION, not mis-sorting — a driver receiving a partial route with nothing telling him so — and it is deliberately **NOT** in this build's scope. It needs its own decision (stay on the undocumented path form · move to the documented `?api=1` form · split a long day across links · surface a warning above the cap) and its own build.
Lauren builds tomorrow's route, reads the stop order off the screen, and texts the link to the installer. **What lands on his phone is the order she read.** Not a different order, not a different count of stops — the same route, because there is only one route. She does not check it, and she does not need to: the link cannot say something the screen did not say, because it is **made out of the same list she is looking at** rather than assembled separately from the same ingredients. When she hands it over three ways — opening it herself, texting it, copying it — all three carry the identical route, and the number of stops she tells him is the number of stops in the link.
🔴 **THE SUBJECT OF THIS STORY IS THE HANDOFF, NOT THE OPTIMISER.** The optimiser has worked correctly since `420e0bc` (2026-07-01) — it asks Google for a shortest-path order, gets a real ordered array back, and renumbers the pins and the on-card list to match. **What it never touched was the URL.** The link was built in `buildRoute()` from the entered order — in cart mode that is `created_at DESC`, so **the newest sale rung up became the first stop driven** — and it was built *before the optimiser had run at all*, then never revisited. Every surface that stayed on Lauren's screen was right; every surface that left the building was wrong. 🔴 **The split falls exactly along the line between the person who could see the defect and the person who received it**, which is why it survived a year: she never opened what she sent, and he had nothing to compare it against. **Lauren replaced ~45 minutes a day of typing addresses into Google Maps with this. An un-optimised handoff makes the optimisation decorative.**
⚠️ **THE RULE THIS STORY ENCODES, and it is why the fix is a deletion rather than an addition (David, 2026-09-08):** *"Delete `routeUrl` as state and derive it from `displayStops`, so the link is structurally incapable of disagreeing with the list beside it. NOT option A — recomputing in an effect leaves two sources of truth, which is the mistake `420e0bc` already made once."* **Derive from what is actually there; never hand-maintain a parallel copy** — the fourth time in one week that this was the answer (§6 r8; [[STD-011]]). A synced copy fixes today's symptom and leaves the next outbound surface free to wire itself to the stale half.
✅ **BUILT 2026-09-08 (#286), THREE OF FIVE PIECES — `handoff_order_parity` · `one_derived_route_url` · `stop_count_parity`.** `routeUrl` no longer exists as state; the URL is derived from `displayStops` at the point of use, so the link, the SMS and the clipboard read the same array the numbered list renders. The stop count in the text now derives from that array too — it previously read `selectedOrders.length`, so selecting five orders where two had no address texted *"(5 stops)"* above a link containing three. **BUILDER-COMPLETE, owner-proof owed** (`docs/owner-tests/delivery-route-handoff-full-surface-test.md`) — and the acceptance test is the point of the story: **it reads the artefact the driver receives, never the screen.**
...
NEEDS: ⚠️ **WRITTEN BY THUNDER 2026-08-31 (ledger #247) AS THE NO-MATCH CASE, from David's own dictation in the build prompt.** The tap it hangs off had a story; the ask did not. 🔴 **WHAT IS OWED BY DAVID — and it is a COPY decision, not an engineering one:** the proposed guidance line was *"It helps most if you mention what we planted and how the crew did."* **Google's Rating Manipulation policy prohibits exactly that** — *"nor should they request that specific content be included"*, and expressly *"content that identifies a staff member"* (support.google.com/business/answer/7400114, read 2026-08-31). **The build ships a neutral default and REFUSES that line at the settings field.** ✅ **RULED SAME DAY — [[R-34]]: the copy is "If you have a moment, we'd appreciate a review." and nothing more, and the RULE IS THE CONSTRUCTION RATHER THAN THE TOPIC** — *"anything of the form 'it helps if you mention…' is the prohibited construction whatever follows it."* That corrected a weaker first check which keyed on the object and would have passed *"it helps if you mention how quickly we arrived"*. 🔴 **The provenance is part of the ruling: the prohibited wording reached the project from a research summary that recommended exactly what Google forbids** — so the two clauses now sit VERBATIM beside the code that refuses, because a reader holding the words can check a proposal and a reader holding a pointer follows the suggestion. **(2)** the repeat-customer window is set at **180 days** by reasoning, not by measurement — one growing season plus a margin; a real answer needs Lauren.
...
✏️ **2026-09-11 (ledger #300) — THE LINK FIELD MOVED, AND TWO THINGS THE FIRST BUILD DID NOT SAY.** **(1) The link is entered on BUSINESS PROFILE**, beside name, phone and tax rate — *"one field, entered once, by the owner"* — because David had LAWNS's link in his hand and could not find the field, which sat four cards down `/settings/all` under a module name. **Same store** (`business_modules.config.review_url`), no migration; Save writes it only when it changed and reports it as its own line. The module card keeps only the customer's line. **(2) The ask is for the door, not for the paperwork:** a stop marked done more than a day after its own date does not prompt. Somebody catching up the record at a desk has no customer to hand the phone to, and the only honest tap there — *Not this one* — would record a skip nobody chose and start that customer's 180-day window. It is also what makes *"never fire on history"* hold whatever wrote the row. 🔴 **(3) THE FINDING: THE ASK HAS NEVER BEEN ABLE TO RUN.** `followup_engine` is a `planned` tile, and `/subscription` offers no Turn on for a tile that is not live, so no tenant can switch it on (tech-debt #270). **OWED BY DAVID:** how Follow-Up gets turned on — the catalog's 30-day clock, by hand, or made core · and whether a SKIP should start the 180-day window or only a shown code (today a skip does, because the window reads `review_asked_at` and a skip writes it too).
...
NEEDS: ✅ **RULED 2026-09-11 — R-121: an owner cannot remove their own permissions, and the rule covers EVERY OWNER-ROLE MEMBER.** ~~ONE RULING FROM DAVID — may an owner remove a permission from themselves?~~ That ruling was the only thing blocking wiring `get_my_permissions` on the ruling side (tech-debt #238); where the guard sits is still unruled. Stage 1 (ACCESS) is BUILDER-COMPLETE, **generalised to all 49 `owner_id` policies on 2026-09-10 (#289, applied — R-119)**, and owner-proof is OWED; Stage 2 (AUTHORITY — an OWNER-role actor may assign roles) is a separate commit and not yet built.
...
- **① GENERALISED, 2026-09-10 (#289, applied — [[R-119]]).** ① fixed the THREE surfaces that were reported. It did not ask how many others had the same fence, and the answer was **49 policies**. David's test settles each one without guessing at a customer's preference: **is this about the BUSINESS ENTITY, or about the WORK?** Entity — who owns it, who is billed, who it transfers to — keeps raw `owner_id` **and gets a comment saying why**; work gets a permission string. Of the 49: **28 re-pointed · 10 dropped as redundant · 8 kept raw, each with the comment it never had · 3 untouched on tables pending DROP. Raw-`owner_id` policies 49 → 12, measured.** ⚠️ **Four that the triage called work are NOT:** `business_members`, `role_definitions` and `invitations` **are the authority store** — a member INSERT there is a permission-granting side door with no funnel — and `service_offerings`, where a `FOR ALL` re-point would create the member DELETE that R2 forbids.
...
🔴 **AND ① HAS A THIRD HALF NOBODY HAD NAMED, WHICH #289 FOUND AND DID NOT FIX: THE CLIENT DERIVES AN AUTHORITY THE SERVER NEVER GAVE IT.** `[TRACE:PERM]` reports `OWNER_LOCKED_SET (computed from the manifest)` with **58** entries against stored arrays of **57** — the 58th is the `owner-only` sentinel, a string in no array, checked by **0 policies and 0 functions**. So the client can render a surface the now-literal `has_permission` refuses. `get_my_permissions(uuid)` exists for this (applied 2026-09-10, **called by nothing, deliberately**): when wired, `can(x)` becomes `array.includes(x)`, `OWNER_LOCKED_SET` is deleted, and the sentinel becomes the `is_account_holder` boolean that call already returns. **NEEDS FROM DAVID — and it is the only thing blocking it: may an owner remove a permission from themselves?** If the stored array is the only truth and an owner drops their own `settings:update`, they cannot grant it back. Tech-debt #238.
...
NEEDS: David to decide the template set (delivery / planting / inspection / netting / subscription…) + how prescriptive the guided editor is (pick-a-template-then-tweak vs free-form-with-guardrails) + where it lives relative to the current Settings service editor (#98). ✅ **PART-ANSWERED 2026-09-11 BY R-120 (ledger #293) — the GUARDRAIL, not the templates:** a transport service's mode field appears and is REQUIRED on every writer (the Settings add and edit forms, the books review, the discovery seed), and `requires_address` defaults from the mode (staff → yes, self → no) — so *"a delivery service with requires_address unset"* is now a default an owner overrides, not a gap they fall into. **Still David's:** the template set and how prescriptive the editor is. ⚠️ **A sibling finding, tech-debt #251:** checkout offers ONE staff delivery per shape, so a business with two (LAWNS: trip charge + tailgate) cannot be modelled correctly by any editor.
...
- **🔴 SETTLE ONE TEST-TENANT SERVICE BEFORE APPLYING `20260911`** — *Test David's new Business* → **Backyard Placement & Staging Service** is `category = transport` with no `transport_mode`. The migration's §0 **REFUSES to run** while any such row exists (measured 2026-09-11). Decide staff / self / not transport, in Settings → Services. _Added 2026-09-11 from the migration report._
- **🔴 THREE TRANSPORT SERVICES ON LAWNS** — staff delivery (`TC`: 533 lines / $40,760 on her own invoices), tailgate (`Tailgate Delivery`: 127 / $18,990), and **self-collect at $0** (most customers use it; no invoice line because nothing is charged). **A walk-in has nothing to select today** — Trip Charge was moved to `addon` as a workaround. ⚠️ **The writer fix landed (R-120, ledger #293), but tech-debt #251 still blocks it:** checkout offers one staff delivery per shape, so a second is never offered. _Transcribed from the 2026-09-10 handoff §7._
- **🔴 TELL LAUREN THREE THINGS** — (1) her profile save works now; (2) stop putting phone numbers in address lines — the installer's printed sheet is being fixed so she no longer needs to; (3) her item clean-up was exactly right. _Handoff §7._
- **SIX WRONG PHONE NUMBERS** — including Archilyn Martin's record holding **LAWNS's own switchboard `(512) 450-3336`**. _Handoff §7; not re-measured._
- **477 RECORDS WITH THE PHONE WRITTEN THREE TIMES** — safe to clear the address line **once Lauren confirms**, and **only after the importer reads `Line2` (tech-debt #254)**, or the next import restores it. _Handoff §7; count not re-measured._
- **THE PHONE ON THE INSTALLER'S PRINTED SHEET** — a BUILD, listed here because it removes the reason for Lauren's phone-in-address workaround; the two clean-ups above wait on it. _Handoff §7._
- **TWO DOCUMENTS READY AND UNREAD BY TERRY** — the address review and the watering ROI. _Handoff §7._
- **SET `nursery_profiles.default_install_price` ON LAWNS** — it is **NULL** (measured 2026-09-11). Until one is set, owner-test CARD 9 (Lauren, install price) reads as a failure when the fix is working. _Handoff §7._
