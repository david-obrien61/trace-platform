...
NEEDS: 🔴 **ONE RULING FROM DAVID — may an owner remove a permission from themselves?** It blocks wiring `get_my_permissions` and nothing else (tech-debt #238). Stage 1 (ACCESS) is BUILDER-COMPLETE, **generalised to all 49 `owner_id` policies on 2026-09-10 (#289, applied — R-119)**, and owner-proof is OWED; Stage 2 (AUTHORITY — an OWNER-role actor may assign roles) is a separate commit and not yet built.
