...
import { reportScreenReady } from '@trace/shared/utils/navTiming';
...
export function Customers() {
  ...
  const loadCustomers = useCallback(async () => {
    ...
    // 🔴 REPORTED HERE, NOT IN AN EFFECT, AND NOT ON EVERY RENDER. This is the moment the rows
    // exist; `reportScreenReady` ignores a second call for one navigation, so a filter or an
    // inline edit re-rendering the grid cannot log a number measured from a move that finished
    // minutes ago. `reads` distinguishes the two paths: 0 means the held copy answered.
    reportScreenReady('customers', { rows: (data ?? []).length, servedFrom: 'database', reads: 1 + Math.max(0, Math.ceil(((count ?? 0) - PAGE) / PAGE)) });
  ...
  useEffect(() => {
    ...
    if (ROSTER_CACHE && ROSTER_CACHE.businessId === businessId) {
      ...
      reportScreenReady('customers', { rows: ROSTER_CACHE.rows.length, servedFrom: 'held copy', reads: 0 });
      return;
    }
    void loadCustomers();
