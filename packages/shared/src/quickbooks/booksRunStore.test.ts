 ...
 */
import { planBooksRun, saveBooksRun, readLatestResult } from './booksRunStore';
import { BOOKS_RULES } from './booksFindings';
import { OPENING_STOCK_RULE_ID } from './openingStock';
...
const finding = (o: Partial<Finding> = {}): Finding => ({
  ...
  // ⚠️ `rows` / `rowsTotal` / `window` / `blocks` / `clean` are NOT on main's `Finding` and are
  // deliberately absent here. They belong to the wider findings-report branch; this pass brought
  // ONE file across (`booksRunStore`) and did not merge the rest, so the fixture is main's shape.
  ...o,
});
...
// ══ §F — READING BACK: THE THREE ANSWERS, AND WHY THEY ARE NOT ONE ════════════════════════
// The seed screen asks this one question and acts on the answer. "You have not read your books
// yet" is an instruction the owner can carry out; "we could not read your saved reports" is OURS.
// Telling somebody to do a thing they already did is how a screen loses their trust in one step.

function reader(rows: unknown[] | null, error: { message: string } | null = null) {
  const q = {
...
async function readBack(): Promise<void> {
  {
    const r = await readLatestResult(reader([{
      rule_id: OPENING_STOCK_RULE_ID, rule_version: 1, matched: 412, of: 638,
      noun: 'units a month for a typical item', value: 4.2, measured: true,
      books_report_runs: { ran_at: '2026-09-14T10:00:00Z' },
    }]), 'biz', OPENING_STOCK_RULE_ID);
    ok(r.found === true, 'F1  a stored result is found');
    if (r.found) {
      ok(r.result.value === 4.2, `F2  and its value survives as a number (got ${r.result.value})`);
      ok(r.result.ranAt === '2026-09-14T10:00:00Z', 'F3  carrying the RUN’s timestamp, which is the axis the comparison is ordered on — not the result row’s own');
      ok(r.result.measured === true, 'F4  and whether the rule actually ran');
    }
  }
  {
    const r = await readLatestResult(reader([{
      rule_id: OPENING_STOCK_RULE_ID, rule_version: 1, matched: 0, of: 0, noun: 'x',
      value: null, measured: false, books_report_runs: { ran_at: '2026-09-14T10:00:00Z' },
    }]), 'biz', OPENING_STOCK_RULE_ID);
    ok(r.found === true && r.result.value === null,
       '🔴 F5  NULL SURVIVES AS NULL. `Number(null)` is 0, and a rule that could not measure would come back claiming a typical item moves zero a month — a fabricated fact wearing a measurement’s clothes');
    ok(r.found === true && r.result.measured === false, 'F6  and it still says it did not measure');
  }
  {
    const r = await readLatestResult(reader([]), 'biz', OPENING_STOCK_RULE_ID);
    ok(r.found === false && r.reason === 'never-run',
       'F7  an EMPTY, error-free read is "you have not read your books yet" — an instruction the owner can act on');
  }
  {
    const r = await readLatestResult(reader(null, { message: 'relation does not exist' }), 'biz', OPENING_STOCK_RULE_ID);
    ok(r.found === false && r.reason === 'unreadable',
       '🔴 F8  AN ERRORED READ IS OURS, NOT THEIRS, AND IS A DIFFERENT ANSWER. Dressed as F7 it would tell somebody to go and read their QuickBooks data when they already had');
    ok(r.found === false && r.detail.includes('relation'),
       'F9  and it carries what actually went wrong, rather than a generic apology');
  }
  {
    // NEGATIVE CONTROL, changing the POPULATION rather than the subject (tech-debt #182).
    const r = await readLatestResult(reader([{
      rule_id: 'something-else', rule_version: 1, matched: 1, of: 1, noun: 'n',
      value: 9, measured: true, books_report_runs: [{ ran_at: '2026-09-14T10:00:00Z' }],
    }]), 'biz', OPENING_STOCK_RULE_ID);
    ok(r.found === true && r.result.ranAt === '2026-09-14T10:00:00Z',
       'F10 NEGATIVE CONTROL — PostgREST may embed the run as an ARRAY, and both shapes must yield the timestamp. §F3 passing on an object alone would be an accident of one driver version');
  }

  // 🔴 THE SCREEN AND THE RULE TABLE MUST NAME THE SAME RULE. The seed screen reads a stored
  // result by id; a typo, or a rule renamed on one side, makes the screen silently permanent-
  // suggestionless — and "no sales history" is a real answer, so nothing would look broken.
  ok(BOOKS_RULES.some(r => r.id === OPENING_STOCK_RULE_ID),
     `🔴 F11 THE SEED SCREEN’S RULE ID EXISTS IN BOOKS_RULES ('${OPENING_STOCK_RULE_ID}'). A mismatch makes the screen fall back to "we have no sales history" forever, which looks exactly like the honest path and is not it`);
}

void main().then(readBack).then(() => {
