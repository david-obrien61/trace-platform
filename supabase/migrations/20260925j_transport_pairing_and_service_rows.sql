-- ═════════════════════════════════════════════════════════════════════════════
-- ⚠️ APPLIED BY DAVID ON 2026-09-25 UNDER THE NAME `20260925c_transport_pairing_and_service_rows.sql`.
--    RENAMED TO `20260925j` — THE FILE ONLY; NOT ONE LINE OF SQL CHANGED (§6 r1), AND NOTHING
--    NEEDS RE-APPLYING. Verified live 2026-09-25 22:33 UTC: `service_offerings.offers_installation`
--    and `order_service_selections.line_rows` both exist.
--
-- 🔴 WHY IT MOVED: SLOT `20260925c` NAMED **THREE DIFFERENT FILES** ACROSS THE REPO —
--    `20260925c_transport_pricing_basis.sql` (on `main`, CONTACTS-349's ring pricing),
--    `20260925c_install_kit_components.sql` (three branches), and this one. David asked for this
--    file to be committed to `main` "under its name so every 20260925 slot names exactly one
--    file" — **and its name was already taken on `main` by somebody else's applied migration**,
--    so committing it as `c` would have created the very collision the instruction removes.
--    `20260925j` was free (`npm run migration:slot`, checked immediately before the rename).
--
--    ⚠️ **THE COLLISION ALREADY HAPPENED IN REALITY, AND THAT IS THE PART WORTH KEEPING:** two
--    different migrations were both applied while both were called `20260925c`. So "apply
--    20260925c" named two files on 2026-09-25 — tech-debt #294a's cousin, one artefact over.
--    This header is the only record of which one this was.
-- ═════════════════════════════════════════════════════════════════════════════
-- ─────────────────────────────────────────────────────────────────────────────
