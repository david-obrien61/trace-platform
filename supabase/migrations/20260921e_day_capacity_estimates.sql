-- ============================================================================================
-- 20260921e — THE DAY'S CAPACITY ESTIMATE, SNAPSHOT AND APPEND-ONLY
...
--
-- V3 · 🔴 A SNAPSHOT CANNOT BE REWRITTEN. Expect: the first UPDATE RAISES
--      'delivery_day_estimates is append-only…'; run it and read the error, do not skip it.
...
-- VALUES ('ed2e5933-45dc-4b9b-a331-ddfd125e7a74', current_date, 'route_save', 4, 10, 7, 30, true, 6.0, true, 1)
-- RETURNING id;   -- keep this id for V4
-- UPDATE public.delivery_day_estimates SET total_hours = 99 WHERE id = '<that id>';  -- MUST RAISE
--
-- V4 · …but the CHOICE may be recorded once → expect the UPDATE to succeed, then the second to RAISE
-- UPDATE public.delivery_day_estimates SET chosen_teams = 1, chosen_at = now() WHERE id = '<that id>';
-- UPDATE public.delivery_day_estimates SET chosen_teams = 2 WHERE id = '<that id>';  -- MUST RAISE
--
-- V5 · DELETE is refused → MUST RAISE 'append-only: DELETE is not permitted'
-- DELETE FROM public.delivery_day_estimates WHERE id = '<that id>';
--
