...
| **cultivar_plants table** | WIRED (untangle catalog-confirmed; policy cleanup pending David apply) | `supabase/migrations/20260613_cultivar_plants_untangle.sql` + `20260613_cultivar_plants_policy_cleanup.sql` | Identity-only join table (tag_id, species, common_name, plant_type, current_container, location_zone, warranty_months + inventory_id FK → business_inventory). Stock-fact cols dropped. nursery_id dropped (AC-1). **Untangle RUN — catalog checks C1/C2/C4/C5 CLEAN** (proof: `docs/verification/20260613_cultivar_plants_verification.md`). C3 surfaced 4 policies (2 redundant leftovers from old `plants`). **Policy cleanup migration WRITTEN** (drops `plants_select_public` redundant public-read; replaces public/ALL `plants_business_owner` with authenticated-scoped `cultivar_plants_owner_all` so owner write survives without the public write-hole — AC-3). Post-cleanup policy set = anon_select_plants (anon QR read) + cultivar_plants_owner_select (auth SELECT) + cultivar_plants_owner_all (auth ALL). Code repointed: usePlant, PlantHero, PlantCard, PlantProfile, AddOns, CartReview, Confirmation, useSubmitOrder, dashboard.ts, submit.ts, qbo/invoice/cultivar.ts, social/generate-posts.ts, Orders.tsx, DeliveryRoute.tsx, Dashboard.tsx. Build clean 2187 modules. ⚠️ David must run `20260613_cultivar_plants_policy_cleanup.sql` in bgobkjcopcxusjsetfob, then run V1–V3 to flip to verified. ⚠️ inventory_id is NULL on all existing rows — QR checkout shows "unavailable" until lot population runs (separate sequenced step). NOTE: anon_select_plants USING(true) = all plant identity public-readable (Tech Debt #32). | CLAUDE.md §HANDOFF 2026-06-13 |
