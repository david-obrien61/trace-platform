...
<!-- Last verified: 2026-06-11 (OCR model externalized to platform_config: gemini-2.5-flash primary, haiku-4-5 fallback; getOcrModels() reads DB → env → default; strict bake-off prompt; BENCH-E Rule 7 applied; migration 20260611_platform_config.sql committed; STANDARDS.md v1.8; build 2180 ✅; Receipt Keeper v1 still WIRED — all pending migrations unverified; live grid test required before advancing to WORKS) -->
...
| **Receipt Keeper v1** | WIRED | `src/pages/ReceiptKeeper.tsx + api/receipts/ocr.ts` | /receipts route (PrivateRoute) · Dashboard header "Receipts" button (isOwner) · GEMINI_API_KEY + ANTHROPIC_API_KEY set ✅ · receipts table applied + 14 cols verified ✅ · PRIVATE bucket created ✅ · STD-010: content-type + size enforced in both layers · **2026-06-11 model externalization (BENCH-E Rule 7)**: Provider fallback now Gemini 2.5 Flash → Claude Haiku 4.5 → 503 · `getOcrModels()` reads `platform_config` DB row → `OCR_PRIMARY_MODEL`/`OCR_FALLBACK_MODEL` env var → hardcoded default · strict bake-off prompt applied · `OcrResult.parsed.line_items` extended with optional `quantity`/`unit_price` · migration `20260611_platform_config.sql` committed — **David must apply (0) as first step** · **2026-06-13 fixes**: (1) Storage RLS migration `20260613_receipts_storage_rls.sql` (3 policies: INSERT/SELECT/DELETE — **David must apply**). (2) Atomic fail: storage fail aborts save; no orphan row. (3) `20260613_receipts_add_line_items.sql` adds `line_items jsonb` — **David must apply** · **2026-06-14 build**: (4) Editable line-item grid: OCR seeds `lineItems[]` state; owner can add/edit/delete rows; amounts parsed to number on save. (5) Live reconciliation readout: `computeReconcile()` runs on every render in confirm step — MATCH_TOLERANCE $0.02 / small_gap ≤$5 or ≤10% / large_mismatch; gapNote shown inline. (6) Conflict dialog + override recording ("Tesla bit"): `large_mismatch` blocks Save → conflict dialog shows delta → "Save anyway" records `reconcile_overridden_at` timestamptz + `reconcile_status='large_mismatch_overridden'`; migration `20260614_receipts_reconciliation.sql` adds 6 reconciliation columns — **David must apply** · Build: 2180 ✅ · **2026-06-15 McCoy's-fallback diagnostic (commit 2d14dee) — 5 root causes patched**: (A) `tryGemini()` AbortController 8000→9000ms — McCoy's 2.2MB raw image with Gemini 2.5-flash thinking layer consistently exceeded 8s, causing fallback. (B) `tryClaude()` `max_tokens` 1024→2048 — same truncation class as June 15 Gemini fix. (C) `OcrResult.parsed` TypeScript interface added `subtotal?`/`tax?` — server was correctly returning tax; client was discarding it at the type boundary. (D) `String(amount)` → `Number(amount).toFixed(2)` for `fields.amount` and line items — integer receipts showed bare "31" not "$31.00". (E) Tax injection: when `data.parsed?.tax` non-null and no OCR line matches `/tax/i`, inject `{ description: 'Tax', amount: '$2.36' }` line item so `computeReconcile` returns `status: 'match'` (eliminates "possibly tax or tip" small_gap gapNote). · **Still WIRED** — all five pending migrations unverified; live acceptance test required (upload McCoy's → Gemini wins, no fallback log → 6 lines including "Tax $2.36" → "$31.00" formatted → green "Lines match total") before advancing to WORKS | CLAUDE.md §HANDOFF 2026-06-15 |
| **receipts table** | WORKS | `supabase/migrations/20260612_receipts.sql` | Applied to bgobkjcopcxusjsetfob · 14 cols verified by David · dual RLS: owner (businesses.owner_id) + member (business_members active=true) · updated_at trigger via existing set_updated_at_generic() · **pending migrations**: `20260613_receipts_add_line_items.sql` (line_items jsonb) + `20260614_receipts_reconciliation.sql` (6 reconciliation columns: line_items_original, amount_original, reconcile_status, reconcile_overridden_at, reconcile_delta, header_amount_edited) — **both David must apply** | CLAUDE.md §HANDOFF 2026-06-14 |
| **receipts reconciliation migration** | EXISTS | `supabase/migrations/20260614_receipts_reconciliation.sql` | 6 additive columns: line_items_original (jsonb), amount_original (numeric 10,2), reconcile_status (text CHECK match/small_gap/large_mismatch_overridden), reconcile_overridden_at (timestamptz), reconcile_delta (numeric 10,2), header_amount_edited (boolean) · AC-1: no vertical nouns · AC-2: existing dual RLS covers all new columns · **David must apply to bgobkjcopcxusjsetfob + run VERIFICATION QUERY** (expect 6 rows) | CLAUDE.md §HANDOFF 2026-06-14 |
| **platform_config migration** | EXISTS | `supabase/migrations/20260611_platform_config.sql` | Infrastructure key/value table · RLS enabled, zero user-facing policies (service key only) · seeds ocr_primary_model='gemini-2.5-flash' + ocr_fallback_model='claude-haiku-4-5-20251001' · AC-1: no vertical nouns · **David must apply to bgobkjcopcxusjsetfob + run VERIFICATION QUERY** (expect 2 rows: ocr_fallback_model + ocr_primary_model) | CLAUDE.md §HANDOFF 2026-06-11 |
...
| **Receipt Keeper v1** | ⚠️ 5 DAVID STEPS REQUIRED. **(0) platform_config table:** apply `20260611_platform_config.sql` + VERIFICATION QUERY (expect 2 rows: ocr_fallback_model + ocr_primary_model). This enables `gemini-2.5-flash` as primary OCR model; without it falls back to env/default. **(1) Storage RLS:** apply `20260613_receipts_storage_rls.sql` + VERIFICATION QUERY (expect 3 rows: receipts_storage_delete, receipts_storage_insert, receipts_storage_select). **(2) line_items column:** apply `20260613_receipts_add_line_items.sql` + VERIFICATION QUERY (expect line_items, jsonb, YES). **(3) Reconciliation columns:** apply `20260614_receipts_reconciliation.sql` + VERIFICATION QUERY (expect 6 rows: amount_original, header_amount_edited, line_items_original, reconcile_delta, reconcile_overridden_at, reconcile_status). **(4) Live grid test** after deploy: upload receipt → OCR seeds grid → edit a line item → confirm reconciliation readout shows → edit total to large mismatch → confirm conflict dialog fires + "Save anyway" records overridden_at on row. Advance to WORKS after step 4 confirmed. | — |
