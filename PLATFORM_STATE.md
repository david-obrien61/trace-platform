...
| **D-11 category + D-12 labor foundation** | Stages 1-3 ✅ OWNER-PROVEN 2026-06-18 · Stage 4 owed | **Schema (Stage 2, catalog-proven):** `cost_objects.cost_category` (nullable, no CHECK) + `labor_resources` table (EMPLOYEE/CONTRACTOR, HOURLY/FLAT_FEE, burden/cost_rate/bill_rate, contractor rate/pass-through; RLS owner_all+member_all) + `resource_id` FK + `labor_hours` — migration `20260618_cost_category_and_labor_resources.sql`. **Labor migrated (Stage 3, owner-proven):** owner $75×160=$12,000/mo CONFIRMED pulled config→`cost_objects` (cost_category='labor', OPEX, RECURRING_FIXED) via `hasMigratedLabor` R1-safe guard in BOTH read path + capture mirror; live `/costs` floor **$12,123** / known **$12,280.67** byte-identical (not $24,123 — no double-count). **Stage 4 OWED:** Settings reorg (recurring/labor/margin/target blocks) + contractor UI + category picker + re-point write path off `config.labor`. Margin engine + P&L + grid still DEFERRED onto the robust schema (no re-migration). `[TRACE:COST]`/`[TRACE:SEAM]` ON until Stage 4 owner-proof. |
