...
| **Vercel functions** | WIRED | `api/*.ts` + subdirs | 🔴 **THE COUNT IS DELETED FROM THIS ROW, NOT CORRECTED — 2026-09-14, ledger #318.** It read **`(11 of 12)` … `11 live functions (1 slot headroom)`** and named eleven; the true count is **12 of 12, ZERO headroom**, and the twelfth — `customers/create` — has been live since **2026-06-20**, the day the deliveries endpoint was folded into it *because* the ceiling was hit. So the row was right on 2026-06-13, went stale six days later, and stood for **86 days in the doc §10 says to read FIRST every session**, inviting the next builder to mint the function that fails the deploy silently. ⚠️ **`docs/inventory-functions.md` carried the identical error and CORRECTED it to `12 of 12` on 2026-09-02** — twelve days before this row, which is the whole argument: a number hand-written in two places gets fixed in one. **It is deleted rather than re-stated because a hand-written count of a list that grows underneath it cannot be repaired by being more careful** — the same resolution David ruled for the bootstrap's cached next-free ids (tech-debt #289, ledger #316: *"a number that is usually right is worse than no number, because it gets trusted"*). ✅ **ASK THE CHECK, NOT THIS ROW:** `npm run verify:api-parses` prints the live count, warns at zero headroom, and **FAILS THE BUILD** at 13 (ledger #318 · proven red on both a flat and a nested 13th file). **`npm run verify` runs it first, so every build re-derives it.** Ceiling doctrine + the reuse-before-mint seams: CLAUDE.md §6 r11 · tech-debt #41 · #60. **Consolidation history:** was 13 → 11 via QBO consolidation 2026-06-13 (`fcdfa97`); deliveries → `customers/create` 2026-06-20 · All three public QBO paths preserved byte-exactly via vercel.json rewrites before SPA catch-all · ⚠️ `packages/shared/src/pages/Settings.tsx:266` has dead fetch() to `/api/services/customer-match` (root gone, will 404) | CLAUDE.md §HANDOFF 2026-06-13 · docs/inventory-functions.md |
...
| **business_voice_samples table** | WIRED (verified 2026-06-13, catalog-confirmed) | `supabase/migrations/20260613_business_voice_samples.sql` | Renamed from `campaign_tone_samples` (20260613). Columns: id, business_id FK→businesses CASCADE, platform, original_text, edited_text, source (provenance — `campaign_generate`\|`social_generate`), created_at. RLS: `business_voice_samples_owner` FOR ALL — ✏️ **RE-POINTED 2026-09-10 (#289, applied): it was owner-scoped (`business_id IN (SELECT id FROM businesses WHERE owner_id = auth.uid())`) and now reads `is_active_member(business_id) AND has_permission(business_id,'campaigns:update')`.** It was also one of the TEN policies that existed in NO migration; its original definition is captured verbatim in `20260910b` §1. Pooled voice store: read-back filters by business_id only; source is metadata. Catalog proof: `docs/verification/20260613_business_voice_samples_verification.md` (C1–C6 all CLEAN). | CLAUDE.md §HANDOFF 2026-06-13 |
...
| **Crew day link** | WIRED (migration `20260917c` APPLIED 2026-09-17, V1–V4; merged to `main`; owner-proof owed) | `src/pages/CrewDay.tsx` (`/crew`) · `components/delivery/CrewLinkPanel.tsx` · `api/members/crewDay.ts` | ledger #347 (2026-09-17). No-login one-day link: the day's stops, items (no prices), Start / Done / Undo / Note with a typed name. Done holds a review ask; no order or stock change. Evidence: `verify:writer-registry` domain `stop-progress` (6 paths · 7 guards), 17/17 mutants. Owner-proof owed: `crew-day-link-full-surface-test.md`. |
...
| **PMI page** | WIRED | `src/pages/PMI.tsx` | /pmi route · **Unblocked 2026-06-13**: shared PMI.tsx rewired off dead pmi_assets onto business_assets + business_pmi_schedule + business_service_log. cultivar-os/src/pages/PMI.tsx is a thin wrapper passing businessId from useBusinessContext() + cultivar assetTypes + serviceTypes. Route registered in router.tsx line 71. Build clean. End-to-end not yet verified by David → **WIRED, not WORKS.** ✅ 20260613_business_service_log_result.sql RUN + catalog-confirmed 2026-06-13 (log result field works). | CLAUDE.md §HANDOFF 2026-06-13 |
| **cultivar_plants table** | WIRED (verified 2026-06-13, catalog-confirmed) | `supabase/migrations/20260613_cultivar_plants_untangle.sql` + `20260613_cultivar_plants_policy_cleanup.sql` | Identity-only join table (tag_id, species, common_name, plant_type, current_container, location_zone, warranty_months + inventory_id FK → business_inventory). Stock-fact cols dropped. nursery_id dropped (AC-1). **Both migrations RUN — catalog-confirmed.** Untangle: C1/C2/C4/C5 CLEAN. Cleanup: V1 (exactly 3 policies), V2 (zero public/ALL — write-hole shape gone, AC-3), V3 (RLS enabled) ALL PASS. Proof: `docs/verification/20260613_cultivar_plants_verification.md` (Parts A+B both filled, VERIFIED). Policy set ⚠️ **CHANGED 2026-09-10 (#289, applied) — V1's *exactly 3 policies* is now FIVE, and the change is a NARROWING:** `anon_select_plants` (anon QR read, unchanged) + `cultivar_plants_owner_select` (now `is_active_member AND has_permission(…,'inventory:read')`) + **`cultivar_plants_owner_all` DROPPED**, replaced by `cultivar_plants_member_insert` / `_member_update` / `_member_delete` on `inventory:create` / `:update` / `:delete`. 🔴 **WHY SPLIT RATHER THAN RE-POINTED: a `FOR ALL` policy gated on ONE string grants EVERY verb on that string**, so a single `inventory:update` policy would have handed DELETE to an update-string holder. ⚠️ **AND IT NARROWS FOR STAFF, DELIBERATELY** — the old predicate was `owner_id OR is_active_member`, so ANY active member could write the catalogue, including STAFF who hold `inventory:read` alone. The count/promote loop writes `business_inventory`, not `cultivar_plants`, so the walk is unaffected. Code repointed: usePlant, PlantHero, PlantCard, PlantProfile, AddOns, CartReview, Confirmation, useSubmitOrder, dashboard.ts, submit.ts, qbo/invoice/cultivar.ts, social/generate-posts.ts, Orders.tsx, DeliveryRoute.tsx, Dashboard.tsx. Build clean 2187 modules. ⚠️ inventory_id is NULL on all existing rows — QR checkout shows "unavailable" until lot population runs (separate sequenced step — see `docs/cost-to-produce/LOT-POPULATION-PRECONDITIONS.md`). NOTE: anon_select_plants USING(true) = all plant identity public-readable (Tech Debt #32). | CLAUDE.md §HANDOFF 2026-06-13 |
...
| **business_service_log table** | WIRED | `supabase/migrations/20260612_business_assets_inventory_pmi_service.sql` + `supabase/migrations/20260613_business_service_log_result.sql` | Structurally verified 2026-06-12 in bgobkjcopcxusjsetfob: RLS enabled · owner_all + member_all policies · 3 FKs (→businesses CASCADE, →business_assets CASCADE, →receipts SET NULL) · append-only ledger (no updated_at trigger — corrections add new rows) · receipt_id is COUNT-ONCE dedup seam. **Wired 2026-06-13**: PMI.tsx handleLogService() INSERTs (service_type, performed_by, cost, performed_at, notes, result). **`result` column VERIFIED 2026-06-13** — `20260613_business_service_log_result.sql` RUN + catalog-confirmed by David: `result` column present (text, nullable) with CHECK constraint `business_service_log_result_check` = (PASS / NEEDS_ATTENTION / FAIL). No pending migration. End-to-end INSERT with live data not yet exercised by David → **WIRED, not WORKS** (schema is verified; the table-level row stays WIRED pending a live service-log INSERT). | docs/cost-to-produce/COST-TO-PRODUCE-DESIGN.md §14 (RECEIPT ROUTING SEAM) |
...
|---|---|---|
| **Cost-to-produce + project-lens** | ✅ OWNER-PROVEN 2026-06-18 | Live `/costs` reads **$12,280.67/mo** (floor $12,123 + est $157.67; verified via service-key capture 2026-06-18 — the earlier $12,279.67 was stale by $1, the "E-mail sender" line); recurring costs edit/assign/add/delete under RLS; project-lens tree (group ordering, unknown-honesty, inline-edit + confidence↔amount coherence) proven end-to-end. `[TRACE:PROJECTLENS]` stays ON until Andrew's asset/inventory widget online+tested. See CLAUDE.md §HANDOFF 2026-06-18. |
| **D-11 category + D-12 labor foundation** | Stages 1-4 ✅ · **LABOR MODEL OWNER-PROVEN** 2026-06-18 · foundation NOT fully closed (small open items) | **Schema (Stage 2, catalog-proven Q–W):** `cost_objects.cost_category` (nullable, no CHECK) + `labor_resources` table (EMPLOYEE/CONTRACTOR, HOURLY/FLAT_FEE, burden/cost_rate/bill_rate, contractor rate/pass-through; RLS owner_all+member_all) + `resource_id` FK + `labor_hours` — migration `20260618_cost_category_and_labor_resources.sql`. **Labor migrated + proven:** owner $75×160=$12,000/mo pulled config→`cost_objects` via `hasMigratedLabor` R1-safe guard; floor held **$12,123** (no $24,123 double-count). **Contractor labor works:** Connor $1,000/mo + Andrew $450/mo as `contract-labor` (`labor_resources` rows + applied `cost_objects` + `resource_id` links; known moved by exactly the **$1,450** delta). **Stage 4 done:** 4-block Settings reorg (recurring/labor/margin/target) live; category picker on recurring rows; `config.labor` clear + button relabel (`fe94ded`). **STILL OPEN (foundation, not the labor model):** (1) save-button vs inline-edit consistency — project-assignment needs Save while other edits are inline (pick ONE model); (2) label the two per-project numbers (one-time capital vs /mo). Margin engine + P&L + grid still DEFERRED onto the robust schema (no re-migration). `[TRACE:COST]`/`[TRACE:SEAM]` STAY ON — D-11/D-12 NOT fully closed. See CLAUDE.md §HANDOFF 2026-06-18. |
| **D-14 per-project cost-to-produce drill-in (Phase 1 + 1.1, cost-only)** | ✅ BUILDER-COMPLETE 2026-06-18 · OWNER-PROVEN owed | Click a project group in the `/costs` By-project tree → modal showing THAT project's cost in isolation: 2 headline figures (Monthly pool + One-time capital, taken VERBATIM from the group's existing `rollup` — no recompute, reconciles with the tree exactly), monthly pool by category (Labor / Other recurring), capital separate, confidence mix. **Phase 1.1:** Labor/Other/Capital aggregates EXPAND to line items (name · amount · confidence · category · 🧾 receipt link); "Other recurring" is a REAL positive group-by (NOT pool-minus-labor) so null/blank `cost_category` surfaces as **Uncategorized** (visible, never absorbed). Reconciles two ways: Σ items === aggregate AND aggregates === tree totals. **ZERO schema** (view-layer; `cost_category`+`receipt_id` added to the tree's existing SELECT + widened row type — no new query). Pricing/margin/N is Phase 2, OMITTED (D-14: cost truth ≠ price strategy). Files: `ProjectCostDrillIn.tsx` (NEW) + `ProjectCostTree.tsx`. **Reconciliation proof PASSED** (`scripts/verify-project-drill-in.ts`): all groups MATCH on real TRACE tenant — CoolRunnings capital $917.31 → 4 hardware rows; BuiltWithCAI Other $156.67 → 4 non-labor rows; **Uncategorized surfaced widely** (most rows untagged — honest, working as intended). 🟡 receipt deep-link `/receipts/:id` absent → links to `/receipts`. `[TRACE:PROJECTLENS]` stays ON. `docs/DECISION-cost-attribution-and-shared-cost.md` · `DECISIONS.md` D-14. See CLAUDE.md §HANDOFF 2026-06-18. |
| **D-13 unified margin store + cost/margin history** | DEFERRED — future cross-vertical arc (NOT near-term) | Captured read-only: shared `MarginEngine.ts` already correct (stateless calculator both verticals feed); margin STORAGE fragmented (Cultivar `config.margin` blob + Ignition's 3 stores, DB one orphaned/display-only); NO DB-level cost/margin history anywhere (last-write-wins). Target = ONE DB-backed RLS margin store + history table, landed WITH/AFTER the BI what-if wedge. `docs/DECISION-unified-margin-store-and-history.md` · `DECISIONS.md` D-13. |
...
| **nursery_modules DROP** | ✅ DONE — by hand, no migration records it | Measured ABSENT 2026-09-11 by `node scripts/verify-migration-apply-state.mjs --catalog`; its policy, index and `business_id` column went with it. ⚠️ This row said *"David must run"* after it had already happened |
