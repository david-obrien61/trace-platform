...
| **business_voice_samples table** | WIRED | `supabase/migrations/20260613_business_voice_samples.sql` | Renamed from `campaign_tone_samples` (20260613). Columns: id, business_id FK→businesses CASCADE, platform, original_text, edited_text, source (provenance — `campaign_generate`\|`social_generate`), created_at. RLS: `business_voice_samples_owner` FOR ALL USING owner. Pooled voice store: read-back filters by business_id only; source is metadata. ⚠️ **David must apply 20260613_business_voice_samples.sql** before voice loop is active. | CLAUDE.md §HANDOFF 2026-06-13 |
