...
<!-- Last verified: 2026-06-11 (Receipt Keeper v1 advanced WIRED → WORKS: live test McCoy's production — provider=gemini, 3-4s, browser-image-compression 2.35MB→373KB, 5 line items + Tax $2.36, $31.00 formatted, green "Lines = Total"; all 5 pending migrations confirmed applied; known limitation: HANDWRITTEN receipts not reliable — logged REQ-2) -->
...
| **Modules · PMI.tsx** | BROKEN | `modules/PMI.tsx` | cultivar-os/src/pages/PMI.tsx:1 — `import { PMI as PMIModule }` · **BROKEN at runtime**: pmi_assets table does not exist in bgobkjcopcxusjsetfob; 20260529_pmi_shared.sql never applied; PMI.tsx errors on load. Module file EXISTS + import compiles; DB layer is missing. | CLAUDE.md §4 Active Tasks |
...
| **PMI page** | BROKEN | `src/pages/PMI.tsx` | /pmi route · **BROKEN at runtime**: pmi_assets table does not exist in bgobkjcopcxusjsetfob; 20260529_pmi_shared.sql never applied; PMI.tsx errors on load. NOTE: business_pmi_schedule (EXISTS 2026-06-12) is a separate Cost-to-Produce schema table — it does NOT replace pmi_assets and does NOT unblock this page. | CLAUDE.md §4 Active Tasks |
...
| **business_assets table** | EXISTS | `supabase/migrations/20260612_business_assets_inventory_pmi_service.sql` | Structurally verified 2026-06-12 in bgobkjcopcxusjsetfob: RLS enabled (relrowsecurity=true) · owner_all + member_all policies (dual-policy = receipts pattern) · FK→businesses (ON DELETE CASCADE) · BEFORE UPDATE trigger via set_updated_at_generic() · CHECK(ACTIVE/IN_REPAIR/OFFLINE/RETIRED) on status. Follow-up columns via `20260612_business_assets_inventory_cost_confidence.sql`: location text + cost_confidence text CHECK(CONFIRMED/DERIVED/ESTIMATED/UNKNOWN). 0 rows. **NO app module reads/writes this table yet — EXISTS only.** | docs/cost-to-produce/COST-TO-PRODUCE-DESIGN.md §§5-8 |
| **business_inventory table** | EXISTS | `supabase/migrations/20260612_business_assets_inventory_pmi_service.sql` | Structurally verified 2026-06-12 in bgobkjcopcxusjsetfob: RLS enabled · owner_all + member_all policies · FK→businesses (ON DELETE CASCADE) · BEFORE UPDATE trigger · location text col in base migration. Follow-up cols via `20260612_business_assets_inventory_cost_confidence.sql`: receipt_id uuid FK→receipts (ON DELETE SET NULL) + cost_confidence text CHECK(CONFIRMED/DERIVED/ESTIMATED/UNKNOWN). 0 rows. **NO app module reads/writes this table yet — EXISTS only.** | docs/cost-to-produce/COST-TO-PRODUCE-DESIGN.md §§5.3, 14 (RECEIPT ROUTING SEAM) |
| **business_pmi_schedule table** | EXISTS | `supabase/migrations/20260612_business_assets_inventory_pmi_service.sql` | Structurally verified 2026-06-12 in bgobkjcopcxusjsetfob: RLS enabled · owner_all + member_all policies · 2 FKs (→businesses CASCADE, →business_assets CASCADE) · BEFORE UPDATE trigger · tasks jsonb + overrides jsonb + last_service_at for "overdue?" queries. 0 rows. **NO app module reads/writes this table yet — EXISTS only. NOT the pmi_assets table referenced by the shared PMI.tsx module — see PMI page row (BROKEN).** | docs/cost-to-produce/COST-TO-PRODUCE-DESIGN.md §5.4 |
| **business_service_log table** | EXISTS | `supabase/migrations/20260612_business_assets_inventory_pmi_service.sql` | Structurally verified 2026-06-12 in bgobkjcopcxusjsetfob: RLS enabled · owner_all + member_all policies · 3 FKs (→businesses CASCADE, →business_assets CASCADE, →receipts SET NULL) · append-only ledger (no updated_at trigger by design — corrections add new rows) · receipt_id is the COUNT-ONCE dedup seam (PRESENT=receipt is authoritative cost source; ABSENT=this cost field stands). 0 rows. **NO app module reads/writes this table yet — EXISTS only.** | docs/cost-to-produce/COST-TO-PRODUCE-DESIGN.md §14 (RECEIPT ROUTING SEAM) |
