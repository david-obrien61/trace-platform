...
<!-- Last verified: 2026-06-13 (PMI module rewired: BROKEN → WIRED on 3 live tables (business_assets + business_pmi_schedule + business_service_log); barcode_id + task checklist + inspection result enum + AI suggest endpoint (api/pmi/suggest.ts, Claude Sonnet 4.6); migration 20260613_business_service_log_result.sql written — David must apply; build clean 2187 modules) -->
...
| **Vercel functions (12 + 1 pending)** | WIRED | `api/*.ts` + subdirs | 12 live functions: campaigns, dashboard, discovery/ingest, members/invite, orders/submit, qbo/auth-url, qbo/callback, qbo/invoice/cultivar, qbo/status, receipts/ocr, social/enable, social/generate-posts · **AT 12-function Hobby limit** · ⚠️ **13th function added 2026-06-13: `api/pmi/suggest.ts`** — exceeds Hobby limit; Vercel will refuse to deploy until David either (a) upgrades cultivar-os project to Pro or (b) removes one existing function to stay at ≤12. `api/social/publish.ts` (Blotato) already deleted in 35913b2 (2026-06-08) · 3 orphaned source files deleted 2026-06-11 (packages/cultivar-os/api/campaigns/generate.ts, campaigns/publish-post.ts, services/customer-match.ts — were dead code, not in Vercel count) · ⚠️ `packages/shared/src/pages/Settings.tsx:266` has dead fetch() to `/api/services/customer-match` (root gone, will 404) | CLAUDE.md §HANDOFF 2026-06-13 |
