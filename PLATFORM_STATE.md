...
| **D-14 per-project cost-to-produce drill-in (Phase 1, cost-only)** | ✅ BUILDER-COMPLETE 2026-06-18 · OWNER-PROVEN owed | Click a project group in the `/costs` By-project tree → modal showing THAT project's cost in isolation: 2 headline figures (Monthly pool + One-time capital, taken VERBATIM from the group's existing `rollup` — no recompute, reconciles with the tree exactly), monthly pool by category (Labor / Other recurring), capital separate, confidence mix. **ZERO schema** (view-layer; added `cost_category` to the tree SELECT + a widened row type). Pricing/margin/N is Phase 2, intentionally OMITTED (D-14: cost truth ≠ price strategy; Surface Honesty — no dead panel). Files: `ProjectCostDrillIn.tsx` (NEW) + `ProjectCostTree.tsx`. **Reconciliation proof PASSED** (`scripts/verify-project-drill-in.ts`): all groups MATCH on real TRACE tenant, incl. BuiltWithCAI $1,156.67/mo (labor $1,000 + other $156.67) + 2 unknown. `[TRACE:PROJECTLENS]` stays ON. `docs/DECISION-cost-attribution-and-shared-cost.md` · `DECISIONS.md` D-14. See CLAUDE.md §HANDOFF 2026-06-18. |
