...
<!-- Last verified: 2026-06-13 (Receipt Keeper: storage RLS migration + atomic fail + line_items capture; build 2180 ✅; two migrations pending David apply; live test required before advancing to WORKS) -->
...
| **Receipt Keeper v1** | WIRED | `src/pages/ReceiptKeeper.tsx + api/receipts/ocr.ts` | /receipts route (PrivateRoute) · Dashboard header "Receipts" button (isOwner) · GEMINI_API_KEY + ANTHROPIC_API_KEY set ✅ · receipts table applied + 14 cols verified ✅ · PRIVATE bucket created ✅ · STD-010: content-type + size enforced in both layers · Provider fallback: Gemini 2.0 Flash → Claude Haiku 4.5 → 503 · **2026-06-13 fixes**: (1) Storage RLS: `receipts` bucket had no `storage.objects` policies → every upload returned 400. Migration `20260613_receipts_storage_rls.sql` written (3 policies: INSERT/SELECT/DELETE, dual owner+member, `split_part(name,'/',1)::uuid` for AC-3 path isolation — **David must apply**). (2) Atomic fail: storage fail now aborts save entirely — user stays on confirm step, no orphan DB row, no lying `[TRACE:RECEIPT] saved` log. (3) line_items capture: `20260613_receipts_add_line_items.sql` adds `line_items jsonb` column — **David must apply** · ReceiptKeeper.tsx handleConfirm() saves `parsed.line_items` + logs line-item count at both confirm and save points · Build: 2180 ✅ · **Still WIRED** — both 2026-06-13 migrations unverified; live receipt test (image lands in storage + row saves with image_url non-null + line_items populated) required before advancing to WORKS | CLAUDE.md §HANDOFF 2026-06-13 |
| **receipts table** | WORKS | `supabase/migrations/20260612_receipts.sql` | Applied to bgobkjcopcxusjsetfob · 14 cols verified by David · dual RLS: owner (businesses.owner_id) + member (business_members active=true) · updated_at trigger via existing set_updated_at_generic() · **line_items column**: `20260613_receipts_add_line_items.sql` written — `ADD COLUMN IF NOT EXISTS line_items jsonb` — **David must apply** | CLAUDE.md §HANDOFF 2026-06-13 |
...
| **Receipt Keeper v1** | ⚠️ 2026-06-13 FIXES APPLIED — 3 David steps required. **(1) Storage RLS:** apply `20260613_receipts_storage_rls.sql` in bgobkjcopcxusjsetfob SQL editor + run VERIFICATION QUERY (expect 3 rows: receipts_storage_delete, receipts_storage_insert, receipts_storage_select). **(2) line_items column:** apply `20260613_receipts_add_line_items.sql` + run VERIFICATION QUERY (expect line_items, jsonb, YES). **(3) Live test** after Vercel deploy: upload real receipt photo → OCR runs → confirm step → save → verify in Supabase: image lands in receipts storage bucket + row has `image_url` non-null + `line_items` populated. Advance to WORKS after step 3 confirmed. | — |
