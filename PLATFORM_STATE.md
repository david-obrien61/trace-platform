...
<!-- Last verified: 2026-06-12 (Receipt Keeper OCR provider fallback: gemini-2.0-flash primary + claude-haiku-4-5 fallback; build 2180 ✅; deploy + re-test with real photo required) -->
...
| **Receipt Keeper v1** | WIRED | `src/pages/ReceiptKeeper.tsx + api/receipts/ocr.ts` | /receipts route (PrivateRoute) · Dashboard header "Receipts" button (isOwner) · GEMINI_API_KEY + ANTHROPIC_API_KEY set ✅ · receipts table applied + 14 cols verified ✅ · PRIVATE bucket created ✅ · STD-010: content-type + size enforced in both layers · **502 root cause confirmed 2026-06-12**: `gemini-1.5-flash` deprecated by Google → returns HTTP 404 → our handler's non-OK branch returned `res.status(502)` (our own code, not Vercel hard-kill — confirmed by `[TRACE:RECEIPT] Gemini error 404` appearing in runtime logs) · **Fix applied 2026-06-12** (two sessions): (1) prior session: client canvas compress + server AbortController 8s; (2) this session: model→`gemini-2.0-flash` + full provider fallback chain (Gemini 2.0 Flash primary → Claude Haiku 4.5 vision fallback → clean 503) · Fallback log: `[TRACE:RECEIPT] provider-fallback fired: gemini→claude, reason: X` · Provider 3 slot left as comment for future expansion · Build: 2180 ✅ · **Deploy + re-test required** to advance to WORKS — real JPEG (IMG_6886.JPG or IMG_6885.JPG) must OCR end-to-end after Vercel deploy picks up updated ocr.ts | CLAUDE.md §HANDOFF 2026-06-12 |
...
| **Receipt Keeper v1** | ⚠️ PROVIDER FALLBACK APPLIED — real JPEG test required. Root cause confirmed 2026-06-12: `gemini-1.5-flash` deprecated → Google 404 → our handler's own `res.status(502)`. Fix: model→`gemini-2.0-flash` + provider fallback chain (Gemini 2.0 Flash primary → Claude Haiku 4.5 fallback → clean 503). Deploy (git push → Vercel) then re-test with IMG_6886.JPG or IMG_6885.JPG. Advance to WORKS when OCR completes without error end-to-end. | — |
