...
# Last updated: 2026-08-02 (2) — the fourth billing value `core_optional`; one field decides the baseline (#183). See §3.
...
### 2026-08-02 (2) — THUNDER "FREE BUT OPTIONAL" HAD NO LEGAL SPELLING IN THE CATALOG, AND FINDING THAT OUT WAS THE ASSIGNMENT. `price_monthly: 0` is legal under `billing:'core'` ALONE — `add_on` requires > 0 (*"0 would read as free"*) and `unpriced` requires null — and `'core'` forces `enabled:true` at seed. So `contractor_tiers` at $0-no-trial-seeds-dark could not be said, and the one spelling that would have seeded correctly, `'unpriced'`, **asserts the price is UNKNOWN when David had just DECIDED it — D-9 pointed backwards, a fabricated absence in place of a real value.** 🔴 **DAVID'S DIAGNOSIS THEN REPLACED MINE AND IT IS THE BETTER ONE: *"three fields doing one job with contradictory rules is the real defect, and contractor_tiers at $0 is what surfaced it."*** I reported a missing value; he named the cause — so the fix is **`enabledByDefault(billing)`, ONE FIELD deciding the BASELINE**, with a running clock as an OVERRIDE on top rather than a second opinion about it. **FOUR RULINGS LANDED; BUILDER-COMPLETE (`npm run verify` exit 0 zero net-new, `build:cultivar` clean, 1023 assertions / 27 files, `moduleSeed` 151→158 red-first ×2, `verify-tile-fields` 35/35); `20260802b` GATED — David applies; cards 28–29, card 27 AMENDED, board 0 of 29.** ZERO new api-fn (12/12), no new table, no policy change.

**Type:** App code + migration + tooling — **NEW `supabase/migrations/20260802b_module_classification_corrections.sql`** (GATED, 6-query V-block) · `tileRegistry.ts` (`ModuleBilling` += **`core_optional`** · NEW **`enabledByDefault()`** · `moduleSeedRow` reads it · three catalog rows reclassified) · `verify-tile-fields.mjs` (`BILLING_RULE` + probes C17–C20) · `moduleSeed.test.ts` (A16–A22, **B6 rewritten a SECOND time**, B6b re-keyed). Docs: ledger **#183** · **4 NEW RULINGS · 2 OWED rows CLOSED · 2 NEW OWED rows** · 2 NEW cards + **1 AMENDED** · `built-inventory.md` (**a body entry that was stale by TWO builds, corrected**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-01 (2) EVERY REGISTRY FIELD #179), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 B6 HAS NOW BEEN REWRITTEN TWICE, AND ADDING THE VALUE IS WHAT EXPOSED THE BUG IN THE FIRST REWRITE.** It read `billing !== 'core'` to mean BILLABLE — *"live, not core, no clock = free forever with nothing to bill."* **That was correct while `core` was the only free value. `core_optional` is ALSO free, by ruling — so the invariant would have fired on the first such module anyone switched on and called its own design a billing defect.** The money question was never *is it core*, it was **is it BILLABLE**, and the field that says so is `billing === 'add_on'`. Nothing caught this; it fell out of writing the value down, which is the argument for minting a value rather than working around one.

**✏️ A PREMISE CORRECTED, AND THE CORRECTION IS THE USEFUL PART.** The build was directed at a *phantom* — a priced module with no tile, and a cap asserting only tile → catalog. **Neither exists.** `verify-tile-fields` has asserted BOTH directions since it was written (C1 forward, **C2 reverse** — *"a catalog entry NO tile references"*), the lists are **11 ↔ 11**, and `qb_invoicing` has a tile at `tileRegistry.ts:202-203`. **But the instinct was right and only the mechanism was wrong:** FIVE of eleven `module_key`s sit on tiles that no renderer ever draws — `placement:'settings'`/`'admin'`, and **`tilesForPlacement()` has ZERO CALLERS.** All five are reachable through `NAV_IA`, so it is a missing SURFACE, not a missing feature. The *catalog-without-tile-with-a-declared-reason* mechanism is therefore **deliberately NOT BUILT: there is no instance of it**, and a declaration list with no member is exactly what `OWNER_ONLY_PENDING` became (#73).

**THE MIGRATION IS ORDER-INDEPENDENT WITH RESPECT TO `20260802`, BY CONSTRUCTION.** `20260802` keys its UPDATE on a clock being present; `20260802b` STRIPS contractor_tiers' clock, so whichever runs first the end state is identical, and every write is an absolute set rather than a toggle. **That is the deliberate fix for the pair-ordering hazard that bit on `20260801`/`20260801b`** — a correct result should not depend on a human remembering an order. And `cost_to_produce`'s clock is started **through `start_module_trial`, never a hand-written `jsonb_set`**: a second writer of that pair is precisely what the 08-01 one-writer ruling ended, and V3 asserts the RPC's own audit row exists rather than trusting it.

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE FOUR COMING MODULES ARE STILL ON LIVE CLOCKS AND I DID NOT TOUCH THEM — you rule.** `followup_engine` · `business_insights` · `online_shop` · `seasonal_module` count down over features that do not exist. **My recommendation, since you asked which I would do: `trial_days: 0` now, and the clock starts when the tile ships.** It needs no model change (A7/A7b already cover `add_on` + no clock), the start-on-ship work is identical either way so deferring costs nothing, and decisively — **an owner cannot evaluate what does not exist**, which is the *"no basis on which to convert"* that yesterday's ruling was made to end. **(b)** **`unpriced` now has ZERO members and I kept it**, as the honest answer for the next module that arrives before its decision does. **(c)** **MASTER_BRIEF:317's $49 SURVIVES ruling 3** — the *Contractor Portal* is a LOGIN surface that does not exist; `contractor_tiers` is the discount switch. Two products, not a $49 write-off. **(d)** ✏️ **Two backticked fragments in commit `919a9c7`'s message were eaten by shell substitution** (*"it read  to mean BILLABLE"*). Left rather than force-pushed over, per the 08-01 precedent; the reasoning is intact in the code and in RULINGS.md.
