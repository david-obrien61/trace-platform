...
# Last updated: 2026-07-23 — the permission funnel: /team Roles now grants (ledger #152, BUILDER-COMPLETE, migration GATED, owner-prove owed). See §3.
...
### 2026-07-23 — THUNDER THE PERMISSION FUNNEL (David's ruling OPTION 1 on the #151 recon): the /team Roles tab wrote a TEMPLATE store while every gate read the MEMBER store and nothing bridged them — so granting a permission on screen granted nothing. **Two SECURITY DEFINER RPCs are now the ONLY way authority changes; the direct-UPDATE side door (incl. the owner's) is CLOSED. BUILDER-COMPLETE; migration GATED; owner-prove OWED (7 cards).** ZERO new api-fn (12/12).

**Type:** Migration (1, GATED) + app/shared code — **2 NEW** (`supabase/migrations/20260723_permission_funnel.sql` · `packages/shared/src/auth/roleFunnel.ts` + its test) + **~9 EDIT** (`roleDefinitions.ts` — writes retired, `resolveRoleDefaults` added · `members.ts` — `updateMemberRole` retired · `auth/index.ts` · `actionPermissions.ts` — `UNWIRED_ACTION_PERMISSIONS` · `MemberConsole.tsx` — RolesTab funnel-wired + owner lit/locked + blast-radius confirm · `TeamConsole.tsx` — hide fake pills, drop retired prop · `roles.ts` — `DEFAULT_PERMISSIONS` DELETED · `Settings.tsx`/`OnboardingWizard.tsx` — mints read the floor · `pmiInterval.test.ts` — block 3 rewritten · `scripts/verify-universals.mjs` — capF/capG re-point to the funnel). Docs: ledger **#152** · owner-test board (**NEW `team-permissions-full-surface-test.md`, 7 cards all `owed`**) · `DECISIONS-INDEX.md` (ruling + drift line) · `built-inventory.md` · `TRACE-SESSION-BOOTSTRAP.md` ⚡ · recon doc (BLOCKED-B answered) · tech-debt **#66 re-scoped** · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-22 CLOSE-OUT), 1 written — entries-in == entries-out, §3 back to 3.** **NO new decision** (implements D-50 funnel shape + STD-011 + AC-4 + D-9; David's OPTION-1 ruling + 4 sub-rulings recorded). **BENCH: no trigger fires.**

**THE DEFECT.** The Roles tab wrote `role_definitions` (`upsertTenantRole`) while `has_permission`/`has_permission_for` read `business_members.permissions`, and NOTHING propagated one to the other — so "Save MANAGER" changed nothing a gate reads, and `import_pricing` was ungrantable to any manager the hour it was minted (#151's card 18, FAILED live). Three stores of one fact (RD template · member rows · the stale `DEFAULT_PERMISSIONS` constant); the gate consulted one.

**THE FIX — A FUNNEL, ENFORCED AT THE DATABASE.** Two `SECURITY DEFINER search_path=''` RPCs are the only way a role→permission fact changes. `save_role_permissions` authorizes the PASSED actor as owner (D-50 scar — never `auth.uid()`, via `assert_movement_actor`), writes the tenant `role_definitions` row, **re-materializes `business_members.permissions` for every ACTIVE member of that role from the resolved role — WIPE not merge (sub-ruling #1)**, and appends the `audit_log` row (the spine's FIRST governance writer) — in ONE txn, returning per-member before/after. `assign_member_role` does the per-member case. **§1 CLOSES THE SIDE DOOR:** the `enforce_member_authority_immutability` trigger now permits a role/permissions UPDATE only under `auth.uid() IS NULL` (service/migration) OR a transaction-local GUC `trace.authority_funnel` that ONLY the funnel sets — so a direct JWT `UPDATE`, **including the owner's SQL workaround (#8)**, is refused. **Stated honestly (David's 🔴): the service-key/postgres path stays open by necessity — named, not hidden. It is a funnel for every JWT caller.** 13 write sites all route through the funnel or cease to change authority; `updateMemberRole`/`upsertTenantRole`/`deleteTenantRole` DELETED.

**BUILD 3 (screen truth) + BUILD 4 (mint/backfill).** Owner row renders every-pill-lit-and-locked from `owner_id` (BLOCKED-B answered by David's policy dump — every gated table carries an `*_owner_all` policy); the save confirm names the blast radius (who loses what) BEFORE the write; count-on-screen == member array after a save; `DEFAULT_PERMISSIONS` RETIRED, mints read the resolved floor via `resolveRoleDefaults`; the two fake pills (`apply_discount`/`override_maintenance`) hidden via the shared `UNWIRED_ACTION_PERMISSIONS`. A new permission enters the floor OFF and reaches a manager only through the funnel — import_pricing is the live grant case (card 1).

**PROOFS.** `npm run verify` exit 0 **zero net-new** (quality-gate + verify:universals both green; capF/capG updated to assert the funnel, not the retired writers); `build:cultivar` exit 0. Pure `diffPermissions` test **10/0** (RED-first); `pmiInterval` **23/0** (block 3 rewritten to assert the fake pills are hidden). The migration carries V1–V8 catalog-backed checks (incl. V3 = the side-door close refuses a direct JWT UPDATE, V4 = the grant reaches the member row).

**FLAGGED FOR DAVID:** **(a)** the **7 owner-test cards** (`docs/owner-tests/team-permissions-full-surface-test.md`), all `owed` — sharpest #1 (grant → member row → manager's import lands = #151's card 18), #3 (a save that REMOVES a permission names the loss + actually removes it), #4 (owner lit+locked), #7 (self-elevation refused + audited). **(b)** **GATED migration `20260723_permission_funnel.sql`** — apply as postgres + run V1–V8 before the cards; the funnel RPCs do not exist until applied, so this is NOT deployable-and-testable yet. **(c)** **🔴 `permission.self_elevation_denied` is durably audited only when a NON-owner CALLS the funnel** — a raw-SQL direct UPDATE is refused by the §1 trigger but CANNOT self-audit (a BEFORE-trigger RAISE rolls back the txn; Postgres has no autonomous transaction). Stated, not overstated. **(d)** **STILL OPEN, recorded not built (sub-ruling #4 scope):** the standing enforcement map + gate · a verify cap asserting every active member's array == resolved role · R5b coverage gaps. **(e)** **tech-debt #66 RE-SCOPED** — it is NOT anon-only: a MANAGER with `view_orders` passes the route gate but orders/order_items RLS filters every row, so they see `roster {count:0}` on an order they created and read 132 committed units as AVAILABLE. A real permission/RLS gap on the order surface, post-demo.

### 2026-07-23 — THUNDER IMPORT-MATCHER DEFECT FIX (found on the FIRST owner-prove of #148): a foreign item # SOURCED a match (D-47/STD-019 reproduced), and SIZE was compared as an exact string. **Both fixed. BUILDER-COMPLETE; owner-prove OWED (cards 20, 21 + refined 7, 9).** Zero schema, zero migration, 12/12.

**Type:** App/shared code — **2 NEW** (`packages/shared/src/utils/sizeLabel.ts` — the ONE size vocabulary · `sizeLabel.test.ts` **30/0**) + **4 EDIT shared** (`import/importPlan.ts` — deleted the SKU fast path, added `applyForeignId` · `inventory/countPromote.ts` — `sameSizeLabel` re-homed · `inventory/stockLineResolver.ts` — `detectSizeCollision` normalizes · `discovery/catalog.ts` — `normalizeSize` re-homed) + **1 EDIT surface** (`pages/InventoryImport.tsx`) + **3 EDIT test** (`import/importPlan.test.ts` **48/0** · `inventory/countPromote.test.ts` **54/0** — flipped the assertion that encoded the bug · fixture CSV). Docs: ledger **#150** · owner-test board (**NEW cards 20/21**, refined 7/9) · CLAUDE.md tech-debt **#56 ✅** · `DECISIONS-INDEX.md` drift line · `built-inventory.md` · `TRACE-SESSION-BOOTSTRAP.md` ⚡ · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (ONE SELLABLE PREDICATE #147), 1 written — entries-in == entries-out, §3 back to 3.** **NO new decision** (implements D-47/STD-019 + D-23 + D-9 + STD-011). **BENCH: no trigger fires.**

**THE DEFECT, AND WHY EVERY SCREEN LIED.** On the first live owner-prove, the plan bound the file's **Basham's** row to our **Texas Redbud** and **'Sierra'** to **Flip Side Vitex** — "Matched by item # DISC-1101/DISC-1104". The SKU-exact fast path resolved the CSV's item-# column against `business_inventory.sku`, where **`DISC-` codes are OUR internal discovery-scrape ids** (ledger #128), not the grower's namespace. Accepting would have cross-written price+attributes onto two unrelated plants. **This is the QBO-email-match (#53, nine mis-billed invoices) on a new surface — sourcing identity on a field the foreign system shares no guaranteed meaning with.**

**FIX 1 — name + size are the authority; the item # never sources.** The fast path is DELETED. Resolution is the resolver we already own and proved (`resolveCountTarget` / token equality / `detectSizeCollision`). `applyForeignId` lets the item # only **CORROBORATE** or **CONFLICT** — and per David's correction it states the **OBSERVATION** ("their item # also matches our SKU on this row"), never the conclusion ("confirms"): a coincidence between unrelated namespaces is not proof. A disagreement is surfaced red and the row defaults **unchecked**. **RULED SOFT-SURFACE (David):** name+size resolved the lot, so a namespace collision is not lot-uncertainty; hard-holding noise would train the owner to click through holds on a 116-row file. The item # is KEPT as a descriptive attribute under its own header ("Item #"), never our sku (D-24/point 3).

**FIX 2 — SIZE is folded before it is compared (David's addition, would have failed card 9).** The catalog stored sizes as bare trade numbers ("30") while CSV/child rows said "30 gal"; exact-string compare → duplicate CREATEs, and the Shoal Creek 45 CONFLICT would never fire. `normalizeSize` (already in the scrape parser) is **MOVED — not forked (STD-011)** — to the ONE shared home `utils/sizeLabel.ts` (sibling of `canonicalName`), extended to fold a bare integer to gallon-class, and reused by BOTH `sameSizeLabel` and `detectSizeCollision`. **COMPARISON-ONLY — the owner's stored value is never rewritten (D-23).** Closes tech-debt **#56**.

**PROOFS (RED-first per STD-002, both fixes).** Identity: added the two `DISC-` collision rows to the synthetic catalog (absent before — *the exact reason the green test hid the live bug: a fixture that cannot express the failure cannot catch it*) → 13 failed. Size: flipped the `countPromote.test` assertion that literally encoded the bug (`!sameSizeLabel('15','15 gal')`) → failed, then passed. GREEN: sizeLabel **30/0** · importPlan **48/0** · countPromote **54/0** · resolveAgainstCatalog 8/0 · columnMap 20/0 · parseCsv 18/0 · catalog-variants **31/0** (scrape unregressed). `npm run verify` exit 0 **zero net-new**; `build:cultivar` exit 0.

**FLAGGED FOR DAVID:** **(a)** cards 20 (item # never sources) + 21 (cross-format size resolves to one lot), refined 7/9 — all `owed`, nothing marked covered (OP-14). **(b)** **CONFLICT holds the WHOLE ROW** (your question): un-ticked writes nothing; ticked writes qty→CSV AND price 205→310. **(c)** **SIZE blast-radius — flagged, not probed (no live DB here):** the comparison fix does not MERGE existing rows; if one variety already holds two same-size spellings under one group, the picker now reads them as a dup needing merge — a read-only probe is owed at owner-prove (#56's deferred merge half). **(d)** bare-integer→gallon changes the shared `normalizeSize` the SCRAPE also uses (§6 r10) — a scraped bare "45" now normalizes "45 Gallon"; low-risk, flagged. **(e)** the DISC-coded fixture rows are KEPT — they are the live collision, now the card-20 proof. **(f)** `planAgainstLot` (AMBIGUOUS owner-pick) does not run the foreign-id cross-check — owner is explicitly picking. **(g)** everything from the #148/#149 flags unchanged; the customer-import half stays a GAP.

### 2026-07-23 — THUNDER CSV CATALOG IMPORT: the second half of onboarding. A grower's price-list CSV → mapped → per-row plan → Accept, writing on-hand through the D-50 ledger and price/attributes through the patch path. **BUILDER-COMPLETE; owner-prove OWED (14 cards).** One GATED migration; ZERO new api-fn (12/12); zero ledger forks.

**Type:** Migration (1, GATED) + app/shared code — **6 NEW** (`supabase/migrations/20260723_inventory_import_columns.sql` · `packages/shared/src/import/{parseCsv,columnMap,importPlan,index}.ts` · `packages/cultivar-os/src/pages/{InventoryImport.tsx,importWrites.ts}`) + **5 NEW test** (`import/{parseCsv,columnMap,importPlan}.test.ts` · `inventory/resolveAgainstCatalog.test.ts` · fixture) + **4 EDIT** (`inventory/stockLineResolver.ts` — extracted `resolveAgainstCatalog` + 2 import columns · `router.tsx` · `BusinessInventory.tsx` · `scripts/verify-universals.mjs` nav-exception). Docs: ledger **#148** · owner-test board (**NEW `## SURFACE: csv-import`, 14 cards all `owed`**) · `built-inventory.md` (line 4 + NEW capability entry) · `DECISIONS-INDEX.md` (D-24 row + drift line) · `user_stories.md` (CSV-import GAP→inventory-half-written; Populate-LAWNS mechanism) · `TRACE-SESSION-BOOTSTRAP.md` ⚡ · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2B DEFECT FIX #146), 1 written — entries-in == entries-out, §3 back to 3.** **NO new decision** (implements D-24 + D-9 + D-49 + D-50 + D-52 + STD-011 + AC-1). **NEW DAVID RULING recorded (not numbered): a CSV never overwrites a physical count without explicit per-row consent.**

**THE WHOLE BUILD RIDES EXISTING RAILS — ZERO NEW api-fn, ZERO NEW MATCHER, ZERO LEDGER FORK.** Parse in the browser (`parseCsv` — hand-rolled RFC-4180 rather than a new dependency; BOM/CRLF/quoted-comma/embedded-newline/blank-line all tested, 18/0), resolve in a pure module (`importPlan`, 34/0), write through the D-50 RPCs. **STD-011 was honored literally, not gestured at:** the name/size matcher was NOT forked — `resolveAgainstCatalog` was extracted PURE from `resolveStockLine` (which now delegates to it, its efficient targeted-SKU query preserved) and guarded with its own tests (8/0), and the FILL/UPDATE/CREATE branch is D-49's `resolveCountTarget` reused. The 12/12 ceiling is untouched.

**TWO NEW COLUMNS, BOTH SETTLED, NEITHER RE-DERIVED.** `price_basis text` is D-35's missing D-9 pair-half — a bare price is a number with no unit. `attributes jsonb` is D-24's flexible-edge BAG, recorded as NOT BUILT and now built to conform: the spine/blob cut is decided per column by *"will the system OPERATE on it?"* — qty/price/size are RIGID COLUMNS, the grower's descriptive columns land in the blob keyed by their own header, verbatim. Both nullable, no CHECK, no new policy (the 20260628/20260707 precedent). Migration is **GATED — David applies + runs verify (A)-(E)**.

**THE MAPPING IS A FOUR-RUNG LADDER OVER ONE PLATFORM-WIDE DICTIONARY (not per-tenant templates).** L1 exact → L2 synonym → L3 shape (infers from VALUES; **PROPOSES, never auto-applies**) → L4 unmapped (keep-as-attribute default, or ignore). Every mapping shows WHICH rung produced it and is overridable. **🔴 A blob field may never be money:** a currency-shaped unmapped column (`Wholesale`) is FLAGGED "looks like a real field; nothing computes on it" and held in the bag — making it a real column is a spine decision + a migration.

**THE WRITE KEEPS PROVENANCE HONEST, DELIBERATELY.** A qty change goes through `adjust_inventory_manual` with `kind='import'` and a reason naming the file — **never** `count_reconcile`, because that is exactly the signal the CONFLICT rule keys on to protect David's physical counts. A CREATE is `count_promote_create_inventory`(qty 0 → `opening_balance` genesis) then the import move, so a re-import can never false-CONFLICT against an import's own genesis. Price/basis/attributes ride the shared `persistInventoryPatch` (no ledger row — a price is not a movement). Partial failure is surfaced per row (#69).

**FLAGGED FOR DAVID:** **(a)** **the 14 cards** — sharpest: **🔴 #9 Shoal Creek CONFLICT does not overwrite a count by default** (your 2026-07-23 ruling, live), **🔴 #5 Wholesale flagged / nothing computes**, **🔴 #10 $0.00 + blank → UNKNOWN, not sellable**, **🔴 #13 `qty == SUM(delta)` = 0 rows and every qty change names the import**, **#1 `Ready` maps to qty only after confirm**, **#12 nothing written until Accept.** **(b)** **the migration is GATED** — apply `20260723_inventory_import_columns.sql` + run the embedded verify (A)-(E) before the cards. **(c)** **TWO PROMPT-vs-REALITY FLAGS reported, not silently reconciled:** the synonym seed listed `ready→qty` but the fixture+card treat `Ready` as the L3 exemplar, so `ready` is EXCLUDED from the dictionary (the card contract wins); and SCOPE "owner-only" vs BUILD-3 "the /inventory permission group" resolved to route owner-only with server write-authority staying the member-scoped RPCs (a true server-side owner-only lock is an RLS change NOT made). **(d)** **the FIXTURE** `docs/owner-tests/fixtures/test-grower-pricelist-FIXTURE.csv` was CREATED (absent) + labelled FABRICATED — if you place your own, it replaces mine and the cards still hold. **(e)** the **customer-import** half of the story stays a GAP. **(f)** #66/#67/#70 and the rest unchanged.
