...
**Step 1 — schema delta WRITTEN + GATED** (`supabase/migrations/20260617_cost_objects_shape_nature_source.sql`, append-only, **NOT applied** — David applies manually): `cost_shape`(6,NOT NULL DEFAULT ONE_TIME) · `cadence`(5,nullable) · `recurring_amount numeric(10,2)`(distinct from acquisition_cost) · `cost_nature`(CAPEX|COGS|OPEX,NOT NULL DEFAULT CAPEX) · `cost_source`(NOT NULL DEFAULT MANUAL, NO check — sources grow without migration) · `node_type` widened with `COST`. Defaults true for every existing row (all ASSET/ONE_TIME/CAPEX/MANUAL). Shapes 4-6 carry-cols deferred (honest-debt, no writer). **⚠️ SCHEMA-VERIFICATION GATE PENDING (CLAUDE.md §9):** `verify-cost-objects.mjs` extended with catalog checks (J)-(P) but NOT RUN — PAT revoked; David runs it AFTER apply. Schema is BUILDER-WRITTEN, **NOT catalog-proven this session.**
...
**NEXT — David's fresh confirm before I proceed:** (1) apply migration `20260617` + run `SUPABASE_PAT=… node scripts/verify-cost-objects.mjs` (catalog gate J-P). THEN steps 4-8: backfill (recurring config → `cost_objects` rows: nature OPEX, source MANUAL, lossless, UNKNOWN→UNKNOWN) → verify flat total == $12,239.67/mo → flip read source → re-point write path (shape+nature+project pickers + substantiation; keep truncation-guard) → owner-prove. **Labor DEFERRED** (stays in config, flagged not dropped — R3). `[TRACE:*]` stays ON until owner-proven. Next layers NAMED not built: API connectors (via `cost_source`) + AI-classify (manual-first).
