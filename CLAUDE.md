...
# Last updated: 2026-08-23 — campaign lifecycle scoped; edit+cancel are free, delete is a ruling. See §3.
...
### 2026-08-22 (2) — THUNDER **THE CATALOG ANSWERED THE FORK I NAMED, AND I HAD TAKEN THE OTHER BRANCH.** The recon's 🔴 headline — *a platform-wide write outage; the ONE WRITER of `business_modules` cannot reach the database* — **IS DISPROVEN.** `pg_proc` shows `set_business_module_state(uuid, text, boolean, boolean, jsonb, uuid, **integer**)` with `proargnames` matching `moduleState.ts:93-101` exactly and OUT param `trial_started`; the console shows `{applied:true, enabled_before:false, enabled_after:true}`. **`20260802c` IS APPLIED. THE WRITE WORKS.** 🔴 **THE ERROR WAS NOT THE REASONING — PostgREST argument-name resolution is correct and unchanged. IT WAS WHERE THE PREMISE WAS READ FROM:** `CLOSE-OUT-LEDGER.md:22` said *"GATED AND UNAPPLIED"*, **and that is a fact about a DOCUMENT.** The recon closed §L1 with the branch table itself — *"seven → `20260802c` was applied and never recorded, the #22 class one table over"* — **and published the other branch.** ⚠️ **Its OWN STD-021 blind spot #3 said *"applied-state is a DOC read, not a fact"*: naming a limit does not discharge it.** **§L1 is KEPT AND STRUCK, not deleted** — a wrong finding that is quietly removed teaches nobody; one carrying its own refutation is the artifact. **RECON + CORRECTIONS ONLY — no app code, no schema, no migration, no cap.** `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · GATE 0 **NOT APPLICABLE** · all `[TRACE:*]` ON.

**Type:** Corrections + a count + a draft rule. `docs/audits/social-campaign-path-recon-2026-08-22.md` (§CORRECTION · §L1 struck · **Q2/Q4 REOPENED** · **THE COUNT** · the closing query moved to the TOP) · `CLOSE-OUT-LEDGER` **#187 cell corrected + #191b** · `tech-debt-log` (**#91 amended · #92 NEW**) · **`ui-control-standards.md` NEW §6** · `RULINGS.md` (2 OWED rows) · front-page · built-inventory · DECISIONS-INDEX drift.

**🔴 AND THE CORRECTION REVEALED THE SHARPER DEFECT THAT THE WRONG HEADLINE WAS CONCEALING — A WORKING WRITE BEHIND A BROKEN READ IS DATA LOSS, NOT A DISPLAY BUG.** `SocialSetup.tsx:59-75` falls back to `defaultChannels()` — **instagram-only, exactly David's 14:15 screenshot** — and cannot distinguish that from a real selection (`error` never destructured at `:67`; `.catch()` at `:74` cannot see a PostgREST error). **Pair it with a Save that genuinely persists:** `:100` sends the **WHOLE array**, `20260801:241`'s top-level `||` replaces `advert_channels` **wholesale**, and `:109` **navigates away without re-reading** — so **one click of a button that looks like a no-op silently destroys four enabled channels, and the one surface that could contradict it never looks.** ⚠️ **NOT ASSUMED — TESTED BY QUERY, and the query is now at the TOP of the recon** with a three-outcome branch table (`tiktok`/`twitter` present ⇒ the read is the ENTIRE defect · instagram-only ⇒ a second `social_drafts` writer exists · no row ⇒ the seed residual), **plus an `audit_log` scan whose `config_keys_patched` says whether a Save ever landed.**

**📏 THE COUNT (#174 — measure before proposing): 30 CONFIRMED, AND 30 IS A FLOOR — 61 further sites match the shape and were NOT individually read, so the ceiling is 91.** Stating 91 is false precision; stating 30 alone understates the class by two thirds. 🔴 **THE DECISIVE RESULT IS AN ARGUMENT AGAINST A HELPER: `readPricingConfig` ALREADY RETURNS `{data, error}` CORRECTLY, and ALL SEVEN of its callers destructure only `data` and coalesce `?? {}`.** **The shared function did the right thing and every call site undid it — the helper was never the problem.** Two adjacent sub-classes counted apart: **7 auth reads** (🔴 `callerPermission.ts:148` is a SECURITY path — **#75's open ruling at a second location**) and **9 HTTP-body parses** (`res.json().catch(() => ({}))` before `res.ok` — **Q6's second error surface, nine sites not one**).

**FLAGGED FOR DAVID:** **(a)** 🔴 **TWO MIGRATIONS RECORDED PENDING WHILE APPLIED, IN ONE EVENING, BOTH FOUND BY QUERYING `pg_catalog` RATHER THAN READING A DOC** — #22 (`20260609`) and now `20260802c`. **Plus ~129 inline CHECKs a `conname` grep cannot match. THE REPO'S RECORD OF THE DATABASE IS UNRELIABLE IN BOTH DIRECTIONS**, and the two directions are different failures: the repo claims work outstanding that is DONE, and the catalog holds objects the repo cannot MATCH. **Three instances behind the SCHEMA-SNAPSHOT CHECKER, owed since the `businesses` recon → now tech-debt #92, NUMBERED AND DELIBERATELY NOT BUILT.** **(b)** 🔴 **NEW DRAFT RULE `ui-control-standards` §6/R1 — A READ WHOSE ERROR PATH RETURNS A VALUE MUST KEEP "FAILED" DISTINGUISHABLE FROM "EMPTY."** A9 on the read side, where we never carried it. **It does NOT require knowing the correct value — only that information is not destroyed.** **DRAFT; David rules.** **(c)** ✅ **WHAT SURVIVED THE CORRECTION, checked not assumed:** C-1 (the INLINE constraint IS in version control), C-2 (the config store exists), **Q1 (the two CHECKs disagree, non-subset both ways — now the ONLY confirmed live defect on this path)**, Q5, Q6, Q7 and tech-debt #22 all STAND; they are repo facts and never rested on the database. **(d)** **Q2 is now "it works" and Q4 is "clean, and now OBSERVED clean rather than inferred"** — smaller findings than the ones they replace, and the truthful ones. **(e)** **STILL NOTHING FIXED BY INSTRUCTION:** no cap, no config store, `campaign_posts_platform_check` NOT widened (a MIGRATION, David's), orphans NOT deleted.

### 2026-08-21 (3) — THUNDER **THE RECON DISPROVED THE PREMISE IT WAS COMMISSIONED ON, AND CLOSING `businesses` MEANT NOT MERGING IT.** `verify-write-paths` counts a path as a FILE, so the platform's RLS anchor read as **seven competing writers**. The column matrix says otherwise and the partition is clean: **CREATION** (`id`/`owner_id`/`business_type`/`trial_started_at`) · **IDENTITY** (`name`/`address`/`phone`/`email`/`website`) · **ACCOUNTING** (`accounting_*`, written by an OAuth callback and a token refresher — **a machine act with no form behind it**), **ZERO overlap in either direction.** Three different ACTS. **Collapsing a signup INSERT, a settings form and a token refresh because a cap counted files would have been the wrong build** — per #168 the honest floor is **THREE, not one**, reached here independently from a different table. **DECLARED with reasons; `businesses` flips `GOAL:FAIL → GOAL:PASS`, goal count 16 → 15.** `npm run verify` **exit 0, ZERO NET-NEW** (5 / 247 / 10 / 12 / 15, byte-identical to the session baseline) · api/ **12/12** · **NO migration, NO policy, NO schema change** · all `[TRACE:*]` ON.

**Type:** Declaration + one type fix + a NEW cap + docs. `scripts/verify-write-paths.mjs` · **NEW `scripts/verify-profile-writer-boundary.mjs`** (chained into `npm run verify`) · `packages/shared/src/context/BusinessProvider.tsx` · `scripts/test-member-login.mjs` · `package.json` · **NEW `docs/owner-tests/businesses-write-paths-full-surface-test.md`** (4 cards, board 0 of 4). Docs: ledger **#190 / #190b / #190c** · **TWO new RULINGS rows** · ⚡ ACTIVE STATUS · `built-inventory.md` · DECISIONS-INDEX drift. Commit `93d8fde`. **§3 RETENTION: 1 archived verbatim (2026-08-02 (7–8)), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 BOTH SLATED REPOINTS WERE STOPPED BEFORE ANY CODE WAS WRITTEN, AND PERMISSION WAS NOT THE BLOCKER.** That was checked FIRST and both would have PASSED: `has_permission_for` is **owner-inclusive by `owner_id`** (`20260726_permission_alias_layer.sql:312-314`) and both sites run after their member row exists — ledger #47's *"auth-free"* describes the front-door **REVEAL** (the service-key ingest fetch), never the write-back. **The real blocker is that the recon compared column SETS while `set_business_profile` SETs all five identity columns UNCONDITIONALLY (`20260727_rbac_flip_corrections.sql:59-64` — no COALESCE, no mask). IT IS NOT A PATCH API,** and a subset writer cannot call a full-record writer without inventing values it does not hold. **`OnboardingWizard:608`** writes only `address`; its state is `{name,address}` (`:439`) and **every** business SELECT it makes is `select('id, name, address')` — routing it sends `p_email=null` and **WIPES the email `OwnerSignup.tsx:277` inserts at signup.** **`DiscoveryGlimpse:183`** writes ONE owner-chosen column and holds none of the other four, so routing widens a 1-column act into a read-modify-write with a clobber window (*narrower and correct beats wider and convenient*, 07-31). **Both declared VISIBLY as second IDENTITY writers, not folded in quietly.**

**🔴 AND D1 WAS WRONG ABOUT ITS OWN MECHANISM, WHICH CHANGED THE FIX.** #189 reported `test-member-login.mjs:144` dying `42703` at **step 5 of 34** with *"29 steps unreachable"*, surviving because the harness is not in `npm run verify`. **The harness is RETIRED** — `refuseRetired()` aborts at **IMPORT** (`:23-30`, 2026-07-30, superseded by `npm run verify:rls`) — so it never reaches step 1, there were never 29 steps to report, and it survived because it **REFUSES TO RUN**. ✏️ **The file declares itself *"THE RECORD OF WHAT RAN, NOT A THING TO RUN"*, so the line is KEPT and ANNOTATED rather than deleted** — the script DID run with that column when it existed, and deleting it would make the record lie. *(The instruction said "remove it, one line"; the premise was disproven, so the record-preserving form was taken and flagged rather than either done silently. One edit to reverse — David's call.)*

**D2 IS FIXED AND IT WAS A PHANTOM.** `Business.tax_rate: number` — **non-nullable, on a column `20260727e:45` dropped** — was **invisible to `tsc` precisely because the loader uses `select('*')`**, so the field never arrived and no type error could fire. **The sweep found ZERO readers anywhere:** both money paths already read through the narrow `get_business_tax_rate` RPC via `fetchTaxRate` (`CartReview.tsx:54`) and `resolveTaxRate` (`submit.ts:280`), and both say so in their own comments. ✏️ **`types/nursery.ts:12` also declares a `tax_rate`** — that is `nurseries`, the pre-`businesses` generation pending DROP, a different table. **Named, deliberately not touched.**

**NEW CAP — THE SENTENCE THAT *WAS* THE SECURITY BOUNDARY IS NOW A CHECK.** The migration already said *"⚠️ THE COLUMN LIST IS THE SECURITY BOUNDARY… `owner_id`, `accounting_*`, `business_type` are UNREACHABLE BY CONSTRUCTION"* — **and that was a COMMENT.** The recon's most durable finding is why it needed teeth: **`business_type` is protected by nothing but the absence of code that writes it** (no column-level RLS; it selects **which tiles a tenant sees**), and **`owner_id` is safe only IMPLICITLY** (`USING` with no `WITH CHECK`, applied as the UPDATE check by default). 🔴 **The SET list is DERIVED from the function source — no column list is typed in the cap** — so `logo_url` would pass and `business_type` fails (**STD-025: if David rules it MAY be edited, the cap goes red with no code change, which is correct and intended**). **RED-FIRST against the REAL migration (STD-024):** both columns planted, each FAILING and NAMED, then restored **byte-identical**. **16 probes** incl. a **negative control**, **absent-function as a HARD FAILURE** (the 08-01 zero-row false-green), **newest-wins both directions**, and **a commented-out later definition must not win**.

**FLAGGED FOR DAVID:** **(a)** 🔴 **SEVEN REMAINS A FLOOR, NOT A TOTAL — the schema-snapshot checker is OWED and NOT built here.** A function created outside the migration path stays invisible (§6 r17). Assume one more writer exists than was found. **(b)** ⚠️ **Q6's residual is unchanged and LATENT:** six of nine app sites don't check affected rows; harmless only because `businesses` has **no member UPDATE policy** — **the day a manager reaches a direct update here, six sites go live at once.** **(c)** ✏️ **STD-021 SELF-CORRECTION, recorded in the cap's own header:** disabling the comment-stripper **still PASSES** on the real corpus (the warning sits above the `UPDATE`, outside the SET-list window), **so stripping is NOT what saves this cap today** — the header says so rather than claiming a save it did not make. **(d)** 🔴 **`verify-write-paths` still prints `goal: 15 table(s)` beside the prose `17 failures` IN ONE SENTENCE** — #188 flagged this at 16-vs-17 and the gap has WIDENED by this build. **Named again, still not fixed.** **(e)** **CARDS 34–37 OWED, and card 4 is non-negotiable** — D2 touched a type on the money path.
