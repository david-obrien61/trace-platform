...
# Last updated: 2026-08-31 — #242 positions: the chooser's missing assertion, and a branch push that deployed nothing — see §3
...
### 2026-08-31 — THUNDER **POSITIONS: A BUSINESS DESCRIBES A JOB BEFORE IT FILLS ONE, AND THE HARD QUESTION IS REPLACED BY ONE EVERY OWNER CAN ANSWER. R-30 AND R-31 FILED. #240.** 🔴 **THE HEADLINE IS THAT DAVID'S TWO STAGE-0 DECISIONS REVERSED THE PROMPT'S OWN MECHANISM, AND ONE OF THEM COULD NOT HAVE WORKED AT ALL.** The prompt said *"only show responsibilities whose module is on — a tenant without the grow ladder never sees 'uppot a lot'."* **There is no `grow_ladder` module_key.** `MODULE_CATALOG` holds eleven keys and the grow ladder is unbuilt, so pointing 16 rows at it would have hidden them from everyone or forced minting a no-cost key — which the 2026-08-01 ruling already rejected and `verify-tile-fields` assertion 2 enforces. David ruled the axis is **VERTICAL**, and gave the better reason: *"a nursery should see the growing responsibilities whether or not they have bought the tile, because the job exists whether or not the software covers it."* ✏️ **The second: `order_compliance_records` was cited for chemical application records and it is an ORDER-SCOPED customer accept/decline receipt** (`order_id NOT NULL`, `decision IN ('accepted','declined')` — the Regina netting record, which the manifest says in as many words). Making `order_id` nullable on a live table to force the fit is worse than an honest blank. **Both are ruled as [[R-30]].** **BUILDER-COMPLETE** `19291f1`→`4fce9f9` · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **51/51 files · 2374 assertions** (+1 file, +48) · **8/8 MUTANTS CAUGHT, MEASURED NOT CLAIMED** · **ONE migration, THREE new tables, ZERO existing table/policy touched** · api/ **12/12, none added** · all `[TRACE:POSITIONS]` ON.

**Type:** Stage 0 recon (G1–G5, three lenses) → build → close-out. Code: **NEW `packages/shared/src/positions/`** — `responsibilityCatalogue.ts` (93 rows) · `responsibilityMarks.ts` · `positionDescription.ts` · `positionFields.ts` · `positionStore.ts` · `positions.test.ts` (48 assertions) · **NEW `supabase/migrations/20260831_business_positions.sql`** · **NEW** `Positions.tsx` / `PositionBuilder.tsx` / `PositionDescription.tsx` · `router.tsx` + `tileRegistry.ts` NAV_IA + `AdminIndex.tsx` (three lines) · `verify-universals.mjs` (two drill-in exceptions, in the pattern the file already uses). Docs: **`docs/RULINGS.md` R-30 + R-31** · tech-debt **#130 + #131** · ledger **#240** · **NEW `docs/owner-tests/positions-full-surface-test.md`** (8 cards · 0 covered · 7 owed · 1 needs-test) · `built-inventory.md` · `user_stories.md` (the story, filed) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-30 (5), the ItemRef entry), 1 written — entries-in == entries-out, §3 back to 3** (archive **267 → 268**, counted with `grep -c '^### '`). ✅ **Own worktree** — `thunder/position-builder`, with three other worktrees live.

**🔴 THE FINDING WORTH MORE THAN THE BUILD IS THAT `permissions: []` WAS CARRYING THREE MEANINGS AND WOULD HAVE PRINTED A LIE ABOUT US.** The draft's header said *"sixteen rows are in that state"* — and the measurement says **twenty rows carry an empty array**. The 16 was right about *"no capability yet"* and wrong as a description of the array: the other four are **PPL-09 "train new staff", which needs no software and is not a gap**, and **PPL-10, which is partly built and reserved to the account holder.** ✏️ **Inferring "not built" from an empty array would have printed *"the software cannot represent this yet"* on a document an owner hands to a person, about work TRACE has nothing to do with — telling them the product is more incomplete than it is.** So `capability` is a declared four-value field with no third option. **This is A9/D-9 — absent is not empty — applied for the first time to our own honesty about what we have built**, and it is [[R-30]]'s first half. **The second half is the product decision that makes the feature work: a `none` row is TICKABLE and prints exactly like a covered one**, because the description says what the JOB is, not what the app covers.

**🔴 AND THE SECOND, WHICH IS THAT THE MARKS COULD NOT BE HAND-DECLARED AND THE DRAFT PROVED IT.** The story asks the picker to mark three kinds *in consequences and never in permission strings*. The draft hand-set `ownerOnly: false` on **nine rows that cite an `owner-only` string** — `team:*`, `subscription:*`, `audit_log:read`, including `MON-07` which my own Stage 0 count missed. Every one is undelegable **by construction**: `CATALOG_PERMISSIONS` filters `owner-only` out of the grantable catalog, so the string cannot be granted to anyone whatever a hand flag says. **Nobody transcribed them wrongly — the fact simply lives somewhere else**, which is what a second list always means (STD-011). All three marks now derive from `permissionManifest.ts`, and the SENSITIVE text is the manifest's own `exposure` sentence. ⚠️ **One exposure leaked a permission string into owner-facing copy** — `margin`'s trailing *"It requires costs:read…"*, measured as the only one of five — so a sentence carrying a `resource:verb` shape is dropped rather than the manifest edited: the Roles page still wants that sentence, because there the string IS the subject.

**FLAGGED FOR DAVID:** **(a)** ✅ **APPLIED, CATALOG-VERIFIED AND MERGED THE SAME DAY — `main` is `3bb36ff`.** David ran it and reported from the catalog: RLS on all three tables · six policies · 🔴 **V3 PASSES — exactly three write policies, all naming `settings:update`, so no over-wide policy slipped in (#124's class)** · ✅ V4 no rows · 🔴 **V5 PASSES — floor UNTOUCHED at OWNER 57 / MANAGER 25 / STAFF 10**, so *the scope bar held as an assertion rather than as a claim*. One trigger, the standard one. **The merge was clean with ZERO conflicts — `origin/main` never moved off `19291f1` during the build, and the shared counters were re-read AT MERGE (ledger 239 · tech-debt 129 · R-29) rather than carried from the build.** **(b)** 🔴 **CARD 5 IS THE BAR AND IT IS A JUDGEMENT, NOT A CHECKLIST: *would you hand this to someone on Monday?*** If it reads like a template with the blanks filled in, **fail it and name the sentence that sounds generated.** ⚠️ **Lauren's corrections to your four hand-written descriptions are the specification and had not come back at close** — where the generated one is thinner, that difference is this build's actual backlog. **(c)** 🔴 **CARD 7 IS THE WALL ASSERTED IN THE NEGATIVE.** As STAFF: the page opens (a staff member reading their own description is the point), every control is disabled, and **a save that reported success while nothing changed would be worse than an error** — that is #238's silent degradation. Every write asserts an exact row count for exactly this reason; the card is the proof it holds under real RLS. **(d)** ⚠️ **THE WRITE GATE IS `settings:update` AND IT IS MY DERIVATION, FLAGGED FOR OVERRULE.** Derived from the ACT per the 2026-07-31 ruling: describing a job creates no role and grants nothing. **`team:update` was rejected in writing** — gating a job DESCRIPTION behind the authority to change who holds POWER is the conflation this feature exists to break, and `team:*` is owner-only so it would put the surface out of reach of the manager the story is about. **(e)** ✅ **THE ZERO-ROW CAP FOUND A REAL DEFECT, NOT A FORMALITY.** The pick-clearing DELETE was `UNCHECKABLE`; a DELETE refused by RLS returns zero rows with no error, **and zero rows is ALSO the legitimate answer for a position with nothing ticked** — indistinguishable from inside the function, so a staff member unticking everything would have been told *"Saved."* The caller now passes the count it loaded. **(f)** ⚠️ **TWO THINGS FILED AND NOT TAKEN:** tech-debt **#130** — replacing ticks is DELETE-then-INSERT and **the sequence is not atomic** (#69's class; the durable fix is one RPC and that is a migration); **#131** — `responsibility_id` has **no FK** because its parent is a code constant, so nothing detects an orphaned pick. **(g)** ⚠️ **`STATUS: needs-build` WAS WRITTEN ON THE STORY AGAIN — R-26 instance 1, ten weeks later, by the same person.** Corrected to `needs-input`, and **the recurrence is recorded on the card rather than the instance**: a vocabulary documented in prose and enforced by nothing gets retyped, and R-26's own mechanisable fix (derive the filter from the data) is unbuilt for this board. **(h)** ⚠️ **NO RLS PROOF SCRIPT WAS WRITTEN, AND I AM SAYING SO RATHER THAN SHIPPING AN UNRUNNABLE ONE.** #238 shipped 13 assertions it could not run; there is still no DB reachable from here, so the migration's V-blocks — which **you** run — are the catalog-backed proof, and CARD 7 is the live one. **(i)** ⚠️ **STILL OPEN AND UNTOUCHED:** `docs/inventory-functions.md` / `inventory-env.md` / `inventory-ai.md` stamped `2026-06-13` · `docs/reference/` still tracked by accident (`8bbaec4`, still yours to revert) · `20260830c` written and not applied · tech-debt **#124**'s eighteen over-wide policies · **#125** · **#126** · **#127**.
