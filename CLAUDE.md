...
# Last updated: 2026-08-22 — social/campaign recon; two premises corrected; #22 resolved, #91 new, #23 escalated. See §3.
...
### 2026-08-22 — THUNDER **TWO OF THE THREE PREMISES THIS RECON WAS COMMISSIONED ON DID NOT SURVIVE THE REPO, AND BOTH CORRECTIONS MADE THINGS WORSE RATHER THAN BETTER.** **① `campaign_posts_platform_check` IS IN VERSION CONTROL** — `20260529_campaigns.sql:26-27`, declared **INLINE** on the `CREATE TABLE`, so Postgres auto-names it and **the string is never typed anywhere**; a grep for the constraint NAME could never have found a constraint committed since May. **So it is NOT a new STD-008 inverse instance and was deliberately NOT filed as one.** **② THERE IS NO CHANNEL-CONFIG TABLE AND THE CONFIG STORE EXISTS ANYWAY** — `business_modules.config.advert_channels`, established by `20260608`, written by ONE writer, read by BOTH generators and the setup page. **Q8's option (c) is not a proposal; it is the status quo.** 🔴 **AND THE REAL DEFECT IS WORSE THAN THE SUSPECTED ONE: the two CHECKs DISAGREE and are NOT a subset in either direction** — `social_drafts` takes 5 (`…tiktok, twitter, sms`), `campaign_posts` takes 4 (`…sms, email`). **`email` is a value NOTHING in the platform can produce; `tiktok`/`twitter` are values the ONLY config UI offers as ticked checkboxes.** The campaign path can be configured, through its own supported UI, into a state its own table refuses. **RECON ONLY — no code, no schema, no migration, no policy, no cap, no fix.** `npm run verify` exit 0, ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · api/ **12/12** · **zero diff under `packages/`, `api/`, `supabase/`** · GATE 0 **NOT APPLICABLE** (no app code ships) · all `[TRACE:*]` ON.

**Type:** Recon + ledger correction. **NEW `docs/audits/social-campaign-path-recon-2026-08-22.md`** (HAVE/NEED/WANT per OP-8, eight questions answered with `file:line`, the nine-enumeration table, STD-021 corpus + **eight blind spots**). Docs: tech-debt **#22 RESOLVED** · **#91 NEW** · **#23 ESCALATED** · ledger row · ⚡ ACTIVE STATUS · `built-inventory.md` · DECISIONS-INDEX drift. **§3 RETENTION: 1 archived verbatim (2026-08-21, the registry measurement), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 THE LIVE DEFECT IS NOT A SOCIAL-MEDIA DEFECT — IT IS A PLATFORM-WIDE WRITE OUTAGE, AND IT ANSWERS Q2, Q3 AND Q4 AT ONCE.** `moduleState.ts:93-101` — **THE ONE WRITER of `business_modules`** — sends **SEVEN** named arguments including `p_trial_days`. The only migration creating a 7-arg `set_business_module_state` is **`20260802c`, recorded GATED AND UNAPPLIED** in `CLOSE-OUT-LEDGER.md:22`. **PostgREST resolves an RPC by its ARGUMENT-NAME SET, so an argument no live function declares is PGRST202 — not a default.** Every call through the one door fails: SocialSetup's Save, the marketplace `Enable`, and both `cost_to_produce` writes. ✏️ **#187 records the OPPOSITE direction — *"old bundles keep working because PostgREST binds by NAME and `p_trial_days` DEFAULTs to NULL"* — which is true of old-bundle→new-function. The live pairing is the INVERSE and has no such protection. Gating the migration was correct; shipping the client that REQUIRES it was not gated with it, and that asymmetry is the finding.** ⚠️ **The apply state is read from the LEDGER, not the catalog — there is no `psql` here. One `pg_proc` query settles it and it is not mine to run: six args → live defect; seven → `20260802c` was applied and never recorded, which is #22's class again on the same evening one table over.**

**🔴 THE ORPHAN MECHANISM IS CONFIRMED AND HAS A SECOND HALF NOBODY NAMED.** `campaigns` commits at `campaigns.ts:103-116`, `campaign_posts` inserts at `:129`, **no transaction spans them** (#69's class — Step 1's reading is correct). **The half that explains ZERO posts rather than SOME: `:129` is a SINGLE multi-row `.insert()`, atomic, so one rejected channel rolls back EVERY post.** ✏️ **AND THAT EXACT SENTENCE IS ALREADY IN THIS REPO ABOUT THE OTHER TABLE** — `20260609:10`: *"the insert is atomic, one sms row rolls back all rows."* **Diagnosed for `social_drafts` on 2026-06-09, fixed for `social_drafts`, never carried to `campaign_posts`, which had the narrower constraint the whole time. #22 and #91 are ONE defect on TWO tables and only one was repaired.** The catch at `:135-138` also never deletes the row it created 20 lines earlier — **every failed generation leaves a permanent orphan, which is why there are exactly two.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **#22 WAS APPLIED AND NOBODY WROTE IT DOWN — it read *"David must apply"* for 74 DAYS while the live constraint already matched `20260609:33` byte-for-byte.** Filed as the STD-008 inverse's **THIRD FORM: migration-applied-but-recorded-as-pending** — *the most dangerous of the three, because the other two make the platform look broken while this one manufactures a backlog entry that is not real, and a backlog with false rows is one people stop reading.* **Same shape as #187's missing row and the 19-days-late OWED flip — the defect fixed, the record not, twice in one month.** **(b)** 🔴 **#23's SWEEP IS STILL OWED AND 08-22 CHANGED WHAT IT MUST DO: the corpus holds ~129 INLINE `CHECK (` declarations, every one invisible to a name-based grep and every one of which would read as a false positive to a sweep keyed on `conname`.** A sweep crying wolf 129 times on its first run gets discarded wholesale (#78's ruling). **Match on `pg_get_constraintdef`, both directions.** Two instances on adjacent tables in ONE evening is an **unmeasured class** per #174 — named with a number rather than left as a memory. **(c)** ⚠️ **Q3 and Q7 ARE ONE DEFECT ON TWO PAGES:** `SocialSetup.tsx:67`/`:74` and `Campaigns.tsx:50`/`:56` both **discard `error`**, so *empty*, *denied* and *errored* render identically — a **DEFAULT presented as a SELECTION** (instagram-only) and an **orphan-filled tenant presented as "No campaigns yet."** The six-state ruling already governs this. **(d)** ✅ **THE ONE THING THAT IS ALREADY RIGHT, said plainly: the config write is E5-CLEAN BY CONSTRUCTION** — it is an RPC returning `applied`/`reason`, and `enable.ts:60-63` surfaces a refusal as a **403 with the reason**. **That is what E5's answer looks like when built, and it already exists on this path** — while three draft-review writes beside it check nothing at all. **(e)** ⚠️ **Three of this path's four `[TRACE:*]` flags are `false` by default** (`campaigns.ts:5`, `generate-posts.ts:5-6`, `SocialSetup.tsx:7`) — the emits exist but are silent. Worth a look against the STD-003 standing instruction; **named, not changed.** **(f)** **NO WIDENING PROPOSED — `campaign_posts_platform_check` is a MIGRATION and it is David's to apply; the orphans are David's to delete.**
...
### 2026-08-21 (2) — THUNDER SEVEN WRITE PATHS TO THE PLATFORM'S RLS ANCHOR, AND THE COLUMN MATRIX SAYS THEY ARE **THREE DISJOINT ACTS, NOT SEVEN COPIES OF ONE.** That is the finding and it is the opposite of what "seven undeclared paths" reads like. `businesses` holds `business_id` (what AC-2 scopes to, what AC-3 makes absolute), `address` (the delivery origin) and `business_type` (which drives `verticalsForBusinessType`, i.e. **which tiles a tenant sees**) — so a stray writer here is not the `customers` risk class. **The matrix splits clean: CREATION (`id`/`owner_id`/`business_type`/`trial_started_at`) reachable from TWO sites and no others · IDENTITY (`name`/`address`/`phone`/`email`/`website`) from five · ACCOUNTING (`accounting_*`) from four, with ZERO overlap in either direction.** The genuine duplication is **exactly two sites**, both inside IDENTITY and both strict SUBSETS of the narrow RPC that already exists. **RECON ONLY — no code, no schema, no migration, no policy, no cap, no fix; zero diff under `packages/`, `api/`, `supabase/`.** `npm run verify` exit 0 zero net-new · api/ 12/12 · GATE 0 **NOT APPLICABLE** (no app code ships).

**Type:** Recon + two doc close-outs. **NEW `docs/audits/businesses-write-paths-recon-2026-08-21.md`** (HAVE/NEED/WANT per OP-8, eleven questions answered explicitly, the column matrix, STD-021 corpus+method+**five blind spots**). Docs: ledger **#187 WRITTEN** (the missing row) + **#189** · RULINGS OWED row FLIPPED · ⚡ ACTIVE STATUS · `built-inventory.md` · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 3 archived verbatim (2026-08-02 (6) #186, (5) #185, (3) #184), 3 written — entries-in == entries-out, §3 back to 3.**

**🔴 TWO LIVE DEFECTS, AT THE TOP OF THE REPORT AND NOT FIXED, AND THEY ARE THE SAME DROPPED COLUMN TWICE.** `20260727e:45` dropped `businesses.tax_rate` (David's ruling: config wins) and the ⚡ front-page records **27e APPLIED**. **D1: `scripts/test-member-login.mjs:144` still INSERTS `tax_rate: 0.0825`** — against an applied 27e that is `42703`, and it is **step 5 of 34**, so the **29 steps after it are unreachable**. It survived because **the harness is not in `npm run verify`** (confirmed absent from `package.json` and `run-tests.mjs`) — nothing has run it since 27e. **D2: `BusinessProvider.tsx:124` still TYPES `tax_rate: number`, non-nullable**, on a column that no longer arrives; benign today only because the provider uses `select('*')` and **no select anywhere names the column.** ⚠️ **Both rest on 27e's apply state, which I read from the front-page and could NOT verify — there is no `psql` here. One query settles it and it is not mine to run.**

**🟢 THE QUESTION DAVID HAS BEEN ASKING FROM THE SCREENSHOTS IS ANSWERED, AND THE ANSWER IS NO.** `/settings/all`, `/settings/business`, `/settings/accounting`, `/settings/services` are **ONE route** (`router.tsx:176`) and **ONE component** with a `section` prop filtering which card renders. The Business Profile card's Save calls **one** function calling **one** RPC. **The section filter did NOT fragment the writer.** Accounting's columns are written by the OAuth callback — a machine act, not a form — and Services writes a different table entirely.

**AND THE STRONGEST THING IN THE RECON IS THE MODEL WORKING:** `set_business_profile` is gated `settings:update`, **audits its denials**, and its column list is the security boundary — with the migration saying so at the line: *"owner_id, accounting_*, business_type and everything else on this table are UNREACHABLE from here BY CONSTRUCTION."* Q9 found **exactly one** function writing `businesses` and **no triggers**; per the standing assumption **7 is a FLOOR, not a total** — a function created outside the migration path is invisible (§6 r17), and the schema-snapshot checker that would see it is OWED and does not exist.

**FLAGGED FOR DAVID:** **(a)** **Q6 — six of nine app sites don't check affected rows and are ALREADY in `zero-row-writes-baseline.json` as unchecked.** LATENT, not live: `businesses` has **no member UPDATE policy**, so a non-owner UPDATE matches zero rows and reports success — but every member-reachable path routes through the RPC, which returns `applied`/`reason`. **The day a manager reaches a direct update here, six sites go live at once.** **(b)** 🔴 **`business_type` is protected by nothing but the absence of code that writes it** — RLS has no column-level restriction and the owner's own UPDATE policy would permit changing it. `owner_id` is safe, but **implicitly**: `businesses_owner_update` specifies `USING` with no `WITH CHECK`, and Postgres applies USING as the check on UPDATE. **(c)** **Q11 answered from git, not inference: `businesses` was NEVER declared** — `ALLOWED_DIVERGENCE` has three entries and none is this table; the count moved 6→7 when `13d920d` folded RPC callers into their targets. **(d)** **Q10: seven parallel write/type enumerations plus twenty select sites carrying fourteen distinct column lists, none derived** — more than the six `customers` had before E6 phase A. **(e)** **NO MERGE ORDER PROPOSED — the matrix is the input that ruling needs, and per the precedent the honest floor looks like THREE, not one.**
