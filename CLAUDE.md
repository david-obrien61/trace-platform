...
# Last updated: 2026-08-02 — A running trial means the module is LIVE (#182); the seed spec reversed. See §3.
...
### 2026-08-02 — THUNDER A 138-ASSERTION TEST SUITE WENT GREEN OVER THE DEFECT, BECAUSE THE MODEL WAS WRONG IN THE TEST AND IN THE MAPPING IDENTICALLY. `moduleSeedRow` mapped `enabled = (billing === 'core')`, so all seven paid add-ons seeded **DARK with a thirty-day clock running over them** — and invariant **B3 asserted exactly that**, forbidding `enabled && start_trial` on the reasoning that a module *"cannot be both included and expiring."* **That is what a trial IS.** The guard and the code were the same mistake written twice, so nothing could disagree with anything. **`enabled:false` is a FUNCTIONAL GATE, not a label** — `api/social/generate-posts.ts:52` refuses on it — so social_media counted toward a $19/mo conversion **while the server refused to generate a single post.** **THAT IS THE FINDING, and the feature is small beside it.** **TRIAL DISPLAY + SEED CORRECTION COMPLETE; BUILDER-COMPLETE (`npm run verify` exit 0 zero net-new, `build:cultivar` clean, 1016 assertions / 27 files, `moduleSeed` 135→151 red-first ×16); `20260802` GATED — David applies; cards 26–27, card 23 AMENDED, board 0 of 27.** ZERO new api-fn (12/12), no new table, no policy change.

**Type:** App code + migration + tooling — **NEW `supabase/migrations/20260802_trialling_modules_are_live.sql`** (GATED, 6-query V-block) · **NEW `packages/shared/src/business-logic/trialClock.ts`** + **NEW `trialClock.test.ts`** · `tileRegistry.ts` (`moduleSeedRow` + the doctrine block reversed) · `useModules.ts` (**`config` is READ**) · `Tile.tsx` (the countdown annotation) · `Dashboard.tsx` · `seedBusinessModules.ts` · `business-logic/index.ts` · `moduleSeed.test.ts`. Docs: ledger **#182** · **1 NEW RULING + 1 marked SUPERSEDED-IN-PART** · **2 NEW OWED rows** · 2 NEW cards + **1 AMENDED** · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-01 THE NOISY RATCHET #177), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 LIGHTNING'S CORRECTION, RECORDED AS LIGHTNING'S.** The ruling I built against on 2026-08-01 was *"paid is seeded disabled with the clock started"*, and a trial-display build was then scoped on top of it — **which would have shipped a renderer for a state the data could not express.** David's correction: *"the site goes live, EVERYTHING WORKS, and fourteen days later the unpaid modules go behind the fuzz. The clock is what ENDS access, not what withholds it."* **It was caught by reading the row against the MODEL rather than against the RULING** — the row was exactly what the ruling asked for, so no check keyed on the ruling could ever have found it. The 08-01 RULINGS row is marked **SUPERSEDED IN PART and left standing**, not rewritten: the rest of it (a row for every module, core enabled, `billing` already being the core-vs-paid field) is correct, and *a ruling edited to look like it was always right teaches nobody why the reversal happened.*

**🔴 THE SECOND, QUIETER FINDING — `useModules` SELECTED `config` AND DROPPED IT.** The trial pair landed in the row on 08-01 and **nothing read it**, so a module on a live 30-day trial and a module nobody had ever touched rendered the same pixel. **It was invisible to tsc AND to knip**: `config` appears in the select string and in `BusinessModuleRow`, so it is "used" everywhere except at the one line that matters. Three row-states collapsed to one — no row, disabled-no-trial, and disabled-mid-trial — where `20260801c`'s own header had already named the first two as the silent-seed-failure hazard and did not extend the list to the third.

**`configured` FLIPS WITH `enabled`, AND THAT IS NOT A RIDER.** `useModules` renders `active` only on both, so correcting `enabled` alone would have fixed the data and left the dashboard showing `[ENABLE]` on all seven — **the customer-visible symptom surviving its own fix.** The precedent was already ruled and already live: `qb_invoicing` seeds `configured:true` with no QuickBooks link, because a green tile says *included*, not *connected*. **NO new `TileState`** — a trialling module is `active` and the countdown is an ANNOTATION, per David's scope collapse.

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE MIGRATION IS THE DELIVERABLE — nothing on your dashboard changes until it is applied.** `20260801c`'s `ON CONFLICT DO NOTHING` is correct and unchanged (it is what makes the seeder safe as the repair path), and it means the code fix reaches every FUTURE tenant and **cannot touch the 11 rows already on disk**. Card 26. **(b)** **`unpriced` is OWED and it now costs something visible:** `cost_to_produce` + `inventory_intake` are BUILT AND WORKING, have no price, get no clock, so they seed DARK and render `[ENABLE]` forever on a purchase nobody can make — the dead-affordance class arriving through a different door. **Not guessed in the migration.** The mapping needs no change either way: price them and they go live on a clock; mark them core and they go live permanently. **(c)** **`[ENABLE]` still writes NOTHING** — `onEnable` and `onNavigate` are literally the same handler (`Dashboard.tsx:706-707` → `openTile`), so it is a navigate dressed as a purchase. Correctly the marketplace's job (ITEM 3); stated plainly here so it is not read as fixed. **(d)** **Expiry is NOT built** — nothing flips a lapsed trial back off, so a trial that runs out keeps working. That is the fuzz, filed, and the countdown deliberately renders nothing at `0` rather than claiming an expiry the platform does not enforce.

### 2026-08-01 (3) — THUNDER A COMPATIBILITY BRIDGE WAS SILENTLY DEFEATING THE MIGRATION IT WAS WRITTEN BESIDE. `moduleState.ts`'s pre-migration fallback matched a **generic `does not exist`** alongside the two real not-found codes — so a transient PostgREST schema-cache miss would have routed a `business_modules` write **AROUND both authority gates** into a direct upsert, which RLS then discards with **no error and zero rows**. Not dead code: live code behind a condition that could still fire, in the one file whose header calls itself *THE ONE WRITER*. **Same class as a guard the write does not depend on (STD-023) — but arriving through ERROR HANDLING rather than through a check**, which is exactly why nothing saw it. **THAT IS THE FINDING OF THIS BUILD, and it is not the feature.** **ITEM 2 COMPLETE + THREE RULINGS LANDED; BUILDER-COMPLETE (`npm run verify` exit 0, `build:cultivar` clean, `moduleSeed.test.ts` 138 assertions red-first ×4, knip 14→13 / 17→15 re-locked); `20260801c` GATED — David applies; cards 23–25, board 0 of 25.** ZERO new api-fn (12/12), no new table, no policy change.

**Type:** Migration + app code + tooling — **NEW `supabase/migrations/20260801c_module_seed_and_trial_clock.sql`** (`start_module_trial` + `seed_business_modules`, 12-query V-block) · **NEW `packages/shared/src/business-logic/seedBusinessModules.ts`** · `tileRegistry.ts` (`moduleSeedRow`/`catalogSeedRows` + the MASTER_BRIEF reconciliation, in place) · `OwnerSignup.tsx` (**`moduleCatalog` REQUIRED in the config**) · `SignUp.tsx` · `AddBusiness.tsx` · `OnboardingWizard.tsx` · `moduleState.ts` (**bridge DELETED**) · `business-logic/index.ts` · **NEW `moduleSeed.test.ts`** · `verify-write-paths.mjs` (one declaration) · `write-paths-baseline.json` · `quality-baseline.json`. Docs: ledger **#181 — and #180, which was MISSING** · **7 NEW RULINGS · 3 OWED rows CLOSED · 1 NEW OWED row** · 3 NEW cards · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-31 (5) BUILD 2 #176), 1 written — entries-in == entries-out, §3 back to 3.** ✏️ **This entry was AMENDED IN PLACE when the three rulings landed, not written as a fourth** — it is the same build, and a fourth entry would have archived #178's for what are this build's own rulings arriving.

**🔴 WHY NOTHING MECHANICAL CAUGHT THE BRIDGE, WHICH IS THE PART WORTH KEEPING.** `verify-write-paths` **correctly reported ONE path** for `business_modules` — because it *is* one path. The cap's unit is a FILE, and it cannot see that one BRANCH of that file's one path bypasses the RPC and its two gates. **A cap that reads which ERROR BRANCHES reach a write is buildable and is NOT BUILT** — named in `RULINGS.md` so the gap is a decision rather than an oversight. The bridge is deleted in the commit that had the standing to delete it (both migrations applied + proven), exactly as its own header promised it would be.

**THREE RULINGS, ALL THREE REVERSING OR SETTLING DRAFTS IN THIS SAME FILE.** **(1) CORE IS RENDERED, NOT MINTED** — a `module_key` on the owner dashboard would make **the shell every tile renders into a purchasable row**, and `verify-tile-fields` assertion 2 is RIGHT to reject a catalog entry no tile references: **the rejected option was rejected by a cap that was correct.** `billing:'core'` already carries the fact and the marketplace reads it whether or not a row exists — **two rows seeded, five shown**, a rendering concern rather than a data one. **(2) 🔴 THE TERM IS SNAPSHOTTED** — the row carries the PAIR `(trial_started_at, trial_days)` and the catalog governs **NEW trials only**, *because a value nobody ratified, inherited from Ignition, silently governing live tenants is the shape that has cost the most this week.* Reading the length at render meant 30 → 14 would **retroactively expire every tenant seeded more than fourteen days ago, with no event marking it.** Checked against STD-011 and cleared: the catalog is the OFFER, the row is the TERM GRANTED. **And the same short-circuit that refuses to restart a clock refuses to rewrite its term**, so the repair path cannot re-term a tenant either. **(3) CORE READS `active` DAY ONE** — `configured:false` put an **`[ENABLE]` button on a working, already-included feature** (dead-affordance, the same defect that kept `campaigns` off `module_key`), and it left this build with **no visible change at all** — *and a build with no observable change reads as a failed deploy*, which is how a real deploy failure gets waved through.

**🔴 THE SILENT-FAILURE ANSWER, unchanged and still the reason this shipped the way it did.** No row ⇒ no clock ⇒ **a customer who is never billed** — and **`useModules.ts:104` renders a MISSING row and a disabled row IDENTICALLY**, so an unseeded tenant looks exactly like a normal one on every screen there is. The RPC **returns COUNTS not a boolean**, it is **idempotent** so the repair path IS the create path, and **the onboarding call sits OUTSIDE the legacy-create branch** so a signup failure self-heals. **RESIDUAL NAMED: an owner who abandons onboarding with both calls failing has no rows and nothing notices** — that home is ITEM 3. David's ruling moved the detection query **onto the owner-test board as CARD 25**, and the reason generalises: *a query that lives only in a migration nobody reopens is a question nobody asks*, and this defect answers no other way.

**FLAGGED FOR DAVID:** **(a)** **the 14-vs-30 trial length is now filed OWED and is no longer urgent** — MASTER_BRIEF:243 and BD-2 say 14, `Help.tsx:524` tells a customer 30, the donor code assumed 30. With the term snapshotted it is a **scheduling question**, answerable before the first real signup rather than before the demo; one customer-facing sentence currently disagrees with the code. **(b)** ✏️ **ledger row #180 was MISSING** — the `20260801` migration claims it in its own header and no row existed. I wrote it, marked as written by another session, correct-don't-duplicate. Found by grepping for the number, not by anything mechanical. **(c)** ✏️ **`quality:baseline` stamps `generated` in UTC**, so it wrote `2026-08-02` at 21:15 CDT on the 1st while every doc says 2026-08-01 — cosmetic, one field, flagged rather than fixed inside a build about something else. **(d)** **`qb_invoicing` reads `active` on a tenant that has never connected to Intuit** — the tile says *included*, not *connected*, and the QB connection indicator is a separate surface. Considered, recorded at the line and on card 23, not missed.

### 2026-08-01 (2) — THUNDER EVERY REGISTRY FIELD WITH A READER WAS CORRECT OR NEARLY SO; EVERY FIELD WITHOUT ONE HAD DRIFTED. That is the whole finding, and it is why the answer was a cap and not a cleanup. `campaigns.status` was wrong for **nine weeks** — the tile said COMING SOON while `nav_campaigns`, the SAME tile at the SAME route, opened the working page. `qr_checkout.kind` said `'action'` (tap-and-done) long after Nav C2 pointed it at a full `/orders` workspace. `nav_eligible` was wrong on **SEVEN of 33 rows in BOTH directions** and nothing could see any of it. **A declaration nobody reads is a declaration nobody corrects.** **ITEM 1 of 3 COMPLETE; BUILDER-COMPLETE (`npm run verify` exit 0 zero net-new, `build:cultivar` clean, 840 assertions); cards 21–22 owed, card 19 AMENDED, board 0 of 22.** ZERO new api-fn (12/12), no schema, no migration, no RLS.

**Type:** App code + tooling — `tileRegistry.ts` (**`nav_eligible` DELETED** · `kind` collapsed `action`\|`context` → **`destination`** · **`campaigns.status` → `live`** + regrouped to `growth` · `seasonal_module.vertical` → **`cultivar`**, the registry's first non-`general` row · `module_key` declared on the five paid add-ons) · **NEW `scripts/verify-tile-fields.mjs`** (15 probes) · `package.json` · `quality-baseline.json` (**knip.unusedExports 14 → 13, win locked**). Docs: ledger **#179** · **4 NEW RULINGS** · 2 NEW cards + 1 AMENDED · `built-inventory.md` · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-31 (4) THE FOURTH STATUS #175), 1 written — entries-in == entries-out, §3 back to 3.**

**`nav_eligible` WAS DELETED, NOT FIXED, AND THE REASON IS THE ONLY ONE THAT MATTERS: `NAV_IA` ALREADY HOLDS THE FACT.** The flag declared *"can be pinned to the optional per-business nav bar"* — a surface never built. `NAV_IA` shipped instead as a hand-authored list, so the flag became a second representation of one fact (**STD-011**), maintained by hand, read by nothing. It had drifted in both directions: `add_business` / `discounts` / `cost_to_produce` declared `false` **while sitting in NAV_IA by `tileKey`**; `inventory_manual` / `inventory_intake` / `opportunities` / `followup_engine` declared `true` with no node. Fixing seven values would have restored a disagreement that has no reason to exist.

**🔴 THE CAP'S RULES ARE DERIVED FROM THE SOURCE, AND THAT IS THE DESIGN.** The required-field list is read out of `interface TileEntry` (`name?:` = optional); the legal value sets are read out of the `export type Tile* =` unions in the same file. **Add a field or a union member and it is enforced here in the same commit, with no second edit.** A hardcoded copy of either would be precisely what this cap exists to prevent, one layer up — which is exactly what `OWNER_ONLY_PENDING` became (#73). **T14 and T15 prove the derivation rather than asserting it**: `vertical:'cultivar'` passes where a hardcoded `general`-only list would have rejected it, and adding `'widget'` to `TileKind` makes it legal with no edit to the cap.

**🔴 SELF-CAUGHT, AND IT IS THE ARGUMENT FOR STD-022 IN ONE LINE.** The first parser used `indexOf('[')` and matched the **empty pair in the TYPE ANNOTATION** (`TileEntry[] = [`); depth-matching closed instantly, the cap read **ZERO ROWS**, and it returned **GREEN over a registry it had never opened**. Caught **only** because nine BAD probes went 0-for-9 — **the third false-green-caught-by-probes this week**, after the `::if#` keys and the field-lists `?`. Second self-catch: probe T15 appended to the union *after* the semicolon, outside the capture, so the one probe proving the cap is derived was proving nothing. Both are commented at the line.

**WHAT WAS DELIBERATELY NOT DONE, and it is a scope call not an omission:** `placement` / `depends_on` / `group` **wiring**. Placement's renderer belongs to the Admin marketplace build — David's own reasoning, that building a separate Settings/Admin renderer first is the work twice. `depends_on` wiring changes the grid days before the demo on a prerequisite rule nobody has ruled on. `group` values are ruled LAST, and they need it: three taxonomies are overlaid today (`readout` restates `kind`, `planned` restates `status`) and **`'planned'` is applied to 5 of the 8 planned tiles.**

**FOUR RULINGS RECORDED, three OPEN.** The trial clock lives in **`business_modules.config`, written only by an RPC** — `created_at` is disqualified by DATA, not preference: the 2026-06-04 `INSERT SELECT` **preserved** timestamps from `nursery_modules`, so a trial read off live rows **expired before it began** (#70's genesis-row shape). The module seeder is a **sibling of `seedPricingConfig` called from BOTH creation paths in one commit** — its own comment already says why. The marketplace gate is a NEW owner-only **`subscription:read` / `subscription:update`**, no delete verb; **`settings:update` was disqualified on evidence** — it is in `MANAGER_DEFAULT_BUNDLE`, so a manager could change what the business PAYS TRACE from the string that lets her fix the business hours. `destination` kept.

**FLAGGED FOR DAVID:** **(a)** **card 21 as MANAGER** — Campaign Scheduler green-dotted, opening the same page as the nav item; its FAIL clause includes *"shows `[ENABLE]`"*, because adding a `module_key` with no `business_modules` row behind it would put a dead button on a two-month-old working feature. **(b)** **card 19 was AMENDED and the reason generalises**: it listed six tiles to look for and **four could not render** (campaigns is now live; Online Shop / Contractors / Seasonal are `placement:'settings'`, which renders nowhere). A card that asks David to hunt for tiles that cannot exist teaches him to distrust the board. **(c)** **build order confirmed: RLS narrowing → seeder + clock RPC → marketplace.** The RLS work carries a dependency that must be STATED not assumed — **an owner with no `business_members` row reads ZERO module rows and sees `[ENABLE]` everywhere**, and `20260730b_owner_member_row_invariant` is what guarantees that row. **(d)** ✏️ **two corrections of mine, stated not absorbed:** the pushed commit message says `ledger #178` when it is **#179** (left rather than force-pushed over), and the first DECISIONS-INDEX edit **deleted eleven historical drift-watch entries** before `git diff --stat` caught it (`1 insertion, 21 deletions`) — restored and re-done as an insert. **The thing that caught it was reading the diff before committing, not the edit.**
