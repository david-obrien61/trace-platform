...
# Last updated: 2026-09-01 — #250 the document-number matcher, #251 the bounded delivery read; R-36 + R-37 — see §3
...
### 2026-09-01 — THUNDER **BOTH BRANCHES MERGED — AND THE MOST USEFUL THING BUILT WAS A TWENTY-LINE CHECK THAT WENT RED THREE TIMES AND CORRECTED ITSELF TWICE. #249. R-35. TECH-DEBT #142.** 🔴 **THE HEADLINE IS THAT THE SESSION OPENED ON A STAGED REVERT OF `main`'s HEAD COMMIT AND NEARLY MERGED ON TOP OF IT.** Eleven doc files sat staged, byte-identical to `83dc2c0` — the PARENT of HEAD — i.e. the entire #246 close-out undone: **R-32 and R-33 deleted from `RULINGS.md`**, ledger row #246 reverted, owner-test cards and `built-inventory` entries stripped. Nothing this session created it. **Merging with it staged would have folded the deletions into the merge commit, where they would read as the merge's doing.** Stopped, proven by comparing every staged blob against `83dc2c0:` and `HEAD:`, and surfaced to David, who ruled DISCARD. ✏️ **A `git status` that says `M` eleven times says nothing about DIRECTION** — the deletions were invisible until the blobs were compared against two commits, and "restore to HEAD" is only safe once you know HEAD is the good copy.

**Type:** Two merges + a build + close-out. **`thunder/fulfilled-tap` FIRST** (David's order: inventory-inert by construction, so it cannot corrupt anything while being proven), then **`thunder/history-order-lines`**. Three collisions settled AT MERGE per #233: **ledger** — both claimed #247, first-in keeps it, second renumbered **#248**; **migration** — both minted `20260831c_`, the fulfilment one became **`20260831d_`** because `20260831c_orders_qb_invoice_uidx.sql` is **APPLIED** and §6 r1 protects it (an unapplied file on an unmerged branch is being NAMED for the first time, not edited); **nine doc ledgers** — both sides kept, incoming first. ⚠️ **THE RENAME'S SECOND HALF WAS THE PART THAT NEARLY ROTTED:** a filename grep found two citations, and a BARE `20260831c` grep found **eleven more** — six of them in code comments and **one in user-facing copy on the stop card**. R-29's lesson arriving through a filename.

**🔴 THE BUILD IS A SENTENCE, AND THAT WAS THE RULING RATHER THAN A COMPROMISE.** `deliveries.status` and `orders.status` are two columns with **no code path between them** — `DeliverySchedule` writes the first and never reads the second. So a completed stop sits against an OPEN order that still HOLDS its commitment (D-52): for a checkout order with lot-anchored lines, **available is UNDERSTATED and on-hand OVERSTATED at the same time.** David ruled the tap **stays inventory-inert** — *"a control that cannot corrupt stock while being proven is worth more than one that does the whole job on day one"* — so `openOrderNotice` NAMES the divergence instead of fixing it, and the decrement gets its own build. **[[R-35]]** also rules the split-delivery clause **in advance**: one order, two trucks, one back — flip the order only when **no other unfulfilled delivery remains on it**. ⚠️ **`holdsCommitment` was EXPORTED rather than "open" re-derived** — on that comment's own standing instruction (*"promote it to an export when a real consumer appears"*), which is the first time that note has been acted on.

**🔴 AND THE FINDING WORTH MORE THAN THE MERGE: A CHECK BUILT TO CATCH ONE DEFECT WAS WRONG IN BOTH DIRECTIONS BEFORE IT WAS RIGHT.** The §3 retention rule has failed three times, and the root cause is structural: **each branch verifies `entries-in == entries-out` against the main it BRANCHED FROM, so the check is true on both branches and false at the merge** — nothing had ever verified it post-merge. `scripts/verify-handoff-retention.mjs` is the mechanism. **It went RED on the real tree three times**, and twice the RED was its own fault: **① it cried wolf** — two DIFFERENT 2026-06-09 sessions legitimately share a title (161 lines vs 97), so heading-comparison called real history a duplicate; **② it then MISSED the real one** — the merge had wedged the archive's own preamble INTO one copy, so the two bodies differed by four lines of boilerplate and whole-text comparison declared them distinct. ✏️ **Both failures are the same lesson from opposite sides: the thing being compared was not the thing that matters.** A check that cries wolf gets deleted, which is how the guarded thing starts failing again — so both corrections are recorded at the code as the REASONS the code is shaped that way, not as tidy final logic. **5/5 mutants on `openOrderNotice`, and M1 SURVIVED FIRST** (probe I3b passed a null id AND a null status, so the next guard returned first and the guard under test was never exercised) — **R-33's exact shape, found by mutating rather than by reading.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE STAGED REVERT IS DISCARDED, NOT EXPLAINED — I do not know what created it.** R-32, R-33 and the #246 close-out are intact; if you staged it deliberately, say so and it can be re-applied as its own commit. **(b)** 🔴 **CARD 12 IS THE NEW TEST AND ITS SILENCES MATTER AS MUCH AS ITS SENTENCE:** a stop with no linked order must show **nothing**, and one whose order already reads Fulfilled or Cancelled must show **nothing**. **(c)** 🔴 **THE INVENTORY COMPOSITION CANNOT BE PROVEN BY ANY DATA IN THE TENANT, AND THE BOARD SAYS SO RATHER THAN LETTING A GREEN RUN STAND IN FOR IT** — the nineteen ingested stops and Saturday 08-29's six captured stops **all carry NULL lots**, so they commit nothing and decrement nothing; it needs a real **CHECKOUT** delivery order with a lot-anchored line, and none exists. **(d)** ⚠️ **`20260831d` IS STILL GATED AND UNAPPLIED** — SQL editor, not the table editor (§6 r17). Until you run it, the stop cards honestly say the control is unavailable. **(e)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs stamped `2026-06-13` · `docs/reference/` tracked by accident · `20260830c` unapplied · tech-debt **#124** · **#125** · **#126** · **#127** · **#132** · **#134** · **#135**.

### 2026-09-01 — THUNDER **THE NINETEEN STOPS HAVE THEIR LINES — AND TWO DIFFERENT BLINDNESSES, ONE MINE AND ONE THE KEY'S. #248.** 🔴 **THE HEADLINE IS THAT `A $0 LINE IS A NOTE` IS OUR OWN RULING, IT IS CORRECT ON THE WRITE SIDE, AND IT IS FALSE READ BACK THE OTHER WAY.** [[R-28]] says a $0 line is a note and carries no item — that is what we EMIT. Their books do something else: invoice **#3648.563** totals **$0.00**, carries a real ship date and a real address, and holds **two real trees** — *Blue Point Juniper (Replacement)* and *Arizona Cypress Blue Ice (Replacement)*, warranty replacements the customer already paid for once. **They are $0 precisely because they were already bought.** ✏️ **A `$0`-means-note rule drops both, the 21 September day sheet reads empty, and a crew arrives at a real address with nothing on the trailer.** So the discriminator is **`DetailType`, never the amount** — `SubTotalLineDetail` is Intuit's running total and is never a line (**exactly one per invoice, 1,469 of 1,469** — counting it doubles every order), `DescriptionOnly` is the note (**194 across the corpus, not one carrying a non-zero Amount**), and a `SalesItemLineDetail` at $0 is a thing that was given away. 🔴 **AND THE MONEY GUARD RUNS ANYWAY: a `DescriptionOnly` line that DOES carry money is kept as a line and counted**, because there are zero of those in 1,469 invoices today and dropping money is the failure nobody notices. **BUILDER-COMPLETE** on **`thunder/history-order-lines`, NOT MERGED** · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **56/56 files · 2802 assertions** (+160) · **24/24 MUTANTS CAUGHT, MEASURED against a green control** (14 on the lines, 10 on the guard) · api/ **12/12, nothing minted** · ZERO Intuit writes · ONE migration, **OWED**.

**Type:** Build + close-out. Code: **NEW `invoiceOrderLines.ts`** (pure: the line parse, the role classification, the money) · **NEW `historyOrderWriter.ts`** (the only new writer; orders + order_items + the join + **the prior-order guard**, and nothing else) · **NEW `QboOrderIngest.tsx`** · `shipmentIngest.ts` (`QboShipmentRow` gains `lines` + `totalTax` — **the same read one level deeper**) · `historyOrder.ts` (**a THIRD door: `receiptId` nullable, pre-built `lines` accepted — both invariants untouched**) · `qbo/router.ts` (two branches, **no new function**) · `vercel.json` · `Settings.tsx` · **ONE migration, `20260831c`, WRITTEN AND NOT APPLIED.** Caps: three declarations (`verify-write-paths` ×3, `verify-field-lists` ×2) and **one cap CORRECTED** — `verify-zero-row-writes` recognised `.length !== 1` and not `.length === 1`, the same inspection written the other way. Docs: ledger **#248** · tech-debt **#139 + #140 + #141** · **NEW owner-test board (11 cards, 0 covered)** incl. **CARD 4b**, and **CARD 4's own arithmetic corrected** — the count no longer rises by nineteen automatically, because a matched sale is not a created one · the delivery board's **CARD 8 rewritten** · `built-inventory.md` · `user_stories.md` (the story, filed) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-31 (5)), 1 written — entries-in == entries-out, §3 back to 3** (archive **274 → 275**, `grep -c '^### '`). ✅ **Own worktree** — `thunder/history-order-lines`, off `main` at `21cb3f2`, with seven other worktrees live. ⚠️ **LEDGER AND TECH-DEBT NUMBERS ARE PROVISIONAL AND MUST BE RE-TAKEN AT MERGE** — David's instruction this session, and ledger #233's own lesson.

**🔴 AND THEN DAVID CAUGHT THE SECOND BLINDNESS, WHICH WAS THE MORE DANGEROUS ONE AND WAS NOT IN THE CLASSIFIER AT ALL — IT WAS IN THE KEY'S INPUT.** *"The nine existing LAWNS history orders carry no `qb_invoice_id` — they came from the OCR capture, not from QuickBooks. The key is blind to them and the import would create a second order for each."* ✏️ **The idempotency key was correct and its INPUT was incomplete**, which is a different failure from a wrong key and much harder to see: `ON CONFLICT (business_id, qb_invoice_id)` does exactly what it says, over a column nine real orders do not have. **And it is not theoretical — SEVEN of the eighteen future-dated invoices carry a `TxnDate` of 26 or 27 August, the exact window those nine were captured in, and SIX of them share ONE date.** 🔴 **THIS IS THE THIRY SHAPE AT NINE TIMES THE SIZE AND WITH MONEY ATTACHED** — one day earlier the delivery ingest hit the identical defect on a hand-entered stop, and the answer there was a second guard keyed on the customer rather than on the id. The answer here is the same in shape and stricter in substance: **the PRIMARY key is the customer's OWN `source_document_number`** — the same string Intuit returns as `DocNumber`, so a match on it is an **identity, not an inference** — with David's three fields (customer, date, amount) as **corroboration**. That ordering is [[D-47]]'s three-way rule and its scar is **#53**, where matching on one field the external system permits to collide cross-billed nine real invoices. 🔴 **ONLY AN IDENTITY WRITES ANYTHING, AND ALL IT WRITES IS THE ID** — two identifier columns on the existing order, guarded `qb_invoice_id IS NULL`, **never its money, status, dates or lines** (asserted on the patch key set, not promised in a comment). **A document number matching over disagreeing money is REPORTED, never reconciled**; an unanchored two-of-three match is reported; more than one candidate is never picked between. All of it renders **above the refusals** on the preview, because *"do not reconcile silently"* is a requirement about the screen and not about a log. **+10 mutants, 10/10 caught.**

**🔴 AND ONE OF THOSE TEN GENUINELY SURVIVED FIRST, WHICH IS THE PART WORTH KEEPING.** N3 — *restrict the candidate set to `order_kind = 'history'`* — broke nothing, because the matcher's own §L tests hand it candidates directly and **cannot see the READ that builds them.** ✏️ **The unit was well tested and the seam above it was not** — tech-debt **#134**'s class exactly, and the reason it matters is that an ordinary CHECKOUT order never pushed to QuickBooks is **equally** invisible to the key and equally duplicable. Closed end-to-end and re-measured. ⚠️ **A fixture defect surfaced in the same hour and is fixed at the class rather than the instance:** the recording stand-in copied seed ARRAYS and shared the row OBJECTS, so an `update` in one test mutated the fixture a later test spread from — §M's clash case silently inherited a `qb_invoice_id` an earlier commit had written, so the ordinary key handled it, **the guard never ran, and three assertions failed against correct code.** *A negative case that is secretly a positive one proves nothing* — the delivery-ingest suite has that sentence in its own fixtures, about a different trap. Rows are deep-copied now.

**🔴 THE SECOND FINDING IS THAT THE SPEC'S OWN LOAD-BEARING NUMBER WAS THREE DIFFERENT FACTS, AND CHECKING IT IS THE ONLY REASON THE RULING IS RIGHT.** The prompt said *"624 of their invoices bake '(Install & Warranty)' into the plant's own line description"* and offered it — correctly, as a signal rather than a fact — as a possible derivation for `service_type`. **Measured against the 2026-08-29 raw capture: 1,469 invoices carry 164 with the parenthesised form, 259 with the phrase in any form, and 624 merely containing the word *"install"* anywhere.** ✏️ **624 is real; it is just not the number for the claim it was attached to** — 42% of the corpus sounds like a strong signal and 11% does not. On the eighteen future-dated invoices: **4 parenthesised · 7 in any form · 1 naming an explicit `Tailgate Delivery` (a DELIVERY-ONLY signal, and the stronger of the two) · 10 with no signal at all.** So the parenthesised reading sets 4 of 18 and misses 3; the loose reading sets 7 and leaves 10. **`service_type` stays NULL and the question goes to David** (tech-debt #140) — a guessed crew is worse than an unset field, and this is [[R-26]]'s shape caught by checking a declaration before using it rather than after.

**🔴 AND THE THIRD, WHICH IS ABOUT MY OWN TOOL: THE MUTANT HARNESS REPORTED A SURVIVOR THAT WAS NOT ONE.** M14 came back **SURVIVED**; the test file was in fact red. The harness grepped the runner's output for the word `FAIL`, and the runner prints *"RED — 1 test file(s) failing"*. ✏️ **A detector that can report the wrong answer, inside the tool measuring for [[R-33]].** Fixed to key off the **exit code**, and a **CONTROL** was added that asserts the unmutated suite is green — because without it every `CAUGHT` below could have been a suite that was already red. **14/14 now, measured.** Two caps also earned their keep during the build and are worth naming: `verify-write-paths` and `verify-field-lists` both went red on the new writer within seconds, and **both resolved to a declaration rather than a build**, which is exactly the 2026-07-29 ruling working as intended.

**FLAGGED FOR DAVID:** **(a)** 🔴 **NOTHING RUNS UNTIL `20260831c_orders_qb_invoice_uidx.sql` IS APPLIED, AND ITS *BEFORE* QUERIES ARE NOT CEREMONY.** Query (b) must return **zero rows**; if it returns anything, two orders already point at one invoice and that is a defect to look at, not a state to index around. The index is created **with NO predicate**, which is #246's one-day-old lesson applied before it could repeat — and `historyOrderWriter.test.ts` **§K reads the real migration corpus off disk** and fails if a predicate ever comes back. **(b)** 🔴 **CARD 5 IS THE CARD, AND IT WANTS BOTH HALVES.** The screen prints *"available to sell did not move"* after a commit — that is the app's own arithmetic over every lot, before the first write and after the last. **Also write down one lot you recognise before you press Record, and check it after.** The panel is an argument; your lot is an observation, and the board does not record arguments as passes. **(c)** 🔴 **CARD 4b IS NOW THE SECOND CARD TO RUN, AND IT CHANGES CARD 4's ARITHMETIC.** The preview shows a panel above everything: *"N of these sales are already recorded here — no second copy was created."* A row reading **"will match, not duplicate"** is safe. 🔴 **A row reading "left for you" writes NOTHING and is yours to settle — open the existing order beside its QuickBooks invoice, because a wrong match there is permanent and invisible.** So `/orders` does **not** automatically rise by nineteen: it rises by what the panel promised, and **higher than the panel is the thing to stop on.** And on **CARD 7**, open invoice **#3648.563** — **both replacement trees must be on its order**, or the 21 September stop has nothing on it. **(d)** ⚠️ **TWO QUESTIONS ARE DELIBERATELY LEFT OPEN AND ARE YOURS, NOT DEFAULTS I MAY PICK.** `service_type` (#140, above) — and **what a DAY SHEET shows** (#139): every line is on the order because the sale would misstate itself without the trip charge, **but a day sheet listing "Trip Charge" as something to load is worse than one that does not**, and the shortcut is measurably wrong — the tree items carry a `Category:CODE` shape and the fees do not, **but `Tailgate Delivery` and `Late fee` are bare names too, and so is `BPJ30REP`, which IS a tree.** The answer is in the item catalogue we already read (`Item.Type`), i.e. [[R-25]]. **(e)** ⚠️ **THE MEASUREMENTS IN THIS ENTRY COME FROM THE 2026-08-29 CAPTURE, WHICH IS TWO DAYS STALE — SAID PLAINLY RATHER THAN GLOSSED.** It holds **18** future-dated invoices; the live ingest on 08-31 read **22** and wrote 19, and your own CARD 1 recorded *"22 of 1,477"*. So every count here describes 18 of the 19 stops, and four invoices created since are unmeasured. **Nothing in the code depends on the number** — it reads what is live — but the *"7 of 18"* install figure will move. **(f)** ⚠️ **A THING IN THE MAIN WORKTREE, NOT MINE, AND I HAVE NOT TOUCHED IT:** `git status` on `/Users/terrenceobrien/Desktop/trace-platform` shows **eleven files STAGED that REVERSE the #246 close-out** — the §3 entry, the ledger row and the built-inventory text for #246 are removed in the index while `HEAD` (`21cb3f2`) still has them. Committing that index would silently delete yesterday's close-out. `git restore --staged --worktree .` on `main` is the fix if it is not deliberate. **(g)** ⚠️ **STILL OPEN AND UNTOUCHED:** `docs/inventory-functions.md` / `inventory-env.md` / `inventory-ai.md` stamped `2026-06-13` · `docs/reference/` still tracked by accident · `20260830c` written and not applied · tech-debt **#73** · **#124**–**#127** · **#132**'s owed `authority:baseline` · **#134** · **#135** · **#137** · **#138**.
