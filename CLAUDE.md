...
# Last updated: 2026-07-16 — §3 HANDOFF RETENTION (N=3): the trim + the rule that makes it self-maintaining (OP-13, ledger #134). See §3 for the current handoff; `docs/handoff-archive.md` for full history.
...
- [~] Lean CLAUDE.md to rules + state + pointers only — no architecture prose duplicated here
      **PARTIAL 2026-07-16 (ledger #134, OP-13) — the two self-maintaining sub-items are DONE; the rest stays open:**
      ...
      - [ ] **STILL OPEN — the structural residual.** 907 → 736; **136 over the ~600 budget.** §3 is now only 91 lines; the weight is §2's infra tables (~155 — Supabase/Vercel/env/domain/folder-map detail that belongs in a pointed-to doc), §6's ten coding rules (~45), §9's standing instructions (~55). This is prose that should be POINTERS. Its own build.
