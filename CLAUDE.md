...
# Last updated: 2026-09-02 — #254 test mode + the order-counting primitive; R-54…R-60; tech-debt #146–#147 — see §3
...
### 2026-09-02 — THUNDER **A BUSINESS CAN PRACTISE FOR A WEEK WITHOUT ONE ORDER REACHING ITS ACCOUNTANT — AND FIVE MUTANTS SURVIVED FIRST, EVERY ONE A REAL HOLE. #254. R-54…R-60 FILED. TECH-DEBT #146 + #147.** 🔴 **THE HEADLINE IS THAT `api/dashboard.ts` SELECTED EVERY ORDER FOR THE BUSINESS WITH NO KIND FILTER AT ALL AND COUNTED `leakage_flag` OVER THE LOT, WHILE `Dashboard.tsx` EXCLUDED CAPTURED INVOICES BEFORE COUNTING THE SAME THING.** One number, two definitions — and a test order lands in the identical hole. ⚠️ **AND THE CLIENT COMMENT DESCRIBING THAT EXCLUSION HAD ALREADY DRIFTED FROM ITS OWN CODE:** it said the read excluded history via `order_kind=is.null`; the query filtered only on status and the exclusion happened later in JS as `!== 'history'` — **a different rule, and one that admits a test order.** A prose description of a filter is not the filter — [[R-26]] arriving inside a comment.

**Type:** Build + close-out. **BUILDER-COMPLETE** on **`thunder/qbo-review-test-mode`** (off `main` at `08fd5a1`), **NOT MERGED** · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 244 / 10 / 12 / 15) · **62/62 files · 3155 assertions** (+114) · `build:cultivar` clean · **api/ 12/12, nothing minted** · **ONE migration, GATED.** Code: **NEW `business-logic/orderKind.ts`** (the primitive) · **NEW `business-logic/testMode.ts`** · **NEW `quickbooks/booksFindings.ts`** (twelve rules as data) · **NEW `TestModeBanner.tsx` · `QboWriteSwitch.tsx` · `BooksReview.tsx`** · guards in `api/qbo/invoice/cultivar.ts` · `UnitPrice`/`Sku`/`PurchaseCost` widening in `invoiceList.ts`/`itemList.ts` · call sites in `api/dashboard.ts`, `Dashboard.tsx`, `api/social/generate-posts.ts`, `CampaignDetail.tsx`, `api/orders/submit.ts` · **NEW `20260902_business_qbo_writes_switch.sql`** · three mutation harnesses. Docs: ledger **#254** · **NEW owner-test board (14 cards, 0 covered)** · **R-54…R-60 + two OWED rows** · the story, written as-built · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · tech-debt **#146/#147**. **§3 RETENTION: 1 archived verbatim (2026-09-01, the #249 merge entry), 1 written — entries-in == entries-out, §3 back to 3** (archive **279 → 280**). ✅ **Own worktree.**

**🔴 THE FINDING WORTH MORE THAN THE FEATURE: THREE SOURCE PROBES WERE READING PROSE, AND ONE HAD BEEN GREEN FOR WEEKS ON THE GUARD WHOSE FAILURE IS UNRECOVERABLE.** My new ordering probe went red against correct code because the guard's own COMMENT names `findOrCreateQBCustomer`. Then **`historyOrder.test.ts` §I went red for the identical reason** — a probe written 27 August, passing ever since, which means it had been readable-as-prose the whole time. Then a third matched *"SERVICE KEY"* inside a paragraph explaining why the component deliberately does not use one. ✏️ **THE REDS ARE THE HARMLESS DIRECTION AND ARE NOT WHY IT MATTERS.** A comment moves an index EITHER way: §I would have **PASSED on a DELETED guard** so long as some comment still mentioned `findOrCreateQBCustomer` or `qbPost` — a green asserting an ordering that no longer exists, on the one guard that writes into a real company's accounting. **That is [[R-33]] exactly, and it was live in the repo.** Fixed in the three files and **proven**: mutants that delete each guard's CODE and leave its COMMENT are both CAUGHT. **The class is not closed** — nothing surveys the other source probes, and this repo leans on them because they are the only way to assert an ORDERING (tech-debt **#146**; the nearest prior art is #119, the same tool needed for a different reason, filed and not generalised then either).

**🔴 AND FIVE MUTANTS SURVIVED THE FIRST RUN. FOUR WERE IN THE FINDINGS SUITE AND ALL FOUR MAKE THE SCREEN SHORTER, CALMER AND MORE CONFIDENT THAN THE TRUTH** — a rule reporting a clean result over a population it never had · unmeasurable rules filtered off the list · a duplicate-customer count that SUMMED two overlapping tallies (a customer entered twice usually shares BOTH an email and a phone, so the sum roughly doubles the very records it is about) · Category FOLDERS counted as unsold stock, which inflates both halves of the ratio so it still looks reasonable. **Not one would have been caught by looking at the screen.** The fifth was mine: a `422` assertion whose fixed-width window reached into the **neighbouring guard's** 422 — the same seam-blindness as a probe short-circuiting on an earlier guard, found by mutating rather than by reading. **All five closed; 27/27 across three harnesses, each against a proven-GREEN control, each keyed off the exit code and each treating a never-applied mutant as an ERROR rather than a skip.**

**🔴 THE TRAP THAT MADE THE PRIMITIVE WORTH CENTRALISING DOES NOT LOOK LIKE A TRAP.** `.neq('order_kind','test')` reads as *"not a test order"* and **drops every row where `order_kind` IS NULL** — SQL `NULL <> 'test'` is NULL, and NULL is what every ordinary checkout order carries. The obvious spelling excludes the entire real business and keeps nothing but the captured invoices. The correct `.or(is.null, not.in.(…))` form does not look correct, which is the strongest argument for it having exactly one home — and both forms are **derived from the same arrays**, with a probe executing the filter strings against the same rows the predicates judge. **A test order counts nowhere; a captured invoice counts as MONEY but not as an ASSESSMENT** — and a probe asserts those two answers must never converge, because if they do, one predicate has swallowed the other and either real revenue has stopped counting or unassessed sales have started proving a clean bill of health.

**FLAGGED FOR DAVID:** **(a)** 🔴 **FIVE OF THE NINE SCOPED ITEMS ARE NOT BUILT, AND I AM NAMING THEM RATHER THAN LETTING THE GREEN VERIFY IMPLY OTHERWISE:** the **file harness** (§3 / [[R-59]] — nothing reads a capture file back, so acceptance #14 is unmet and nothing here has been proven from Test Dave's) · **retire-and-replace** (§4 / [[R-57]]) · **normalisation with consent and its two `audit_log` rows** (§5 / [[R-58]] — so acceptance #11 is unmet) · the **printable artefact** (§7) · the **re-run-writes-only-what-is-new** proof (#13). The findings ENGINE and its PANEL are built; **a place to record an answer to the two findings that ask one is not** — the panel says so on screen rather than offering a button that does nothing. **(b)** 🔴 **CARD 6 IS THE OWNER TEST AND ITS SECOND POINT IS THE ONE THAT MATTERS: after a test order, read QUICKBOOKS — no new invoice AND NO NEW CUSTOMER — never our own logs.** A log saying we did not write is a claim about the thing under test. **CARD 9 MUST NOT BE SKIPPED**: cards 6 and 7 both pass if the push is simply broken, and nothing else on the board can tell the difference. **CARD 8** proves the exclusion is a property of the row rather than a mode — the test order stays out of the figures **after** go-live. **(c)** 🔴 **GATE 0 ② IS LOAD-BEARING IN AN UNUSUAL DIRECTION: until `20260902` is applied, every order is written as a test order** (a failed read of the switch means test mode — the safe direction, stated at the code), **so an unapplied migration looks like a working feature.** Run the VERIFY block; **query 3 is the one that matters** — any UPDATE policy on `businesses` other than `businesses_owner_update` and [[R-55]]'s authority claim is false. **(d)** 🔴 **OWED AND ON THE BOARD: DOES A TEST ORDER DEPLETE STOCK?** Raised, not decided, as the prompt asked. **Built as NO on the [[R-21]] precedent because that is the reversible direction** — stock a week of test orders consumed cannot be un-consumed. ⚠️ **The prompt's own clause *"if the answer is no, the screen must SAY so"* is NOT yet honoured**: the banner is silent about stock, so an owner watching a test order not move the count reads it as a defect, and one who never checks assumes depletion was tested. **That sentence cannot be written until you rule.** **(e)** ⚠️ **`api/dashboard.ts` HAS NO CALLER** — measured, no `fetch('/api/dashboard')` exists anywhere — so the divergence was LATENT rather than live. Fixed anyway (it is deployed and reachable), but **the real question is the SLOT**: it is one of twelve on a plan with zero headroom (tech-debt **#147**). **(f)** ⚠️ **§3's two-minute check is ANSWERED and it is the good answer: BOTH unique indexes are `(business_id, qb_invoice_id)` — tenant-scoped, not global — so Test Dave's import cannot collide with LAWNS's existing 19.** **(g)** ⚠️ **ONE CAP DECLARATION IS YOURS TO RATIFY** (`QboWriteSwitch` as a fourth `businesses` writer — it cannot ride an existing path for an AUTHORITY reason, not a column one), and **`field-lists-baseline.json` was RE-KEYED BY HAND** because `--update` would have grown it from 34 to 39 with entries exempt by path declaration. **(h)** ⚠️ **NOT MERGED.** Re-read `CLOSE-OUT-LEDGER.md`, `tech-debt-log.md`, `RULINGS.md` and the §3 count at merge (#233's lesson; my numbers are **#254**, **#146–#147**, **R-54…R-60**). **(i)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs stamped `2026-06-13` · `docs/reference/` tracked by accident · tech-debt **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#143**–**#145**.

### 2026-09-01 — THUNDER **THE SIX WRITE-ONLY COLUMNS FINALLY HAVE A READER — AND THE MOST USEFUL THING I DID ALL SESSION WAS RE-MEASURE A NUMBER THAT TURNED OUT TO MEAN SOMETHING ELSE. #252. R-38…R-53 FILED. TECH-DEBT #143 + #144 + #145.** 🔴 **THE HEADLINE IS THAT `/receipts` COULD CAPTURE AND COULD NOT SHOW, AND ON 1 SEPTEMBER THAT COST EIGHT DOCUMENTS.** Lauren photographs a receipt, presses Save, the page thanks her — and that is the last anyone ever sees of it. Seventeen receipts had accumulated at LAWNS, **eight of them captured in one afternoon**, and **two produced orders while six produced nothing** — a difference that existed nowhere on any screen. **A receipt that landed and a receipt that vanished looked identical: both looked like a thank-you message.** The six reconciliation columns had been **WRITE-ONLY since 14 June** — banked at the moment of save by a wizard that then had no reader. This is that reader, plus `receipt → order → delivery` so a capture can say what it became.

**Type:** Build + close-out, **READ-ONLY**. **BUILDER-COMPLETE** `ab617b2` on **`thunder/receipts-view`** (off `main` at `84cc695`), **NOT MERGED** · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **58/58 files · 2994 assertions** (+87) · **14/14 MUTANTS CAUGHT, MEASURED against a green CONTROL** · `build:cultivar` clean · **api/ 12/12, nothing minted** · **NO migration · NO new permission · NOT ONE WRITE.** Code: **NEW `lib/receiptsList.ts`** (pure — the projection, the banked-verdict read-back, the outcome chain, the honest count) · **NEW `receiptsList.test.ts`** · **NEW `components/receipts/ReceiptsList.tsx`** · `pages/ReceiptKeeper.tsx` **+11 lines, an import and one mount** — the seven-state wizard is otherwise untouched · **NEW read-only `scripts/measure-receipts-view.mjs`**. Docs: ledger **#252** · **NEW owner-test board (11 cards, 0 covered)** · **R-38…R-53 filed in `RULINGS.md`** with their evidence in a new decision doc · **NEW as-built + drift table** · tech-debt **#143/#144/#145** · `user_stories.md` (the story, written as-built — no matching story existed) · `built-inventory.md` **Wave 3** · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift + two doc-map rows · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-31 (8)), 1 written — entries-in == entries-out, §3 back to 3** (archive **277 → 278**, `grep -c '^### '`). ✅ **Own worktree** — `thunder/receipts-view`, with ten others live.

**🔴 THE FINDING WORTH MORE THAN THE SCREEN: THE RECON'S LOAD-BEARING FACT WAS TRUE AND MEANT THE OPPOSITE OF WHAT IT LOOKED LIKE.** The recon reported the bwi duplicate as producing orders `dc943a79` and `eb3ab2b0` — which reads as **one receipt with two orders**, and a build aimed at that shape would have rendered it wrong. Measured: **`receipts with >1 order` is 0 of 17.** It is **two receipts, one order each**, both carrying document number **19837964**, both with a delivery whose `delivery_date` is NULL. ✏️ **Same facts, different shape, and the shape is what the screen has to draw** — which is [[R-26]] arriving not as a false claim but as a true one read one level off. **And the re-measure paid a second time:** `header_amount_edited` is **false on 17 of 17** while `accept_vs_edit` reads `edited` on 17 of 17, which **narrows the open question without answering it** — whatever the owner changed before saving, **it was never the total.** The question stays David's.

**🔴 THREE PROHIBITIONS, AND EACH IS MECHANICAL RATHER THAN PROMISED — WHICH IS THE ONLY KIND THAT SURVIVES THE NEXT EDIT.** ① **NO RE-EVALUATION.** The verdict on screen is the one the owner was actually shown, not a second verdict computed today with no way to tell the two apart: `computeReconcile` is not imported **and `line_items` is not even selected**, so re-deriving it is impossible rather than discouraged. The one arithmetic is RECONSTRUCTION — `delta` was written as `lineSum − amount`, so `lineSum = amount + delta` recovers the banked figure exactly and invents nothing. ② **NO ADJUDICATION.** Six of seventeen produced no order; the row reads *"No order recorded for this receipt."* **and stops.** Not *missing*, not *orphaned*, not *an error*, and **not a blank** — an absence a reader must interpret is the defect, not the fix. They read as vendor PURCHASE invoices, which correctly should not become customer orders, **but that is a READING and the screen is not allowed to hold it.** ③ **NO DERIVED DOCUMENT TYPE.** Measured live: `receipts` has **21 columns and none of `origin`/`shape`/`source`/`doc_type`/`document_type`/`kind`** — the recon could not answer this and now it is answered. So LAWNS's own nine sales invoices sit beside eight supplier invoices with nothing distinguishing them, and **retro-classifying from the vendor name works on these seventeen rows and is not a rule** ([[R-50]]).

**🔴 AND THE MUTANT THAT SURVIVED FIRST IS THE ONE WORTH KEEPING, BECAUSE THE BLIND SPOT GENERALISES.** Coercing an unusable number to `0` instead of to absent **broke nothing**: every *never fabricate a figure* probe had been written against `null`, which short-circuits on the first guard **before ever reaching the coercion under test.** ✏️ **The guarantee was proven for one input and assumed for the rest** — [[R-33]]'s class, and the same seam-blindness as #248's N3. Closed and re-measured 14/14. The harness itself keys off the **exit code**, never a grep for the word FAIL, and asserts a **green CONTROL** first — without that, every CAUGHT could be a suite that was already red. **M1 (orders embed removed) and M2 (deliveries embed removed) both go red**, which was the prompt's own bar: a probe set that passes with either join reverted has proven neither.

**FLAGGED FOR DAVID:** **(a)** 🔴 **SIXTEEN RULINGS ARE FILED — R-38…R-53 — AND FOURTEEN OF THEM GOVERN BUILDS THAT DO NOT EXIST YET.** That is the point: they were paid for today and four rulings have already evaporated by living only in §3 prose. **Provenance checked before filing — none of the sixteen was already on the board**, including the two flagged as possibly pre-existing from 28 August. ⚠️ **AND ONE THING WAS DELIBERATELY *NOT* MINTED AS A RULING:** *the screen does not adjudicate*. You set it as a caveat on this build, not as a platform rule, and putting a number on it would put words in your mouth on a board whose whole value is that every line is yours. **It is enforced in code and asserted by a probe; if it should be R-52, it is one line and yours to say.** **(b)** 🔴 **CARD 5 IS THE CARD, AND IT ASKS YOU TO READ THE SENTENCE RATHER THAN CHECK FOR ITS PRESENCE** — *"No order recorded for this receipt."* must not say *missing*, *orphaned* or *error*, **and a blank there is equally a fail.** **CARD 4** is the one that proves the shape: **each** bwi 2026-07-29 row shows its **own** order, both #19837964. **CARD 9** is the one that proves the build wrote nothing. **(c)** 🔴 **TWO DUPLICATE PAIRS AND 452 EMPTY SEAMS ARE SURFACED, NOT REPAIRED, AND BOTH ARE YOURS.** One vendor invoice appearing as two sales is a **$1,283.88 revenue overstatement sitting in live data** (tech-debt **#143** — the durable fix is a partial unique index that **cannot land until the four rows are settled**, exactly #58's shape). And **`cost_objects.receipt_id` is 0 of 5, `business_inventory.receipt_id` is 0 of 447** — the count-once dedup seam designed on 12 June has **never been written by anything** (**#144**), which the cost-assignment build cannot proceed without. **(d)** ⚠️ **THE RECEIPT KEEPER SURFACE HAS NO ID ON THE 24-CAPABILITY BOARD** — 3.1 is Leakage, 4.2 is `/inventory/reconcile`; the capture half has shipped since 2026-06-11 with no row. Checked, flagged, **not minted** — yours to assign. **(e)** ⚠️ **TWO QUESTIONS STAY OPEN AND MAY NOT BE ANSWERED BY A DEFAULT IN CODE:** why `accept_vs_edit` reads `edited` 17/17 (narrowed above), and why 6 of 8 captures on 1 September wrote a receipt and no order. **(f)** ⚠️ **NOT MERGED, and ten other worktrees are live** — re-read `CLOSE-OUT-LEDGER.md`, `tech-debt-log.md`, `RULINGS.md` and the §3 count at merge (#233's lesson; my numbers are #252, #143–#145, R-38…R-53). **(g)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs stamped `2026-06-13` · `docs/reference/` tracked by accident · ~~`20260830c` and `20260831d` written and unapplied~~ ✅ **BOTH APPLIED — measured live 2026-09-02, not inherited.** `20260831d`'s four columns all return rows; `20260830c`'s `count_group_variant_sizes` exists and refused a fake actor at its own authority check (`42501`), which a missing function cannot do. **Carried forward as *pending* through three sessions, including by me — [[R-26]] and tech-debt #22's class, the STD-008 inverse** · tech-debt **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139** · **#140** · **#141**.

### 2026-09-01 — THUNDER **THE DEPENDENCY WAS UNMET, AND CHECKING THAT WAS THE MOST VALUABLE THING I DID ALL SESSION. #250 + #251. R-36. R-37.** 🔴 **THE HEADLINE IS THAT A BUILD PROMPT ARRIVED SAYING *"do not start this until the matcher fix is owner-proven"* AND THE MATCHER FIX DID NOT EXIST.** `origin/main` was still `84cc695`, no branch matched `match`/`doc.?num`/`corrobor`, no ledger row existed, and `historyOrderWriter.ts:490` still read `if (c.differed.length === 0)` verbatim in both worktrees holding it. **Nothing was written until David ruled.** He ruled *both, matcher fix first* — two builds, two branches, in sequence. ✏️ **A precondition stated in a prompt is a claim like any other, and the cheapest possible thing to check.**

🔴 **AND THE PREMISE THE MERGED CODE RESTED ON WAS MEASURABLY FALSE.** #248's guard calls a matching document number *"an identity, not an inference."* **It is not.** LAWNS's own books — the 2026-08-29 raw capture, **1469 of 1469, `complete: true`** — carry **22 DocNumbers used by TWO DIFFERENT INVOICES EACH, 44 invoices**, every pair a different customer AND a different amount: `#3274` is Dyan Bourne at $811.88 **and** Jeffrey Gyurkovic at $4,717.50. Two defects followed, and the exposure numbers are why the dependency clause was right: **ZERO of those pairs are among the eighteen future-dated invoices the merged code runs against today, and FOUR are inside the 564-invoice import.** Both defects are **latent on `main` and go live at Stage 1**. **① `unknown` was computed and never consulted** — `corroborate()` sorts fields into agreed/differed/**unknown** and the verdict tested only `differed.length === 0`, so an existing order whose customer, date and money were all null **disagreed with nothing on earth** and got a permanent `qb_invoice_id` written onto it, after which the ordinary key skips it forever. **② Collisions were checked among OUR priors and never among INCOMING invoices** — two invoices sharing a number each returned `same-invoice` on the SAME order, the id-recording pass wrote for whichever the loop reached first, the other was logged as a benign race, **and the loser, consumed by the guard, was never planned, so its order was never created.** A real sale disappeared and loop order decided which.

🔴 **THE FINDING WORTH MORE THAN EITHER FIX: A MUTANT SURVIVED TWELVE GREEN UNIT PROBES.** Reverting the ONE line handing the new pass's result to the report — `priorOrders: settledPriors` → `priorOrders` — broke nothing. The function ran, cost nothing, and was thrown away. ✏️ **That is #248's own recorded lesson arriving one build later** (*"the unit was well tested and the seam above it was not"*, tech-debt **#134**), and it is why the fix was §M23–M30 driving `previewOrderIngest`/`commitOrderIngest` rather than a thirteenth assertion beside the twelve. **12/13 caught, 1 PROVEN equivalent** by a differential run over 10 input sets rather than asserted as equivalent, **0 real survivors.**

**Type:** Two builds, two branches, neither merged. **#250 `thunder/docnumber-matcher` (`546c16a`)** — `historyOrderWriter.ts` (`DOC_MATCH_MIN_CORROBORATION = 2`, **§②'s own settled bar reused rather than a second standard invented for one question**; new pure `refuseCompetingPriorClaims`) · `+28` assertions · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **57/57 files · 2935 assertions** · `build:cultivar` exit 0. **#251 `thunder/deliveries-window`** — **NEW `lib/deliveryWindow.ts`** + 19 assertions · `DeliverySchedule.tsx` · the §9 story · **[[R-36]]** + **[[R-37]]** · `npm run verify` **exit 0, eslint 245 → 244 and the baseline RE-TAKEN to lock it** · **58/58 files · 2926 assertions** · `build:cultivar` exit 0. Both: **api/ 12/12, nothing minted · ZERO schema change · ZERO Intuit writes.** **§3 RETENTION: 1 archived verbatim (2026-08-31 (8)), 1 written — entries-in == entries-out, §3 back to 3** (archive **277 → 278**).

🔴 **AND #251 IS THERE BECAUSE THE STAGE 0 RECON WAS RIGHT ABOUT A DEFECT AND WRONG ABOUT WHICH ONE.** Recon predicted that 564 imported stops would fill an unbounded `.limit(200)` and push the nineteen future stops off `/deliveries`. True, and the lesser half. **Reading the code found `filterDate` applied CLIENT-SIDE** — `rows.filter(r => r.delivery_date === filterDate)` over that same unbounded read — so selecting **any** day outside the oldest 200 returns empty and the calendar's drill-in reports **"Nothing scheduled on this day" for a day that HAS stops.** ✏️ **Absence asserted without ever being established, on the exact surface the fulfilment tap and `openOrderNotice` mount on.** The bound now reaches the query, decided in a pure lib per **#134**; a selected day is `.eq()`'d **unclamped at any distance in the past**, and the working list is **`.or('delivery_date.is.null,…gte')`** — the `is null` half kept deliberately, because a bare floor would silently drop undated stops, which is the same defect one layer down. **Two header totals were REMOVED rather than recomputed** — both became false once the read was bounded to one day.

⚠️ **THREE THINGS I GOT WRONG AND CORRECTED, RECORDED BECAUSE THE PATTERN IS ONE THING.** **(i)** My first `npm run verify` was piped to `tail -25`, so **exit 0 was `tail`'s** and every earlier stage was discarded — a green that could not have gone red, inside the build about exactly that. Re-run capturing the real code. **(ii)** Every doc edit for #250 landed in the **main worktree on `main`** rather than on the branch; caught at `git status`, and the five files were moved by comparing **direction** (`--numstat` plus every deleted line) before restoring — #249's staged-revert lesson, applied to my own mess. **(iii)** I wrote that the calendar renders `DeliverySchedule` as its day drill-in **before checking the router** — `/deliveries` is `DeliveryRoute`; `DeliverySchedule` is mounted as a component with a `filterDate` prop, which is what exposed defect ②.

**FLAGGED FOR DAVID:** **(a)** 🔴 **#250 MUST BE OWNER-PROVEN BEFORE THE IMPORT — that is the whole reason it shipped alone.** **CARD 4c** is NEW and `needs-test` **with its reason**: it needs two invoices sharing a number AND an existing order carrying it, a state your tenant may not be in, and **an empty panel is not a pass** — provoke it in the SQL editor or leave the card red. **CARD 4b is a re-read, not a re-run: fewer of your nine may now say "will match", and that is the tightened bar rather than a failure** — each says which fields it could not compare. **(b)** ⚠️ **A DECISION THAT IS YOURS TO OVERRULE: the bar is TWO of three**, stated at `DOC_MATCH_MIN_CORROBORATION` with its reasoning. **Its effect on your nine is UNMEASURED** — it needs a read of `orders` this session did not have (the env read was denied and I did not press it). **(c)** 🔴 **CARD 16 (operations-calendar) IS WRITTEN TO BE RUN BEFORE AND AFTER THE IMPORT WITH THE SAME ANSWER.** Step 4 is the one that matters: after the import, open a **2025** day from the grid and confirm the stops are listed. **(d)** ⚠️ **THE IMPORT IS RULED, STORIED AND UNBUILT** — 564 deliveries + 564 orders + 1,565 item lines + up to **489 customer resolutions**, a table your prompt's non-negotiables never mention. **(e)** ⚠️ **`Tailgate Delivery` is deferred by ruling, but the measurement is already done so nobody re-derives it: TG and `TC` are MUTUALLY EXCLUSIVE across all 1,469 invoices (zero co-occurrences), TG does NOT predict shipping (40% either way), and 0 of 124 TG invoices carry `(Install & Warranty)` against 163 of 1,345 without.** **(f)** ⚠️ **#250 AND #251 BOTH INSERT A LEDGER ROW BEFORE #249 ON SEPARATE BRANCHES** — a merge conflict by construction; the resolution is *keep both* and re-take the numbers (#233). **(g)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs stamped `2026-06-13` · `docs/reference/` tracked by accident · `20260830c` and `20260831d` unapplied · tech-debt **#124**–**#127** · **#132** · **#134** · **#135**.
...
19. **A CHECK THAT CANNOT DISAGREE IS NOT A CHECK — AND A TEST DOUBLE MUST BE ABLE TO REFUSE WHAT THE REAL THING REFUSES (binding; [[R-33]]).** Before trusting a green result, ask the only question that matters about it: **could this have failed?** Three mechanisms produced the same false green inside a fortnight — a hardcoded gap list that asserted nothing (#73), a seam no test could reach so 25 assertions tested around it (#242 / tech-debt #134), and a **test double more forgiving than the real system**, which stamped an `onConflict` Postgres rejected on all 19 live rows (tech-debt #138). **The shape is one: the thing asserting was incapable of disagreeing, and its green was read as evidence.**

    So: **(a)** a fake, stub or in-memory double MODELS THE CONSTRAINTS THAT CAN REJECT A REAL WRITE — unique indexes and their predicates, NOT NULL, FK, RLS refusal — or it is a rubber stamp, and the assertions resting on it are decoration. **(b)** A new check is proven by **making it fail once, deliberately, and watching it go red** before it is trusted; a check nobody has seen refuse is a claim. **(c)** When a defect reaches production past a green suite, the finding is **why the suite could not have caught it**, and that is fixed FIRST — fixing only the defect leaves the blind spot that will pass the next one. **(d)** Prefer a seam that makes the condition reachable over more assertions aimed near it.

    ⚠️ **Numbered 19 by APPENDING.** Rule 18 records that a first draft slotted itself in at 16 and silently broke a live citation of *"§6 r17"* in `docs/RULINGS.md`. Rules are appended because other documents cite them by number; renumbering to satisfy a reading order breaks references to fix a preference.
