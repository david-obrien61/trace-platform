...
# Last updated: 2026-08-24 — build: the swallowed storage failure, a write that stored nothing (see §3)
...
### 2026-08-24 — THUNDER **BUILD: THE SWALLOWED STORAGE FAILURE — AND THE ONLINE HALF WAS WORSE THAN THE RECON KNEW.** `store.ts:57` discarded a quota / disabled-storage exception (*"best-effort, never throw into a Save"*), so `enqueue()` returned normally over a queue that had not grown and every caller above it reported success. 🔴 **MEASURED, NOT INFERRED — OFFLINE the engine said `queued` under a banner reading *"counts are saved on this phone and will sync when you're back in signal"*; ONLINE it said `applied`, its STRONGEST claim, with `drain applied:0 failed:0 remaining:0` and THE DATABASE CALLED ZERO TIMES.** ✅ **`save()` STILL NEVER THROWS — the exception is RETURNED** (`StoreWriteResult`, R-11's union discipline on the write side); **a write-then-READ-BACK probe warns BEFORE she walks a row.** **AFTER: offline → `failed` naming STORAGE; online → `applied` with the database called ONCE — the claim is now TRUE.** **BUILDER-COMPLETE + DEPLOYED `4056de8`** · NO schema, NO migration, NO policy, NO new permission string, NO cap · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **29/29 test files · 1092 assertions** · api/ **12/12** · 🔴 **GATE 0 PROVEN BY WATCHING THE TRANSITION** · all `[TRACE:*]` ON.

**Type:** Build + close-out. Code: `packages/shared/src/sync/store.ts` · `offlineQueue.ts` · `syncEngine.ts` · `index.ts` · `storeDurability.test.ts` (**NEW, 24 probes**) · `packages/cultivar-os/src/pages/InventoryCount.tsx`. Docs: ledger **#207** · `RULINGS.md` (**R-12 RULED-IN-SHAPE → PARTIAL — its client-side instance landed, its own PostgREST subject still OWED; NEW OWED row S-A, the IndexedDB move, WITH its data answer**) · `built-inventory.md` (**1 net-new entry, 1 amended**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · owner-tests (**cards 1 and 5 updated; card 5 `needs-test` → `owed`**) · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 (13) #204), 1 written — entries-in == entries-out, §3 back to 3** (archive 237 → 238). ✏️ **CLAUDE.md read IN FULL (verified: only line 3 and §3 differ from the session-start copy) and `docs/RULINGS.md` read IN FULL (184 lines incl. the whole OWED table).**

**🔴 THE FINDING THE RECON DID NOT HAVE, AND IT IS THE REASON TO READ THIS ENTRY: THE ONLINE PATH WAS WORSE THAN THE OFFLINE ONE.** The recon named the offline banner. But online, the op was never persisted, so `queue.list()` was EMPTY, the drain loop never ran, `stillQueued` was false — and `submit` concluded *"not in the queue, so it landed."* **`applied` is the strongest claim the engine can make, and it was being made over a database that had not been touched.** That is `submit` inferring success from an **ABSENCE** — **A9 / D-9 (*absent is not empty*) on the WRITE path, and R-12's class (*a write must prove it wrote*) arriving on the client side of the wire.** ✏️ **THE FIX KEEPS THE ORIGINAL AUTHOR'S INTENT AND DROPS ONLY THE SILENCE:** the comment was right that a Save on a phone in a row must never throw. **Not throwing and not reporting are different things, and only the first was ever wanted.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **A METHOD FINDING WORTH MORE THAN THE FIX, AND IT IS NOW PINNED IN A TEST: A STATUS-ONLY ASSERTION IS BLIND TO THIS DEFECT CLASS.** With the defect re-planted, `E3 (status === 'applied')` **still passed**. Only **E4, which counts DATABASE CALLS**, caught it. **The claim and the act have to be checked separately, or the check inherits the lie** — and that is the same shape as #206's card 9, one layer down. **(b)** 🔴 **CARD 5 IS THE ONE TO RUN, AND RUN IT ON THE LOGOUT-LOOP DEVICE.** The store holding a day's counts is the same mechanism you are separately chasing, so one trip may inform both. It now expects a **red warning before the walk**, and a refusal that names STORAGE rather than the network. **(c)** ⚠️ **ONE JUDGEMENT CALL, RECORDED RATHER THAN BURIED: online-with-unusable-storage now writes DIRECTLY instead of refusing.** The drain cannot carry an op that is not in the queue, so this is the only path to the database — it converts a FALSE `applied` into a TRUE one and keeps a capability that genuinely works. **Refusing would have been simpler and strictly worse for Lauren days before LAWNS.** **(d)** ✅ **THE PROBE READS BACK, NOT JUST WRITES** — a Safari Private tab can ACCEPT `setItem` and persist nothing, and a write-only probe would have passed over exactly the case it exists to catch (pinned as P3). **(e)** ⚠️ **TWO SENTENCES, NOT ONE WITH A TENSE FLAG, AND THAT IS DELIBERATE:** `storageError()` says *a save did not happen*; `storageWarning()` says *nothing has failed yet, and here is what will*. Saying *"we could NOT save that"* before she has saved anything would be its own false claim. **The DIAGNOSIS — quota vs unavailable — is the part that must not drift, and it is one switch three lines apart.** Keeping them distinct matters: **telling someone to clear space on a phone with 60 GB free is the same class of false-but-actionable instruction as *"check the tag"* on a network failure.** **(f)** ⚠️ **INDEXEDDB IS NOT DONE AND IS FILED AS S-A WITH ITS DATA ANSWER, because the answer is the expensive half:** the queue is ONE key per (business, domain), so the migration is *read → write → delete at startup, before the first drain* — **and if it is skipped, a phone holding an un-drained walk upgrades, looks in the wrong store, shows `0 waiting to sync`, and the work is gone with no error.** That is the defect just closed, arriving through a deploy instead of a throw. **(g)** ✅ **`AssetCapture` — the queue's other consumer — INHERITS THE FIX FOR FREE and was not touched** (`assetCapture.ts:119,170` already return `res.error` on `failed`). **(h)** ✅ **`git log`/`git status` re-run at write-up.** **(i)** **CLAUDE.md is 584 lines, under its ~600 budget.**
