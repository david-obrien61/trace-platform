...
# Last updated: 2026-09-11 — #301 one stop, three screens: one stop card on schedule, route and order; ship-to editable; checkout stop carries its order — see §3
...
- AC-1: 🔴 **NEW 2026-09-03 (#272, David's instruction) — `packages/shared/src/discovery/populate.ts:77` QUERIES THE TABLE `cultivar_plants` FROM INSIDE SHARED.** `supabase.from('cultivar_plants').delete(...)`, with the result keyed to `counts.cultivar_plants` at `:78`. **A vertical table name, hardcoded in platform code** — AC-1 in its most literal form (*"vertical identity is a value, never a table name"*), and it is a RUNTIME literal, not a comment. ⚠️ **It is the ONLY one: population, not a sample** — every other `cultivar` hit under `packages/shared/src` is prose, a `'cultivar'` *value* (`notifications/types.ts`, `positions/`, `design-system/tokens.ts` — all legitimate under AC-1), or a test reading app source. **Found while measuring the DataSheet promotion, filed by David's instruction, NOT fixed in that pass** — it is a discovery/seed-data path and repairing it inside a component move is the scope creep that makes a diff unreviewable.
- AC-2: Some RLS policies are `USING(true)` — documented intentional, post-demo tighten
- AC-4: Cultivar green `#27500A` default in shared UI primitives (post-August 2026) — 🔴 **QUANTIFIED 2026-09-03: 42 occurrences across 21 files** in `packages/shared/src`, and the fix mechanism (`design-system/tokens.ts`, a complete per-vertical palette) **has ZERO importers**. Tech-debt **#157**, filed against `shared` rather than against any one component.
...
**Tables: the inline list is GONE — it declared itself stale and it was.** Canonical per-table state (LEVEL + LOCATION + EVIDENCE): **PLATFORM_STATE.md**. Still true and worth carrying: ~~`nursery_modules` exists pending DROP~~ — ✅ **CORRECTED 2026-09-11: `nursery_modules` IS GONE**, dropped by hand (no migration records it; the repo's only `DROP TABLE nursery_modules` is a commented-out line) — measured by `node scripts/verify-migration-apply-state.mjs --catalog`, which also shows its policy, index and `business_id` column went with it. And `nurseries` + `losses` are the pre-`businesses` generation — both EMPTY, both pending DROP via GATED `20260727d_drop_losses_and_nurseries.sql` (ledger #162); `losses` keys on `nursery_id`, and its successor shape is `plant_events['lost']`.
...
Active open items: **#271–#277 (NEW 2026-09-11 — ledger #301, filed not fixed: 🟡 a stop has no phone of its own · 🟡 a stop marked done cannot be reopened, and the tap is now on three screens · 🟡 nothing ever writes `cancelled` while every stop read filters it · 🟡 `submit.ts:206` says `deliveries` has no `order_id` beside the line that writes it · 🟡 crew permissions on the stop are unmeasured, and a checkout line's name needs `inventory:read` · 🟡 a ship-to change and its history row are two calls, not one RPC · 🟡 `/deliveries` without a date still lists cart orders, not stops — 27 Test Dave's orders have no stop)** · **#139 (RE-SCOPED 2026-09-11, R-144 — fee lines are LISTED, not filtered; the classification exists in QuickBooks, the missing piece is the link from an `order_items` row to its item)** · **#270 (NEW 2026-09-11 — 🔴 the review ask cannot run on any tenant: `followup_engine` is a `planned` tile, `/subscription` files it under COMING with no Turn on, and the board card that said "turn the tile on" named a control that does not exist. The link half is fixed by ledger #300; turning it on is David's call — catalog clock, by hand in SQL, or core)** · **#267–#269 (NEW 2026-09-11 — recon, ledger #299: 🟡 `orders.install_date` is a live column nothing has ever written or read; retire or wire waits on R-143 · 🔴 7 of 19 QuickBooks-ingested LAWNS stops are installs typed as delivery, so under R-143 they would never start a warranty · 🟡 `cultivar_plants.warranty_months` defaults to 12 months while LAWNS warrant six)** · **#266 (NEW 2026-09-11 — 🟡 the zone walk capture tool exports for `business_inventory.zone` and "the irrigation zone records"; NEITHER EXISTS, measured live. Where zones live is David's call before any import — ledger #298)** · **#261–#265 (NEW 2026-09-11 — PMI, ledger #297: 🔴 `/api/pmi/suggest` checks no login · 🔴 Joel holds `pmi:*` without `costs:read`, so `/pmi` shows him nothing · the generator suggests intervals its converter cannot read · hour intervals have nowhere to live and `Kubota Hours` is linked to nothing · one cadence per schedule, and NO SCHEDULE beside real tasks)** · **#253 (NEW 2026-09-11 — 🔴 **SHIPPED CODE READS AND WRITES THREE TABLES THAT DO NOT EXIST** — `20260905_production_planning` is not applied, so Settings → Operations cannot save and the Uppot plan page cannot commit, on every tenant. Apply-or-retire is David's call)** · **#254–#259 (NEW 2026-09-11 — transcribed from the 2026-09-10 handoff: importer skips `BillAddr.Line2` · leftover `Harness STAFF` row · 14 membership-only policies · no check that an `enforced` string is live · 7 movement RPCs with no permission check · offline staged-insert conflicts untraced)** · **#252 (NEW 2026-09-11 — 🟡 `price_type` and `price_unit` are two representations of one fact: checkout multiplies on `price_type`, the books review DERIVES it, the Settings editor lets the two disagree. R-120 Q2; unifying reverses a 2026-07-08 decision and needs a live read first)** · **#251 (NEW 2026-09-11 — 🔴 checkout offers ONE staff/flat transport row; a second is never offered and no flag names it. **Blocks the LAWNS service rows** — trip charge and tailgate are both staff, both once per order)** · **#250 (NEW 2026-09-11 — 🟡 the TRACE business `45830ba7` is GONE and no `business_type='general'` business exists, so the two `20260614` cost-to-produce migrations that wrote to it are unverifiable; deletion recorded nowhere found)** · **#249 (NEW 2026-09-11 — 🟡 `customers.shop_id` is gone and NO migration removes it — schema changed outside the repo, #39's class; `nursery_modules` is the second instance)** · **#248 (✅ RESOLVED 2026-09-11 — retired in place, R-139, ledger #297. WAS: 🟡 `20260529_pmi_shared.sql` NEVER RAN and nothing says whether it will; likely abandoned for `20260612`'s `business_*` tables, but every apply-state run will list it NOT_APPLIED beside two migrations that genuinely wait on David — his call whether to retire it)** · **#241 (NEW 2026-09-10 — 🔴 **THE DUAL-RLS CAP ENCODES THE MODEL THE `owner_id` TRIAGE RETIRES, AND IT KEEPS PASSING.** `verify-universals` #3 asserts *an owner policy AND a member policy* on every tenant table; #289 replaced the owner half with a permission on 28 policies and DROPPED it on 10. It failed on exactly ONE table — `cultivar_plants`, whose pinned policy name was dropped — and passed on the rest, because `tableHasOwnerPolicy` (`:105`) is a plain corpus grep with **no drop-tracking**: it reads `CREATE` statements the applied migration removed. **Passing on history.** Pin repointed, checker NOT rewritten)** · **#240 (NEW 2026-09-10 — 🟡 `SET LOCAL role authenticated` — the V-block impersonation form written into three migrations (`20260828` V7–V9) — **has never been run and FAILED on first use**: `permission denied to set role`. Setting the CLAIMS GUC alone is sufficient and is proven. [[R-33]]'s class; three cards on a 0-covered board carry the broken form)** · **#239 (NEW 2026-09-10 — 🟡 eight of the ten undocumented policies are still uncaptured; #289 captured two verbatim before touching them. Five of the eight are the open `USING(true)` public-QR grants and several are DUPLICATES of each other)** · **#238 (NEW 2026-09-10 — 🔴 **THE CLIENT COMPUTES 58 AND THE SERVER STORES 57.** The 58th is the `owner-only` sentinel — a string in no array, checked by 0 policies and 0 functions — so the client can render a surface the now-literal `has_permission` refuses. The fix is `get_my_permissions` (#237), ~~blocked on David's ruling: may an owner remove a permission from themselves?~~ ✅ **RULED 2026-09-11 — R-121: no, for EVERY OWNER-role member**)** · **#237 (NEW 2026-09-10 — 🟡 `get_my_permissions` carries the postgres default ACL, so `anon` holds EXECUTE. **A convention gap, NOT a hole — measured**: with no claims it returns `[]`/false, and AC-3 holds under a real principal. The REVOKE/GRANT pair is written COMMENTED at the foot of `20260910c`; David's one-line call)** · **#236 (2026-09-10 — 🟡 **the install-price save, B.1's defect at a second address.** ✅ Clauses 1–2 FIXED IN THE DATABASE by `20260910b` — the policy is re-pointed at `settings:update` **and a new `_member_select` on `settings:read`** was added, because a read gated on the WRITE string is a different lie. 🔴 **Clause 3 — the section is not permission-gated in the UI — is UNTOUCHED**, and CARD 9 (Lauren editing it for real) has not been run)** · **#195 (NEW 2026-09-06 — 🔴 **TECH-DEBT #186 THROUGH #191 ARE CITED IN THIS FILE AND DO NOT EXIST IN `docs/tech-debt-log.md`.** The log stops at **185**; this line and three §3 entries describe six items above it. Found while claiming an id. **R-26's shape inside our own ledger**, and it means #275's and #276's close-outs were incomplete on the tech-debt gate. New ids start at 192 so nothing is overwritten; transcribing six other sessions' findings inside a catalogue-import build is the drift the gate exists to catch)** · **#194 (NEW 2026-09-06 — 🟡 `api/orders/submit.ts` reads a lot BY ID without the retired filter. Not reachable today — a retired lot cannot be PICKED, so this is only ever reached with an id the picker handed out — but the second site uses `.single()`, which ERRORS rather than returning null when a filter excludes the row, and re-shaping checkout's error path inside a catalogue-import build is scope that build declined. **DECLARED in place** in `retiredFilter.test.ts`'s EXEMPT map, so it prints its reason on every run)** · **#193 (NEW 2026-09-06 — 🟡 a size stated at the FRONT of a QuickBooks description is not read — ~30 items, all fertiliser, compost and bags: *"50lb Bag: Micromax Granular Micronutrients"*, *"15gal Bucket: Regular Compost Mix"*. They report `not_stated`, which is honest and incomplete. 🔴 **THE BLOCKER IS A QUESTION, NOT A PARSER: is `50lb` the SIZE of a bag, or is the size `each` and 50lb a property of it?** `parseUnitOfMeasure` reads it as WEIGHT, correctly — and recording a bag as a weight changes what the uppot ladder and every count screen do with it. David's answer decides whether this is twenty lines or a data-model question)** · **#192 (NEW 2026-09-06 — 🟡 **`soft_delete_inventory` WRITES `status='deleted'`, WHICH IS NOT IN `ALL_STATUS_VALUES`** — `20260720…:744` and `:875` both set it, `inventoryStates.ts:154,161` define the vocabulary as `available · depleted · damaged · returned · archived`, and **5 rows carry it live.** So the one state a person would most want to isolate — *what did we tombstone?* — is the one the grid filter cannot select. ✏️ **AND IT CORRECTS A PROMPT PREMISE:** #277's prompt said the RPC sets `'archived'`; the corpus and the live data both say `'deleted'`. `__harness_replay_lot` IS archived — at qty 0, by another path)** · **#186 (NEW 2026-09-04 — 🔴 **THE TEST RUNNER REPORTED 72 OF 74 FILES AND STILL SAID *All test files pass*.** Two consecutive runs minutes apart on one tree: `74/74 · 3838 assertions` then `72/72 · 3775`. Both omitted files existed and passed when run alone. `readdirSync` racing three other sessions writing into the same tree — and **the short run is indistinguishable from the full one**, because nothing compares the discovered count to anything. [[R-33]] in our own tooling, and it degrades the ratchet and every mutation harness that shells out to it. Fix = a discovered-file floor in `quality-baseline.json`)** · **#187 (NEW 2026-09-04 — 🟡 the UI divergence cap's `SCAN_ROOT` is `packages/cultivar-os/src`, so `QboBooksReader` — in `shared` — was **never counted as bespoke and is not credited as converged**; `undeclared_bespoke_surfaces: 23` is the population of ONE DIRECTORY and nobody reading it is told which. ⚠️ Distinct from #156, which is about import edges)** · **#188 (NEW 2026-09-04 — 🟡 `positionStore.ts:16` says *"a STAFF member holds `settings:read`"*; **`STAFF_DEFAULT_BUNDLE` holds nine strings and no `settings:*` at all.** The FOURTH comment-contradicts-its-own-repo finding in a fortnight. Comment-only fix)** · **#189 (NEW 2026-09-04 — 🟡 the authority cap flags `{isOwner && (` and says nothing about `if (!isOwner) return null` — the same decision, the stronger form, invisible. Not a false green; a cap whose coverage depends on spelling)** · **#183 (NEW 2026-09-04 — 🟡 **RE-INVITING A PERSON MINTS A SECOND MEMBER ROW AND NOTHING STOPS IT.** `create_invitation` (20260828:383-387) INSERTs into `business_members` with **no prior SELECT, no `ON CONFLICT`, no `WHERE NOT EXISTS`**, and the table has **no unique index of any kind** — a whole-corpus grep for `unique|constraint|index` returns ONE hit, the `fk_business_members_invite_id` foreign key. Accept resolves by `invite_id`, so one row activates and the other is a **permanent orphan**; `removeMember` deletes by `id`, so removing the person through the UI clears one and leaves the other on the roster. It grants nothing today (`is_active_member` requires `active=true`), **which is exactly why nobody would notice.** 🔴 **Named in three places and fixed in none** — the RESET INVITE migration header, the client function, and the owner-test board's not-covered list — because the durable fix is a partial unique index and that is **#54/#58's shape: it cannot land until the live rows are known clean, and this session could not read them** (service key would not load, HTTP 000). **OWED, with its blocker stated**)** · **#184 (NEW 2026-09-04 — 🟡 **A DEAD CLEANUP FUNCTION WOULD SILENTLY UNDO THE RESET PATH.** `expireInvitations` flips every expired invitation to `used = true`, and `used = true` is precisely what `reset_invitation_expiry` REFUSES — so wiring it once would tombstone every invitation the Reset invite button exists to rescue, and remove the row from both team surfaces again. **ZERO callers, and it has never had any** (grepped `packages/`, `api/`, `scripts/`; the only reference is its own export). 🔴 **The hazard is not that it runs — it is that it READS LIKE HOUSEKEEPING**: its comment said *"Run this as a cleanup task."* A red warning now stands on it; the DECISION whether expiry-sweeping has any place at all is David's and is unmade)** · **#185 (NEW 2026-09-04 — 🟡 **A RATCHET BASELINE DECLARES A WRITER THAT WRITES NOTHING.** `write-paths-baseline.json` (stamped 2026-08-28) lists `packages/shared/src/auth/invitations.ts` as an **`audit_log` writer** and that file contains **zero occurrences of `audit_log`**. Permissive slack, not a false green — but it means an audit write from that file lands **without the cap noticing**, which is convenient today and is the opposite of what a cap is for. Same class as #73: a declaration nobody re-derives)** · **#182 (NEW 2026-09-04 — 🔴 **A HARNESS THAT CANNOT REACH ITS TARGET REPORTS THE SAME AS ONE THAT PASSED.** David's class name, after the FOURTH instance in a week: #146 (probes matching their own file's PROSE — one *would have passed on a DELETED guard*), #138 (a double more forgiving than the real system), ledger #269 (`[a-z]+` skipping a column keyed `receipt_id`), and #273 (**the probe written to catch #179 could not match `address_line1`, the very column the defect was about** — it reported 13 of 14). 🔴 **[[R-33]]'s sharper sibling: there the check could not FAIL; here it never REACHED the thing.** The tell is a scanner that reports a count it never states an expectation for. **The mechanical fix is a mutant that changes the POPULATION, not the subject — none of our 13 do**)** · **#179 (NEW 2026-09-04 — ✅ **RESOLVED IN THE BUILD THAT FOUND IT, FILED AS THE INSTANCE ON DAVID'S INSTRUCTION.** `VENDORS_SELECT` named **10 columns while its migration creates 14** — the four missing were the ADDRESS. 🔴 **Nothing we own could have caught it: a column with no reader and no writer is invisible to tsc, eslint, knip and every probe.** The list is now the SOURCE and the select DERIVED; `vendorEdit.test.ts` §A parses the columns out of the migration and fails both directions; mutant M1 caught. ⚠️ **The CLASS is open — a declarative list that does not match what its migration creates, across every `*_SELECT` in the repo**)** · **#180 (NEW 2026-09-04 — 🟡 **A COMMENT DEFERRED A FIELD TO A SURFACE THAT CANNOT EDIT IT.** `ReceiptKeeper` said correcting the invoice number *"belongs to the record's ONE edit surface, /receipts/:id (E1)"* — **`/receipts/:id` does not edit `receipt_number`**, proven by grep. So the field was editable NOWHERE while a comment, citing a real clause, said otherwise. 🔴 **A deferral to a surface that was never built reads exactly like a deferral to one that was, and nothing catches the difference — [[R-26]] in our own corpus, THIRD instance this week** (#61, #145, this))** · **#181 (NEW 2026-09-04 — 🟡 **THE DIVERGENCE CAP ABSOLVES ANY FILE THAT IMPORTS `sheetStyles`**, because the shared MODAL chrome is exported from `DataSheet.tsx` and `usesSharedGrid` matches the PATH. A modal that centres itself correctly is counted as using the shared GRID. ⚠️ Not a false green today, but **check C1 would order a still-true declaration deleted** on the strength of a style import)** · **#156 (NEW 2026-09-03 — 🔴 **THE `shared → cultivar-os` BOUNDARY IS ENFORCED BY NOTHING.** tsconfig, eslint, package.json, knip and vite each checked and named: a shared file could import from a vertical today and **every check in `npm run verify` would pass.** It is a convention, not a constraint — *"the reason this decision was ever available to get wrong, and the next person will assume a rule exists"* (David). ⚠️ A cap must assert IMPORT EDGES, not the string `cultivar-os` — five legitimate shared test files read app source by path)** · **#157 (NEW 2026-09-03 — 🟡 **THE PER-VERTICAL PALETTE HAS ZERO IMPORTERS WHILE 42 CULTIVAR LITERALS SIT IN `shared`.** `design-system/tokens.ts:210` exports a complete `cultivar`/`ignition` palette and **nothing imports it**; `#27500A` appears **42 times across 21 files** in `packages/shared/src`. §1.5's known AC-4 violation, quantified for the first time. Filed against `shared`, NOT against `DataSheet` — the 35 were there first. 🔴 The real blocker is unanswered: how does a shared component learn which vertical it renders in)** · **#178 (NEW 2026-09-02 — 🟡 **`packages/shared/src/supabase/types.ts` DECLARES THREE TABLES THAT DO NOT EXIST.** `GrowthGoal`, `NotificationLog`, `AIUsageLog` — probed live against a NEGATIVE CONTROL (a name that cannot exist) plus `receipts` as a positive one: all three **ABSENT**. All carry `tenant_id`, the pre-`business_id` naming; none is imported anywhere. The fourth, `Vendor`, was removed by #259 — **only it, because deleting types unrelated to vendors inside a vendor build is the scope creep that makes a diff unreviewable.** ⚠️ **The file's own header still calls these *"tables that exist in every vertical's schema"***, which is the claim that needs correcting deliberately. **The cost is not clutter — it is a CONFIDENT WRONG ANSWER**: anyone reading this file to settle *"do we have X?"* gets YES. Same class as the inventory doc that said a function slot was free at 12/12)** · **#145 (✅ **RESOLVED-IN-FACT, AND IT HAD BEEN FOR TWO DAYS — CORRECTED 2026-09-03.** The row read *"🟡 no `/receipts/:id`"* while the route was SHIPPED (`router.tsx` → `<Route path="/receipts/:id" element={<ReceiptDetail />} />`), boarded (`receipt-detail-full-surface-test.md`, 12 cards) and WIRED — `ProjectCostDrillIn.tsx:199` navigates to `` `/receipts/${receiptId}` ``. **TWO records said the route did not exist and the second was that same file's own header, 170 lines above the code that used it.** Both corrected. ⚠️ **THE RESIDUAL IS REAL BUT IT IS A DIFFERENT DEBT: the fallback to `/receipts` fires whenever a cost item carries no `receipt_id`, which today is ALL of them — `cost_objects.receipt_id` is populated 0 of 5 (tech-debt #144). The link is right; the data it needs is absent.** 🔴 **The finding is not the stale row, it is that a stale row on the exact question a build prompt was about went unchallenged — R-26 inside our own corpus.**)** · **#144 (NEW 2026-09-01 — 🔴 **THE COUNT-ONCE DEDUP SEAM HAS NEVER BEEN WRITTEN BY ANYTHING**: `cost_objects.receipt_id` 0/5, `business_inventory.receipt_id` 0/447, measured. Three migrations describe it as the join that PREVENTS DOUBLE-COUNT; a seam nothing populates prevents nothing. **Blocks the cost-assignment build**)** · **#143 (NEW 2026-09-01 — 🟡 nothing stops the same document being captured twice and TWO PAIRS ARE LIVE; one vendor invoice appears as two sales, **$1,283.88 overstated**. `receipts` has no unique index but its PK. Durable fix is a partial unique index that **cannot land until the four rows are settled** — #58's shape. SURFACED, not repaired)** · **#78 (✅ resolved 2026-08-01 — ratchet caps re-keyed on `file::binding#table.verb`; counts proven identical before re-locking. → **ledger #177**)** · **#89 (✅ resolved 2026-07-31 — `useModules` filtered `can()` before reading `status`; `<BeingBuilt>` mounted. → **ledger #176**)** · **#90 (2026-07-31 — 🟡 **PARTIAL 2026-08-28: the BLOCKER is gone and the scope SHRANK to two verbs.** Originally: REMOVE `team:create/update/delete` rather than leave them mis-described, since team changes go through the funnel — owner-only by design, enforced by a trigger — which is *"never"*, not *"unbuilt"*. 🔴 **`team:create` IS NO LONGER ONE OF THEM: R-22 wired it**, and it is enforced by `create_invitation` plus two `invitations` member policies (#228). **`team:update`/`team:delete` remain the original item.** 🔴 **AND THE BLOCKER RESOLVED ITSELF BY BEING HIT FROM THE OTHER SIDE:** the obstacle was capQ's retained `migration ⊆ manifest` direction failing when a string leaves BOTH manifest sets while `20260727_rbac_resource_action_flip.sql` cannot be edited (§6 r1). #228 hit exactly that with three strings and fixed it properly — a **self-pruning declaration** (`r-b2-wired-since-declarations.json`) that fails the build if an entry goes stale or was meaningless to begin with. **Removing a verb is now a declaration away, not blocked.** Ledger #175 · #228)** · **#87 (2026-07-31 — 🟡 `navPermission()` falls back to **`view_dashboard`**, retired into the `member` sentinel. **UNREACHABLE TODAY (0 of 26 nav nodes)** — a landmine, not a live defect: the next node added without a `tileKey` or its own permission goes invisible to everyone including the owner. DECLARED in `authority-grants-baseline.json`, prints RED every capA run. Fall back to `member`, or delete the fallback and make the field required? Ledger #174)** · **#88  by MINTING `reports:read` as the NEW `planned` status — the string was gating a tile without existing in ANY manifest. Ledger #175)** · **#85/#86 (✅ resolved 2026-07-31 — two legacy-string client gates that admitted NOBODY (`view_costs` → `costs:read`, `import_pricing` → `inventory:import_price`). → **ledger #174**)** · **#84  BY RULING, NOT BY BUILD (✅ resolved 2026-07-31 BY RULING, not by build — a permission gates a CAPABILITY, not a field. → **ledger #173**)** · **#75 (NEW 2026-07-28 — 🟡 **A DEVICE CHECK THAT DISABLES ITSELF ON ERROR** — `[TRACE:DEVICE] device read failed` FAILS OPEN, observed live in the MANAGER session on `11c2e48`. A security-shaped check whose error path is "allow" is not a check, and the failure is INVISIBLE (a trace log is not a surfaced error) — the #158 class again. Logged as OBSERVED, not investigated: what is owed first is which call fails and whether the guarded surface is one where fail-open is a deliberate recorded choice. **Convenience → fine; authorisation → security defect.**)** · **#74 (✅ resolved 2026-07-28 — a Save that changed nothing keeps its audit row as `no_change`; compared as SETS. → **ledger #163 · STD-023**)** · **#73 (NEW 2026-07-28 —`verify-universals.mjs`'s `OWNER_ONLY_PENDING` is a HARDCODED gap list and SIX of its nine tables now carry member policies in repo migrations (`orders`/`order_items`/`order_service_selections`/`order_compliance_records`/`customers` via `20260724`, `social_drafts` via `20260727g`); only `plant_events`/`addons`/`nursery_profiles` are still real. **Stale noise, NOT a false green** — the cap passes on its real assertion — but a gap list that only grows stops being read, and it prints on every `npm run verify`. Fix = DERIVE it from the policies the script already parses (STD-011); tech-debt #63's class. Surfaced not fixed — rewriting a checker inside a table-DROP pass is the drift the gate exists to catch)** · **#71 (NEW 2026-07-22 — one `status` column, two authors: D-42's qty-derive overwrites D-52's tombstone; a deleted lot reads `depleted`, a manual `archived` is reverted, and the grid still offers Delete on a tombstoned lot surfacing raw `already_deleted`. Durable fix = lifecycle state in its own field — a MIGRATION. Logged not fixed, per David)** · **#72 (NEW 2026-07-22 — the `sale` ledger row's `reason` is NULL while every neighbouring kind explains itself; carry the order number at the emit sites next time the order path is touched)** · #2 (QB hardcode), #3 (social in cultivar), #4 (nursery footer), #8 (RLS unverified), #10 (SavingsReport missing), #12 (Ignition AI dark / Railway kill path), #13 (stub duplication), #16 (MarginEngine orphaned — A callers + plants.cost_price), #17 (dead migration), #18 (pin_hash unverified), #19 (instagram fallback), #20 (platform union), #21 (orphaned campaigns files), **#22 (✅ resolved 2026-08-22 — the migration WAS applied and the row read "David must apply" for 74 days: the STD-008 inverse's third form, migration-applied-but-recorded-as-pending. → **recon 2026-08-22**)** · **#91 (NEW 2026-08-22 — 🔴 **THE TWO PLATFORM CHECKS DISAGREE AND ARE NOT A SUBSET EITHER WAY.** `social_drafts` takes 5 (`…tiktok, twitter, sms`), `campaign_posts` takes 4 (`…sms, email`) — `email` is a value NOTHING can produce, `tiktok`/`twitter` are values the ONLY config UI offers. ⚠️ **PREMISE CORRECTED: `campaign_posts_platform_check` IS in version control** — `20260529_campaigns.sql:26-27`, declared INLINE, so Postgres auto-names it and the NAME is never typed; a name-grep could never find it. **~129 inline CHECKs share that property**, which is what #23's sweep must match on DEFINITION not `conname`. Blast radius total not partial: one atomic multi-row insert + an already-committed campaign row = **a campaign with zero posts**, two live)** · **#23 🔴 ESCALATED 2026-08-22 (two instances on adjacent tables in ONE evening = an unmeasured class per #174; sweep written 2026-06-09, never run, and 08-22 changed what it must do — match on DEFINITION, both directions)**,  #24 (opaque names), #25 (6 AI features dark), #26 (orphaned DataBridge keys), #27 (10 tables no migrations — IGNITION only; `losses` is NOT this entry, see #39), #28 (pilot_all RLS open), #29 (receipts naming), #30 (voice-samples RLS scope), #31 (catalog-verify process), #32 (cultivar_plants anon read open), #33 (widget-header backfill), #57 (✅ resolved 2026-07-16 — counting a variety made it unscannable; fixed by D-49's one invariant in `countPromote.ts`. → **ledger #133**) · #59 🟡 NEW 2026-07-16 (`TRACE-SESSION-BOOTSTRAP.md`'s header carries the **STD-011 duplicate-header disease AND IS LOADED EVERY SESSION** — §10's Session Starter opens it FIRST, so its `Last updated:` prose block is a per-session token tax exactly like CLAUDE.md line 3 was. **OP-13's own triage put it with the ledger/DECISIONS-INDEX as "not loaded every session" — that was WRONG for this one**; the triage stands for the other two. The header-is-a-POINTER clause should extend to it — David's call. Ledger #135) · #58 🟡 NEW 2026-07-16 (**the DB-level guard for the (variant_group, size) pair** — the durable form of ledger #135's defect 2: a partial unique index `(business_id, variant_group, size) WHERE variant_group IS NOT NULL AND size IS NOT NULL`. This is **ledger #74's deferred option C**. It is a MIGRATION and **it would REJECT the live Acoma dup, so it cannot land until the data is clean** (i.e. after the regenerated remediation). The code guard `findSizeTwin` is proportionate at a single-owner nursery's volume. **Sibling of #54** — the `qb_customer_id` partial unique index, named-not-taken on the same reasoning) · #56 (✅ resolved 2026-07-23 — size vocabulary normalized before COMPARISON via the one shared `normalizeSize`. → **ledger #150** ⚠️ residual: existing same-size-different-spelling rows still need a MERGE) — original defect was: (SIZE VOCABULARY not normalized on the count path — **the last live defect of the D-49/#135 family**; `findSizeTwin`/`sameSizeLabel` are exact equality and the catalog carries SIX spellings of THREE sizes (`15`/`30`/`45`/`5 gal`/`30 gal`/`45 gal`), incl. on **'Sierra', a DEMO variety**. Unlike #135's four, it can **MERGE existing rows** → needs a read-only blast-radius probe first — `'Sierra'` is live with `["15","30 gal"]`, so counting "15 gal" against the "15" row mints a THIRD row: two spellings of one physical size, on-hand split across both. D-45 did this for NAMES, nobody did it for SIZES. The next defect in the D-49 family — its own blast radius: unlike D-49 it can MERGE existing rows) · #34 (✅ resolved 2026-06-19 — `router.ts:15` import depth. → **commit 14a9a82**) · #35 (✅ resolved 2026-06-15 — `nursery_profiles` 406 → `maybeSingle`) · #36 (/assets + /pmi nav-dead) · #37 (PMI UI polish pass) · #38 (frictionless multi-channel cost capture — NEXT MAJOR BUILD after Core-2b; capture≠classification, hard-blocked on Core-2b sameCost dedup) · #39 (live schema not in version control — orders/customers/order_items + qb/leakage/netting cols live-only; **🟡 RE-SCOPED 2026-07-28: `losses` leaves this class by DELETION, not by capture** — GATED `20260727d`, ledger #162. A legitimate way off the list but a DISTINCT one, and the blind spot it created is exactly what BLOCKED 27d: no migration ⇒ a source grep could not see `losses_nursery_id_fkey`, so the check had to be `pg_constraint`-based. `nurseries` leaves the same way) · #40 (built-inventory flag correction — 5.1 inventory / 5.2 PMI are resolved, not ⚪) · #41 (Vercel Hobby 12-function ceiling — `api/` at limit; new functions silently fail deploy; mitigated by folding deliveries→customers; upgrade to Pro before next module wave) · #55 (✅ resolved 2026-07-16 — `canonicalName` apostrophe defect; 4 of 6 possessive varieties were broken in live data. → **archive #132**) · #54 🟡 (qb_customer_id collision guard is code-level/TOCTOU — durable form is a partial unique index `(business_id, qb_customer_id) WHERE NOT NULL`; a MIGRATION; theoretical at zero links, needed before real billing volume) · #53 (✅ resolved 2026-07-16 — QBO matched on EMAIL ALONE and cross-billed nine real invoices; fixed by the D-47 three-way rule. → **archive #131**) · #42 (✅ resolved 2026-06-21 — `seed.ts` D-9 silent coercion) · #60 🟡 NEW 2026-07-17 (**DEPLOY-VERIFICATION GAP — Vercel deploys the TREE not the COMMIT, and nothing between push and test confirms the build SUCCEEDED.** `313de44` (#135) never deployed — its Vercel build FAILED after clone — and #135 went live ~20h later only as a side effect of pushing the #137 markdown (`77ffd8e` carries it; merge-base CONFIRMED). One layer deeper than #128/#129: "the bundle can't be stale if it was never built." **✅ RULE RATIFIED 2026-07-17 → OP-15** (owner-prove STEP ZERO = confirm the deploy for THIS SHA is READY, before hard-refresh; homed PRIMARY on the owner-test board's GATE 0 where David stands, per row-19B). **Mechanical SHA-STAMP SHIPPED 2026-07-20 (ledger #141) — BUILDER-COMPLETE, owner-prove OWED, so this row stays 🟡:** vite `define` → `__COMMIT_SHA__` (7 chars, `'dev'` off-Vercel) → DebugPanel footer; GATE 0 ① is now "does the app say the SHA I pushed?" **Closes only on David's live match** — a stamp nobody has read is a claim, and a wrong one would silently un-verify every future GATE 0) · #61 🟡 NEW 2026-07-17 (**`countPromote.ts:24` comment is FALSE** — asserts scrape-reads-variations "never built"; `fetchProductVariants`/`extractSizeVariants` EXIST + wired (added 06-28/06-30). A comment contradicting its repo fed two days of wrong reasoning. Comment-only fix) · #62 🟡 NEW 2026-07-17 (**DataSheet viewport standard DRIFTED, one item both halves** — h-scrollbar anchored to table-content bottom not a fixed-height viewport (123 rows down at 123 rows); AND sell-price/conf columns past the RIGHT fold, the sell-decision column cut off on the "board 5.1 reconcile surface." A set convention drifted, not a missing pixel; inventory is the first DataSheet consumer long enough to surface it. Recon: is the viewport in `DataSheet.tsx` or per-page — do assets/customers pin while inventory doesn't?) · #70 🟡 NEW 2026-07-22 (**GENESIS `opening_balance` ROWS CARRY A SYNTHETIC `occurred_at`** — the D-50 backfill dates them at MIGRATION-APPLY TIME (`20260720…:360`, `now()`), not the lot's origin, so ~126 rows carry a timestamp describing *when we adopted the ledger*, not when anything happened. Produced the 2026-07-22 reconcile defect: for a lot last counted before 2026-07-20 the genesis row fell INSIDE the window and replayed as fresh arrival (prior 60 + genesis 60 = 120 vs book 60 — an apparent *exact doubling* from ONE read). **FIXED at the consumer** (`isMovement()` excludes position-assertions — independently correct, not a date patch) **but the hazard is in the DATA** for the next time-windowed reader. Re-dating is **REJECTED** — an UPDATE on an append-only table whose trigger rejects even `postgres`. Ledger #146) · #67 🟡 NEW 2026-07-21 (**BLIND CAPTURE IS THE MISSING HALF OF RECONCILE — and it is one of the D-50 story's own two OPEN build inputs.** `InventoryCount.tsx:438` calls `count_reconcile_inventory` **at capture**, so the phone count APPLIES ITSELF: by the time the desk reconcile screen opens, book == counted and the residual is **0 by construction**. The new DELTA mode is real for movements landing **after** a count, but it is **not** the count-then-review loop the story describes (*"the owner reviews the walk as a unit"*). `blind_capture_mode` — phone records to `inventory_counts` WITHOUT moving qty, desk becomes the applier — is named in that story's `PIECES:` and left as **BUILD INPUT (2), explicitly owed to David**. A CAPTURE-path change, deliberately not taken in the reconcile build. Ledger #145) · #68 🟡 NEW 2026-07-21 (**RECONCILE IS NOT SESSION-SCOPED — the spec asked and it is honestly not built.** Works per-lot across the catalog instead. Blocked by #67 (a session view would render a screen of "agrees — done" proving nothing) AND `inventory_count_sessions.status` already has ONE owner in `InventoryCount.tsx` — a second writer of one lifecycle field is the STD-011 drift we keep paying for. Lands WITH blind capture, as one build. Ledger #145) · #69 🟡 NEW 2026-07-21 (**A MULTI-STEP ACCEPT CAN PARTIALLY LAND, AND APPEND-ONLY MEANS NO ROLLBACK.** An attributed reconcile issues 2-4 sequential RPCs; each is atomic, **the sequence is not**. Mitigated not fixed: the UI names the step it stopped at and says earlier rows are already permanent, and the closing step lands ABSOLUTELY on `counted` so a *successful* run self-corrects against concurrent sales. Durable fix = ONE RPC taking the whole plan as `jsonb` in one plpgsql transaction — a MIGRATION. Sibling in shape to #54/#58. Ledger #145) · #63 🟡 NEW 2026-07-17 (**THE CAPTURE INDEX HAS NO GATE** — every other canonical surface has a reconciliation gate; the 📚 retrieval index is maintained by a step-6 REMINDER, and a reminder rots (row-19B). Found because David asked Lightning its humor style and it said "not encoded anywhere" — it was OP-2, canonical since 2026-06-03, absent only for lack of an index row. Sweep filled 21 missing rows (0 stale). Through-line: **the artifact does not carry its own provenance.** NOT an OP — named, numbered, left to earn a gate)
...
**Type:** BUILD, on `main`. ⏳SHA. **BUILDER-COMPLETE** — no migration, api/ 12/12, no new permission string. `npm run verify` ⏳VERIFY · four new test files, 108 assertions, surface probes each beside a mutant. **§3 RETENTION: 1 archived verbatim (#298), 1 written — entries-in == entries-out.**
...
- [x] `nursery_modules` → `business_modules` ✅ 2026-06-04 — migration written, 6 API/hook files repointed, membership-scoped RLS, build clean. ⚠️ David must run migration in Supabase SQL editor before deploying, then run `node scripts/verify-business-modules.mjs` to confirm counts, then `DROP TABLE nursery_modules CASCADE;` ✅ **The DROP is done** — measured absent 2026-09-11.
...
### 🔴 PENDING DATA TASK — DAVID RUNS IT (R-120, ledger #293)

- [ ] **LAWNS's service rows — THREE, not one.** Their invoices carry three fulfilment modes: **TC trip charge** (533 lines, $40,760 — staff) · **Tailgate Delivery** (127 lines, $18,990 — staff, but a DIFFERENT service: a curb drop, not carried in) · **self-collect** (most customers; NO invoice line, because nothing is charged). All four LAWNS rows read `category='addon'` today — Lightning's 2026-09-09 workaround, a symptom, not a choice. **LAWNS stays read-only to Thunder; David writes these.** 🔴 **Blocked on tech-debt #251** — checkout offers one staff/flat row, so trip charge + tailgate would hide one.
- [ ] **Run services-review CARD 19 (the half-bound sweep) BEFORE applying `20260911_service_offerings_transport_requires_mode.sql`** — its pre-flight refuses while any such row exists, by design.
...
11. **FUNCTION THRIFT — the Vercel Hobby 12-function ceiling (binding, STOP-and-surface):** the deployed backend is the repo-root `api/` directory (per §2 "api/ at repo root" — 12 shim files re-exporting `packages/cultivar-os/api/*`), and Vercel Hobby caps a deployment at **12 serverless functions**. We are at **12 of 12 — zero headroom.** A 13th function does NOT error loudly; it makes the whole deploy **fail silently** and Vercel keeps serving the last-good bundle (this bit us on 2026-06-20 — the deliveries endpoint was function #13, so every deploy silently failed and prod served stale code until `api/deliveries/create` was folded into `api/customers/create`). Therefore:
    ...
    ✅ **PLAN VERIFIED HOBBY — David, 2026-09-08. The 12-function ceiling is REAL and we are AT it: `find api -name '*.ts' | wc -l` = 12, counted the same day.** Recorded so nobody re-litigates the ceiling. ⚠️ **Supabase is the PAID plan (backups); Vercel is NOT** — do not reason from one to the other.

    🔴 **AND THE DURATION NUMBERS ARE NOT WHAT EITHER OF US THOUGHT — CHECKED AGAINST VERCEL'S OWN DOCS 2026-09-08, TWO PAGES, BOTH STAMPED `last_updated: 2026-08-24`** (`/docs/functions/limitations#max-duration` and `/docs/functions/configuring-functions/duration`):
    ...
    ⚠️ **THREE STATED FIGURES WERE WRONG AND ARE CORRECTED HERE RATHER THAN QUIETLY:** *"Vercel's unstated 10-second Node default"* (Thunder, 2026-09-08 recon — an assumption stated as a measured fact, [[R-26]]'s shape) · *"Hobby maxDuration ceiling is 60s"* (the 2026-09-08 prompt — true historically; the Hobby raise to 60s is a Vercel changelog entry that has since been superseded by 300s) · and the conclusion drawn from both, that `handleBooksPreview` carries a *"latent 10-second timeout"*. **On the published numbers there is no such timeout.**

    ✅ **FLUID COMPUTE IS ENABLED — CONFIRMED IN THE DASHBOARD BY DAVID, 2026-09-08.** That was the one fact nothing readable from outside could settle (the response headers carry only `x-vercel-id` / `x-vercel-cache`; the pulled env files hold no such flag), and it is what makes the table above the operative one. **So this project's real ceiling is 300s default and 300s maximum, and it always was.**

    🔴 **THEREFORE NOTHING IS SET, AND THAT IS THE CORRECT STATE — NOT AN OMISSION.** `maxDuration: 60` would have **CUT the ceiling from 300s to 60s**, a fifth of what we have. Anyone who reads *"no `api/` file configures a duration"* and reaches for a fix is about to make it worse: **the project default IS the answer here.** The only value worth setting would be one BELOW 300 to cap a runaway function deliberately, and that is a decision about a specific function, never a blanket one. ⚠️ **`maxDuration: 300` is also not worth adding** — it is a no-op at the ceiling, and a number written in twelve places is a number that goes stale when a plan changes.

    ⚠️ **NO `api/` FILE CARRIES `maxDuration` OR `export const config` — all 12 run on the project default.** Grepped across `api/`, `packages/cultivar-os/api/` and `vercel.json`, 2026-09-08: zero occurrences.
