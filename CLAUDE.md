...
# Last updated: 2026-08-24 (6) — recon: the QuickBooks invoice push; four issues on one sent invoice (see §3)
...
### 2026-08-24 (5) — THUNDER **RECON: WHERE SELLABLE THINGS LIVE, AND WHERE OUR LINE ITEMS LIVE — DAVID'S PREMISE IS WRONG IN THE DIRECTION THAT MATTERS, AND THE CORRECTION IS GOOD NEWS.** David: *"how many tarps did we sell… it is a line item on the receipt why is this hard?"* 🔴 **ON THE SELL SIDE IT IS NOT HARD AND IT IS NOT JSON: `order_items` IS A REAL RELATIONAL LINE-ITEM TABLE — one row per thing sold, `quantity` · `unit_price` · `subtotal` · FK `business_inventory_id`** — and `order_service_selections` carries the service lines with the same shape. **QuickBooks reads FROM it: `pushQboInvoice` assembles every QB line from our own rows (`cultivar.ts:381-440`) and only TWO pointers come back (`qb_invoice_id`/`qb_invoice_url`, `:660-668`).** ✅ **`business_inventory` IS GENERIC, NOT PLANT-SHAPED — the deciding question, answered from the CREATE TABLE: the whole NOT NULL set is `business_id` + `name` + `qty` + `status` + timestamps; `size`/`variant_group` are NULLABLE columns added 16 days later, and the table's ONE check constraint is about `cost_confidence`.** 🔴 **THE REAL GAP IS ON THE OTHER SIDE OF THE BUILDING: there is NO purchase-line concept anywhere — `purchase_order`/`goods_received`/`purchase_line`/`bill_line` return ZERO hits across migrations, cultivar and shared.** **LOOK ONLY — no app code, no schema, no migration, no policy, no permission string, no cap** · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **30/30 test files · 1124 assertions** · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · **GATE 0 NOT APPLICABLE — nothing ships** · all `[TRACE:*]` ON.

**Type:** Recon + close-out. Docs: **NEW `docs/decisions/2026-08-24-sellable-things-and-line-items-recon.md`** (the one deliverable, 333 lines) · ledger **#213** · `tech-debt-log.md` (**#102 · #103 NEW**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **`built-inventory.md`: ZERO net-new entries and ZERO amendments — nothing was built and no capability's state changed, which is the honest answer rather than a touched date.** **Owner-tests: no card added, updated or flipped — no surface was added, changed or polished.** **§3 RETENTION: 1 archived verbatim (2026-08-24 (2) #208, `in`-check confirmed on 5,994 chars), 1 written — entries-in == entries-out, §3 back to 3** (archive 241 → 242). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (186 lines incl. the whole OWED table).**

**🔴 THE FINDING WORTH MORE THAN EITHER ANSWER, BECAUSE IT REFRAMES WHAT DAVID SHOULD ASK FOR NEXT: THE TWO HALVES OF HIS QUESTION ARE NOT THE SAME PROBLEM AND DO NOT HAVE THE SAME FIX.** *"How many tarps did we sell"* is answerable today from `order_items ⋈ business_inventory.name ⋈ orders.created_at`. *"How many happy hoses did we buy"* is not answerable at all — **and the sharpest instance is that the receipt OCR ALREADY ASKS THE MODEL FOR `quantity` AND `sku` per line (`ocr.ts:64,94,108`) and the save THROWS BOTH AWAY in one assignment (`ReceiptKeeper.tsx:259`)**, writing only `{description, amount}` to `receipts.line_items`. ✏️ **So the honest deliverable was a recon that says "this half is built, stop redesigning it" as loudly as it says "this half is missing"** — because the storage-format framing would have sent a build at the half that already works.

**FLAGGED FOR DAVID:** **(a)** ✅ **THE ANSWER TO YOUR THIRD QUESTION, PLAINLY: the invoice's CONTENT is stored here in full** — lines, quantities, unit prices, discounts, tax, total — **and `DemoQBInvoice.tsx:65-77` PROVES it by re-rendering an invoice from our own rows.** What lives only in QuickBooks is the **rendered document and the money lifecycle after it**, which is the boundary **D-37 chose on purpose** (`2026-07-10-money-boundary.md:43-45`), not an accident. **The invoice NUMBER is generated here too — `orders.notes` (`submit.ts:696`).** **(b)** 🔴 **DELIVERY AND INSTALL ALREADY HAVE HOMES AND ARE SEEDED — I did not design one and did not need to.** `'We deliver'` (`20260529…f_service_offerings.sql:104`) and **`'We deliver and plant'` at `225.00` per plant (`:115`)**. **So C1(iv) — *what did we charge for install last quarter* — is FULLY answerable today with nothing missing.** **(c)** 🔴 **TECH-DEBT #103 IS A LATENT MONEY DEFECT AND THE TRIGGER IS A CATALOG ROW, NOT A CODE CHANGE: every `per_unit`/`'plant'` service multiplies by `itemCount` = the sum of ALL cart lines (`submit.ts:218`), with no notion of which are plants.** 1 tree + 2 tarps on self-transport bills **netting ×3 = $30**; planting would bill **×3 = $675**. **Mitigated not silent** — `qtyFor` honors an owner override first (`:490-494`) — **but it goes live the day someone adds a tarp.** **(d)** ⚠️ **THERE IS NO PRODUCT-KIND COLUMN ANYWHERE ON `business_inventory` — no `type`, no `category`, no `kind`.** A tarp fits the table with nothing added, but **the only handle for *"which rows are tarps"* is `name` text-matching**, and #103 cannot be fixed until something distinguishes the two. **(e)** ⚠️ **A CLAIM I DELIBERATELY DID NOT MAKE: whether any tarp or fertilizer ROW exists live.** This machine has no catalog access and the 2026-08-22 ruling says a database claim is sourced from the catalog or not made — **so I proved the SHAPE from the CREATE TABLE and declined the row count.** **(f)** ⚠️ **ONE STALE SOURCE NAMED SO NOBODY CITES IT LATER: `2026-07-07-live-schema-map.md:24,44` asserts *"There is NO price, sell_price… column on this table"* — TRUE when written, FALSE now** (`sell_price` landed the same day in `20260707_business_inventory_sell_price.sql`). Useful for shape; **do not cite its money claims.** **(g)** ⚠️ **`order_items` and `orders` still have NO CREATE-TABLE migration** — tech-debt **#39**, live-only. Their columns are proven by four ALTERs and by the code; **`orders.created_at` is sourced from CODE not the catalog** (four readers, one sorts on it — `Orders.tsx:50,59`) and **`order_items` has no timestamp of its own**, so a date always comes through `orders`. **(h)** ⚠️ **ONE STATED DEVIATION (§6 r10 / §9 gate 10, no silent divergence):** the three-lens gate normally requires **OPTIONS spanning NEED→WANT; this prompt's scope bar forbade options in Parts A and B and forbade designing a goods model.** **I honored the prompt and present none** — the lenses carry HAVE/NEED/WANT without collapsing to a recommendation, and **options are owed the moment you ask for a build.** **(i)** ✅ **`git log`/`git status` re-run at write-up.** **(j)** **CLAUDE.md is under its ~600 budget.**

### 2026-08-24 (4) — THUNDER **BUILD: THE TILE ENABLE/DISABLE ROUND TRIP — THE FUZZ'S SWITCH, AND ALMOST NOTHING BEHIND IT IS NEW.** David: *"build the mechanism."* 🔴 **THE PLATFORM HAS DOCUMENTED AN OFF PATH NOBODY COULD REACH: `set_business_module_state` has accepted `p_enabled=false` since it was written, its own refusal string reads *"enabling **or disabling** a module changes what this business pays"* (`20260802c:127`), and `ModuleStatePatch.enabled` is already `boolean` — #201's "ZERO callers pass false" CONFIRMED at all three write sites.** ✅ **`Turn off` now renders in Included and Active; disable is ONE UPDATE, there is NO DELETE anywhere in the function, and the config merge preserves the trial pair — the data is unreached, not destroyed.** ✅ **R-1 LANDS AND IS DERIVED, NOT RE-SPELLED: `mayBeSwitchedOff = !enabledByDefault(billing)` — the ruling's own *"same one decider, read one more place"* — and a core row renders NO control AT ALL, absent rather than greyed.** ✅ **R-12: `enabledAfter` was being read off the RPC, LOGGED, and thrown away; it is now RETURNED as a tristate and ASSERTED.** 🔴 **B3 STOPPED AT THE SWITCH ON PURPOSE — THE SERVER-SIDE FUZZ AGGREGATE DOES NOT EXIST (twelve `fuzz` hits, every one prose).** **BUILDER-COMPLETE `357f481`→this commit** · NO schema, NO migration, NO policy, NO new permission string, NO new cap, NO new api/ fn · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **30/30 test files · 1124 assertions** (+1 file, +32) · api/ **12/12** · **GATE 0 OWED — David confirms Vercel READY for THIS SHA before reading any screen** · all `[TRACE:*]` ON.

**Type:** Build + close-out. Code: `packages/shared/src/business-logic/moduleState.ts` · `moduleRoundTrip.test.ts` (**NEW, 26 probes**) · `packages/cultivar-os/src/registry/tileRegistry.ts` · `moduleSeed.test.ts` (**+6 probes**) · `packages/cultivar-os/src/pages/Subscription.tsx`. Docs: ledger **#212** · `RULINGS.md` (**R-1 OPEN → IMPLEMENTED**; R-2/R-3 annotated STILL OPEN and now REACHABLE; R-12 second client-side instance; **F-A annotated NOT ANSWERED**) · `tech-debt-log.md` (**#100 · #101 NEW**) · `built-inventory.md` (**1 net-new entry, 1 amended**) · owner-tests (**cards 34 · 35 · 36 ADDED, card 32 AMENDED — nothing marked `covered`**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-24 #207, `in`-check confirmed), 1 written — entries-in == entries-out, §3 back to 3** (archive 240 → 241). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (186 lines incl. the whole OWED table).**

**🔴 THE FINDING WORTH MORE THAN THE SWITCH, AND IT IS ABOUT WHAT I DID *NOT* WRITE: THE RULING FORBADE THE OBVIOUS FIX, AND THE OBVIOUS FIX WAS IN SCOPE.** B3 asked me to stop a disabled module reading as `'available'` (`useModules.ts:122` — the same pixel as never-enabled AND as a missing row, three facts collapsed into one). **The cheap patch is a fifth `TileState`. R-2 closes exactly that door in its own words — *"not absent, **not a fifth flat state** — the fuzz"* — so minting `'disabled'` would be ANSWERING A RULING WITH A CONSTANT**, and a CSS blur is refused by the same ruling BY NAME (*"#81 with a filter on it"*: the payload is still in the response and one devtools line reads it). **The ruled destination is a SERVER-SIDE AGGREGATE and it does not exist.** ✏️ **So the honest deliverable was the switch plus a filed gap, not a switch plus a plausible-looking tile** — tech-debt **#100/#101**, and owner-test **card 36** exists so the absence is not refiled as a bug by the person testing it.

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE LIMIT YOU NEED BEFORE YOU DEMO IT: ONLY ONE MODULE ANYWHERE GATES ON `enabled` — `api/social/generate-posts.ts:52`.** For ten of eleven catalog entries, *"off"* today means **off on the marketplace page**: the tile still looks normal and the route still works. **Card 34 says to test the round trip on Social Media specifically**, because it is the one place "off" currently means anything. **(b)** 🔴 **`/delivery-schedule` WITH DELIVERY OFF IS A SEPARATE PROBLEM AND IS FILED, NOT FIXED (#101).** It is gated at `router.tsx:149` on `deliveries:read`, a PERMISSION; a grep of all 48 routes for enablement returns **ZERO**. It has been true since the router was written — **this build did not cause it, it made it REACHABLE** — and R-3 rules the fix is the fuzz overlay and **never a guard**, so it is blocked on the aggregate. **(c)** ⚠️ **F-A IS NOT ANSWERED AND NO DEFAULT WAS PICKED.** The confirmation copy DOES differ (`core_optional`: *"your settings are kept"* · `add_on`: *"the current period runs out as normal"*) because the two facts differ and **neither claims a refund** (R-8) — but F-A is about what a disabled TILE does when SELECTED, on a surface that does not exist. Different moment, still yours. **(d)** ✅ **THE ROUND TRIP IS PROVEN BY COUNTING, NOT BY READING A STATUS — the `4056de8` lesson applied to its own successor.** The fake HOLDS STATE and is modelled statement-by-statement on `20260802c:60-185`: enable → **7 rows** → disable (`enabledAfter===false`, **7 rows still**, **0 deletes**, **1 rpc call**, trial pair byte-identical) → re-enable (**the same 7 rows BY IDENTITY**, `trialStarted===false`). **D1 plants the silent no-op and proves `applied:true` rides over a database that was never written** — the probe that would have caught #207 a day earlier. **(e)** ✅ **RED-FIRST THREE WAYS (STD-024), all restored `diff`-identical:** the coercion defect planted into the real `moduleState.ts` fails 2; `mayBeSwitchedOff` forced always-false fails 4, always-true fails 3. **(f)** ⚠️ **A CLAIM I DELIBERATELY DID NOT MAKE: whether `20260802c` is applied.** This machine cannot read the catalog and the 2026-08-22 ruling says a database claim is sourced from the catalog or not made. **It does not gate this build** — no parameter, signature or migration changed, and the client already sends all 7 arguments on every enable. **(g)** ✏️ **ONE STALE HEADER CORRECTED IN A FILE I WAS ALREADY TOUCHING (§1.6 fix-in-one-pass):** `Subscription.tsx`'s header asserted *"it does not start a trial either — RULING OWED"*, which you ruled on 2026-08-02 (8) and the code below it has done ever since. **R-14's class: nothing mechanical reads a comment, so a stale claim survives its own correction.** **(h)** ⚠️ **ONE STATED DEVIATION (§6 r10, no silent divergence):** the `Turn off` button is `minHeight: 40` matching its `Turn on` sibling where §6 r3 sets 48 — **row coherence over restyling a proven control days before LAWNS**, on a desktop-only owner surface. Flagged; it covers both buttons. **(i)** ✅ **THE STORY GATE WAS A MATCH, NOT A CONFLICT — and M-B is already answered.** `user_stories.md:1054` is `scoped-out`, but its **2026-08-23 amendment says so explicitly**: *"It was never about a module's ON/OFF SWITCH, and that switch is IN SCOPE… does NOT contradict this row and does not need it flipped."* **No `scoped-out` row was flipped.** **(j)** ✅ **`git log`/`git status` re-run at write-up.** **(k)** **CLAUDE.md is under its ~600 budget.**
