...
# Last updated: 2026-08-24 (4) — build: the module off switch; the fuzz is not built (see §3)
...
### 2026-08-24 (2) — THUNDER **RECON: WHAT DOES THUNDER ACTUALLY USE IGNITION FOR? THE ANSWER IS *ALMOST NOTHING*, AND IT UNBLOCKS THE ENVIRONMENT SPLIT.** David's stated obstacle to a second Supabase project was *"the Ignition app is in the current database and I am using it as a reference."* 🔴 **NEITHER HALF SURVIVES: I BARELY TOUCH IGNITION, AND I HAVE NEVER ONCE READ A ROW FROM ITS DATABASE.** Everything consulted is SOURCE in git — `CoreApp.jsx` (read on **every `npm run verify`** by `verify-universals.mjs:152`) plus four one-off donor reads (`MarginEngine.js` · `PredictiveKey.jsx` · `DataBridge.js` · `IgnitionProt.jsx:266` as a counter-example). **The only artifact ever authored to QUERY that project was committed 2026-06-08 and `PLATFORM_STATE.md:246` still reads `David must run`** — never run, 76 days, across 260 commits to `cultivar-os`/`shared`. ✅ **NO DEPENDENCY, and stronger than "absent": two separate Postgres instances cannot hold a cross-database FK, enum or sequence.** **LOOK ONLY — no app code, no schema, no migration, no cap, no deletion** · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **29/29 test files · 1092 assertions** · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · **GATE 0 NOT APPLICABLE — nothing ships** · all `[TRACE:*]` ON.

**Type:** Recon + close-out. Docs: **NEW `docs/audits/ignition-consultation-recon-2026-08-24.md`** (the one deliverable) · ledger **#208** · `tech-debt-log.md` (**#94 NEW**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **`built-inventory.md`: ZERO net-new entries and ZERO amendments — nothing was built and no capability's state changed, which is the honest answer rather than a touched date.** **Owner-tests: no card added, updated or flipped — no surface was added, changed or polished.** **§3 RETENTION: 1 archived verbatim (2026-08-23 (14) #205, `diff` confirmed IDENTICAL), 1 written — entries-in == entries-out, §3 back to 3** (archive 240 → 241). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (185 lines incl. the whole OWED table).**

**🔴 THE FINDING DAVID DID NOT ASK FOR AND SHOULD ACT ON FIRST, BECAUSE IT POINTS AT WEDNESDAY RATHER THAN AT IGNITION: THE TWO APPS SHARE ONE SUPABASE CLIENT WHOSE TARGET IS AN ENV VAR.** `packages/ignition-os/supabase.js` is a single line re-exporting `shared/src/supabase/client`, which reads `VITE_SUPABASE_URL` — **nothing in the code pins Ignition to a project.** And the shapes collide: `20260529_ignition_businesses.sql` creates a **`businesses` table of its own** defaulting `business_type:'shop'`, which `ignition-os/modules/OnboardingWizard.jsx:36,136` INSERTs into (`member_devices` and `customers` collide by name too). **If that variable is ever pointed at the Cultivar project, Ignition's onboarding writes tenant rows into the table LAWNS lives in.** ⚠️ **NOT ASSERTED IN EITHER DIRECTION — I cannot read a Vercel dashboard and did not guess.** It is one glance: Vercel → the Ignition project → Environment Variables. **The environment problem was never Ignition's data; it is one setting.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE ONE REASON TO DUMP ANYTHING, AND IT IS NARROW: tech-debt #27's TEN TABLES** (`dtc_codes, eval_photos, tools, tool_signout_log, repair_logs, customer_authorizations, concept_aliases, purchase_orders, pmi_schedules, ai_usage`) **have no committed migration — their DDL and RLS exist NOWHERE but that running project.** Everything else a dump would produce, git already has, **and a file on disk cannot drift, which makes it better than a live project for reference, not worse.** **(b)** ⚠️ **`pg_dump`, `psql`, `supabase` and `docker` are ALL NOT INSTALLED on this machine — checked, not assumed**, which is why the recon hands over TWO routes: the zero-install SQL-editor path this repo already uses (`docs/audits/ignition-db-verification-2026-06-09.sql`, READ-ONLY by its own header), and the real dump with its flags. **(c)** 🔴 **THREE DUMP DON'TS, EACH WITH ITS REASON: `--schema=public` ONLY** (a bare `pg_dump` sweeps `auth.users` password hashes and `vault`); **`--no-owner --no-privileges`** (a restored `supabase_admin` ACL is §6 r17's exact defect, and RLS cannot filter TRUNCATE); **and a DATA dump of `businesses` is a CREDENTIAL FILE** — `20260529_ignition_businesses.sql:18-19` stores QuickBooks OAuth tokens as plain `text`, so it must not be `git add`ed. **(d)** ✅ **Q6 IS ANSWERED AS A QUERY AND A BRANCH TABLE, NEVER A HEADLINE**, per the 2026-08-22 catalog ruling — this machine has no catalog access, and the repo's record has been wrong in BOTH directions. Source says to expect: **no other vertical has tables** (`conduit`/`pantry`/`coolrunnings`/`kinna` are `VerticalId` strings only — KINNA is unbuilt, not hiding) · `trace-app` already shares the DB · **four superseded generations still standing** · a possibly-undeleted debris tenant row · **and if query 1 returns `shops`/`jobs`/`shop_members`, STOP — the collision in (a) has already happened.** **(e)** 🔴 **TWO NET-NEW ORPHANS FOUND WHILE COUNTING, ON NO BOARD ANYWHERE → tech-debt #94:** `pmi_assets`/`pmi_service_logs` and `campaign_tone_samples` are **dead generations of LIVE capabilities with ZERO runtime readers** — and `20260622`/`20260623` have already spent effort maintaining RLS on them, which is why the cost recurs. **(f)** ⚠️ **NAMED, NOT TOUCHED: `shared/src/supabase/auth.ts:128` does `.from('shop_members')` inside `authenticate()`, which has ZERO CALLERS** (Ignition calls its own `DataBridge.authenticate`). It is dead code in a file that is **OFF LIMITS (§7)**, and knip cannot see it (`shared` is deep-import scoped out, §6 r9). **(g)** ⚠️ **Ignition is already half-broken as a live reference:** `ForgotPinFlow` and `JoinFlow` query `pin_resets`/`shop_invites`, dropped 2026-06-02 and never recreated. **(h)** ✅ **`git log`/`git status` re-run at write-up.** **(i)** **CLAUDE.md is 584 lines — unchanged from session start, and under its ~600 budget.**
