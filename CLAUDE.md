...
# Last updated: 2026-09-02 — #257 the receipt detail view + owner line-editing (migration UNAPPLIED); R-64 — see §3
...
### 2026-09-02 — THUNDER **THE RECEIPT OPENS, THE LINES ARE THERE, AND THE DEFECT WAS UPSTREAM OF THE QUESTION I WAS ASKED. #257. R-64. TECH-DEBT #148 + #149.** 🔴 **THE HEADLINE IS THAT THE WORKING COPY WAS LESS INFORMATIVE THAN THE RECORD IT WAS MADE FROM, AND NOBODY HAD LOOKED.** `line_items` carried **TWO** keys on **171 of 171** stored lines; `line_items_original` carries **FIVE** on **141 of 141**. The confirm path read quantity, unit_price and sku off the document and **threw all three away on save** — so the Sudderth rate (**20.72 at $35.00**, **21.31 at $25.00**) survived only in a snapshot nothing had ever rendered. The prompt asked whether quantity should be editable; David's answer was that the question was downstream — *start by not throwing them away* — and that is the shape of the build. `/receipts/:id` now shows every line with its quantity and rate, the tax, the document, the banked verdict, and an owner-only pencil.

**Type:** Build. **BUILDER-COMPLETE** on **`thunder/receipt-detail-view`** (off `main` at `c542e7e`), **NOT MERGED** · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 244 / 10 / 12 / 15) · **60/60 files · 3130 assertions** (+89 in a new file) · **20/20 MUTANTS CAUGHT against a green CONTROL** · `build:cultivar` clean · **api/ 12/12, nothing minted** · **NO new permission** · 🔴 **CARRIES A GATED MIGRATION.** Code: **NEW `lib/receiptDetail.ts`** (the model — field states, the envelope recovery, the preview verdict, the vendor question) · **NEW `lib/vendorKey.ts`** · **NEW `receiptDetail.test.ts`** · **NEW `pages/ReceiptDetail.tsx`** · **NEW `20260902_receipt_line_edit_and_vendor_preference.sql`** · `ReceiptKeeper.tsx` (the five keys, and the `|| 0` that fabricated a $0.00) · `receiptsList.ts` (the sentence) · `ReceiptsList.tsx` · `router.tsx` · `ProjectCostDrillIn.tsx` (the deep link it has wanted since it was written) · `receiptReconciliation.ts` · `verify-universals.mjs` (the drill-in declared, with its reason) · **NEW read-only `scripts/measure-policy-command-scope.mjs`** + `measure-receipt-detail-mutants.mjs`. Docs: ledger **#257** · **NEW owner-test board (12 cards, 0 covered)** · **R-64** · tech-debt **#148/#149** · `user_stories.md` correction · this §3. **§3 RETENTION: 1 archived verbatim (2026-09-01 #250+#251), 1 written — entries-in == entries-out, §3 back to 3** (archive **280 → 281**).

**🔴 THE FINDING WORTH MORE THAN THE SCREEN: SEVEN MUTANTS SURVIVED THE FIRST RUN AND ALL SEVEN SHARED ONE ROOT CAUSE.** The probes asserted that a string existed **somewhere in the file** rather than that the guard was **wired**. U1's slice ran from the `UPDATE` to an **earlier** `WHERE id = p_receipt_id` — the SELECT at the top of the body — so end < start and it tested an **empty string**, which satisfies every negative assertion for free. `IF false AND <comparison>` still contains the comparison. `IF false THEN RAISE 'only the business owner…'` keeps every message intact. And **`line_items` is a SUBSTRING of `line_items_original`**, so an `includes()` check on the projection stayed green when the column was deleted and the page would have rendered no lines at all. ✏️ **All seven now assert the BRANCH, and SQL/TSX comments are stripped before matching — because U10 had been reading my own comment explaining the removed defect as the defect.** 20/20 caught on the re-run, against a control asserted green first.

**⚠️ AND I WAS WRONG IN STAGE 0, IN THE DIRECTION THAT MATTERS.** I reported *"any active member holding `costs:read` can UPDATE any receipt row"* from `receipts_member_all`. **That policy was DROPPED on 2026-07-27** by the RBAC flip and replaced with four per-command policies; UPDATE is `receipts_member_update`, gated on **`costs:update`**. The gap is real and differently shaped — and it is not theoretical: measured live, the **MANAGER on `Test Dave's Tree Nest` holds all four `costs:*` verbs**, so that manager could edit any receipt in that business today. The sweep David asked for found **23 of 137 live policies carry no `FOR` clause** (all 23 with `WITH CHECK`, i.e. write-capable) — `receipts_member_*` is **not** among them; `receipts_owner_all` is, and that one is owner-scoped and intended. **Reported, not fixed beyond the row this build touches.** ✏️ **My own probe lied first, too:** the first Stage 0 pass searched the OUTER `ocr_raw` envelope for `"tax"`, found **0 of 36**, and reported the figure absent — the parsed object sits one level down as an escaped string, present on **35 of 35**. Caught by re-probing and frozen as fixture **B4**.

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE MIGRATION IS GATED AND UNAPPLIED — CARDS 6–11 CANNOT RUN UNTIL YOU APPLY IT**, and un-applied the READ cards still pass, so a half-green board reads as a code defect and is not one. It creates `edit_receipt_line_items`, `trg_receipts_snapshot_and_line_guard` and `vendor_preferences`; run its own CATALOG-VERIFICATION GATE (A)–(F) after. **(b)** 🔴 **CARD 8 NAMES A SPECIFIC MANAGER ON PURPOSE — the one on `Test Dave's Tree Nest` who holds all four `costs:*` verbs.** A manager holding none would be refused by the READ long before reaching the guard, and the card would pass **with the guard deleted** — [[R-33]]. **(c)** 🔴 **CARD 3 ASKS YOU TO READ A SENTENCE, NOT CHECK ITS PRESENCE:** the amber note on the missing quantity must put the omission on the **platform**, never say *changed*, *removed* or *deleted*. Nobody edited those; we dropped them, on 171 of 171 lines. **(d)** ⚠️ **R-64 IS FILED IN YOUR WORDS** (per-vendor billing unit, Option C, `preferred` + `preference_note`, no per-product matrix; `hour` deliberately absent). **The vendor key is a FOLD, not an identity** — `vendorKey()` folds punctuation and corporate suffixes, so "Sudderth Brothers Contracting, Inc." and "Sudderth Brothers" answer as one. **There is deliberately NO dangling `vendor_id` column** — that is tech-debt #144's exact shape; the re-point is a single UPDATE joining on `vendor_key` when the vendor recon lands. **(e)** ⚠️ **TECH-DEBT #148 — `accept_vs_edit` STILL MEASURES OUR OWN FORMATTING AT WRITE TIME. This build fixed the SENTENCE, not the FLAG**, deliberately: fixing the writer gives the column two meanings either side of an unrecorded date, and choosing how to record that boundary is yours. **(f)** ⚠️ **CARD 11 USES `bwi`, NOT SUDDERTH** — there is only one Sudderth receipt, so it cannot demonstrate *"the next invoice"*; bwi has 4. **(g)** ⚠️ **NOT MERGED**, and `thunder/qbo-review-test-mode` is live and holds **R-63** and tech-debt **#146/#147** — mine are **R-64** and **#148/#149**, claimed at file time against `main` `7952cb1` **and against that branch**. Re-read `CLOSE-OUT-LEDGER.md`, `tech-debt-log.md`, `RULINGS.md` and the §3 count at merge (#233). **(h)** ⚠️ **STILL OPEN AND UNTOUCHED:** tech-debt **#143**/**#144** (the duplicate pairs and the 452 empty seams — still yours) · the three `inventory-*` docs bannered but not re-audited · `docs/reference/` tracked by accident · **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#141** · **#145** (now resolved by this build — see the ledger).
