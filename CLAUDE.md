...
# Last updated: 2026-09-03 — #262 retire-and-replace + normalisation as a mechanism; appliers and screens OWED — see §3
...
### 2026-09-03 — THUNDER **RETIRE-AND-REPLACE, AND NORMALISATION AS A MECHANISM. #262. R-70…R-72.** 🔴 **THE HEADLINE IS THAT THE MUTATION HARNESS FOUND A DEFECT NO PROBE COVERED, AND IT WAS THE WORST-SHAPED ONE AVAILABLE: the first draft consumed the matched item on the RETIRE path, so a retired row's replacement was never created and the product DISAPPEARED FROM THE CATALOGUE — a silent deletion dressed up as a retirement, on a build whose first rule is that nothing is deleted.** The harness surfaced it by flagging a line's **REMOVAL as an improvement** (T10 survived), which is a shape no assertion was looking for. ✏️ **A probe checks a property you thought of. A surviving mutant reports a property you did not.**

**Type:** Build. **BUILDER-COMPLETE** on **`thunder/qbo-review-test-mode`**, **NOT MERGED, PUSHED** · `npm run verify` **exit 0 ZERO NET-NEW** · **api/ 12/12 from the filesystem, nothing minted** · **NOT ONE WRITE** — both migrations additive, and nothing yet applies a plan. **22/22 mutants caught across two new harnesses**, each against a control asserted GREEN first. **60 assertions.** Code: **NEW `inventory/retireAndReplace.ts` · `inventory/normalisationConsent.ts`** (both pure) · **NEW `20260903_inventory_retire_lifecycle.sql` · `20260903b_display_standards.sql`** (both GATED) · 2 NEW harnesses. Docs: ledger **#262** · **[[R-70]]…[[R-72]]** · owner-test cards **21–23** (board 23, 0 covered, 2 `needs-test`) · built-inventory · ⚡ ACTIVE STATUS · DECISIONS-INDEX · the story · this §3. **§3 RETENTION: 1 archived verbatim, 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 A SECOND DEFECT, CAUGHT BY A PROBE, AND IT IS THE ONE THE WHOLE FILE EXISTS TO PREVENT.** My first matcher used a single *SKU-wins-if-present* key. Given **447 rows with NO SKUs and 685 items that ALL have SKUs** that key is asymmetric, so **nothing ever matched** — every counted row retired and a duplicate created for each. Two passes now: SKU only when **both** sides carry one, then name-slug + parsed unit. **And the clause that fell out of building rather than out of the ruling: a counted row that matches is ADOPTED, not carried beside a fresh twin** — the obvious reading makes two rows for one product and **splits the on-hand across them**, which is tech-debt #56/#58's live defect manufactured deliberately at import time, on the one row nobody can reconstruct.

**🔴 NORMALISATION GROUPS BY PARSED MEANING, WHICH IS WHAT MAKES THE COSMETIC/MEANING SPLIT STRUCTURAL RATHER THAN A JUDGEMENT CALL.** `30 gal` · `30 Gallon` · `#30` · `30G` parse to one shelf → a **spelling** question with a computed default and a computed WHY. **`#3/5` parses to a RANGE** → a **meaning** question carrying **no suggestion at all**, because whether a #3 and a #5 are one product or two is a fact about Terry's business and a default would be us answering it for him. An unparseable label is a **finding**, returned even when it appears once. ✏️ **An earlier draft decided *is this a range?* by pattern-matching the composed key STRING** — a format change of one character would have turned every range into a confident spelling suggestion; the classification now comes from the same parse that built the key. **§A of the suite reads the module's own SOURCE and fails if it names a single size** — *a mechanism, not a vocabulary*, asserted mechanically rather than promised.

**⚠️ RETIREMENT GETS ITS OWN COLUMN, AND DECLINING TO REUSE `status` IS THE WHOLE POINT.** `status` already has two writers that overwrite each other (tech-debt **#71** — the D-42 qty-derive vs the D-52 tombstone). A third would fail in the worst way available: **all 443 retired rows have qty 0**, so the derive would recompute `retired` → `depleted` and they would silently come back. It does not close #71; it declines to add to it.

**🔴 AND THE CORRECTION DAVID SENT REACHED FURTHER THAN THE CARD IT WAS ABOUT.** *"Lauren — a manager, not an owner"* is false: she holds `role = OWNER` at LAWNS with a `user_id` that is **not** `businesses.owner_id`, the only such row in the database. **CARD 4 now names the tenant and the actor** (`test obrien` on Test Dave's — the only ACTIVE manager anywhere; `joel joiner` is a manager at LAWNS but `active=false`), and records that running it as Lauren and passing it is itself a FAIL. **The new `business_display_standards` table carries an owner policy AND a member policy for exactly this reason** — an `owner_id`-only policy would have locked her out of her own tenant. ⚠️ **AND IT IS ALREADY LIVE ONE LAYER DOWN, NAMED NOT FIXED: `audit_log`'s `audit_owner_read` is `owner_id`-keyed, so Lauren can WRITE the record of her own normalisation decision and CANNOT READ IT BACK.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **WHAT IS BUILT IS THE TWO DECISIONS AND THE TWO PLACES THEY LIVE. THE APPLIERS AND THE SCREENS ARE OWED**, and CARD 23 is `needs-test` saying so in those words — nothing yet writes `retired_at`, creates the 685, or applies a chosen label, **and a retired row that still appears everywhere is not retired.** Without that card, #262 reads as *retire-and-replace: done*. **(b)** 🔴 **CARD 22 IS THE SHARP ONE — it proves BOTH policies exist on the new table**, which is the Lauren case; if only the owner policy is there, stop. **CARD 21's V2 looks like a formality and is not:** a migration that quietly retired rows would be indistinguishable from the feature working. **(c)** ⚠️ **CARD 21's V5 IS A GO/NO-GO:** if the live counts are not ~447 total and ~4 counted, the data has moved since the ruling and the plan was written against those numbers. **(d)** ✅ **[[R-72]] records the delta proof as DEFERRED BY RULING** — minted precisely so a deferral is not later mistaken for a miss. **(e)** ⚠️ **ID DISCIPLINE IS NOW A MERGE-TIME CHECK, NOT ONLY A FILE-TIME ONE** — #260 renumbered because `main` took `#257` and `R-64/R-65` while this branch sat unmerged. **An id is free *as of a read*.** **(f)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs are bannered, not re-audited · `docs/reference/` tracked by accident · tech-debt **#71** · **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#147**.
