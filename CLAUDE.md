...
# Last updated: 2026-08-21 (2) — recon: the seven write paths to `businesses` (#189); ledger #187 written. See §3.
...
### 2026-08-02 (7–8) — THUNDER **RECONSTRUCTED 2026-08-21 FROM THE COMMITS, NOT FROM ITS CITATIONS.** THE MARKETPLACE'S `Enable` CREATED A BILLABLE MODULE THAT WAS LIVE WITH NOTHING THAT EVER ENDED IT — **free forever, no conversion date** — and the card's own copy promised the opposite. **David's ruling: ONE CALL, NOT TWO.** `set_business_module_state` now CALLS `start_module_trial` **inside its own transaction**, so the enable and the clock land together or not at all. 🔴 **TWO SEQUENTIAL RPCs WOULD NOT HAVE FIXED IT, WHICH IS THE WHOLE REASON FOR THE SHAPE:** a partial failure between call one and call two produces **precisely the state the ruling exists to prevent** (#69's class). One call makes it **UNREACHABLE rather than merely unlikely.** **THREE COMMITS; BUILDER-COMPLETE (`npm run verify` exit 0, `build:cultivar` clean, 185 assertions); `20260802c` GATED — David applies; card 32 AMENDED.** ZERO new api-fn (12/12), no new table.

**Type:** App code + migration + docs, across **`6b96cfe` → `d89b124` → `9da6790`** — `Subscription.tsx` · `moduleState.ts` · `moduleSeed.test.ts` · **NEW `supabase/migrations/20260802c_enable_starts_the_clock.sql`** (GATED) · **NEW CLAUDE.md §6 r18** · `ui-control-standards.md` §5/S1 · `20260802`'s supersession header. **⚠️ Its ledger row was NEVER WRITTEN — this entry and ledger #187 were both reconstructed on 2026-08-21 from the diffs, after #188 found the number cited three times in `RULINGS.md` and zero times in the ledger.**

**🔴 DAVID'S FINDING CAME FIRST AND IT WAS A GLYPH: the Contractors card carried a green CHECK — *done, nothing to do* — beside a `[Turn on]` button saying it was off. Both states asserted on one card.** The cut generalises into **§6 r18**: *the section header already asserts the billing fact for every row in it, so a per-row glyph must not repeat it* — a second representation of one fact (STD-011), **and the redundant copy is the one that drifted.** The sweep that followed found **two more, one of them Thunder's own and worse than the original**: Available's copy said *"A 30-day trial starts when it is enabled"* — **it did not**, and the card described an outcome the button could not produce; and Available's header told a MANAGER *"turn one on and it works immediately"* while every row below read *Owner only*. **Coming and Active swept CLEAN, reported in full because a clean sweep is worth knowing after that one.**

**INLINING THE CLOCK WAS REJECTED ON EVIDENCE, NOT TASTE, AND ITS PREDECESSOR'S CHECK IS THE EVIDENCE.** `start_module_trial` is the only function anywhere that spells `trial_started_at`/`trial_days` as write targets, and **`20260801c` V2 ASSERTS that against `pg_proc`** — *"a third function name there is a second clock writer."* Inlining would have made this file that third name and **failed its own predecessor's check.** Calling it keeps the literals in ONE function while still being ONE client call. **The 6-arg signature is DROPPED in the same transaction** — two overloads would give PostgREST *"could not choose the best candidate"* **on the money path** — and old bundles keep working because PostgREST binds by NAME and `p_trial_days` defaults to NULL.

**✏️ CORROBORATED AGAINST THE DIFF, WHICH IS THE POINT OF A RECONSTRUCTION:** every claim above is in the source — the call at `20260802c:159`, the DROP at `:56`, the default at `:67`, the denied-audit row at `:126`, and **`COALESCE(p_trial_days,0) > 0` at `:156`, which is what keeps `core` AND `core_optional` clock-free** so starting a countdown on free Contractors cannot arrive *through the fix*. **`20260802c`'s own header names ledger #187**, which is how the number was confirmed rather than inferred. **SEED-IF-ABSENT ALSO CLOSED** here — it was producing a visible contradiction, *"Included ✓"* from the catalog beside `[ENABLE]` from an absent row — and it repairs a **SHORT** seed, not only an empty one.

**FLAGGED FOR DAVID:** **(a)** 🔴 **`20260802c` IS GATED AND UNAPPLIED — the money path is code-complete and the database has not been told.** **(b)** 🔴 **B6's BLIND SPOT IS NAMED IN ITS OWN COMMENT, per David: it asserts over the SEED PROJECTION, and a row created by a BUTTON never passes through `moduleSeedRow`.** No sibling assertion can cover it — a CHECK constraint would need `billing` and the DB has no catalog; a mirror over the RPC's inputs **would have passed on 08-02 because the client sent nothing.** The route taken is making the state unreachable; **the honest guard for the button path is a standing QUERY (`20260802b` V3b), not an assertion.** **(c)** ✏️ **§6 r18 was numbered BY APPENDING:** the first draft inserted it at 16 and pushed the table-editor rule 17 → 18, **silently breaking `RULINGS.md:68`'s live citation of "§6 r17"** — caught by grepping for the reference before committing, **not by review.**
