...
# Last updated: 2026-09-03 — #272 the grid engine moves to @trace/shared — see §3
...
### 2026-09-03 — THUNDER **SHOW LAUREN HER DATA — AND THE LOUDEST NUMBER ON THE PAGE WAS 52% OF THEIR REVENUE AND MOSTLY WRONG. #267 (recon) + #268 (build). R-76 · R-77 · R-78 · R-79.** 🔴 **THE HEADLINE IS THAT THE PROMPT CALLED A SAFEGUARD A BUG, AND ITS OWN EVIDENCE SENTENCE WAS THE COMMENT IMPLEMENTING THE SAFEGUARD.** §2 asked for Lauren's customers' NAMES on screen and called their absence *"the gap you measured"*, quoting `api/qbo/router.ts:694` — *"there is not even a preview"* — which is the line implementing [[R-24]] clause (b), *"a read of personal data is summarised, never listed."* Stopped and put to David; he ruled **OPTION B** and recorded it as Lightning's error. ✏️ **The restraint was STRUCTURAL, not stylistic: `QboInvoiceRow` has no customer-name field**, which is why `BooksReview.tsx:31` could already say the clause *"holds by construction, not by care."*

**Type:** RECON → STOP (#267) → BUILD (#268), both on `main`. **BUILDER-COMPLETE** · `npm test` **68/68 files · 3506 assertions** (booksFindings **93/93**, booksReport **33/33**) · `build:cultivar` ✓ · **29/29 mutants caught, 0 survived** · **NO migration · api/ 12/12, nothing minted · NO new permission.** ⚠️ **`npm run verify` reads eslint 245 vs baseline 244 — the ONE net-new is `ReceiptsList.tsx:87`, another session's UNCOMMITTED work; ZERO problems in all 10 files this commit touches, proven per-file.** **§3 RETENTION: 1 archived verbatim (#263 vendor chain), 1 written — entries-in == entries-out, §3 back to 3** (archive **287 → 288**).

🔴 **THE FINDING THAT WOULD HAVE BEEN HANDED TO TERRY ON PAGE ONE.** `sold-below-price-card` was **worded about their printed price card and computed against the QuickBooks `UnitPrice`** — two different floors, one sentence — and on LAWNS's real books it read **$1,607,416: 52% of their entire $3,187,796 of revenue**, sorting FIRST under R-66's money-at-stake ordering. David: *"Hand Terry that on page one and the report loses its credibility before he reaches anything true."* **Withdrawn rather than re-worded** ([[R-78]] — keeping the id alive with new arithmetic leaves one id that has meant two things). Decomposed before deciding: median charged/list is **0.87**, so list price IS broadly real, but **49 of 185 items with ≥5 sales were never once sold at or above list** (carrying **$671,741**), **74 lines were charged exactly $0** and counted at full list, and quantity **more than doubled** the total ($761,504 → $1,657,696) on 36 bulk lines. The successor **names its floor in its own sentence**: **$724,273, *"typically people paid 87% of the recorded price."***

🔴 **AND A CANNOT-COMPUTE THAT WAS FALSE ABOUT OUR OWN READ — [[R-79]], the sharpest class here.** Receivables told owners *"the invoice read does not include how much of each invoice is still unpaid, or when it was due."* **`Balance` and `DueDate` are on 1,469 of 1,469 rows;** `invoiceList.ts` discarded them. ✏️ **A missing finding is a SILENCE; a false refusal is an ASSERTION** — it tells an owner their books lack something their books carry, and forecloses the cheapest fix for every reader who believes it. Two parser fields: **$30,736 owed · $11,158 more than 30 days past due · oldest due 2026-04-22**, matching the figures David had all along.

🔴 **THE MUTATION HARNESS CAUGHT TWO OF MY OWN PROBES LYING, AND THE FIRST ONE IS THE BETTER LESSON.** M21 **SURVIVED**: my giveaway probe asserted the $0 exclusion using a $0-amount line — and `pricedLines` (`amount > 0`) had **already removed all 74 real giveaway lines before the rule ever saw them**. So the guard was dead code, `freeLines` was always 0, and the `limits` sentence I wrote **described a filter that never fired**. ✏️ *The exclusion David asked for was already in force one layer up; what I nearly shipped was a false claim about it.* M24 **SURVIVED** as an equivalent mutant, proving the null-balance comment overclaimed — an unreadable balance is now **DECLARED**, not absorbed into the same silence as a settled invoice.

**FLAGGED FOR DAVID:** **(a)** 🔴 **§0 SAID *"THERE IS NO INGEST BUTTON ON THIS SURFACE AT ALL"* — TRUE OF THE PANEL, NOT OF THE PAGE.** `QboDeliveryIngest` and `QboOrderIngest` are siblings immediately below it on the Accounting card, gated on `deliveries:create`/`orders:create` — **permissions a MANAGER holds.** Not changed: removing your own tools is your call. **(b)** ⚠️ **`sold-below-quickbooks-list` is still the largest money finding at $724,273 and still sorts first** — honest and named now, but read the sentence before Lauren does. **(c)** ⚠️ **`QboBooksReader` lives in `packages/shared`, which never imports `packages/cultivar-os`, so `<DataSheet>`/`renderExpand` are NOT reachable from this surface** — the invoice list is a plain bounded table. My #267 report implied the widget was available; that was right about the widget and wrong about the reach. **(d)** ⚠️ **9 owner-test cards OWED** on a new board; CARD 3 is `needs-test` with its reason — provoking a mid-sequence failure needs a deliberately expired token and **should not be done to LAWNS's live company for a checkbox**. **(e)** ⚠️ **A THIRD SESSION HOLDS `ReceiptsList.tsx` + `receiptsList.ts` UNCOMMITTED** (clean tree at 14:32, modified during my session, and `trace-platform-e2` denies owning it). I committed by explicit path, left their index untouched, and told them — [[R-62]]. **(f)** ⚠️ **STILL OPEN:** `origin/assets` · tech-debt **#143**–**#145** · **#148**–**#154**.
