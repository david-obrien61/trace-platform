...
# Last updated: 2026-08-24 (5) — recon: where sellable things live, and where our line items live (see §3)
...
### 2026-08-24 (3) — THUNDER **RECON ×2: THE VARIETY FIELD MATCHES ON EXACT NAME ONLY — AND THE THING DAVID ASKED FOR IS ALREADY BUILT, WIRED TO A DIFFERENT SCREEN.** David typed `VITEX` on the count screen expecting a choice between his two vitex varieties (Shoal Creek · Flip Side) and got the *"Didn't recognize this"* sheet; `Live Oak` resolved. 🔴 **THE MISS IS ONE LINE — `tokenSetsEqual`'s `if (a.size !== b.size) return false` (`canonicalName.ts:82`): `{vitex}` vs `{shoal, creek, vitex}` is 1 ≠ 3, so a strict SUBSET is a non-match BY CONSTRUCTION.** ✅ **AND THE ANSWER WAS ALREADY IN `packages/shared/`: `searchStockLines` (`stockLineResolver.ts:278-311`) does substring AND token-SUBSET matching, returns a LIST, is under test — and its OWN DOCBLOCK USES THE WORD "vitex" AS ITS EXAMPLE.** **Its one caller is `ScanOrder.tsx:329` — CHECKOUT.** **LOOK ONLY — no app code, no schema, no migration, no policy, no permission string, no cap** · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **29/29 test files · 1092 assertions** · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · **GATE 0 NOT APPLICABLE — nothing ships** · all `[TRACE:*]` ON.

**Type:** Recon ×2 + doc corrections. Docs: **NEW `docs/decisions/2026-08-24-variety-lookup-exact-match-recon.md`** · **NEW `docs/decisions/2026-08-24-storage-probe-private-tab-recon.md`** · ledger **#210** (+ the C3 header fix) · `tech-debt-log.md` (**#97 · #98 · #99 NEW**) · owner-tests (**cards RE-MAPPED BY CONTENT — see below**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **`built-inventory.md`: ZERO net-new entries and ZERO amendments — nothing was built and no capability's state changed, which is the honest answer rather than a touched date.** **§3 RETENTION: 1 archived verbatim (2026-08-23 (15) #206), 1 written — entries-in == entries-out, §3 back to 3** (archive 241 → 242). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (186 lines incl. the whole OWED table).**

**🔴 THE FINDING WORTH MORE THAN EITHER RECON, BECAUSE IT IS ABOUT HOW THE QUESTION GOT ASKED: THE OMISSION IS DECLARED, NOT ACCIDENTAL.** `QrScanner.tsx:24-27`, on the `onLookup` prop: *"when provided, the manual 'Look up' field routes here instead of onScan, so a human can SEARCH by a partial id / name token (searchStockLines)… **Callers that want the count-style exact match (e.g. InventoryCount) omit this.**"* `git log -S onLookup` returns **ONE commit — `a1ad0d1`, a CHECKOUT fix-pass.** **So the search field was built for checkout, the count screen was named in a comment as deliberately excluded, and nobody revisited it.** ✏️ **This is why the recon proposes no matcher: CORE MANDATE rule 1 turned a build question into a wiring question, and building one would have been the rebuild r1 exists to stop.**

**FLAGGED FOR DAVID:** **(a)** ⚠️ **MY DESCRIPTION OF THE FIELD DID NOT SURVIVE CONTACT WITH THE SOURCE, AND IT WAS WRONG IN BOTH DIRECTIONS.** I called it *"variety names, not tag codes"* from screenshots. **It is DECLARED AS A TAG FIELD** — placeholder *"Or type the tag (e.g. SCV-0031)"*, button *"Look up"*, **`autoCapitalize="characters"`** (`QrScanner.tsx:134-138`) — 🔴 **which is why the typed input arrived as `VITEX` in capitals: the uppercase in the report is the code's own fingerprint.** **And it matches names too**, over THREE things (`cultivar_plants.tag_id` · `business_inventory.sku` · `.name`), **case-insensitively at every lane.** **(b)** 🔴 **A SECOND COST NOBODY ASKED ABOUT AND IT IS THE WORSE ONE (tech-debt #98): typing `Vitex` + a size into the unknown sheet and pressing Save MINTS A THIRD VARIETY** (`InventoryCount.tsx:701`) beside the two real ones, on-hand split across it — **the D-49/#135 duplicate family arriving through a SHORT NAME instead of a spelling.** The sheet promises *"different spellings don't split into separate items"* (`:951`) and **rests all its weight on the word *spellings***; the code's own comment (`:665-667`) already names subset as a deferred layer. **And it fires on SAVE, never on entry — there is no typeahead anywhere in the tree.** **(c)** ✅ **FIVE OPTIONS, NEED→WANT, DELIBERATELY NOT COLLAPSED** (copy-only · pass `onLookup` · + close the mint hole · live typeahead · deepen L4). **All five are client-side: ZERO migrations, ZERO new `api/` functions — 12/12 untouched.** ⚠️ **Option 4 is FLAGGED NOT RECOMMENDED: smallest diff, widest blast radius** — it changes all six call sites including the customer-facing QR profile and the CSV importer, **and it does not even fix the reported case** (`vitex` subset-matches TWO rows). **(d)** ⚠️ **STORY DRIFT SURFACED, NOT ACTED ON:** `user_stories.md:337` asserts the shape is *"the SAME … whether the walk ends in a count or a sale"* and declares it **BUILT** — since `a1ad0d1` the two endings differ on this control. **No story edited.** **(e)** 🔴 **PART B's ANSWER IS NEITHER OPTION OFFERED: THE PROBE RAN AND IT WAS RIGHT.** It fires at `InventoryCount.tsx:214` on **page mount** — ungated on `online`, ungated on having started a count — and returned `ok`, proven by the absence of a banner that has no other condition (`:815` idle, `:788-792` counting). **It writes, reads back and cleans up ALL INSIDE ONE PAGE SESSION (`store.ts:168-183`), so it measures *can this store hold a value now*, never *will it still hold it after the tab closes* — and a Private tab passes the first and fails the second.** ⚠️ **So CARD 5's EXPECTATION is what the measurement falsifies, not the code**, and `store.ts:163-166`'s comment asserts the opposite as fact (tech-debt **#99**) — **named, deliberately not fixed.** **(f)** ✅ **THE BY-PRODUCT WORTH MORE THAN THE FINDING: login worked and DID NOT LOOP in the Private tab, so the Private tab is CLEARED as the logout-loop suspect and the loop is UNEXPLAINED.** ⚠️ **Private-tab-WHILE-OFFLINE is still unproven — David was online throughout, so nothing ever had to survive in the store**; the recon carries the 11-step sequence whose decisive step is *close the Private tab, reopen a new one, see whether the pending count survives.* **(g)** 📋 **OWNER-TEST CARDS RE-MAPPED BY CONTENT AFTER AN INTEGER COLLISION — my error, not the board's.** T1–T6 are David's phone tests and are **not** the numbered cards. **T2 AND T3 BOTH LIVE ON CARD 1** (its *Airplane mode OFF* clause is T3 — the evidence David cited) · **T1 → card 2** ⚠️ *flagged, not guessed: card 2 is checkout and card 8 is the count loop, and the surface was never reported — one sentence from David settles it* · **T4 → CARD 9**, moved off card 3 · **T5 → card 5**, the one case where number and content agree · **T6 (custom domain) has NO card — said so, and none was created.** **Annotations on cards 3 and 4 REMOVED as misattributed.** 🔴 **THUNDER MARKED NOTHING `covered`; no `STATUS` and no `LAST-PROVEN` line was changed (OP-14).** **(h)** ✅ **C3 done — the ledger header's `Newest:` read #208 while the table held #209; fixed, and `Last updated:` was stale at #207 and fixed with it.** **C4 filed as tech-debt #97.** **(i)** ✅ **`git log`/`git status` re-run at write-up.** **(j)** **CLAUDE.md is under its ~600 budget.**
