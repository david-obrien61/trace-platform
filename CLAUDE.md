...
# Last updated: 2026-09-03 — #268 show Lauren her data; a $1.6M headline withdrawn; R-76–R-79 — see §3
...
### 2026-09-03 — THUNDER **THE PROMPT WAS STOPPED, AND THE REASON A SURFACE DIVERGED FROM THE DESIGN SYSTEM TURNED OUT TO BE FALSE WHEN IT WAS WRITTEN. #266. R-75. THE R-38 AMENDMENT DRAFTED, UNNUMBERED. TECH-DEBT #153 + #154.** 🔴 **THE HEADLINE IS THAT `<DataSheet>` HAS BEEN ABLE TO DO THE THING WE SPENT A BUILD SAYING IT COULD NOT DO, SINCE 1 JULY.** `ReceiptsList.tsx` declared the grid dropped because *"a fixed-column grid can only render the chain by truncating it or by exploding one receipt into several rows."* `DataSheet.tsx:81-82` has carried **`renderExpand`** — *"Optional per-row detail drawer"* — since **2026-07-01, `e3e6796`**. The comment was written **2026-09-01, `ab617b2`: two months later.** ✏️ **The divergence was not a gap in the widget; it was an unchecked claim about our own widget** — [[R-26]]'s class, and the **second instance this week** of a comment contradicting its own repo (tech-debt **#61** was the first).

**Type:** RECON → STOP → RECORD. **NO BUILD.** 🔴 **ZERO non-comment changed lines under `packages/`** — 24 insertions, 7 deletions in one file, **every one a comment, proven by filtering the diff rather than asserted.** Zero under `api/` and `supabase/`. `npm run verify` **exit 0, zero net-new** · the new divergence cap **exit 0** · `build:cultivar` exit 0. **§3 RETENTION: 1 archived verbatim (#262), 1 written — entries-in == entries-out, §3 back to 3** (archive **288 → 289**).

🔴 **THE PROMPT WAS STOPPED ON THREE CONTRADICTIONS AND DAVID UPHELD ALL THREE.** ① It required one receipts modal to implement **M3/M4/M5**, which the standard files as *"NOT yet implemented platform-wide … the next modal rung"* — a per-surface answer to a platform question, which the standard's own clause 5 forbids in terms (*"① this doc is updated → ② then the shared display widget is updated, once → ③ surfaces use the widget"*). **Withdrawn by David; the modal rung lands in the shared control and receipts inherits.** ② It re-asked list ordering and modal-vs-route, both already settled (**G9** and **E1**) — and clause 5's own worked example, written hours earlier by a peer session, *is this prompt*. ③ 🔴 **[[R-38]]'s structural test is measurably wrong on our own corpus.** It says *"a line with no SKU AND no quantity is not goods"*; measured against the 17 LAWNS receipts: bwi's fuel surcharge is caught, **Bailey Bark's FREIGHT (`sku:'099', quantity:245`), its Fuel Surcharge (`sku:'110565', quantity:0.14`) and Sudderth's CREDIT CARD FEE (`quantity:1`) are not — 3 of 10 caught, 7 misclassified as goods.** Derived from bwi, never run against the two vendors already in the corpus.

🔴 **AND THE FALSE REASON WAS COMMITTED TO `main` WHILE I WAS WRITING THE REPORT.** A peer session (`da661dea`) landed the divergence cap as **#265** with the receipts declaration as its first entry — carrying the withdrawn claim as its `reason` and grading it *"REASONED AND CORRECT about the shape."* **All three copies are now corrected against David's ruling**: the declaration's `reason` and `note` (plus a new `premise_withdrawn` block naming the four clauses — G1/G2/G3/G5 — that are downstream of it and re-answerable), the `ReceiptsList.tsx` header where a builder actually reads it, and the peer's report, **annotated in place with its original text preserved**. ⚠️ **Their `G4`/`G6`/`G7` `owed` verdicts and the `23 UNAUDITED` baseline are untouched and remain the cap's real finding** — only the reason and its grading were overturned.

🔴 **THE HAND-OFF COULD NOT BE MADE, AND [[R-62]] PREDICTED EXACTLY THAT.** While the peer held fifteen uncommitted files in the shared worktree — including `CLAUDE.md`, `RULINGS.md` and the ledger — I could not write a single canonical record. `ListAgents` resolves nine peers and **none maps to `da661dea`**; its transcript carries no self-name, so it has never run `ListAgents` and **cannot be addressed**. R-62's own text: *"A hand-off that depends on a live listener is not a hand-off, so it was filed as a committed document plus a ⚡ ACTIVE STATUS row instead."* Filed exactly that way, then folded into the canonical files the moment they were released.

**FLAGGED FOR DAVID:** **(a)** ⚠️ **THE R-38 AMENDMENT IS DRAFTED, UNNUMBERED, IN YOUR REGISTER AND NOT IN YOUR VOICE** — by your instruction. **R-38 stands as filed until you write it, and must not be built on.** **(b)** 🔴 **[[R-75]] FILED IN YOUR WORDS** — the line-item search ships labelled, never a bare *"nothing found"* above the 100-row cap. ✏️ **It generalises past search: it is D-9 turned on a NEGATIVE** — an absence asserted over a set you never examined is as false as an invented number, and harder to notice because nothing appears on screen. **(c)** 🔴 **THE MODAL IS NOT A MOUNTING** — `ReceiptDetail.tsx:141` is one monolithic component with no extracted editor, so **E1 cannot be satisfied by mounting anything that exists.** Extracting the editor **is** the work; scope it as such. **(d)** ⚠️ **Invoice number is a MIGRATION** (the reader IS asked at `ocr.ts:66` / `:100` and there is no column — tech-debt **#153**, and it is the dedup key #143 needs); **UOM is a prompt change** and the reader was never asked — the cheap one is the one that stops the screen asking bwi a question bwi's own invoice answers twice. **(e)** ⚠️ **The card shape is still an OPEN question.** The premise is withdrawn; the choice is not thereby decided either way. **(f)** ⚠️ **A peer inverted §3 to newest-first** — I followed it rather than fight it, but it is now inconsistent with how the archive was written yesterday. **(g)** ⚠️ **STILL OPEN:** `origin/assets` (remote-only, 67 days) · the three `inventory-*` docs bannered not re-audited · tech-debt **#143**–**#145** · **#148**–**#154**.

### 2026-09-03 — THUNDER **THE STANDARD OUTRANKS THE PROMPT — AND THE DIVERGENCE THAT WAS *CORRECT* WAS STILL THE DEFECT. #265. R-73 · R-74.** 🔴 **THE HEADLINE IS THAT `ReceiptsList` DID EVERYTHING §6 r16 ASKS AND IT DID NOT HELP.** Its header names the standard it diverges from, names the deviation, and gives a real reason — a receipt row carries 0..n orders each with 0..n deliveries, which a fixed-column grid can render only by truncating the chain or exploding one receipt into several rows. **That divergence is right, and it was still defective twice.** ① **THE DIVERGENCE RECORDED IS NARROWER THAN THE DIVERGENCE TAKEN:** it explains dropping the SHAPE and is silent on **G4 (sortable) · G5 (column show/hide) · G6 (search/filter)** — the surface has none of them, plus G7's density half. ② **IT WAS FILED IN EXACTLY ONE PLACE, A COMPONENT COMMENT** — not the as-built doc (checked: no mention), not ledger #252, not `ui-standards.html`, not the standard. **A prompt-writer could not find it without opening the file, and did not.** ✏️ *A narrow record read as a complete one, filed where nobody looks.*

**Type:** Build + Records, on `main`. **BUILDER-COMPLETE** · `npm run verify` **exit 0, ZERO NET-NEW** (tsc 5/5 · eslint 244/244 · knip 10/12/15) · **68/68 files, 3442 assertions**; receipts suite 94/94 · **NO migration · api/ 12/12, nothing minted · NO new permission.** Code: `receiptsList.ts` (+`receiptSortKey`, `compareReceiptsForDisplay`) · `ReceiptsList.tsx` · `receiptsList.test.ts` · `ProjectCostDrillIn.tsx` (header). NEW: `scripts/verify-ui-standard-divergence.mjs` · `docs/decisions/ui-standard-divergences.json` · `docs/decisions/2026-09-03-ui-standard-divergence-report.md`. **§3 RETENTION: 1 archived verbatim (2026-09-03 #261), 1 written — entries-in == entries-out, §3 back to 3.**

🔴 **G9 IS NOT A TIDY-UP — DAVID'S CORRECTION TO MY OWN PROPOSED WORDING CHANGED SHIPPED BEHAVIOUR.** I proposed *"the most recent record first"*; David ruled *"the most recent record **DATE** — the date the document or event itself carries, NOT the row's creation timestamp."* `ReceiptsList` ordered on `created_at`. **On LAWNS's live rows the two disagree: the 2026-07-02 bwi invoice was captured AFTER the 2026-07-29 one**, so July 2nd sat above July 29th — *a list of invoices that contradicted the invoices.* ⚠️ **AND THE PROBE COULD NOT HAVE CAUGHT IT: the old `E7` varied `created_at` ALONE across three rows sharing one `date`, so both candidate orders agreed on every input it supplied — it would have gone GREEN against either.** [[R-33]] inside our own suite. Rebuilt so the two orders DISAGREE, and **proven red against the old sort** before being trusted. **An undated row is positioned by its capture DAY and still reads *"No date recorded"*** — the fallback buys a POSITION, never a displayed value (D-9 / A9), and `nullsFirst: true` at the query keeps it from being the first row dropped at the 100-row cap. ✏️ **The other option put to David was unbuildable and is recorded so it does not return: *"unreviewed-first"* needs a column that does not exist** — [[R-50]]: `receipts` has 21 columns and no reviewed/unreviewed bit; `reconcile_status` is the verdict banked at capture.

🔴 **THE CAP'S CLAUSE LIST IS DERIVED FROM THE STANDARD, WHICH IS WHAT MAKES THE ORDER OF OPERATIONS MECHANICAL RATHER THAN REMEMBERED.** `verify:ui-divergence`: a bespoke record-list surface carries a declaration, and **that declaration answers EVERY clause of the sections it diverges from** (`met` / `dropped`+reason / `owed`). **Adding a clause to the doc automatically invalidates every declaration until it is re-answered** — proven by adding a throwaway `G10` and watching both B1 and D1 go red in one run. Self-pruning both directions (#228's pattern): a declaration for a deleted file, or for one that has since CONVERGED onto `DataSheet`, fails as stale. **Proven RED seven ways total.** Baseline-and-ratchet on population (23) and board coverage (11) — **a day-one hard fail would have been paid off with 23 rubber stamps, which is [[R-33]]'s exact defect.** ✏️ **AND THE CAP'S OWN FIRST-RUN DEFECT IS THE BEST SHAPE IN THIS ENTRY: it read COMMENTS AS CODE, so `ReceiptsList` — whose header contains the words `<DataSheet>` *while naming the standard it diverges FROM* — was classified as a CONSUMER and excluded. The cap exonerated the one file it was written for, because that file's prose was the most careful.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **`ui-standards.html` RENDERS 3 OF THE STANDARD'S 6 SECTIONS — its own build, you authorise.** 11 clauses are defined and rendered nowhere (`G9, F4, E1–E6, S1, R1, R2`), and **E1 — *one record, one edit surface* — is the clause that answered modal-vs-route and is not on the board a prompt-writer would check.** The doc's own enforcement clause 2 promises gaps are *"visible, not buried."* **(b)** 🔴 **`receipts-view` CARD 1 is `owed` again** (board 9 covered · 1 owed · 1 needs-test). **The discriminating step: bwi 07-29 must appear ABOVE bwi 07-02.** It rides the same one print as six other cards, so the re-proof is free. **(c)** ⚠️ **`baseline.undeclared_bespoke_surfaces: 23` IS NOT 23 KNOWN-GOOD SURFACES** — unaudited: not found wanting, not looked at. **(d)** ⚠️ **`receipts.date` NULL count is UNMEASURED from here** — the undated-row path is probe-covered with no live population behind it; one query settles it. **(e)** ⚠️ **G9 is unaudited against `DataSheet`'s own consumers** (inventory / assets / customers). **(f)** ⚠️ **THREE STALE RECORDS CORRECTED, AND THE PATTERN IS THE FINDING: `built-inventory.md` had tech-debt #145 marked resolved the whole time, while the two records a prompt-writer actually reads — CLAUDE.md's tech-debt row and `ProjectCostDrillIn.tsx`'s own header, 170 lines above the code that uses the route — both still said `/receipts/:id` does not exist.** Plus §6 reading `DRAFT — DAVID RULES` eleven days after the ruling that settled it. **(g)** ⚠️ **I HAVE NOT SEEN THE RECEIPTS BUILD PROMPT** — it is not in the repository, and I am not claiming to have audited it for a second editor. E1 catches that at build time; I did not catch it by reading. **(h)** ⚠️ **STILL OPEN AND UNTOUCHED:** `origin/assets` (remote-only) · `docs/reference/` tracked by accident · tech-debt **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#141** · **#143**–**#144** · **#148**–**#151**.
