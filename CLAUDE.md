...
# Last updated: 2026-08-23 (3) — Arbor Day refined in place; two stories added. See §3.
...
### 2026-08-23 (3) — THUNDER **REGINA SUPPLIED THE ELEMENT THE STORY WAS MISSING, AND IT IS THE ONE THE SURFACE IS MISSING TOO: A SOCIAL POST IS CONTENT; A CAMPAIGN CARRIES AN ASK.** The entry filed at `f7aa53c` described drafting, editing and posting and **never said what Lauren is asking anyone to DO** — and the create form has the same hole. **REFINED IN PLACE, NOT REPLACED — David's ruling and it is right:** the scene, the seasonality, the under-production constraint and the edit/cancel loop all stand, Lauren still confirms them Wednesday, and `needs-input` exists precisely so a story can be refined while it is open. **Replacing it would destroy the trail of what was believed and what changed — the same argument that kept §L1 struck through rather than deleted.** Three surgical edits, title and all other prose untouched: **PIECES** gains `campaign_call_to_action` in position 2 · **NEEDS** replaced with one that **carries its source**, so the ruling is traceable to the customer who gave it (Regina's bad-press scenario — *"my bad, we didn't give you all the details, thank you for bringing that to our attention"*) · **two paragraphs inserted** between the drafting paragraph and the Terry paragraph. **TWO NEW ENTRIES filed alongside, verbatim, no tag needing a fix.** **DOCS ONLY — no app code, no schema, no migration, no cap; `stories.html` NOT edited; nothing deleted or retitled.** `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · GATE 0 **NOT APPLICABLE** · all `[TRACE:*]` ON.

**Type:** In-place refinement + 2 additions + close-out. `user_stories.md` (85 → **87** stories). Docs: ledger **#194** · `RULINGS.md` (**Q4 + Q5 moved from bare OWED to OWED-WITH-A-CUSTOMER-ANSWER; TWO NEW OWED rows**) · ⚡ ACTIVE STATUS · `built-inventory.md` · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-22 (2) #191b), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 THE STEP-6 FINDING IS SHARPER THAN "THERE IS NO FIELD", AND THE SHARPER VERSION IS THE USEFUL ONE: THE ASK HAS SOMEWHERE TO BE TYPED AND NOWHERE TO BE HONORED.** `campaigns.description` exists, and its create-form placeholder **is already an ask** — `Campaigns.tsx:165`, *"We're running 20% off all large containers. Great time to move the B-grade stock."* **Three layers each demote it:** the label reads *"Context for the AI **(optional** — shapes the posts)"* (`:162-163`), the tag says **optional**, and `generate.ts:100` hands it to the model as **`Context:`** — background, not the thing every post must carry. **A fourth, and it is the tell:** `CHANNEL_GUIDANCE.sms` (`generate.ts:29`) is the ONLY place *"one clear call to action"* appears anywhere in the codebase — **the ask exists as per-channel FORMATTING GUIDANCE on one channel, rather than as a campaign-level fact.** So this is not a missing concept; it is a **demoted** one, which is a different fix and a cheaper one to get wrong.

**🔴 AND THE SECOND INSERTED PARAGRAPH IS A CONSTRAINT THE CODE DOES NOT MERELY LACK — IT CURRENTLY LEANS AGAINST IT.** There is **no anti-fabrication instruction anywhere in the generator**, and both prompts ask for posts that are *"warm, local, **specific**"* — `SYSTEM_PROMPT` (`generate.ts:19`) and the user prompt (`:106`). **"Specific" with no facts to be specific about is an invitation to invent them**, and the story names the consequence in the owner's terms: *a fabricated energy-saving percentage in her Instagram feed is her liability, not ours.* **It is one line of system prompt. DELIBERATELY UNFIXED** — a prompt change inside a docs-only task is exactly the drift the scope bar exists to catch, and it is recorded in `built-inventory.md` in place rather than carried out.

**📐 THE RE-ESTIMATE, AND THE HONEST PART IS NOT THE NUMBER: COHERENT GOES ~5–7 PROMPTS / ZERO MIGRATIONS → ~7–9 / ONE ADDITIVE MIGRATION.** *Migration-free* was one of the two reasons COHERENT was recommended on a demo horizon, so **the migration changes its character, not just its size** — that is the part David should weigh. **Recommended form (b): the CTA takes its OWN column, not a relabel of `description`.** The ask and the AI context are two different facts; the form already conflates them by placeholder; and one column carrying two facts is **precisely #71's open shape**, which this platform is already paying for once. One additive, append-only column on a table holding two orphan rows is cheap — a second STD-011 instance is not. **PER-CHANNEL CODES → COMPLETE AT MINIMUM, AND HONESTLY A FOURTH SCOPE OF THEIR OWN** (~4–6 prompts, 1–2 migrations, 1 ruling): they touch **CHECKOUT and the money path** — `submit.ts`, the path David deferred #84 on days before a demo — and the **existing discount model** (`discountTypes` in `business_pricing_config`, `order_discount:apply`, with D-55's AC-4 hold still in place). **That is a different blast radius from a form field. Recommended, NOT rescoped.**

**FLAGGED FOR DAVID:** **(a)** ✅ **THE NON-DELETION IS THE SESSION'S DEFINING CHOICE AND IT WAS VERIFIED, NOT ASSERTED: 58 pure additions · 2 in-place modifications (the PIECES and NEEDS tag lines) · 0 deletions.** Both sibling entries confirmed **BYTE-UNTOUCHED** by line-exact comparison against the pre-session snapshot, `grep -c "plan the season once"` = **1**, and 87 stories parse under `stories.html`'s **own extracted parser** (not a reimplementation — that method is what caught the `PIECES: —` chip asymmetry last time). **(b)** ✏️ **TWO NEW OWED ROWS WENT INTO `RULINGS.md`, WHICH THE PROMPT DID NOT ASK FOR AND WHICH I JUDGED REQUIRED:** the two new stories each say *"David to rule…"* in a `NEEDS:` tag, and **a ruling that lives only in prose is the exact failure `RULINGS.md` was created for** (four rulings evaporated out of §3 at N=3). Say the word and they come out. **(c)** 🔴 **Q4's customer answer is NARROWER THAN ANY OF THE THREE OPTIONS THE RECON OFFERED, and that is worth reading before ruling:** Regina split it on **PUBLICATION**, a boundary (a)/(b)/(c) did not have — before publication the drafts follow the edit; after publication **the posts have left the building and editing the row cannot reach them**, so the honest act is a reply and a restart. The recon was arguing about a database row while the artifact that mattered was already public. **(d)** **Nothing flipped to `written`** — all three campaign entries stay `needs-input` until you rule and Lauren confirms Wednesday. **(e)** **CLAUDE.md is 713 lines against its own ~600 budget**, unchanged in kind by this session; the residual is the still-open §4 *"lean CLAUDE.md"* item.

### 2026-08-23 (2) — THUNDER **THE THREE STORIES ARE FILED, AND #192's BLOCKING RULING IS CLOSED — THE ONE PIECE OF THE CAMPAIGN SCOPE THAT WAS NEVER A THUNDER PROMPT.** Yesterday's scoping named it the prerequisite of MINIMUM, COHERENT and COMPLETE alike: `user_stories.md` had **ZERO campaign entries**, none of the eight `## ARC:` sections covered the surface, and §9 says a story precedes any build spec. Three entries now sit under `## NEEDED — cross-cutting`, **filed VERBATIM as David wrote them** — **Arbor Day — plan the season once, change it when Terry changes his mind** (`needs-input`) · **Generating "more posts for this campaign" creates a second campaign, silently** (`needs-input`) · **Deleting a campaign — deliberately not built** (`scoped-out`). ✏️ **The surface shipped without a story BECAUSE the requirement came from Regina's nonprofit and was generalised into the platform without the story travelling with it — which is what `SCOPE: platform, vertical:cultivar, vertical:kinna` on the Arbor Day entry is the record of.** **DOCS ONLY — no app code, no schema, no migration, no cap; `stories.html` NOT edited; no existing story altered.** `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15, byte-identical to the session baseline) · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · GATE 0 **NOT APPLICABLE** · all `[TRACE:*]` ON.

**Type:** Story filing + close-out. `user_stories.md` (+3 entries, 85 stories parsed). Docs: ledger **#193** · `RULINGS.md` (**the story-blocker OWED row FLIPPED to ✅ ANSWERED; ONE NEW OWED row**) · ⚡ ACTIVE STATUS · `built-inventory.md` (Campaign Scheduler) · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-21 (3) #190), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 THE ONE FIELD I WAS ASKED TO DERIVE CAME BACK `—`, AND THE DERIVATION IS THE FINDING: THE CAMPAIGN SCHEDULER IS A LIVE, BUILT CAPABILITY WITH NO ROW ON THE 📋 24-CAPABILITY BOARD.** It has a tile (`tileRegistry.ts:189`, `status:'live'`, `depends_on:'social_media'`) and its own `built-inventory.md:1270` section, and the board simply has no id for it. The nearest neighbour, **4.3 Social media (gen + publish)** (`TRACE-SESSION-BOOTSTRAP.md:417`), is **a different surface, not a parent**: 4.3's feeder is `social/generate-posts.ts` → `social_drafts` (the weekly generator on `/social/setup`), while campaigns ride `shared/campaigns/generate.ts` → `campaign_posts` via `api/campaigns.ts` — two generators, two tables, two inventory sections. **Tagging these `4.3` would have made 4.3 read as STORIED while its own surface still has none — a false green on the board's one key link (`MAPS-TO` is the link the whole board exists for).** Per the header, `—` means no capability exists yet and renders as *"→ no capability yet"* / *"→ not building"*, so the gap is visible rather than papered over. **An id was not invented to fill a field.**

**✅ VALIDATED AGAINST THE REAL RENDERER, NOT A REIMPLEMENTATION OF IT.** `stories.html`'s own `parseStories`/`splitIds`/`splitList` were extracted and run against the file: **85 stories parse, all three found.** Every tag legal — `needs-input`/`needs-input`/`scoped-out` all RECOGNIZED · every SCOPE value in `SCOPE_ORDER` (incl. `vertical:kinna`) · `BUILD: active` · ARC omitted → **Unfiled**, correct for a `## NEEDED` cross-cutting item · `NEEDS:` present on both non-terminal entries and absent on the terminal one · **`Reason:` is the FIRST narrative line of the `scoped-out` card, so the card SHOWS why.** **WHAT'S OWED view: two owed, one terminal — exactly as expected.** ⚠️ **One cosmetic renderer asymmetry, reported not fixed: `splitIds` filters `—` but `splitList` does not**, so `PIECES: —` draws a single chip reading `—`. It parses, it renders legibly, it mirrors David's own `MAPS-TO: —` convention, and `stories.html` may not be edited — **so the tag was left exactly as written rather than deleted to suit a chip.**

**🔴 STEP 5 — THE MECHANISM WORKED AS DESIGNED, ONE INSTANCE LATE; AND THE CHEAP COUNT SAYS IT IS NOT ONE INSTANCE.** `campaigns:delete` was ruled twice AND cap-enforced (`verify-universals.mjs:1705-1712` fails the build on the literal) and **still came back this morning as a question that cost a recon** — because it was not on the board, which is precisely what `scoped-out` exists to prevent. **The exact class is one literal list behind one cap line, so the count is free: of the FIVE UNMINTABLE DELETES (`permissionManifest.ts:195-202`), campaigns is now the ONLY member with a `scoped-out` entry — `customers:delete`, `service_offerings:delete`, `deliveries:delete`, `assets:delete` have NONE.** ⚠️ **Deliberately NOT batch-filed, for a reason found while counting: `service_offerings:delete` would contradict its own code on the day it was written** — `Settings.tsx:415-419` ships a live, ungated, unconfirmed `.delete()` (E5's shape, already OPEN). **The BROADER class is NOT cheap and is left as a NAMED QUESTION rather than a false number:** it is a per-row judgement over ~65 `RULINGS.md` rows, not a grep, **and that file is admittedly incomplete** (created 2026-07-30; three rulings landed six weeks to two months late), so any count over it is a floor with an unknown ceiling — #174 restated. Filed to RULINGS OWED.

**FLAGGED FOR DAVID:** **(a)** ⚠️ **THE PROMPT SAID `docs/user_stories.md`; THAT FILE DOES NOT EXIST.** The board is **`user_stories.md` at repo ROOT** and `docs/user-stories/` is the long-form flow-spec directory — a **different artifact type** the §9 story-location rule says is not board-tagged and not parsed. **Filed to the root board.** The near-miss is the rule's own subject and worth one line. **(b)** 🔴 **NOTHING WAS BUILT, BY INSTRUCTION, AND THE L2 DEFECT IS STILL LIVE** — *"Generate more posts for this campaign"* still mints a second campaign and navigates onto it, silently; the two orphan rows are still there. **Recorded in `built-inventory.md` in place, unfixed.** **(c)** **ARBOR DAY STAYS `needs-input`** — it does not flip to `written` until you rule the EDIT question (RULINGS OWED) and Lauren confirms the scene Wednesday. **(d)** **CLAUDE.md is 715 lines against its own ~600 budget** — flagged per the CONTEXT BUDGET CHECK; the structural residual is the still-open §4 *"lean CLAUDE.md"* item, unchanged by this session.

### 2026-08-23 — THUNDER **DAVID IS RIGHT, AND TWO THIRDS OF WHAT HE ASKED FOR IS FREE — THE THIRD IS BLOCKED BY A RULING HE MADE, NOT BY AN ABSENCE.** *"why can i not edit each campaign… i can add but not edit or cancel or delete"* splits three ways and only one is a build gap. **EDIT: nothing blocks it** — `campaigns_member_update` exists (`20260727c:21-23`), `campaigns:update` is `enforced`, there is simply no UI. **CANCEL: nothing blocks it** — `campaigns.status` **already permits `'cancelled'`** (`20260529:15`) and `Campaigns.tsx:103` **already paints it red**; the display half shipped 2026-05-29 and nothing has ever written the value. **DELETE: ruled against, twice, with a cap behind it** — spec R2 (*"per David's ruling **likely never**: a deleted campaign destroys its own history"*), `permissionManifest.ts:195-202`, and `verify-universals.mjs:1705-1712` **fails the build on the literal `campaigns:delete`.** 🔴 **AND THE SHARPEST FACT: THE OWNER CAN ALREADY DELETE A CAMPAIGN AT THE DATABASE — TODAY.** `campaigns_owner` is **`FOR ALL`** (`20260529:19-20`, incl. DELETE) with `campaign_posts` **CASCADE** (`:24`), and `20260727c:14-16` says so in as many words: *"DELETE stays owner-only through the existing `*_owner` policies."* **The SQL editor is not a workaround for a missing capability — it is a workaround for a missing BUTTON.** What stands in the way is that the model has **no string to gate it on** (cap-enforced) and the obvious substitute, `isOwner`, was ruled **DISPLAY-ONLY on 2026-07-30**. **RECON + ESTIMATE ONLY — no app code, no schema, no migration, no cap.** `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · GATE 0 **NOT APPLICABLE** · all `[TRACE:*]` ON.

**Type:** Scoping + estimate. **NEW `docs/audits/campaign-lifecycle-scoping-2026-08-23.md`** (HAVE/NEED/WANT per OP-8, nine questions with `file:line`, the F1/F2/F3 dependency proof, three scopes, eight rulings owed, STD-021 §7 blind spots). Docs: ledger **#192** · **RULINGS OWED — 5 NEW ROWS** · ⚡ ACTIVE STATUS · `built-inventory.md` · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-21 (2) #189), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 LIVE DEFECT FOUND OUTSIDE THE NINE QUESTIONS, AT THE TOP AND UNFIXED — THE "GENERATE MORE" BUTTON MINTS A SECOND CAMPAIGN AND WALKS YOU ONTO IT.** `CampaignDetail.tsx:400-411` reads verbatim *"✦ Generate more posts for this campaign"*; `handleGenerateMore` (`:143-167`) POSTs `action:'generate'` — **the CREATE branch** — and `campaigns.ts:103-116` INSERTs a new row unconditionally (no id in the body, no upsert), then `:164` navigates to **the NEW campaign's id.** The campaign the owner was looking at never receives a post. 🔴 **And it is the ONLY generate path with NO error surface at all:** `:165` is `catch { /* silent */ }` and `:164` navigates `if (resp.ok)` **with no `else`** — a 500 does nothing observable. **So it is a SECOND, SILENT ORPHAN FACTORY.** ✏️ **This is the shape of the gap David is asking about, not a slip: there is exactly ONE write path into this feature — CREATE — so a build needing *"add posts to an existing campaign"* had nowhere to go and rode CREATE. A MISSING LIFECYCLE DOES NOT STAY MISSING; IT GETS IMPERSONATED BY THE PATH THAT EXISTS.**

**🟢 Q3 ANSWERED AND IT DEFUSES THE ORPHANS: THERE IS NO PUBLISH PATH. NOTHING WILL EVER ACT ON THEM.** Four independent negatives — `vercel.json` has **no `crons` key** (only rewrites) · the auto-publish seam is declared **inert** in writing (`campaigns.ts:9-13`, *"Blotato was removed — it misrepresented capability"*) · the only `status='published'` write is keyed on an explicit `postId` from a human's button (`:181-184`) · **no site anywhere filters `campaigns` by `status`.** `scheduled_date` is a date the AI wrote for a human to read; **no code consumes it to decide when to do anything.** **The cleanup is a correctness-of-display fix, not a safety fix — which removes the only argument for rushing DELETE ahead of its ruling.**

**🔴 THE DEPENDENCY ORDER IS PROVEN, NOT PREFERRED — AND THE REAL PREREQUISITE IS THE ONE NOBODY WOULD LOOK FOR. F1 DOES NOT DEPEND ON F2:** not one of edit/cancel/delete writes `campaign_posts.platform`, so `campaign_posts_platform_check` is on **none** of their code paths — the prompt's own instinct was right and the lifecycle could ship against a vocabulary that stays broken. **F3 IS THE PREREQUISITE — of F1's VISIBILITY, not its code:** `Campaigns.tsx:50` discards `error` so a failed list read renders *"No campaigns yet"* over live rows, and 🔴 **`:62` does the same on the post COUNT, so `:247-257`'s fall-through renders a zero-post orphan as "All posts published ✓"** — **three states, one green check, the strongest affirmative signal in the UI given to a failed generation** (§6 r18 + A9). **Ship F1 without F3 and the observed outcome is "the new buttons don't work" when they were never drawn.** **ORDER: F3 → F1 → F2.**

**📐 THE ESTIMATE — THREE SCOPES, IN THUNDER-PROMPT UNITS. MINIMUM ~2–3 · COHERENT ~5–7 · COMPLETE ~11–14.** 🔴 **MINIMUM AND COHERENT NEED ZERO MIGRATIONS** — every piece rides `campaigns:update`, an existing policy, an existing status value, and `api/campaigns`' existing `action` switch (**api/ is 12/12; no new file, §6 r11**). **MINIMUM** (the honest read + cancel + a compensating delete) closes *"I cannot undo what I created"* **without touching R2** — ⚠️ **and its limit is stated: the rows still EXIST and there is still no edit, so it does not answer David's literal question.** **COHERENT** adds edit + the L2 fix + validate-before-generate and IS the ruled standard — **delete is then the only absence, and it is a RULING rather than a gap**, which is a materially different answer to give him. **COMPLETE** adds delete, the derived vocabulary and the transactional RPC: **2 migrations, 3 rulings, and F is the piece most likely to overrun.** ✏️ **BIGGEST REUSE WIN, and it is not where anyone would look: THE EDIT FORM ALREADY EXISTS AS THE CREATE FORM** — `Campaigns.tsx:120-198`, the identical six fields. Building an edit modal fresh would author a §6 r8 semantic dup deliberately.

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE STORY BOARD HAS ZERO CAMPAIGN ENTRIES** — grep across all 98KB of `user_stories.md` returns nothing and none of the eight `## ARC:` sections covers this surface. Per the §9 story-reconciliation gate that is **IN CODE BUT NOT ON THE BOARD**, and a story is CREATED before any build spec. **This is a hard prerequisite for every piece and it is not a Thunder prompt.** **(b)** 🔴 **Q5 IS ALREADY RULED AND #71 IS NOT A RISK HERE — IT IS THE DEFECT THIS PARAGRAPH PREVENTS, WRITTEN ABOUT THIS EXACT TABLE:** *"a `status` column is NOT a tombstone… overloading lifecycle would be one column carrying two facts, which is the STD-011 defect this program exists to end."* **CANCEL is lifecycle, DELETE is removal, and `campaigns.status` may not carry both.** **(c)** ⚠️ **THE PRECEDENT CUTS BOTH WAYS AND IT IS NAMED SO NOBODY FINDS IT LATER AND READS IT AS PERMISSION: the unmintable-delete rule forbids a STRING, never a delete OPERATION** — `service_offerings` is on the same list and ships a live, ungated, unconfirmed `.delete()` today (`Settings.tsx:415-419`, route gated only `settings:read`, member policy `FOR SELECT` only — **E5's shape, already an OPEN ruling**). A hard delete on `campaigns` **would not break any cap**; it would break the ruling, which is different and more serious. **(d)** 🔴 **PER YESTERDAY'S OWN RULE (`RULINGS.md:121`), THE HEADLINE FINDING IS A DOC READ AND SAYS SO:** that `campaigns_owner` is `FOR ALL` **in the live database** rests on `20260529` being applied as written. **The one query is in the doc, with its branch.** **(e)** **NOTHING FIXED, BY INSTRUCTION** — L2 unfixed, orphans NOT deleted (David's), no string minted, no constraint widened, Q4/Q5 recommended-and-stopped.
