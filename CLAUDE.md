...
# Last updated: 2026-07-08 (THUNDER CHECKOUT FIX-PASS [ledger #99] — 4 owner-prove-surfaced fixes on the restored transport workflow: (1) ScanOrder manual field now SEARCHES (new shared searchStockLines, partial id/name → pick list) not exact-match; (2) checkout modals CENTERED; (3) CustomerCapture requires address+phone when a delivery branch is selected (reads selectedTransport.requires_address/staff-shape, not hardcoded); (4) owner/manager delivery-date field threaded cart→submit, GATED migration 20260708_orders_delivery_date.sql (deploy-window-safe fallback in submit.ts). APP CODE + 1 gated migration, ZERO new api-fn (12/12); npm run verify exit 0 zero NET-NEW (tsc 6 / eslint 250 / knip 10·14·15). STORY MAPS-TO 2.1 (in-store purchase workflow). NEW gap sub-story logged (template-driven service setup, needs-input). UNCOMMITTED — David commits.)
