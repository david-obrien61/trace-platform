...
# Last updated: 2026-08-25 (8) — build: checkout writes a scheduled delivery; zero migrations (see §3)
...
### 2026-08-25 — THUNDER **BUILD: CHECKOUT WRITES A SCHEDULED DELIVERY — AND THE PREMISE CHECK MATTERED, BECAUSE THE WRITE WAS ABSENT RATHER THAN BROKEN.** 🔴 **THE GAP, MEASURED BEFORE ANY CODE: `orders` AND `deliveries` WERE UNCONNECTED — `submit.ts` contained ZERO `deliveries` references.** Every stop on `/delivery-schedule` and `/deliveries` arrived through the OCR-invoice door (`api/customers/create.ts`), so **an order rung up at the counter for delivery next Thursday appeared on NEITHER screen** and the truck's day was assembled from photographed paper only. So this is a net-new connection, not a repair — the different fix David's prompt told me to report and stop on **was not the case.** ✅ **ONE INSERT, `scheduleCheckoutDelivery` (step 12b):** a row when `transport_method` is `delivery` or `install`; **`self` writes nothing and the insert is NEVER ATTEMPTED** (a no-op that still round-trips passes a row count and is wrong). `install → 'planting'` · `delivery → 'delivery_only'` — **the OCR door's own two words, no third invented** · `source:'checkout'` · `notes` = the invoice number · address read **canonical `billing_*` first, legacy mirror as fallback (D-41)**, absent stays **NULL not `''`**. 🔴 **IT CANNOT FAIL THE ORDER, STRUCTURALLY: every failure path returns an outcome object, so nothing it can produce reaches `handleCreate`'s catch** — the order completes, the invoice pushes, the customer is confirmed. 🔴 **IT PROVES IT WROTE (R-12) IN BOTH DIRECTIONS — `rows.length !== 1` refuses the zero-row PostgREST shape AND a two-row insert; red-first showed a null-guard-only variant passes the zero case incidentally.** **BUILDER-COMPLETE `64a799a`→this commit** · NO schema, NO migration, NO policy, NO new permission string, NO new cap, NO new api/ fn · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **32/32 test files · 1206 assertions** (+1 file, **+55, all row-counting**) · api/ **12/12** · **GATE 0 OWED** · all `[TRACE:*]` ON.

**Type:** Build + close-out. Code: `packages/cultivar-os/api/orders/submit.ts` · `checkoutDelivery.test.ts` (**NEW, 55 probes**) · `write-paths-baseline.json` (**`deliveries` 2 → 3, ONE explicit line**). Docs: ledger **#216** · `tech-debt-log.md` (**#108 · #109 NEW**) · `built-inventory.md` (**1 net-new body entry**) · **NEW `docs/owner-tests/checkout-delivery-scheduling-full-surface-test.md`** (**7 cards — 1-5 `owed`, 6-7 `needs-test` WITH REASONS; NOTHING marked `covered`**) · `user_stories.md` (**1 NEW `STATUS: gap` story**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-24 (5) #213, `in`-check confirmed on 6,541 chars), 1 written — entries-in == entries-out, §3 back to 3** (archive 244 → 245). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (186 lines, every row incl. the whole OWED table; R-12 read verbatim at source).**

**🔴 THE FINDING WORTH MORE THAN THE BUILD, AND IT CAME OUT OF THE PRE-FLIGHT GATE RATHER THAN THE CODE: THE OBVIOUS MOVE WAS FORBIDDEN BY A RULING MADE ABOUT THIS EXACT LINE.** `customers/create.ts:67` gates its delivery insert on **`deliveries:create`**, and copying that gate into checkout is what any careful reader would do. **The 2026-07-31 ruling — *A PERMISSION GATES A CAPABILITY, NOT A FIELD* — was made about the checkout delivery-date field and names `deliveries:create` as the WRONG string for it**, because granting it *"would also hand her the ReceiptKeeper delivery INSERT she was never owed."* **Tech-debt #84 closed on the same reasoning: *submit.ts needs nothing and its silence is a RULING, not an oversight.*** ✏️ **And the corroborating fact settles it from a second direction: the anon QR checkout path carries NO TOKEN AT ALL, so a gate here would not have narrowed the act — it would have deleted it.** The ruling did not tell me what to write; it stopped me writing something.

**FLAGGED FOR DAVID:** **(a)** 🔴 **WHAT YOU RUN LIVE, IN ORDER — and CARD 5 is the one that matters, because *"a stop appeared"* is not the claim: GATE 0 (the SHA on screen == `git log -1`, and Vercel READY for THAT SHA) → COUNT the stops on your target day → ring a **Delivery only** order for that day → `/delivery-schedule` shows **EXACTLY ONE** new stop, right day, right address, `Delivery only` badge, invoice number in notes → repeat with **Delivery + planting**, expect the `Planting` badge → repeat with **self-haul**, expect the board COMPLETELY UNCHANGED (⚠️ **check the UNDATED bucket too**). Expected counts: **+1 / +1 / +0.**** **(b)** 🔴 **TWO KNOWNS SHIPPED BY YOUR DECISION, NOT DISCOVERED LATER — and one of them is bigger than the duplicate you named. #108: there is no `order_id` column** (*"KNOWN AND ACCEPTED, DO NOT SOLVE… Do NOT add a column to prevent it"* — honored, no column added). You named the duplicate; **the quieter one is STALENESS: `handleUpdate` changes `orders.delivery_date` and CANNOT REACH the delivery row**, so a customer moved from Thursday to Tuesday keeps a stop on Thursday and Lauren plans a truck around it. **A deleted order leaves an orphan stop.** Both are in #108 with the durable fix (an FK + a partial unique index — a MIGRATION, which is why it was excluded). **(c)** ⚠️ **ONE STATED DEVIATION, AND IT IS THE ONLY ONE (§6 r10): `deliveries` now has a THIRD app write path, and its column mapping is a near-twin of `customers/create.ts:103-114` — the fork §6 r8 exists to prevent.** The durable form is ONE shared writer both doors call; **that refactor touches the OCR capture path, which you scoped out**, so I took the copy and flagged it rather than taking it silently. **`write-paths-baseline.json` `deliveries` went 2 → 3 in ONE explicit line — BASELINED (*known today*), deliberately NOT declared in `ALLOWED_DIVERGENCE` (*correct forever*), because unlike every declared multi-path table the column overlap here is TOTAL and a declaration would be the wrong claim.** The table still prints **GOAL:FAIL** so it stays visible. Filed as **#109**; best landed WITH #108's migration as one build. **(d)** ⚠️ **§9 STORY GATE = NO MATCH, and I did not treat that as permission.** Every delivery story on the board sources its stops from the OCR door; **none said a checkout order becomes one.** I wrote a **`STATUS: gap`** story from your own prompt (the *IN-CODE-NOT-ON-BOARD / as-built* branch of the gate) — **you own promoting it off `gap`.** Thunder does not dictate stories. **(e)** ✅ **RED-FIRST AGAINST THE REAL SOURCE (STD-024), not internal controls: FIVE defects planted into `submit.ts` in turn** — the count check dropped · the null-guard-only variant · `self` made to schedule · the catch removed · the invoice number dropped — **each FAILING and NAMED**, and the source restored **`diff`-confirmed identical** each time. ⚠️ **The first run of RED 1 caught only ONE assertion, which was too weak, so the test was STRENGTHENED with a two-row probe before I accepted it.** **(f)** ⚠️ **A DELIBERATE OMISSION, stated because it looks like an oversight: the OCR door strip-and-retries `service_type` on a missing column and the checkout door does NOT.** Reason: `DeliverySchedule.tsx:122` already selects that column in a bare list, so a live screen depends on its presence; and a row landed WITHOUT it is a **silently degraded stop** (no badge, no planting/drop distinction) — worse than a loud, non-blocking failure. **(g)** ⚠️ **A CLAIM I DID NOT MAKE:** whether the `deliveries.service_type` column is live in the catalog. This machine has no catalog access and the 2026-08-22 ruling says a database claim is sourced or not made — **I proved the SHAPE from the CREATE TABLE and the ALTER, and relied on your prompt's assertion that the table already has every column needed.** **(h)** ⚠️ **UNTOUCHED, as instructed: the OCR capture path, every existing `deliveries` row, `/delivery-schedule`'s rendering and duplicate handling, `/deliveries`' filter.** ⚠️ **Consequence worth knowing: whether a checkout-sourced stop appears on the route MAP depends on that untouched filter — unverified, and owner-test card 7 says so rather than implying coverage.** **(i)** ✅ **`git log`/`git status` re-run at write-up.** **(j)** **CLAUDE.md is under its ~600 budget.**

### 2026-08-24 (7) — THUNDER **BUILD: THE QB INVOICE STOPS PRINTING THE OVERRIDE REASON — AND FIX 2 STOPPED AT ITS GATE, WHICH IS THE MORE USEFUL RESULT.** ✅ **FIX 1 SHIPPED AND IT IS ONE LINE:** `cultivar.ts` no longer interpolates `override_reason` into the QB line description; the adjusted line reads `<Service> — price adjusted`, **the negative-adjustment shape is untouched** (retail 1575 + a separate −575), and **the reason is NOT lost — it stays on the row, on the internal order screen, and in the `[TRACE:QBO]` emit**, which is R-7 attribution working as ruled: internal by design. 🔴 **FIX 2 DID NOT RUN, BY THE GATE'S OWN RULE, AND THE GATE ANSWER IS THE FINDING: `'1'` IS THE ONLY QUICKBOOKS ITEM ID THIS PLATFORM HOLDS ANYWHERE** — no config, no env var, no column, no lookup; the only query builder is `qbQueryCustomers` (`:68-76`), hardcoded to `select * from Customer`, and **nothing anywhere queries `Item`.** **Inventing one would push an invoice QuickBooks rejects, and a push that fails is worse than a push that mis-categorizes.** 🔴 **THE PAYLOAD IS ASSERTED BY A TEST FOR THE FIRST TIME — 27 probes, fixture = INVOICE 436 RECONSTRUCTED FROM THE RENDERED DOCUMENT** (496 + 399 + 125 + 1575 − 575 = 2020; × 7.6% = 153.52; total 2173.52, all four asserted). **BUILDER-COMPLETE `4725375`→this commit** · NO schema, NO migration, NO policy, NO new permission string, NO new cap, NO new api/ fn · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **31/31 test files · 1151 assertions** (+1 file, +27) · api/ **12/12** · **GATE 0 OWED — David confirms Vercel READY for THIS SHA before pushing any invoice** · all `[TRACE:*]` ON.

**Type:** Build + close-out. Code: `packages/cultivar-os/api/qbo/invoice/cultivar.ts` · `qboInvoiceLines.test.ts` (**NEW, 27 probes**). Docs: ledger **#215** · `tech-debt-log.md` (**#104 → 🟡 partially resolved · #106 annotated with the gate answer**) · `built-inventory.md` (**1 net-new body entry**) · **NEW `docs/owner-tests/quickbooks-invoice-full-surface-test.md`** (**cards 1 · 2 ADDED as `owed`; cards 3 · 4 as `needs-test` WITH REASONS — nothing marked `covered`**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-24 (4) #212, `in`-check confirmed on 7,119 chars), 1 written — entries-in == entries-out, §3 back to 3** (archive 243 → 244). ✏️ **CLAUDE.md read IN FULL, `docs/RULINGS.md` read IN FULL, and the QB recon read IN FULL and QUOTED rather than paraphrased.**

**🔴 THE FINDING WORTH MORE THAN EITHER FIX, AND IT CAME OUT OF THE GATE RATHER THAN THE BUILD: `'1'` IS A PER-TENANT FACT HARDCODED INTO SHARED PLATFORM CODE.** It resolves in LAWNS' company file — **invoice 436 proves item 1 exists there and is named "Services"** — but **a second tenant's QuickBooks has no obligation to have an item `1`, or to have it mean anything similar.** So #106 is not only *"plants book as services"*; it is a **multi-tenant landmine that fails differently for the next customer**, and nothing in the code would say so. ✏️ **The gate was written to stop me guessing an id, and what it actually surfaced is that the id we already ship is a guess about someone else's books.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **WHAT YOU MUST DO TO UNBLOCK FIX 2 — A READ IN QUICKBOOKS, NOT A BUILD: Sales → Products and services → find (or create) a PRODUCT item for nursery stock → open it and read the id from the browser URL → hand back the id AND the exact item name.** Then FIX 2 is a one-seam change, because **the payload already separates goods from services** — asserted by probes D2/D2c. **Only the id is missing.** ⚠️ **Do not change item `1` itself — everything already invoiced points at it.** **(b)** 🔴 **THE RECEIPT HALF IS NOT FIXED AND THAT IS DELIBERATE — say it plainly before you test:** the Confirmation screen still shows the reason via `OrderTotals.tsx:97`, **a component shared with the internal order screen**, and you scoped it out. **Owner-test CARD 3 exists precisely so it is not refiled as a bug by whoever tests CARD 1.** **(c)** ✅ **THE TEST ASSERTS THE PAYLOAD, NEVER A STATUS — the reason being that invoice 436 WAS a 200.** *"A test that asks the far end whether it liked what we sent inherits whatever we sent."* A1 searches the **entire serialized payload**, not the one line I fixed. **(d)** ✅ **RED-FIRST AGAINST THE REAL SOURCE (STD-024), not just internal controls:** the old interpolation planted back into `cultivar.ts` fails **exactly A1/A2/A3**, and A3's failure message reproduces invoice 436's line verbatim; **file restored and `diff`-confirmed identical.** Three internal negative controls (E1/E2/E3) additionally prove the leak detector, the reconcile check and the pair check can each fail. **(e)** ⚠️ **ONE STATED DEVIATION, AND IT IS THE ONLY THING I DID BEYOND THE TWO FIXES (§6 r10):** to assert the payload at all, the line assembly had to be callable without QuickBooks, so it was **extracted into `buildQboInvoiceLines()` — MOVED VERBATIM, proven mechanically: a normalized diff of the 198-line region against `HEAD` shows the ONLY content changes are FIX 1's description line and its comment.** That region contains the tax line and the legacy-addon block, which you told me not to touch — **they were RELOCATED, not modified, and nothing about their behaviour changed.** I took it because a test built around a hand-written copy of the payload is the drifting fake this codebase keeps paying for; **flagged rather than taken silently.** **(f)** ⚠️ **A NUMBERING CORRECTION IN YOUR PROMPT, because these rows get cited later:** the prompt labelled the ItemRef fix **#105** and put *"the tax line"* at **#106** and *"the strip and edit path"* at **#107**. As filed yesterday: **#104** = the printed reason · **#105** = the validation + attribution loss (the strip and the edit path) · **#106** = the hardcoded ItemRef · **#107** = the discount TYPE. **The tax line has no row of its own.** I built to your INTENT, which was unambiguous. **(g)** ⚠️ **api/ IS STILL 12/12 and no migration, schema, policy or permission string was touched.** **(h)** ✅ **`git log`/`git status` re-run at write-up.** **(i)** **CLAUDE.md is under its ~600 budget.**

### 2026-08-24 (6) — THUNDER **RECON: THE QUICKBOOKS INVOICE PUSH — FOUR ISSUES ON ONE INVOICE THAT WAS ALREADY SENT AND ALREADY READ.** Evidence: QB `txnId=436`, order `2661dbe4-e26d-486f-b65f-50e0f56716c3`, 07/16/2026, status **"Opened"**. ✅ **THE HARD PART WORKS AND NO OPTION MAY BREAK IT:** the arithmetic reconciles, line detail reaches QB, and the retail-line + negative-adjustment pair preserves BOTH the original price and the concession — R-7's shape, and the scar of QBO error 6070 (`cultivar.ts:481-486`). 🔴 **Q1 — THE OVERRIDE REASON IS INTERPOLATED INTO THE CUSTOMER'S INVOICE LINE AT `cultivar.ts:500-503`, AND IT REACHES A SECOND CUSTOMER SURFACE THROUGH A SHARED COMPONENT: `OrderTotals.tsx:97` serves BOTH the Confirmation receipt AND the internal order screen, with no idea which audience it is rendering for.** 🔴 **Q2 — (i), A SINGLE HARDCODED ITEM: `ItemRef: { value: '1', name: 'Services' }`, TWELVE identical inline literals — and plant-vs-service SURVIVES to the push (two tables read separately, looped separately, `offering.category` read at `:459`), so the distinction is KNOWN AND DISCARDED, never lost.** ⚠️ **Q3 — a computed tax LINE is the ONLY mechanism: `TxnTaxDetail`/`TaxCodeRef`/`GlobalTaxCalculation`/`TaxRateRef`/`SalesTaxRef` return ZERO hits repo-wide.** 🔴 **Q4 — THREE independent gates require the reason and NONE checks its content; `trim() !== ''` is the whole test.** **LOOK ONLY — no app code, no schema, no migration, no policy, no permission string, no cap** · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **30/30 test files · 1124 assertions** · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · **GATE 0 NOT APPLICABLE — nothing ships** · all `[TRACE:*]` ON.

**Type:** Recon + close-out. Docs: **NEW `docs/decisions/2026-08-24-quickbooks-invoice-push-recon.md`** (the one deliverable, 346 lines) · ledger **#214** · `tech-debt-log.md` (**#104 · #105 · #106 · #107 NEW** — #104 tagged **CUSTOMER-FACING · ALREADY SENT**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **`built-inventory.md`: ZERO net-new entries and ZERO amendments — nothing was built and no capability's state changed.** **Owner-tests: NO card added, updated or flipped, and none created for unbuilt fixes (as instructed); nothing marked `covered`.** **§3 RETENTION: 1 archived verbatim (2026-08-24 (3) #210, `in`-check confirmed on 7,165 chars), 1 written — entries-in == entries-out, §3 back to 3** (archive 242 → 243). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (186 lines incl. the whole OWED table; R-6 and R-7 read verbatim at source).**

**🔴 THE FINDING WORTH MORE THAN ALL FOUR, AND IT IS A CORRECTION TO THE PROMPT'S OWN PREMISE: THE REASON ON INVOICE 436 IS NOT OUR VALIDATION MESSAGE. NO CODE PATH PRODUCED IT.** A grep across `packages/` and `api/` for *"cannot be empty"*, *"must be filled"* and *"if discount applied"* returns **ZERO** — our actual message is *"A reason is required to change this price — it goes on the invoice and the record"* (`CartReview.tsx:850`). **So a human hit the required-reason gate and typed a description of the rule into the box to get past it, and because CONTENT is never checked, the note ABOUT the requirement SATISFIED the requirement — and QuickBooks emailed it to the customer.** ✏️ **That is worse than an echoed error string and better news for the fix: the control did exactly what it was built to do. The gap is between *a reason exists* and *a reason is meaningful*, and nobody ever drew that line on a field that gets printed to a customer.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **Q1's FOUR OPTIONS ARE COSTED AND DELIBERATELY NOT COLLAPSED — YOURS TO PICK.** **A** stop sending it = **ONE line** (`:501`), but **does not touch the receipt** · **B** split internal/customer = ~4 files + 1 gated migration **+ teaching `OrderTotals` its audience**, and getting that wrong prints the internal reason on the receipt anyway · **C** a discount TYPE = **largest, and partly BUILDS R-6** · **D** ~2 lines. ⚠️ **D IS ALREADY HALF-DONE AND THAT IS THE POINT: the sentence *"it goes on the invoice and the record"* EXISTS but renders ONLY inside `{reasonErr && …}` (`:848`) — i.e. only after you already tried to save an empty reason.** The always-visible copy is the label `Reason *` and the placeholder *"e.g. loyal contractor"*. **(b)** 🔴 **OPTION C IS NOT REACHABLE TODAY AND IT IS BLOCKED TWICE (tech-debt #107): the vocabulary is scoped to CUSTOMER TIERS not per-line overrides, AND the chosen tier's NAME is never persisted on the order** (`orderBase` writes no tier column) — **which is exactly why a tier prints `Discount (10% off)` while an override prints free text.** ✅ **But the presenter half is already built AND already imported by this very file:** `taxExemption.ts`'s code→label shape, called at `cultivar.ts:596` for the tax-exempt line. **The push uses this pattern for one reason field and not the other.** **(c)** 🔴 **Q2's CONSEQUENCE, SAID PLAINLY BECAUSE IT IS AN ACCOUNTING FACT, NOT A COSMETIC ONE: booked this way your QuickBooks shows 100% service revenue, zero product sales, and NO COGS against inventory** — the goods lines never touch an inventory-tracked QB item, so product gross margin cannot be computed inside QuickBooks at all. **A P&L built from this says LAWNS is a service company that sells no plants.** **(d)** 🔴 **TWO PATHS DESTROY THE ATTRIBUTION R-7 DEPENDS ON (#105): the deploy-window strip (`submit.ts:851-857`) drops all five override columns and retries — the concession still lands because it is already in `subtotal`, so the money moves and NOTHING records who or why; and the edit path (`:1249`) updates `quantity`/`subtotal` only, leaving `original_price`/`reason` describing a price that no longer exists — while the GOODS breakdown beside it IS refreshed.** **R-7's whole cost-benefit is Lauren seeing the discount afterward.** **(e)** ⚠️ **Q3 IS CONSISTENT WITH D-37 AND I CHECKED IT RATHER THAN ASSUMED IT:** computing tax and putting the figure on an invoice IS charge computation (scoped IN); pushing a line processes, collects and reconciles nothing (scoped OUT). ⚠️ **What is NOT a D-37 question and is NOT answered here: the tax lands on the same `'Services'` item, so it books as REVENUE rather than into QB's tax handling. That is a question for your accountant, not for us — no tax opinion is offered.** **(f)** ✅ **Q5 — ONE LIVE SEAM, TWELVE SITES INSIDE IT.** `cultivar.ts` is the only live builder (24 hits); `packages/shared/src/quickbooks/invoice.ts` is **DEAD donor code** (zero callers, `localhost:8000` default) **and hardcodes `value:'1'` too**; `ignition-os/ExternalBridge.js` noted and untouched. **So Q1 is one line, Q2 is a twelve-site sweep in one file, Q3 is one site, Q4 is three.** **api/ 12/12 — NO option here needs a new function.** **(g)** ⚠️ **A CLAIM DELIBERATELY NOT MADE: which case the three NULL `override_reason` rows are.** A NULL on a NON-overridden line is **correct and expected** (`overrideCols` writes `{is_manual_override:false}` with no reason key) — **not every null is a defeated validation**, and this machine cannot read the catalog (2026-08-22 ruling). **(h)** ⚠️ **STATED DEVIATION (§9 gate 10):** the prompt directed no options for Q2–Q5 and no collapsing for Q1 — **honored; options for Q2–Q4 are owed when you ask for a build.** **(i)** ✅ **`git log`/`git status` re-run at write-up.** **(j)** **CLAUDE.md is under its ~600 budget.**
