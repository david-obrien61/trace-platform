...
# Last updated: 2026-08-21 (3) — `businesses` closed by declaration; the phantom column; a new boundary cap (#190). See §3.
...
### 2026-08-21 — THUNDER TWO REGISTRY DEFECTS FOUND ON A PHONE TURNED OUT TO BE **1 OF 12 AND 1 OF 22**, AND THAT RATIO IS THE ENTIRE POINT. The class, stated once: **A REGISTRY ROW CAN BE INTERNALLY VALID AND STILL BE FALSE ABOUT THE WORLD.** `verify-tile-fields` passes over every instance **correctly** — it asserts a row is COMPLETE and VALID against its own SCHEMA and reconciles catalog↔tile in both directions, and it has **no assertion comparing one row to ANOTHER ROW DESCRIBING THE SAME THING**, nor a declared `section` against an actual `route`. **Two found by accident is an UNMEASURED class, not a small one (David's ruling, #174) — the fix for two is two edits, the fix for thirty is a derivation, and nobody had the number. MEASUREMENT ONLY; NOTHING FIXED, NOTHING RULED.** `npm run verify` exit 0 zero net-new · api/ 12/12 · `tileRegistry.ts` byte-identical · GATE 0 **NOT APPLICABLE**.

**Type:** Measurement + docs. **NEW `scripts/measure-registry-contradictions.mjs`** (⚠️ **NOT chained into `npm run verify`**) · **NEW `docs/audits/registry-contradiction-count-2026-08-21.md`**. Docs: ledger **#188** · ⚡ ACTIVE STATUS · `built-inventory.md` · DECISIONS-INDEX drift. Commit `8e0224f`.

**🔴 THE NUMBERS: Q1 = 12 contradicting capability pairs · Q2 = 22 section/route disagreements of 27 routed nav nodes · Q3 = 5 modules paired to a tile no renderer draws · `businesses` = 7 undeclared write paths.** `settings:read` alone produces **8 of the 12**. Q2 disaggregates and the disaggregation is the finding: **`dashboard` has ZERO children under `/dashboard` (uniform), `admin` has TWO under `/admin` and SEVEN not (mixed)** — whether those are the same defect is the prior question the measurement **refuses to answer**.

**METHOD — esbuild-EVALUATE, NOT TEXT-PARSE, and the decision not to reuse the proven walker is recorded rather than silent.** The registry is bundled with `lucide-react` aliased to an inert stub and imported, so the **REAL `dashboardTiles()` and `navRoute()` are CALLED** — RENDERABLE and tileKey-route resolution cannot drift from the code they describe, and **no status/placement/section list is typed anywhere in the script.** `verify-tile-fields`' walker was the FIRST choice: it is **not exported** and the module has **no `import.meta.main` guard**, so importing it runs its probes and its `process.exit` — reuse meant editing a live cap mid-measurement, and copying meant forking the one parser whose failure mode is documented in its own header (§6 r8 weighed, not ignored).

**12 PROBES, ALL GREEN, AND THREE OF THEM ARE WHY THE NUMBER IS TRUSTWORTHY:** **P1** row-count (evaluated 33 == an INDEPENDENT text count 33 — the 2026-08-01 zero-row false-green cannot recur silently) · **P7** negative control (a clean fixture reports 0/0/0, **without which P5/P6 are satisfied by a script that flags everything**) · **P12** module-cache (node caches ESM by URL — a reused outfile would have made **every planted-bad pass while measuring the wrong file**). **PLANTED-BAD AGAINST THE REAL FILE, not only fixtures:** 33→34 in both count methods, 12→13, 22→23, 5→6, **each NAMED by key**, then restored with `git diff` EMPTY and `diff -q` IDENTICAL.

**FLAGGED FOR DAVID:** **(a)** ⚠️ **The script is NOT a cap, deliberately — minting one before David rules answers a ruling with a constant**, which is the thing the measurement exists to prevent. **(b)** 🔴 **`verify-write-paths` prints `goal: 16 table(s)` beside the prose `17 failures` IN ONE SENTENCE** — the computed number and the 07-29 ruling's constant have diverged. **This measurement's own class, inside a cap's output.** Named, not fixed. **(c)** 🔴 **LEDGER #187 IS CITED 3× IN `RULINGS.md` AND DID NOT EXIST IN `CLOSE-OUT-LEDGER.md`** — written in the following session. **(d)** **STD-021's required *what this CANNOT see* section lists NINE blind spots**, including that **Q1 pairs on `required_permission` ALONE** — had David's two rows also disagreed on their permission, this measurement would report **ZERO** for the defect he actually found.
