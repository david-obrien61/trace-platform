...
# Last updated: 2026-08-23 (15) — build: the scanner's lie fixed, R-11's first instance (see §3)
...
### 2026-08-23 (15) — THUNDER **BUILD: THE SCANNER STOPS BLAMING THE TAG FOR A DEAD ZONE — AND THE TYPE FOUND ALL FIVE CALLERS BY ITSELF.** `stockLineResolver`'s three reads discarded the Supabase `error` and substituted `(rows ?? [])`, so a network failure returned the **byte-identical** value a genuinely absent tag returns — and the app told Lauren, by name, in a modal, to **check the tag**. 🔴 **THE FIX IS A TYPE, NOT A DISCIPLINE (R-11, ruled one day earlier): NEW `ReadResult<T> = {ok:true;value:T} | {ok:false;error:ReadFailure}`** — `.value` is unreachable without handling the failure, so `tsc` refuses the careless caller. ✅ **`isConnectivityError` MOVED, not copied, to `utils/supabaseError` — one predicate, two consumers (§6 r8).** 🔴 ***"Check the tag" can no longer appear on a connectivity failure, on any of the three screens.*** **BUILDER-COMPLETE + DEPLOYED `3d0944b`** · NO schema, NO migration, NO policy, NO new permission string, NO cap · `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · **28/28 test files · 1068 assertions** · api/ **12/12** · 🔴 **GATE 0 PROVEN FROM THE DEPLOYED BUNDLE** · all `[TRACE:*]` ON.

**Type:** Build + close-out. Code: `packages/shared/src/utils/readResult.ts` (**NEW**) · `utils/supabaseError.ts` (**NEW**) · `inventory/stockLineRead.test.ts` (**NEW, 18 probes**) · `stockLineResolver.ts` · `inventory/index.ts` · `sync/syncEngine.ts` · `usePlant.ts` · `PlantProfile.tsx` · `ScanOrder.tsx` · `InventoryCount.tsx`. Docs: ledger **#206** · `RULINGS.md` (**R-11 RULED-IN-SHAPE → PARTIAL, first instance recorded, the 30-site build still OWED**) · `built-inventory.md` (**2 net-new entries, 1 amended**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · owner-tests (**card 2 rewritten, cards 8/9/10 added**) · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 (12) #203), 1 written — entries-in == entries-out, §3 back to 3** (archive 236 → 237). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (184 lines incl. the whole OWED table).**

**🔴 THE RULING'S CENTRAL CLAIM IS NOW MEASURED, NOT ARGUED — AND THAT IS THE MOST TRANSFERABLE THING HERE.** R-11 says a returnable `{data,error}` *can* be ignored and *will* be, citing `readPricingConfig` — correct shape, seven callers, every one throwing the error away. **Flipping the two resolver signatures produced 36 NET-NEW `tsc` ERRORS across ALL 5 call sites in 3 files** (`usePlant:146` · `ScanOrder:277` · `ScanOrder:311` · `InventoryCount:290` · `InventoryCount:630`) — **the compiler enumerated every careless caller with no grep, no list and no reviewer.** The cheaper option available was a fourth `kind:'unavailable'` member on the existing union; **it would have produced ZERO errors and every caller could have gone on ignoring it**, which is precisely the failure R-11 exists to end. ✏️ **One deviation from the ruling's literal spelling, declared at the site rather than silent:** the failure branch carries `ReadFailure {kind:'offline'|'error'; message}` instead of `Error`, because the dead-zone-vs-refusal distinction is the only thing a consumer can act on — **telling someone to go find signal for an RLS refusal is a second lie.** The ruled property is unchanged.

**FLAGGED FOR DAVID:** **(a)** 🔴 **ONE CHANGE IS LOAD-BEARING BEYOND COPY AND IT IS THE ONE I WOULD RE-CHECK FIRST: `InventoryCount:630`, resolve-before-create, now REFUSES on a failed read.** It previously fell through to the *genuine NEW variety* branch — so **an unreadable catalog looked identical to an empty one and the answer was CREATE IT**, the D-49/#135 duplicate-variety family arriving through a wire failure instead of a spelling. ⚠️ **The offline refusal 120 lines below does NOT cover this**: a NON-connectivity failure (an RLS refusal, a bad column) leaves `isOnline()` **true**, so the create would have proceeded and split the variety. **(b)** ✅ **THE COPY IS ENFORCED BY SEPARATING THE STATES, NOT BY REWORDING ONE STRING** — checkout gets its own `'unreachable'` phase beside `'unknown'`; the count sheet swaps heading and lead on a flag **and KEEPS TYPED ENTRY OPEN** (Skip & flag still records through the durable queue, so a dead-zone walk is still recordable); the QR profile gets a `readFailure` branch **above** its not-found branch. **The old strings SURVIVE and must** — they are true on a real miss, which is what card 9 exists to prove. **(c)** 🔴 **CARD 9 IS THE ONE NOT TO SKIP: a fix that turned every miss into "network error" would be the same defect pointed the other way, and just as wrong in the lot.** H3/H7 pin it in code; card 9 pins it on the phone. **(d)** ⚠️ **STEP 3 — THE 24-HOUR QR READ CACHE IS DECLARED, NOT REMOVED**, and it is the QR page's only offline capability. **Its cost is unfixed and named: on a cache HIT the page renders SILENTLY STALE, up to 24h.** `usePlant` now reports `readFailure`, which is HALF of a staleness signal; **the other half is a PlantProfile render change with a real design question in it (a customer-facing staleness banner is a different decision from an operator-facing one) and I did not invent an answer.** Card 10 opens by telling the tester to clear storage first, or the cache answers and the card proves nothing. **(e)** 🔴 **GATE 0 WAS PROVEN FROM THE BUNDLE IN ITS STRONGEST FORM, AND IT CAUGHT A HOST PROBLEM WORTH KEEPING: `cultivar-os.app` (the APEX, the domain CLAUDE.md calls the live production app) serves a GoDaddy-style PARKING LANDER on `/assets/*`** — a 114-byte redirect, not a bundle. **A single read against it would have proven nothing while looking like a pass.** `cultivar-os.vercel.app` carries `"3d0944b"`, the five new strings, **and** the three old strings. **(f)** ⚠️ **R-11's 30-SITE BUILD IS UNTOUCHED AND STILL OWED — `readPricingConfig`'s SEVEN CALLERS INCLUDED**, which is the ruling's own headline evidence and was deliberately out of this build's scope. **(g)** **NAMED, NOT FIXED (STEP 5):** the raw `Load failed` on checkout submit · the two silent no-ops (pricing config, route anchor) · the delivery-route raw error · the cart's non-survival of a reload (**card 4 LEFT FAILING on purpose — a passing card over an unbuilt fix is what OP-14 clause 3 forbids**) · `store.ts:57`'s swallowed quota failure · the poisoned-op escape hatch. **(h)** ✅ **`git log`/`git status` re-run at write-up.** **(i)** **CLAUDE.md is 584 lines, under its ~600 budget.**

### 2026-08-23 (14) — THUNDER **DOCS: NINE RULINGS RECORDED + ONE STORY CORRECTION — AND THE CORRECTION IS THE MOST VALUABLE ITEM, BECAUSE THE SPEC HAD ALREADY ANSWERED LAUREN'S QUESTION TWICE.** Nine decisions David made in conversation on 2026-08-23, none of which existed anywhere in the repo, are filed as RULED rows **R-8…R-16** — not re-argued, not improved, not defaulted in code: **a trial term is a TERM, not a meter** (day 12 of 14 with the module off still leaves two days; a month turned off RUNS OUT; **no partial refund — *"that's bad math"***) · 🔴 **a confidential column is enforced IN THE DATABASE, not by a linter** · **tile capability is DECLARED `phone|desktop|either`** · 🔴 **read honesty is a TYPE — a discriminated union, not a discipline** · **a write must PROVE it wrote** · **Juan logs his own PMI** · **the manager MAY see maintenance cost** · **the free-form discount already exists and is settled** · **three items DEFERRED BY THE OWNER, with a date on them.** ✅ **FIVE OWED QUESTIONS CLOSED WITH THEIR RULINGS NAMED — M-A (R-8) · C-B (R-9) · C-D (R-14) · F-B (R-15) · R-A** — and **TWO opened rather than folded in (N-A, N-B).** **DOCS ONLY** — no app code, no schema, no migration, no cap; **zero diff under `packages/`/`api/`/`supabase/`**; `stories.html` untouched. `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · 27/27 test files · 1050 assertions · api/ **12/12** · **GATE 0 NOT APPLICABLE — nothing ships, so there is no bundle to prove** · all `[TRACE:*]` ON.

**Type:** Docs + close-out. Docs: ledger **#205** · `RULINGS.md` (**9 RULED + 2 NEW OWED; M-A/C-B/C-D/F-B/R-A flipped ANSWERED, F-A flipped DEFERRED-BY-OWNER, E5 and the standing DRAFT RULE flipped RULED-IN-SHAPE**) · `user_stories.md` (**ONE appended DESIGN CALL paragraph — no tag, no STATUS, no existing prose touched**) · `tech-debt-log.md` (**#93 NEW**) · ⚡ ACTIVE STATUS · `built-inventory.md` (**ZERO net-new entries — nothing was built; one existing entry amended**) · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 (11) #202), 1 written — entries-in == entries-out, §3 back to 3** (archive 237 → 238). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (173 lines incl. the whole OWED table).**

**🔴 THE STORY CORRECTION, AND WHY IT IS WORTH MORE THAN THE NINE ROWS: `user_stories.md`'s Billy Bob spec CONTAINS BOTH TALLY CLAUSES AND BOTH ARE CORRECT.** *"Running tallies ('4, 6, 8, 12, 16' → 16), self-correction, last value wins"* and *"location-spanning tallies (same variety here + in the greenhouse, SUMMED)"* sit in the same paragraph. **They are DIFFERENT ACTS: a running tally is one person counting up IN ONE SPOT — each number restates the total, so the last one IS the total; a location-spanning tally is the same variety found in TWO PLACES, and it adds.** 🔴 **Lauren's walk (3, then 6 more, then 5 more) is the SECOND act at fine grain — same row, different clumps — which is why the code's running-tally reading gets it wrong.** **The app CANNOT know which act she is performing** (both look identical at the keypad: a second number typed against a lot already counted) — **so it must ASK, and the conflict sheet recon #198 found ALREADY ASKS. What it is missing is a third option: ADD.** It offers REPLACE (the running-tally reading) and KEEP (abandon the new number), and has no way to express the one act the spec explicitly names. ✅ **R-A is answered with it: ONE ledger row at session close carrying whatever she chose — the three passes are session detail in `inventory_counts`, not three ledger events.** ⚠️ **Riding with that build, filed as tech-debt #93 rather than fixed: the sheet's *"first count"* label goes FALSE after the first recount** — `prevQty` reads `sessionCounts[key]` and `commitCount` overwrites it, so her third pass reads *"first count 6"* when 6 was the second and the real first (3) is not on screen. §6 r18's class, in the one sheet whose whole job is helping a human choose between two numbers.

**FLAGGED FOR DAVID:** **(a)** ⚠️ **TWO RECEIVED PREMISES WERE CHECKED RATHER THAN COPIED, AND ONE WAS WRONG. `labor_resource_wages` IS A CHILD TABLE, NOT A VIEW** (`20260621_financial_wall_phase2.sql:48-83`) — the ruling's substance is untouched, but the wrong precedent would have been copied into the #81 build, and **the child-table split has the stronger property: the confidential value is not merely unselected, it is NOT PRESENT in the parent row** (the parent's copies were cleared to NULL in the same migration). A view omitting the column is a legitimate alternative implementation of the same ruling; it is not what this repo did. **The PostgREST claim HOLDS and was verified, not assumed:** every policy in the corpus is written `TO authenticated` and role differentiation happens INSIDE the policy via `auth.uid()` → `business_members.permissions`, so `GRANT SELECT (cols)` genuinely cannot distinguish an owner from a manager from staff. **(b)** ✅ **C-B IS ANSWERED *NO CAP*, and the reason is now on the record so it is not re-proposed: a select-string linter is BLIND TO `select('*')` — which is exactly how C-D got in** (`PMI.tsx:225`). The cap would have been green over the defect it was commissioned to find. **(c)** 🔴 **R-13 IS RULED AND NO PERMISSION BUNDLE WAS CHANGED, DELIBERATELY — and the fact underneath it is worse than a polish gap.** `STAFF_DEFAULT_BUNDLE` is ten strings (`permissionManifest.ts:1260-1271`) and contains **no `pmi:*`** — `pmi:read`/`pmi:update` are manager strings (`:1242`) — **so Juan cannot open the PMI surface at all today.** Moving two strings into the staff floor would pre-answer **N-B** (clone MANAGER and subtract, or clone STAFF and add) in the direction hardest to reverse, and **saving the floor's own set with no tenant row MINTS AN OVERRIDE — a ONE-WAY DOOR.** That constraint is now written where you will meet it, not rediscovered on Wednesday. **(d)** ⚠️ **R-14 CLOSES C-D AS RULED, AND LEAVES ONE DOC FIX NAMED-NOT-MADE:** `PMI.tsx:70-80`'s header asserts *"a manager can legitimately hold pmi:read and NOT costs:read"* and describes a redaction — **TRUE of the asset list, FALSE of the service log 145 lines below it in the same file.** A header claiming a protection it does not provide is the class that has cost this repo repeatedly; the correction is one paragraph saying the cost column is deliberately visible per your ruling. **(e)** 🔴 **R-11 AND R-12 ARE RULED IN SHAPE WITH THEIR BUILDS OWED AND UNSCOPED, and R-11's evidence is the part that should size it: `readPricingConfig` ALREADY returns `{data, error}` correctly and ALL SEVEN of its callers throw the error away.** That is why the answer is a type and not a helper — a shape that *can* be ignored *will* be. **30 sites CONFIRMED is a FLOOR, ceiling 91**, and the 7 auth reads are explicitly not covered. `tsc` is already in the gate, so the ruling costs no new tooling. **(f)** ⚠️ **R-8 CHANGES NO MECHANISM AND THAT IS THE HALF WORTH SAYING:** `trial_days` is snapshotted at seed and `term_rewrite_refused` (`20260801c:281`) proves the term travels with the row. **What is owed is a SENTENCE on the card** — a countdown that keeps running while a module is off is honest under your ruling and looks exactly like a bug without it. **(g)** ✅ **NO DEFAULT WAS PICKED IN CODE ANYWHERE THIS PASS** — R-10's tile-capability default, the UNDER-PRODUCTION word, and Juan's role shape are all left open on purpose, because answering a ruling with a constant is the failure this file exists to prevent. **(h)** ✅ **`git log`/`git status` re-run at write-up — HEAD `bdc35a9`, tree carrying only docs; `git diff --stat -- packages/ api/ supabase/` EMPTY.** **(i)** **CLAUDE.md is under its ~600 budget and the §4 residual OP-13 names is unchanged.**

### 2026-08-23 (13) — THUNDER **RECON: THE OFFLINE STORY. STORE-AND-FORWARD IS REAL, IT IS LOCATED, IT COVERS EXACTLY TWO SURFACES — AND THE READ IN FRONT OF THE ONE THAT MATTERS DEFEATS IT.** David's airplane-mode proof was testing something real, and it is now RECORDED rather than remembered: `packages/shared/src/sync/` (4 files, 418 lines) is a genuine offline outbox — always-enqueue-then-drain (`syncEngine.ts:143-155`), FIFO replay, idempotent (`insert` PK = `clientId`, so a replay's `23505` reads as applied, `:195`), reconnect on the `online` event (`:96`) **plus** a drain of leftovers at `start()` (`:97`) **plus** a manual **Sync now**. 🔴 **CONSUMERS: `InventoryCount.tsx:194` AND `AssetCapture.tsx:62`. THAT IS THE WHOLE LIST** — corroborated independently, because those are also the ONLY two files in the app containing any `navigator.onLine` / offline UI. **Checkout end-to-end, the QR profile, the delivery route and the desk reconcile have NO QUEUE. No service worker and no manifest exist anywhere**, so the shell cannot load offline and the capability lives inside an already-open session. **RECON ONLY. NOTHING BUILT** — no app code, no schema, no migration, no policy, no cap. ONE document: `docs/audits/offline-store-and-forward-recon-2026-08-23.md` (904 lines). `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · 27/27 test files · 1050 assertions · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · GATE 0 **NOT APPLICABLE** (nothing ships) · all `[TRACE:*]` ON.

**Type:** Recon + estimate + close-out. Docs: ledger **#204** · ⚡ ACTIVE STATUS · `built-inventory.md` (**ONE net-new body entry — the sync slice had NO line here after two months**) · DECISIONS-INDEX drift · **`inventory-full-surface-test.md` — the single `needs-test` offline stub replaced by SEVEN cards** (board 95 → 101; 32 covered / 58 owed / 10 needs-test) · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 (10) #201), 1 written — entries-in == entries-out, §3 back to 3** (archive 236 → 237). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (173 lines, five chunks incl. the whole OWED table).**

**🔴 THE DEFECT IS OUTSIDE THE TWELVE QUESTIONS, IT IS AT THE TOP OF THE DOCUMENT, AND IT IS REPORTED UNFIXED PER THE SCOPE BAR.** `stockLineResolver.ts` **discards the Supabase `error` at all three of its reads** (`:223`, `:236`, `:266`) and substitutes `(rows ?? [])` — so a dead zone hands `resolveAgainstCatalog` an EMPTY CATALOG and it returns **`{ kind:'miss', reason:'no_match' }`, the identical value a genuinely-absent tag returns. There is no third state.** Lauren scans a real tree and gets a modal reading *"Didn't recognize this — Scanned SCV-0031 — it didn't match a stock line. **Check the tag.**"* **A network failure rendering as a confident, false, ACTIONABLE domain answer** — worse than a silent failure, because it sends her to do work that cannot help. ✅ **And it is a live instance of a rule already sitting OWED in `RULINGS.md`, written before the code that would need it:** *a read whose error path returns a value must keep FAILED distinguishable from EMPTY* — A9 on the read side. That draft rule now has a named, reproducible instance on the busiest surface in the product. 🔴 **THE SECOND-ORDER CONSEQUENCE IS THE ONE THAT MATTERS: it lands in front of the surface the queue DOES cover.** `InventoryCount.tsx:290` uses the same resolver, so offline every scan misses → typed entry → `create` → **correctly refused offline** (`:507-513`, an honest refusal for the wrong reason). **The write half is sound; the read half in front of it stops the loop** — which is almost certainly why the proving sequence resolved the item BEFORE going offline.

**FLAGGED FOR DAVID:** **(a)** 🔴 **Q3 — THE QUEUE IS IN `localStorage`, AND CAPACITY IS NOT THE PROBLEM; DURABILITY IS.** Key `trace:sync:<biz>:<domain>:queue`; a second, **deliberate** IndexedDB store holds asset bytes (`assetBlobStore.ts:4-11` says why in its own header). **MEASURED, not estimated: ~1.5 KB per counted item → ~1,600–3,300 items before quota** — far beyond a day's walk. **But `store.ts:57` SWALLOWS a quota/disabled-storage failure while `submit()` still returns `queued` and the banner still says *"counts are saved on this phone"*** — a save that reports success and did not happen. **And a Safari PRIVATE tab clears the store with the tab** — the same storage you are chasing for the logout loop, stated as an inference from the storage model rather than as an observed instance, and put on card 5 to be tested on the same device. **(b)** 🔴 **Q11 — 14 PRICE-AFFECTING CONTROLS, AND TODAY EVERY ONE COSTS ZERO** (`computeOrderPricing` runs in `CartReview`'s **render body**, `:200`, over a config fetched once on mount). **13 become genuinely new round trips under R-5; exactly 1 rides an existing call.** A four-item order goes from 4 requests to ~11, on the worst signal in the business. 🔴 **THE QUEUE DOES NOT ABSORB THEM — three independent reasons: checkout is not a consumer, the queue holds WRITES and a price resolution is a READ whose value never returns to a component, and FIFO is the wrong shape for a latest-wins question.** ⚠️ The *"estimated, confirming"* UI is still buildable — **from the retail `sell_price` the client already holds and which R-5 does not remove** — but that is an optimistic-display build, not the outbox, and I have not reported it as the queue doing it. ✏️ **A stronger input to R-5's cost-benefit than the round-trip count: the failure mode is not "slow", it is "no total renders at all"** — and per Q4 the cart does not survive a reload, so the LAWNS worst case is a sale that can neither be priced nor recovered. **(c)** ✅ **Q12 — NO NEW `api/` FUNCTION. `submit.ts:180-192` is ALREADY an `action` dispatcher whose own comment cites §6 r11 by name**, and it already imports and runs `computeOrderPricing`/`resolveTier`/`resolveTaxRate` server-authoritatively at `:605`. `action:'price'` is a fifth branch on the file that already holds the machinery. **R-5 is shippable at 12/12; the ceiling is not the blocker.** **(d)** 🔴 **3a — ADJACENT, NOT THE SAME, AND OPPOSED WHERE IT COUNTS.** `blind_capture_mode` has **zero hits** anywhere (R-B confirmed independently). Shared: durable idempotent deferral, proven on this table and RPC, with an envelope already carrying what blind capture needs. Opposed: **the drain has NO HUMAN IN IT**, so on reconnect everything applies automatically and the desk still opens to *"agrees — done"* — **it reproduces #67's *residual is 0 by construction* a few minutes later rather than fixing it.** Scope them as one build anyway, because blind mode is a change to what `InventoryCount.tsx:438` queues and #68/**R-C** hangs off the same line — and ⚠️ **R-C's hazard would be CREATED by that build: `inventory_count_sessions` has exactly ONE writer today** (three sites, all in `InventoryCount.tsx`; `InventoryReconcile.tsx` never touches it — verified by grep). **(e)** 🔴 **Q10 — A HARD REJECT AT QUEUE POSITION 2 BLOCKS POSITION 3 PERMANENTLY.** `drain()` breaks on both retry (`:176`) and reject (`:180`) to preserve FIFO — correct for a transient failure, head-of-line blocking for a permanent one. **`forget()` has TWO call sites, both the missing-table case, and NO UI can clear a stuck op**; the badge reads *"N waiting to sync"* forever. ⚠️ **And the drain's failures are console-only** — all three `syncNow()` invocations discard the `DrainResult` — **on `DEVICE: phone`, where OP-14 says a console is not available.** ⚠️ Also: `clientTs` rides the envelope described as *"reconcile ordering for SETs"* and **has no reader in `sync/`**. **(f)** ✅ **Q9 — #57's ACCUMULATION CLASS CANNOT RECUR HERE, structurally: `count_reconcile_inventory` computes `delta = counted − current` under a `FOR UPDATE` lock at APPLY time**, so a replayed count lands absolutely. The one imperfection (a lost response → a second zero-delta ledger row) was **already documented at `types.ts:34-39`**, not discovered by me. **(g)** ✅ **Q6 — AN UNDOCUMENTED CAPABILITY FOUND: `usePlant.ts:41-62` has a `localStorage` read cache, 24h TTL, seeded into initial state BEFORE any fetch** — so a tag scanned in the last 24h renders offline. It is in no inventory doc, no board, no handoff. **On a cache hit it is silently stale; on a miss it shows a raw `TypeError: Failed to fetch`** on a customer-facing page. **(h)** ⚠️ **ONE CLAIM CLASSIFIED, AS ASKED: ledger #136's *"four surfaces ship no test incl. … OFFLINE SYNC"* is about TEST COVERAGE and it is ACCURATE** — the board carried exactly one card, `needs-test`, `LAST-PROVEN: never`, body reading *"NO TEST WRITTEN."* **It says nothing about whether the mechanism exists, and the mechanism exists.** Those two facts were being read as one, and that conflation is what cost tonight's re-establishment. **(i)** **THE FLOOR (STEP 4): ~2 prompt units, 0 migrations, 0 rulings, no new function, no cap** — Floor A a third `unavailable` resolution state + the two discarded `readPricingConfig` errors; Floor B connectivity-shaped error copy at the two unqueued write points **reusing `isConnectivityError`, which already exists at `syncEngine.ts:35`** (§6 r8 — a second copy is the drift the rule exists to catch). **NOT in the floor and named so: cart persistence, the poisoned-op escape hatch (needs a ruling, same shape as T-E/`campaigns:delete`), moving the outbox to IndexedDB, and any part of R-5.** **(j)** **OP-14: 7 cards written, `covered` set on NONE. Card 1 is `owed` — your airplane-mode run. Cards 2 and 4 are EXPECTED TO FAIL and are written that way deliberately**, so the hole is recorded rather than rediscovered a third time. **(k)** ✅ **`git log`/`git status` re-run at write-up — HEAD still `5bcdbf4`, tree carrying only close-out docs and the new recon; `git diff --stat -- packages/ api/ supabase/` EMPTY.** **(l)** **CLAUDE.md is under its ~600 budget and the §4 residual OP-13 names is unchanged.**
