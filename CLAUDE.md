...
# Last updated: 2026-09-03 — #260 the file harness + the eight shapes + the first-look report (renumbered from #257 at the merge; main's #257 is the receipt detail view) — see §3
...
### 2026-09-02 — THUNDER **THE REHEARSAL DOOR, THE EIGHT SHAPES, AND A REPORT THAT ASKS FOR NOTHING. #260. R-66…R-67.** 🔴 **THE HEADLINE IS THAT SIX MUTANTS SURVIVED A GREEN SUITE AND EVERY ONE WAS A REAL HOLE WHOSE FIXTURE SIMPLY COULD NOT TELL THE TWO ANSWERS APART.** Four were in the findings suite: with one measured money finding in a tier, "ordered by money" and "ordered by the order somebody typed" produce the SAME list; with nothing else in the tier, a `null` and a `0` TIE. The assertions were not wrong — they were aimed NEAR the property and never at it, so the mutant and the truth agreed. The new probes are built specifically so the two **disagree**. ✏️ **That is [[R-33]] and the #248 seam arriving a third time, and it is now the thing I check first rather than last.**

**Type:** Build, **READ-ONLY**. **BUILDER-COMPLETE** on **`thunder/qbo-review-test-mode`** (adopted at `da68eb4`), **NOT MERGED, PUSHED** · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 244 / 10 / 12 / 15) · **65/65 files · 3279 assertions** (+116) · `build:cultivar` exit 0 · **api/ 12/12 confirmed FROM THE FILESYSTEM, nothing minted** · **NO migration · NO new permission · NOT ONE WRITE.** 🔴 **67/67 real mutants caught across SIX harnesses, 1 PROVEN equivalent, 0 survivors** — each against a control asserted GREEN first, keyed off the EXIT CODE. Code: **NEW `captureReplay.ts` · `captureProjection.ts` · `booksReport.ts`** (all pure) · `booksFindings.ts` (+4 rules, shape/value/recommendation, re-sort) · `invoiceList.ts` · `BooksReview.tsx` · `QboBooksReader.tsx` · **3 NEW mutation harnesses**. Docs: ledger **#260** · **[[R-66]]…[[R-69]]** · owner-test cards **16–20** (board now 20, 0 covered) · built-inventory · ⚡ ACTIVE STATUS · DECISIONS-INDEX · the story · this §3. **§3 RETENTION: 1 archived verbatim (2026-09-01, the six-write-only-columns entry), 1 written — entries-in == entries-out, §3 back to 3** (archive **281 → 282**).

**🔴 THE FILE HARNESS IS THE KEYSTONE, AND THE REASON IS THE TEST DESIGN RATHER THAN THE FEATURE.** David cannot be the first to run the import against LAWNS — if he goes first there is nothing left for Lauren to show him, and **her** running it is the actual test ([[R-69]]). So he rehearses on Test Dave's loading LAWNS's own saved file. **A file replaces a CONNECTION, not a CODE PATH:** the replay hands back verbatim row bodies and the same parsers run downstream, and the projection reproduces the live payload **per entity, asymmetry included** — Item ships rows, Customer ships five, Invoice ships none. Making those uniform would be a tidier function and a worse one; it would render a screen that does not exist in production, and the rehearsal would be a rehearsal of something else. **And the file door is gated STRICTLY HARDER than the live door** — the live read counts its own pages and may trust its arithmetic; a file came off a disk, so `complete:true` is never read as an answer, `expected_total` is checked against the **count page's own body**, and `retrieved_total` against **the rows actually present**. ⚠️ **The count page is page zero and is NOT a page of rows** — parsing it as rows yields zero, so including it would break nothing and quietly make every re-count one page short. Split off by its DERIVED query string, never by position.

**🔴 THE ANALYSIS SHIPS SHAPES NOW, WHICH IS THE DIFFERENCE BETWEEN CUSTOMER TWO GETTING A WORSE REPORT AND A BETTER ONE ([[R-67]]).** Eight shapes, every rule declares one, all eight covered, and a probe greps every rule's STATIC text for vertical vocabulary — generated sentences excluded deliberately, since a sentence built from her own catalogue legitimately contains her words. Two shapes had no rule and now do: **`discount-in-wording`**, the largest money finding there is and one about MEASURABILITY rather than loss — the owner cannot answer *what does discounting cost me* because the discount was typed into a line's wording — and **`markup-formula-not-achieved`**, the catalogue's rule against the sales' rule. ⚠️ **A catalogue with NO consistent markup has not broken a formula**, so below 90% consistency it reports not-measured rather than inventing a rule nobody adopted and then reporting they broke it. And **`overdue-receivables` exists precisely because it CANNOT run** — a silence there would read as a clean bill of health, so it names the two fields that would answer it.

**⚠️ ONE PROMPT INSTRUCTION WAS CHECKED AGAINST THE REPO AND THE REPO WON.** The prompt's header says *"Claim ids at file time. Never leave a gap."* Clause (6) of the numbering rule — landed on `main` in `63ac35b`, **after** that prompt was written — says an unminted ruling carries **no id at all**, and the standing OWED row about pushing correctly claims none. **So `R-64` was FREE, and the `R-64` string already in `RULINGS.md` is that row declining to claim it.** Verified before use rather than assumed, which also caught that `#437`/`#438` in the ledger are QuickBooks invoice numbers, not row ids. ✏️ **A prompt is a claim like any other** — [[R-26]].

**FLAGGED FOR DAVID:** **(a)** 🔴 **THREE OF THE FIVE SCOPED ITEMS ARE NOT BUILT AND ARE NAMED RATHER THAN IMPLIED: RETIRE-AND-REPLACE (§9), NORMALISATION WITH CONSENT (§4), AND THE RE-RUN-IS-THE-DELTA PROOF (acceptance 15).** ✏️ **I first wrote *two* here and it was wrong — the delta proof was the one I dropped, and I caught it only by re-counting the list against the prompt rather than against my own memory of it.** The first two need database work this pass did not take: a retire/replace over 447 live inventory rows, and the two `audit_log` rows plus a **mutable** home for the standard, since an immutable event log cannot express *this is still current*. **The third is not buildable here at all — it proves an IMPORT is idempotent, and this build imports nothing; it is named so the next session does not inherit it as done.** **The report is honest about the second meanwhile: it states it reflects "no corrections" rather than going quiet, because an absent line reads as "none were needed".** **(b)** 🔴 **THE CARD-PROCESSING RECOMMENDATION CANNOT BE BUILT FROM THESE THREE READS** — it needs payment-method fee data Item/Customer/Invoice do not carry. Named as a gap rather than authored, which is [[R-68]] applied to itself; the four-part class, one computed instance and the renderer all ship. **(c)** 🔴 **CARD 17 IS THE SHARPEST ON THE BOARD** — delete one page from a saved file and confirm it is REFUSED BY NAME, with both numbers. A truncated download looks exactly like that, and a review computed over 61% of a business's invoices is confidently wrong about a real company. **CARD 18 step 3:** a `$0 at stake` is a FAIL, not a near-miss. **CARD 20** proves the whole surface wrote nothing, **read in QuickBooks, never in our logs.** **(d)** ✅ **§8's two-minute check re-measured independently and it is the good answer: BOTH unique indexes are `(business_id, qb_invoice_id)`, tenant-scoped, NOT global.** Test Dave's import cannot collide with LAWNS's. **(e)** ⚠️ **The engine's original within-tier sort rule was SUPERSEDED, not overwritten** — it forbade *worst-first*, which is a different axis from *worth-to-her*; the reasoning is preserved in the code beside the change so nobody re-derives it as a defect. **(f)** ⚠️ **NOT MERGED, and other worktrees are live** — re-read `CLOSE-OUT-LEDGER.md`, `RULINGS.md` and the §3 count at merge; my numbers are **#260** and **R-66…R-67**. **(g)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs are bannered, not re-audited · `docs/reference/` tracked by accident · tech-debt **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#147**.
