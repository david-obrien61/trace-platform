...
# Last updated: 2026-08-31 — #244 calendar: the day was below the fold and the window could not move — see §3
...
### 2026-08-31 (4) — THUNDER **CALENDAR: THE DAY WAS BURIED AND THE WINDOW WAS A CAGE — BOTH DISPLAY, AND NEITHER ONE WAS THE DAY VIEW. #244.** 🔴 **THE HEADLINE IS THAT THE REPORTED SYMPTOM AND THE ACTUAL DEFECT WERE DIFFERENT THINGS, AND BELIEVING THE SYMPTOM WOULD HAVE REBUILT A WORKING SCREEN.** The report was *"the day view is too small"* — David clicked a day, saw a truncated `Josh Ph…` in a cell, and concluded the drill-in was undersized. **It renders the stop, the customer, the full address, the phone, Edit customer and Route this day, and CARD 9 had already audited every affordance on it.** It was mounted **at the very bottom of the page, AFTER the "What this calendar shows" footnote**, under four weeks of grid — so a click outlined a cell and appeared to do nothing, and the ellipsis in that cell read as the entire answer. ✏️ **So the fix is scroll position, ordering and what a cell prints, and NO new day screen was built** — which is also what keeps §3-of-the-prompt's coming split (*today* and *the calendar* as two screens with two jobs) cheap instead of expensive. 🔴 **AND THE SECOND DEFECT WAS HIDING REAL WORK WHILE COUNTING IT.** The grid was fixed at *"this week and the three ahead"* with no way forward past Sep 26 and **no way back at all**, and the drill-in states the cost in its own words — *"1 stop on this day · 9 SCHEDULED IN TOTAL."* **Nine deliveries, one reachable.** Saturday 2026-08-29 — seven stops, six made, one rescheduled, the day David spent the week reconstructing — was **one day** outside it. **BUILDER-COMPLETE** `044183a`→`c435620` on `thunder/calendar-window` · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **52/52 files · 2463 assertions** (+44) · **8/8 MUTANTS CAUGHT, MEASURED — M1 is the defect itself ("the window cannot move") and eight assertions fail on it** · **ZERO schema · ZERO migration · ZERO policy · no permission string** · api/ **12/12, none added** · all `[TRACE:CALENDAR]` ON.

**Type:** Fix + close-out. Code: `packages/cultivar-os/src/lib/operationsCalendar.ts` (**`offsetWeeks` · `WINDOW_STEP_WEEKS` · `weekLabel` · `windowHeading` · `cellSummary`**; `CalendarWeek` gains `relativeIndex`) · `operationsCalendar.test.ts` (**+44 probes**, §G–§I) · `pages/OperationsCalendar.tsx` (the day section moved under the grid + `dayRef` scroll-into-view · the window control · the cell). **ZERO other files touched.** Docs: ledger **#244** · tech-debt **#135** · owner-tests **CARDS 12–15 added, CARDS 1 and 9 REWRITTEN; board 15 cards · 0 covered · 14 owed · 1 needs-test** · `built-inventory.md` · `user_stories.md` (PIECES + the window clause + the landing split, filed) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift + **its `Last updated:` header corrected to a pointer, four sessions stale** · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-31, the #240 positions entry), 1 written — entries-in == entries-out, §3 back to 3** (archive **270 → 271**, counted with `grep -c '^### '`). ✅ **Own worktree** — `thunder/calendar-window`, off `main` rather than off the in-flight `thunder/workbook-reconcile`, with five other worktrees live.

**🔴 THE FINDING WORTH MORE THAN THE FIX IS THAT TWO OWNER-TEST CARDS HAD DECAYED WITH NO CODE CHANGE AT ALL.** CARDS 1 and 9 were written 2026-08-28 and asked the reader to look at **Saturday 08-29** — seven stops, and the header count of 9. The home window rolled past that date on **08-30**. So for two days both cards had been **impossible to run as written**, they still read as live instructions, and nothing anywhere watched them. ✏️ **This is [[R-26]]'s shape carrying [[R-29]]'s half-life: a card naming an absolute DATE is a claim with an expiry, exactly like a line number, and it decays on its own without anyone touching the code.** Rewritten **relative to the window**, with the absolute dates that remain marked by what makes them reachable. ⚠️ **The same rot is what made the labels wrong the moment the window moved:** the week rows were labelled by their POSITION in the grid (`wi === 0 ? 'This week'`), which is correct for exactly one window and silently calls a four-weeks-ago week *"This week"* for every other. **A label anchored to the grid describes the grid; a label anchored to TODAY describes the calendar** — so `relativeIndex` and `windowHeading` are both derived from the offset in the MODEL, and §6 r18's rule that a header is a CHECKABLE CLAIM is now enforced by a unit test rather than by a reader noticing.

**🔴 AND THE SECOND, WHICH IS ABOUT WHAT A 90px CELL CAN HONESTLY SAY.** `Josh Ph…` is a truncated name and nothing else; three of them identify nobody and read as a rendering fault rather than as a busy day. **The count is strictly more information than the names it replaces** — one stop still prints its name (a single label fits, and knowing *who* is on an otherwise empty day is the whole point of the glance), more than one prints `3 stops` · `3 stops · 1 planting` · `2 stops · all planting`. ✏️ **The planting sub-count is not decoration and was the one real design decision here:** it is the exact axis the day-type flag is about to use, so a bare *"3 stops"* would flatten the distinction the amber flag underneath is making. And a mixed-KIND day falls through to **`countPhrase`, the same function the conflict text uses** (§6 r8), so the cell and the flag physically cannot disagree about how to say *"2 maintenance jobs"* the day PMI lands.

**FLAGGED FOR DAVID:** **(a)** 🔴 **CARD 14 IS THE TEST AND THE NUMBER IS THE ASSERTION: move back four weeks, open Saturday 29 August, SEVEN STOPS, all seven listed by name.** If it is any other number, say what it is before judging anything else on the page. A four-week step from home lands on **Aug 2 – Aug 29** with that Saturday as the last cell, which is why the step is a whole window and not a partial one. **(b)** 🔴 **CARD 12 IS DEFECT ① AND IT IS NOT *"the day view renders"* — it always did.** Click a day from the top of the page: the day must be **on screen without you scrolling**, and the sources footnote must now be **BELOW** it. That inversion was the whole defect. **(c)** ⚠️ **CARD 15 IS THE ONLY CARD ON THIS BOARD THAT NEEDS A PHONE, and that is deliberate** — the placement was **your** ruling, not a default (*"the desktop already has dropdown navigation and does not need arrows"*), so it needed a check that both halves got built: dropdown on desktop with **no arrows**, 48px arrows on the phone, and one press home from either. The board is otherwise still `DEVICE: desktop` and this build did not check whether the four-week grid reads well at 380px. **(d)** 🔴 **THE SCROLL IS ASSERTED BY NOTHING AND I AM SAYING SO RATHER THAN IMPLYING OTHERWISE.** It is a `useEffect` on a `ref` inside a `.tsx` component needing router, context and Supabase — **tech-debt #134 verbatim, filed one build earlier by the same class of gap.** 44 assertions were added and **not one of them can see a scroll**; CARD 12 is the only proof that exists, and the board says so under *"what this test deliberately does not cover"*. **(e)** ⚠️ **TWO THINGS FILED AND NOT TAKEN:** tech-debt **#135** — the selected day's date now prints twice ~80px apart, and **the copy I would rather remove is CARD 9's proven text**, so it waits for a pass allowed to touch the drill-in; and the second `matchMedia` hook in the codebase is **named in code as the second, not extracted** — it answers a different question from `ReceiptKeeper`'s (*"is there room for a select"* vs *"can this device take a photo"*), and reusing that one would have made a touchscreen desktop navigate by arrows. **(f)** ⚠️ **THE LEDGER NUMBER IS `#244` AND IT MAY COLLIDE — `#243` IS TAKEN BY `thunder/workbook-reconcile`, WHICH IS UNMERGED.** Per ledger #233's own lesson (*take the number AT MERGE, not at build*), re-read `CLOSE-OUT-LEDGER.md`, `tech-debt-log.md` and the §3 count at merge; **that branch will also have written a §3 entry**, so the archive arithmetic must be redone across both, exactly as 08-30 (6)/(7) had to be. **(g)** ⚠️ **STILL OPEN AND UNTOUCHED:** `docs/inventory-functions.md` / `inventory-env.md` / `inventory-ai.md` stamped `2026-06-13` · `docs/reference/` still tracked by accident (`8bbaec4`, still yours to revert) · `20260830c` written and not applied · tech-debt **#124**'s eighteen over-wide policies · **#125** · **#126** · **#127** · **#132**'s owed `authority:baseline`.

### 2026-08-31 (3) — THUNDER **THE CHOOSER'S MISSING ASSERTION — AND THE FAILING TEST WAS RUN AGAINST A BUNDLE THAT DID NOT CONTAIN THE FEATURE. #242.** 🔴 **THE HEADLINE IS THAT CARD 9 FAILED HONESTLY AND THE DEFECT IT FOUND WAS NOT IN THE CODE: `origin/main` IS `14ea7d0` AND DOES NOT CONTAIN `positionStartingPoints.ts` AT ALL.** #241 was pushed to `thunder/position-starters` and **never merged**, and a branch push deploys nothing. **Every symptom matches #240 exactly** — no chooser (the code is absent), *"About the business populated"* (it is #240's **placeholder**, which only renders when the field is EMPTY), *"Nothing ticked yet · no note…"* (#240's list line, unchanged by me). ✏️ **This is OP-15 / #60 in its purest form and it is MY failure, not the tester's:** I wrote *"THE SHA UNDER TEST IS THE MERGE OF `thunder/position-starters` TO `main`"* into GATE 0 **and then never merged it**, which is a rule filed where the actor is standing and ignored by its own author. **Recorded as NOT RUN rather than FAILED** — an observation against the wrong bundle is fiction, and filing it as a fail would send the next session hunting a defect that is not there. **BUILDER-COMPLETE** `3272110`→`6844975` · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **52/52 files · 2419 assertions** (+20, +1 file) · **10/10 MUTANTS CAUGHT, MEASURED** · ZERO schema · ZERO migration · api/ **12/12** · **NO new dependency** · all `[TRACE:POSITIONS]` ON.

**Type:** Reproduce → fix the gap → close-out. Code: **NEW `packages/cultivar-os/src/components/positions/StartingPointChooser.tsx`** · **NEW `startingPointChooser.test.ts`** (20 assertions) · `positionStartingPoints.ts` (**`shouldOfferStartingPoints()`**, a pure guard returning `{offer, reason}`) · `PositionBuilder.tsx` (calls both; the TRACE line now logs the REASON) · `Positions.tsx` (the three placeholders) · `contextProposals.ts` (*"forty varieties **of tree**"*). Docs: register **`P2` CLEARED** · tech-debt **#134** · ledger **#242** · owner-tests (**GATE 0 gains the paragraph naming what happened; CARD 9 gains the reopen path and the reason-trail; CARD 1 gains the placeholder check**) · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-30 (7), the dead invoice line), 1 written — entries-in == entries-out, §3 back to 3** (archive **269 → 270**, `grep -c '^### '`).

**🔴 THE FINDING WORTH MORE THAN EITHER DEFECT, AND DAVID NAMED IT BEFORE I DID: A CONDITION THAT CANNOT BE ASSERTED IS A CONDITION THAT WILL NOT BE.** #241 shipped **25 assertions about starting points and killed 9 of 9 mutants** — and **not one of them put the chooser on a screen.** ✏️ **That was not carelessness, and treating it as carelessness would guarantee the repeat.** Every probe tested `startingPointIds` — the SETS — **because the sets were the only part reachable.** The render guard was an inline boolean and the set-to-button mapping was JSX, both inside a page component that reads React context, calls Supabase and needs a router; `scripts/run-tests.mjs` bundles `*.test.ts` with esbuild and runs it in **node**, deliberately (§6 r10, no vitest by design). **Nothing in this repository could reach either, so nothing did.** **Diligence cannot compensate for an unreachable seam — only a seam fixes a seam.** ✅ **Two were cut:** the decision moved to a **pure function** whose `reason` distinguishes five withholding states, and the chooser became a **context-free component** that `react-dom/server` renders to HTML **inside the existing runner, with NO new dependency** — the probes read the actual markup. 🔴 **N2, the mutant that injects the exact reported symptom (*"the guard never offers"*), is caught by three assertions.** ⚠️ **And the honest residual: `renderToStaticMarkup` does not run `useEffect`, so ONE `chooser.offer &&` in JSX is still provable only by CARD 9** — a far smaller unasserted surface than the guard, the mapping, the counts and the wiring all being outside reach. **The CLASS is tech-debt #134, and its cheap next step is a COUNT of unassertable render guards, not a framework** (#174 — an unmeasured class stays unmeasured).

**🔴 AND THE SECOND, WHICH IS A9/D-9 RUNNING BACKWARDS AND I HAD NOT SEEN THAT BEFORE: *"FORTY ACRES"* IS A PLACEHOLDER, AND AN OWNER READ IT AS HIS OWN DATA.** It is #240's, `4fce9f9`, `Positions.tsx`: `placeholder="grows and sells shade trees on forty acres in Leander"` — **not a proposal, not a value, saved nowhere.** ✏️ **Its presence is diagnostic rather than incidental: a placeholder renders ONLY when the field is EMPTY**, so *"About the business is populated"* was in fact the blank-page problem the proposal exists to fix, reported as its opposite. **The literal was LAWNS-shaped, sat in LAWNS's own field, and carried a number — forty acres — that nothing measured supports (everything this week says twenty).** *Absent is not empty*, inverted: **a placeholder that reads like a value tells someone their blank field is filled.** All three now read *"For example: …"* with the tenant identity and the unsupported number gone → register **P2 CLEARED**, and my proposal now says *"forty varieties **of tree**"* so two unqualified forties cannot sit in one card. ⚠️ **My proposal never said acres** — but the distinction is worth nothing next to the fact that the card as a whole was unreadable.

**FLAGGED FOR DAVID:** **(a)** ✅ **MERGED ON YOUR INSTRUCTION AND VERIFIED LIVE THE SAME DAY — `main` IS `ce09942`.** Fast-forward, zero conflicts (`origin/main` had not moved off `14ea7d0`). 🔴 **The deploy was proven FROM THE ARTEFACT rather than from a dashboard:** `cultivar-os.app`'s content-hashed bundle was polled until it changed, then read — `/assets/index-Cu2Ia3V_.js` contains **`ce09942`**, the chooser copy, the corrected placeholder and the guard's five reason strings, and **`14ea7d0` and `"forty acres"` are both gone.** The index was re-read on every pass rather than a remembered bundle path reused — trusting a remembered artefact is the same class of mistake this gate exists to catch. **Nothing else is owed: no schema, no migration, nothing to apply.** ✅ **AND THE ONE-GLANCE CHECK IS NOW WHERE THE ACTOR STANDS: all 15 owner-test boards open with *READ THE STAMP* as their first line, above the metadata.** On the positions board it had been **step ③**, behind a `git log` and a Vercel round-trip — *the cheapest and most direct check sitting behind two slower ones that answer a different question* (**"what did I intend?"** rather than **"what am I looking at?"**). You had that stamp on every PDF you printed this week and never used it. **A check nobody reaches is not a check.** **(b)** 🔴 **GATE 0 STEP ③ BEFORE ANY RE-RUN, WITHOUT EXCEPTION — the `VersionStamp` at the foot of the screen reads `built <time> · <sha>`. If it says `14ea7d0` or `3bb36ff`, you are on #240 and nothing below CARD 8 can pass.** That stamp exists precisely for this (#60, ledger #141) and this is the first time it would have paid for itself. **(c)** ⚠️ **IF THE CHOOSER IS STILL ABSENT AFTER THE MERGE, DO NOT GUESS — THE APP NOW TELLS YOU WHY.** `[TRACE:POSITIONS] builder-loaded` carries `chooser:` with one of five reasons. 🔴 **`nothing-ticked` with no chooser on screen is the one genuine render defect no test can reach** — that is the report I need. Any other value names the state, and `read-only` on an OWNER session means the business context had not resolved. **(d)** ✅ **YOUR EXTRA CASE IS COVERED AND IT WAS A REAL HOLE:** a SAVED position left at zero ticks now offers the chooser again, asserted as **J4**. #241 had it working by accident and had never considered it separately — you named the state that most needs the offer, and it was the one nothing tested. **(e)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs stamped `2026-06-13` · `docs/reference/` tracked by accident · `20260830c` unapplied · **#132** (the `authority:baseline` blessing, owed AFTER CARD 7) · **#133** (the starting sets' membership is my derivation, not the workbook's) · **#124** · **#125** · **#126** · **#127** · **#130** · **#131**.

### 2026-08-31 (2) — THUNDER **POSITIONS: NEVER SHOW A BLANK PAGE — AND THE DEFECT WAS THE FLOW, NOT THE RENDERING. #241.** 🔴 **THE HEADLINE IS THAT THE THING DAVID REPORTED AS BROKEN WAS WORKING PERFECTLY.** He created "Production Manager", was shown 93 rows with nothing pre-selected, ticked nothing, and got a document reading *"Nothing has been ticked for this position yet · 0 responsibilities."* **That page is TRUTHFUL AND USELESS, and there is no bug in it** — every line of it is correct. **The defect is upstream: a list of 93 with nothing selected IS a blank form, and the whole capability exists because blank forms do not get filled in.** Fixing the rendering would have polished the symptom. ✅ **AND THE HARDER HALF ALREADY WORKED, WHICH IS WORTH SAYING BEFORE THE FIXES:** the week's shape rendered from `business_operating_days` — *"Sunday, Thursday, Friday and Saturday delivery/placement; Monday service/maintenance"* — **read, not asked for.** The read-don't-ask rule landed on its first outing. **BUILDER-COMPLETE** `14ea7d0`→`27dd0f8` on `thunder/position-starters` · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **51/51 files · 2399 assertions** (+25) · **9/9 MUTANTS CAUGHT, MEASURED** · **ZERO schema · ZERO migration · ZERO policy · no permission string · no role · no catalogue row changed** · api/ **12/12, none added** · **NO URL FETCHED** · all `[TRACE:POSITIONS]` ON.

**Type:** Build (four pieces) + close-out. Code: **NEW `packages/shared/src/positions/positionStartingPoints.ts`** (six sets + start blank, and one resolver) · **NEW `contextProposals.ts`** · `PositionBuilder.tsx` (the chooser, ten collapsed areas, the sticky total) · `Positions.tsx` (the proposal beside each empty field) · `PositionDescription.tsx` (the ④ empty state) · `positions.test.ts` (**+25 assertions, blocks F/G/H**). Docs: **`docs/decisions/HARDCODED-REGISTER.md` — item `P1` FILED, the first OPEN item on that register in seven weeks** · tech-debt **#132 + #133** · ledger **#241** · owner-tests **12 cards** (CARDS 9–12 new; 1/2/7 rewritten) · `built-inventory.md` · `user_stories.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-30 (6), the staff count walk), 1 written — entries-in == entries-out, §3 back to 3** (archive **268 → 269**, counted with `grep -c '^### '`). ✅ **Own worktree** — `thunder/position-starters`, with four others live.

**🔴 THE FINDING WORTH MORE THAN THE BUILD IS A CORRECTION TO TWO CANONICAL DOCS, AND IT IS R-26'S SHAPE WITH A TWIST I HAVE NOT SEEN BEFORE: TWO TRUE FACTS AND A FALSE CONCLUSION DRAWN FROM THEM IN THE SAME SENTENCE.** `built-inventory.md` and the story both said the website scrape is **NOT BUILT** *"(`AIEngine` is dark with no `api/ai/*` route behind it)"*. **Both premises are correct and both still stand.** But a website identity read depends on neither: **`runIdentity` and `runAnalysis` (`shared/src/discovery/engine.ts`) extract `businessName`, `location`, `yearsInBusiness`, `staffSize`, `servicesFound`, `certifications` and `tone` from a live site, and they are WIRED AND DEPLOYED** on the default path of `api/discovery/ingest.ts:176-179` — one of the twelve live functions, gated on `ANTHROPIC_API_KEY` with an explicit refusal when it is absent. ✏️ **R-26's thirteen instances are declarations false when written; this one is a declaration TRUE when written whose INFERENCE was false** — it named the two places a scrape isn't and concluded there is none. **The prompt's own framing was also off, in the other direction:** it said *"the crawl exists and extracts the wrong thing."* The CATALOGUE crawl does; **a separate identity extractor exists and extracts exactly the right thing.** ⚠️ **What is genuinely missing is much smaller than "its own build": the mapping from a `BusinessIdentity` to three owner-facing sentences, and somewhere to keep it** — and **`business_discovery_profiles` is NOT that place**, which is the one thing the earlier note had right: its `raw_extract` holds `{items, counts}` (`populate.ts:328`) and it is unique per `source_url`. **Measured from the repository. No URL was fetched, per the scope bar.**

**🔴 AND THE SECOND, WHICH IS ABOUT MY OWN HONESTY RATHER THAN A DOCUMENT'S: FIVE SETS LANDED EXACTLY ON DAVID'S FIVE STATED COUNTS, AND THAT IS CONSTRUCTED AGREEMENT, NOT CORROBORATION.** The prompt said the sets *"EXIST AND ARE MEASURED, not invented — from the same workbook the catalogue came from"*, and gave 34 / 27 / 9 / 8 / 10. 🔴 **The workbook is not in this repository** — nothing under `docs/discovery/`, `docs/user-stories/` or anywhere else holds a per-role responsibility list. So the **counts are David's measurement and the MEMBERSHIP is mine**, derived from the catalogue and **tuned until each set hit its stated number**. ✏️ **Writing *"measured, not invented"* over that would have been R-26 instance fourteen, filed by the person who has been counting them** — and the matching numbers are the most persuasive-looking part, which is exactly what makes them dangerous. The file header says what actually happened, `F11` pins the counts against silent drift and its own comment says it is not evidence of membership, and it is tech-debt **#133**. ⚠️ **It shipped rather than blocking because the exposure is bounded and stated: a starting point grants nothing, shows the owner every row it ticked, and is adjustable in one click.** Unlike a permission, being wrong here costs a tick.

**🔴 AND THE THIRD, WHICH IS THE DESIGN DECISION I WOULD MOST LIKE OVERRULED IF IT IS WRONG: A NON-OWNER STARTING SET NEVER *SUGGESTS* AN UNDELEGABLE ROW, WHILE A `capability:'none'` ROW IS SUGGESTED FREELY.** Both are rows the software cannot fully carry, and R-30 says a `none` row is TICKABLE because *the description says what the JOB is, not what the app covers* — so treating them differently needed a reason rather than a preference. **The reason is who is blocked:** `none` means **nobody** is blocked, the software simply cannot help; **undelegable** means the platform will actively **refuse THIS person** — `CATALOG_PERMISSIONS` filters `owner-only` out of the grantable catalog entirely, so the string cannot be granted whatever anyone ticks. **A starting point is a suggestion the PLATFORM makes, and suggesting work the platform then refuses to delegate is the platform contradicting itself.** The owner may still tick one by hand and the mark explains why — **only the SUGGESTION is held to the stricter bar.** The line is derived from `marksFor`, never hand-drawn, and `F3`/`F12` assert it in both directions (F12 proves the subject is non-empty — the OWNER set carries all eleven).

**FLAGGED FOR DAVID:** **(a)** ⚠️ **NO BLOCKER AND NOTHING TO APPLY — this build touches no schema, so no migration is waiting on you and no catalog verification is owed.** **(b)** 🔴 **CARD 9 IS THE CARD THIS BUILD EXISTS FOR, AND ITS SECOND HALF IS THE ONE PEOPLE SKIP:** apply a set, then **change something** — untick two, tick one it missed, Save, reload. Your edit must survive; a set that reasserted itself would be a template, not a starting point. **And the negative:** go back into a position that HAS ticks — **the chooser must be GONE**, because applying a set REPLACES and offering that beside real work is one tap from destroying it. **Then type `Yard Manager` on a new position: NOTHING may be pre-selected.** **(c)** 🔴 **CARD 11 MUST RUN ON LAWNS** — the only tenant with a proposal. **Use a proposal, then DO NOT SAVE, and reload: the box must be empty again.** That is the whole mechanism — a fact we found is not a fact you have agreed to, and the way that is made visible is that it is not stored anywhere until you save. ⚠️ **And read the three proposals as facts about your business:** their About page says **1985** and their own structured data says **`foundingDate: 1984-01-01`** (measured in the captured page at `discovery/__fixtures__/lawns-vitex-real.html:46`). **I did not pick one.** One edit by you or Lauren settles it, which is the mechanism working rather than a defect in it. **(d)** ⚠️ **THE SETS' MEMBERSHIP IS MINE AND I WOULD RATHER HAVE THE WORKBOOK THAN BE RIGHT** — see the second finding above. If it exists, checking it in lets #133 close by reconciliation instead of by Lauren. **(e)** ⚠️ **`npm run authority:baseline` IS OWED AND I DELIBERATELY DID NOT RUN IT.** Six positions sites have printed `new` on every capA run since #240; **I proved they pre-date this build by stashing the tree and re-running**, and all six resolve to OWNER+MANAGER, which is right. **Baselining is the act of BLESSING a grant set, and that is not a builder's to give** — same reason Thunder never marks an owner-test card `covered`. Run it **after CARD 7 passes** and a future widening goes red instead of blending into six lines everyone has learned to scroll past (tech-debt **#132**). **(f)** ⚠️ **P1 IS THE FIRST OPEN ITEM ON THE HARDCODED REGISTER IN SEVEN WEEKS AND I FILED IT AGAINST MYSELF IN THE SAME COMMIT** — LAWNS's facts in shared code. It caps positions at amber, which costs nothing today (0 of 12 cards covered), and **the exit is the correction in the first finding**: when a `BusinessIdentity` can feed the proposal, P1 is DELETED rather than extended. **(g)** ⚠️ **STILL OPEN AND UNTOUCHED:** `docs/inventory-functions.md` / `inventory-env.md` / `inventory-ai.md` stamped `2026-06-13` — **flagged at session open per the health check and not used to answer anything** · `docs/reference/` still tracked by accident (`8bbaec4`, still yours to revert) · `20260830c` written and not applied · tech-debt **#124**'s eighteen over-wide policies · **#125** · **#126** · **#127** · **#130** · **#131**.
