...
**Step 1 done (migration applied + 32/32 catalog-proven 2026-06-17 with David's PAT — since revoked).** **NEXT — David's fresh confirm before I proceed (the data-move, held by design):** steps 4-8: backfill (recurring config → `cost_objects` rows: node_type COST, nature OPEX, source MANUAL, lossless, UNKNOWN→UNKNOWN) → verify flat total still == $12,239.67/mo (mismatch = STOP) → flip read source (page selects `cost_shape`/`cadence`/`recurring_amount`, feeds pool) → re-point write path (shape+nature+project pickers + substantiation; keep truncation-guard) → owner-prove. **Labor DEFERRED** (stays in config, flagged not dropped — R3). `[TRACE:*]` stays ON until owner-proven. Next layers NAMED not built: API connectors (via `cost_source`) + AI-classify (manual-first).
