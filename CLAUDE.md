...
# Last updated: 2026-07-31 — the pill is real, the feature is not built: a fourth permission status, `planned` (#175). See §3.
...
### 2026-07-31 (4) — THUNDER LIGHTNING CONTRADICTED ITSELF INSIDE ONE CONVERSATION AND DAVID CAUGHT IT. The morning's SIX-STATE ruling says BEING BUILT renders distinctly with a "coming soon" hover. Hours later #88 ruled `reports:read` **declared-unwired** — which per §7.1 **FILTERS THE STRING OUT** of the Roles catalog. **Opposite answers about the same string, from the same source, the same day.** David's resolution: **THE PILL IS REAL, THE FEATURE IS NOT BUILT — and that is not a fake pill.** A fake pill claims something WORKS when it does not; **a planned pill says a thing is COMING, which is TRUE**, and it is the conversation starter — the customer sees it, taps it, and tells us what they need from it before we build the wrong thing. **FOURTH STATUS SHIPPED; BUILDER-COMPLETE (`npm run verify` exit 0); card 18 owed, board 0 of 18. BUILD 2 — the tile path — NOT STARTED.** ZERO new api-fn (12/12), no schema.

**Type:** Manifest + tooling + UI — `permissionManifest.ts` (fourth status · **`reports:read` MINTED** · 2 reclassified · `PLANNED_PERMISSIONS` + `UNGRANTABLE_PERMISSIONS` · `OWNER_LOCKED_SET` predicate) · `MemberConsole.tsx` (the chip + `onPlannedSelect` seam) · `TeamConsole.tsx` (wiring + **a comment that argued the superseded position**) · `verify-universals.mjs` (capQ) · `permissionManifest.test.ts` (**+9 assertions, 74 total**) · `authority-grants-baseline.json`. Docs: ledger **#175** · **1 NEW RULING** + **2 NEW OWED rows** (#89 BUILD 2, #90 team:* removal) · **NEW card 18** · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-31 THE LOSSY CONVERSION #172), 1 written — entries-in == entries-out, §3 back to 3.**

**WHY A FOURTH STATUS AND NOT A SECOND AXIS.** `declared-unwired` conflated two different facts: *"nothing will ever enforce this"* (an accident, or a deliberate no) and *"scoped for a planned feature."* Option A — a fourth status — was chosen over a `roadmap: true` field for the reason the week supplied: **every consumer here is status-keyed, and a second axis is a second thing to forget.** The counter-argument was recorded and **knowingly rejected by David**: yes, `planned` and `declared-unwired` share the un-grantable invariant and differ only in rendering — **but they also differ in INTENT, and intent is what a reader needs.** Semantic, not presentational. 🔴 **THE FLOOR: `planned` requires a build behind it** — a named gap, a card, a slot, or a decision. `campaigns:create` is the worked counter-example and stays `declared-unwired`: **the next verb being obvious is not a scoped build.** Without that floor everything imaginable becomes planned and the distinction dies.

**🔴 capQ FAILED ON THE MINT, AND THE FIX IS THE PART WORTH KEEPING.** Extended to UNGRANTABLE = declared-unwired ∪ planned, it immediately failed: `reports:read` is absent from the R-B2 `NOT IN` list inside `20260727_rbac_resource_action_flip.sql` — **an APPLIED migration that §6 r1 forbids editing.** The assertion was demanding an impossible edit. That list is a **HISTORICAL SNAPSHOT** of what was stripped from real member arrays that day, and **a string minted afterwards cannot be seeded by a rewrite that already executed.** The `manifest ⊆ R-B2` direction was removed with the reasoning recorded IN the cap; it is not a gap, because `qBundleViolations` (no un-grantable string in any bundle) plus `qFloorViolations` (the LATEST `_align_floor` reconciled against those bundles) cover the **live** seeding path — the deleted direction guarded a spent one. **A derivable replacement was TESTED BEFORE DELETING, not assumed:** *"the R-B2 list is the alias-reachable un-grantable strings"* is **FALSE — only 3 of 8 members have a legacy antecedent.** Removing half a reconciliation is normally how a cap goes quietly blind; this one says why, in place, and kept the direction with teeth.

**🔴 THE TEST SUITE CAUGHT A REAL DEFECT BEFORE THE COMMIT DID.** `OWNER_LOCKED_SET` filtered `!== 'declared-unwired'`, so the three new planned strings **would have entered the owner's computed set** — the owner "holding" `reports:read` for a readout that does not exist, **capA assertion 3 then demanding the SQL literal grow 52 → 55**, and an applied migration needing an edit to record a grant of nothing. The correct predicate is UN-GRANTABLE. **It is the same conflation the fourth status exists to end, reappearing one derivation later, in the commit that ends it** — the 2026-07-29 lesson holding exactly: knowing a rule does not make you apply it while writing the code the rule is about.

**THE HONEST RESIDUALS.** The **requirement hook is NOT built** — `onPlannedSelect` is an undefined prop and a planned chip is informational today; the seam is declared so the chip is not re-plumbed later. **`reports:read`'s category is `financial`**, a small judgement call flagged in the code rather than presented as derived. And **BUILD 2 is not started, with a real defect named in it:** `dashboardTiles()` excludes `kind:'readout'` and `useModules` filters `can()` **BEFORE** reading `status`, so **`planned` is reachable only for tiles gated on strings people already hold — the inverse of the state's purpose.** `<BeingBuilt>` is still unmounted.

**FLAGGED FOR DAVID:** **(a)** **card 18** — `/team` → Roles → a non-OWNER role: three dashed-amber 🚧 chips, clicking does NOT tick, and **after Save + reload the count is unchanged**; `campaigns:create` / `team:create` must NOT appear at all, which is the distinction proven. **(b)** **BUILD 2 (#89)** on your word, including the one-direction cap you specified. **(c)** **#90** — removing `team:create/update/delete` is blocked on the same applied-migration reconciliation this build solved from the other side; it needs the retained direction handled before those three can leave both sets. **(d)** the `financial` category call is yours to overturn cheaply.

### 2026-07-31 (3) — THUNDER FOUR INSTANCES, FOUR DISCOVERY ROUTES, NOT ONE OF THEM A CHECK. `manage_orders` — David noticed nobody held it. `view_customers` — David could not search a customer at checkout (#170). `view_costs` and `import_pricing` — capA assertion 4's first run, hunting something else entirely. **Every one a CLIENT gate naming a string the model had retired; every one narrowed to zero while the surface looked completely fine.** David: *four found by accident means the class is UNMEASURED, not small — I want the count, not another instance.* **THE COUNT IS 4 STRINGS · 5 SITES, and TWO WERE LIVE DEFECTS.** **capA ASSERTION 5 SHIPPED + 2 STRINGS FIXED; BUILDER-COMPLETE (`npm run verify` exit 0, 51 probes); cards 16–17 added, board 0 of 17.** ZERO new api-fn (12/12), no schema.

**Type:** Tooling + app code — **capA 4 → 5 assertions** (35 → **51 probes**) · `authority-grants-baseline.json` gains an `a7Declared` block · **2 EDITS** (`Dashboard.tsx`, `InventoryImport.tsx`) · 2 baselines re-locked for line-shift. Docs: ledger **#174** · **2 NEW RULINGS** + **2 NEW OWED rows** (#87/#88) · **2 NEW cards** · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-30 (2) THE AUTHORITY MODEL #171), 1 written — entries-in == entries-out, §3 back to 3.**

**WHY capP DID NOT COVER IT — precisely, because "add it to capP" was the obvious wrong answer.** capP's assertion 1 (P15) scans for `fate: 'retire'` strings in the ROUTER text and in the strings RLS policies check. **`view_costs` is `fate: 'split'` and `import_pricing` is `fate: 'rename'`** — legacy WITH live successors, so neither is in P15's set — and neither lives in the router. **The client layer is a corpus capP has never read.** That is the whole reason four of these survived every green run.

**THE TWO LIVE ONES.** `Dashboard.tsx:91` gated the inventory-value metric on `view_costs`, so `can()` returned false for **every session including the owner's** — the metric was dark for everyone and `:160` did not even request `unit_cost`. **Nobody reported it because a tile that is missing looks exactly like a tile that was never built**; you do not file a ticket about a feature you assume is unfinished. Ruled → **`costs:read`**: the metric is qty × unit_cost, the cost basis and nothing else, so of eight successors only one describes what is read. **The `unit_cost` SELECT was never deleted** — it was gated on the same dead flag, so fixing the string restored it, and it is recorded that way rather than as a restoration that did not happen. `InventoryImport.tsx:71` was the same shape → **`inventory:import_price`**, a 1:1 rename; its comment still read *"Owner ⇒ can() short-circuits true"*, **describing the line Phase 2 deleted** — #164's class sitting directly above the defect it was describing.

**🔴 FOUR SELF-CAUGHT ERRORS IN THE CAP, EVERY ONE BEFORE IT REPORTED A NUMBER — and this is the entry's real content, because a cap that reports a wrong list is worse than no cap.** **(1)** A `{3,60}` length bound INSIDE the quote regex left 1-char literals unconsumed, so the scanner paired the WRONG quotes: `{ key: 'x', required_permission: 'manage_orders' }` yielded the "string" `, required_permission: ` and **missed the real one.** Caught by **probe S3** — the argument for writing probes before the scan. **(2)** `readModel` parsed `PERMISSION_MANIFEST`, which is `buildManifest()` — **a DERIVATION** — and got **ZERO permissions**, so the first list was **49 strings and flagged every live one.** Now it reads the `RESOURCES` seed table and **THROWS on zero** rather than reporting a catastrophe as a finding. **(3)** Not every permission is resource+verbs: `'tax_exempt:apply'` and `'order_discount:apply'` are SINGLETON keys with no `verbs` array, and the parser would have reported **two live, correct, ENFORCED strings** as defects. **(4)** Position matching was LINE-level, so a tile row whose KEY is `'qr_checkout'` — colliding with a legacy permission NAME — was flagged because `required_permission:` appeared later on the same 200-column line; it is ARGUMENT-level now. **49 → 7 → 4.**

**🔴 AND A PRE-EXISTING DEFECT IN capA ITSELF, COMMITTED SINCE 2026-07-30.** `decomment()` blanked string literals with **a NUL byte** instead of a space, which made the file **BINARY to `grep`** — so `grep` over `scripts/` had been **silently skipping the cap that guards the entire authority model.** Found only because a routine grep for my own new function name returned nothing while `awk` showed it plainly present. One byte, no effect on the assertions, and the file is searchable again. **A tool that fails silently on the file you trust most is the same shape as everything else in this ledger.**

**WHAT I DID NOT DO, deliberately.** `view_dashboard`'s nav fallback (**unreachable today — 0 of 26 nav nodes reach it**, a landmine for the next one added without a `tileKey`) and `reports:read` on the planned `business_insights` tile are **DECLARED with their reasons and print RED on every run** — not fixed, because picking a successor silently is exactly what the day's method ruling forbids. They are #87/#88 on David's desk.

**FLAGGED FOR DAVID:** **(a)** **#87/#88.** For `view_dashboard`: fall back to the `member` sentinel, or delete the fallback and make the field required? For `reports:read`: mint it in the model, or point the tile at an existing string? **(b)** **cards 16–17** — the dashboard Inventory value must be a real figure; the import must not claim your prices will not save. Both are OWNER cards and both are one glance. **(c)** the **method ruling is recorded in RULINGS** and it is the one line from these two days worth carrying forward: *a string is derived from the ACT, never from who needs to pass.* **(d)** **tech-debt #78 fired on cue** — two baselines re-locked for pure line-shift, in the same session whose own new baseline is keyed `file::binding` to avoid exactly that.

### 2026-07-31 (2) — THUNDER A NARROWING IS REPORTED BY THE PERSON WHO LOST SOMETHING. A WIDENING IS REPORTED BY NOBODY, EVER. That asymmetry is the whole argument for capA's new assertion, and the evidence is this morning's own audit: **four findings across 15 sites, and THREE WERE WIDENINGS.** David found the one narrowing in about twenty minutes because a person noticed something missing; nobody was ever going to notice that `ScanOrder` had quietly started admitting STAFF. **DAVID RULED: BUILD IT** — and separately overturned this morning's own string, because `orders:update` was right for the wrong reason. **capA ASSERTION 4 SHIPPED + 1 STRING; BUILDER-COMPLETE (`npm run verify` exit 0); card 15 updated, board 0 of 15.** ZERO new api-fn (12/12), no schema.

**Type:** Tooling + app code — **capA extended** (`scripts/verify-authority-checks.mjs`, 3 → **4 assertions**, 16 → **35 probes**) · **NEW `authority-grants-baseline.json`** (41 sites) · **NEW npm script `authority:baseline`** · **1 EDIT** (`CustomerCapture.tsx` — the string + the reasoning). Docs: ledger **#173** · **2 NEW RULINGS** + 1 marked SUPERSEDED + **2 NEW OWED rows** (#85/#86) · tech-debt **#84 CLOSED ruled-not-built** · card 15 rewritten · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-30 RETIRED-STRING #170), 1 written — entries-in == entries-out, §3 back to 3.**

**RULING 1 — THE GRANT SET IS BASELINED.** Assertions 1–3 are all about SHAPE. A site can be immaculately shaped and admit a different set of people than it did yesterday, and **that is exactly what happened inside the build whose purpose was to make authority explicit.** Assertion 4 resolves every authority site — `can('literal')` bindings, inline control renders, route `permission=` props — to the ROLES that pass it, against the three bundles. A role-set change **fails the build**; the fix is to revert or re-baseline **with a `why`**. **Keyed `file::binding`, deliberately NOT `file:line`** — tech-debt #78 is that the other three caps report a false NEW on any comment edit above a tracked site, and a binding name survives the edit. **19 new probes both directions (STD-022): a narrowing fails, A WIDENING FAILS, a declared change passes, an empty grant set with no reason fails on first sight.** **STD-024 red-first against REAL HISTORY rather than a synthetic tree** — restore `CustomerCapture.tsx` at `123ad3a`, the post-Phase-2 file capA passed clean that day, and assertion 4 names it: `grant-narrowed: OWNER+MANAGER+STAFF → OWNER`.

**🔴 THE FIRST RUN FOUND TWO LIVE DEFECTS NOBODY WAS LOOKING FOR — the #170 class, instances 3 and 4.** `Dashboard.tsx::canViewCosts` gates on **`view_costs`**, a LEGACY string, and `can()` is literal membership with no alias resolution — so it **admits NOBODY INCLUDING THE OWNER.** The dashboard's inventory-value metric is dark for everyone and the query at `:160` does not even SELECT `unit_cost`. `InventoryImport.tsx::canImportPricing` is the same shape, and the comment above it still reads *"Owner ⇒ can() short-circuits true"* — **describing the short-circuit deleted in Phase 2.** **Neither is fixed and neither is silently accepted:** both are DECLARED `KNOWN-BAD, NOT ACCEPTED` and **print in red on every run.** `view_costs` maps to **EIGHT** successors, and choosing which one gates a metric is David's call, not a builder's.

**RULING 2 — A PERMISSION GATES A CAPABILITY, NOT A FIELD.** *"May create an order" implies "may set its fields."* So this morning's `orders:update` was **right for the wrong reason** — it happens to include Lauren, when the rule is that whoever may perform the act may set the act's data. The string is now **`orders:create`**, and **STAFF GET THE FIELD**: they hold it and they take orders, so gating on `:update` would have handed the field to the manager and withheld it from the person standing in the lot — *the same defect with a different victim.* **`submit.ts` needs NOTHING, and tech-debt #84 closes RULED-NOT-BUILT** with the reasoning in the code file itself, so nobody adds a server gate later believing its absence was an oversight. `order_discount:apply` and `tax_exempt:apply` stay separate **because they are MONEY and TAX acts, not because they are fields.**

**🔴 THE HONEST LINE.** The cap **prints its own blind spot** — 6 dynamic sites (`can(navPermission(n))`, the tile registry, `PermissionRoute`'s own call) whose grant set is decided at runtime, and it knows the three DEFAULT bundles, **not a tenant's edited arrays**. Both are in the header, because a cap silent about its own limits is #164. **Two parser false-positives appeared on the cap's own first run** — a block comment mentioning `can()` and `can()`'s own definition, both listed as "dynamic blind spots" — caught, fixed, and each given a probe (G17b/G17c). **A blind-spot list padded with prose teaches its reader to skim the one section that must be read.** And the day's arithmetic is worth stating plainly: **one string, ruled three times in about twelve hours** — `deliveries:create` → `orders:update` → `orders:create`. Only the last one is reasoned from the capability rather than from the holder.

**FLAGGED FOR DAVID:** **(a)** **#85/#86 are on your desk** — `Dashboard::canViewCosts` needs one of `view_costs`'s EIGHT successors chosen for the inventory-value metric (`costs:read` is the obvious candidate); `InventoryImport::canImportPricing` → `inventory:import_price` is unambiguous and needs only your yes. **(b)** **card 15 now runs TWICE — as MANAGER `df7723be` AND as STAFF.** A staff session that sees the customer form but not the date means the string drifted back to `:update`. **(c)** **card 6 re-walked with the delivery branch; card 1 on your mark.** **(d)** the **6 dynamic sites** are genuinely unresolvable statically — if any of them matters, the fix is to make the permission literal at the call site, not to make the cap cleverer. **(e)** `npm run authority:baseline` after any deliberate grant change, and **write the `why`** — an undeclared re-baseline is the one way to defeat this assertion.
