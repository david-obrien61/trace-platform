...
# Last updated: 2026-09-04 — #273 a vendor gets an edit path; the per-record control leaves the row — see §3
...
### 2026-09-03 — THUNDER **`/receipts` IS A `<DataSheet>`, AND THE CAP NAMED THE CONVERGED FILE BY ITSELF. #269 (renumbered from #268).** 🔴 **THE HEADLINE IS THAT THREE CLAUSES THIS SURFACE HAD RECORDED AS `owed` FORTY-EIGHT HOURS EARLIER ARRIVED WITHOUT BEING BUILT.** David ruled the move: one row per receipt, the chain in a `renderExpand` drawer. **G4 sortable columns · G6 global search + a quick status filter · G7 density in a bounded scroll box** came from the engine. ✏️ *That is the answer to why the design system outranks a surface, stated as a cost rather than a principle: the divergence had been charging us three clauses nobody was ever going to fund separately.*

**Type:** Build, on `main`. **BUILDER-COMPLETE** — `8c83574` · `npm run verify` **exit 0, ZERO NET-NEW** (tsc 5/5 · eslint 244/244 · knip 10/12/15) · **68/68 files, 3506 assertions**; receipts suite **134** (was 94) · **NO migration · api/ 12/12 · NO new permission.** Records: `53d38c4`. **§3 RETENTION: 1 archived verbatim (my own #265 entry), 1 written — entries-in == entries-out, §3 back to 3.**

🔴 **THE SELF-PRUNING CHECK FIRED UNPROMPTED, ON THE GOOD OUTCOME — WHICH IS THE HALF OF IT NOBODY EVER SEES.** The moment the `DataSheet` import landed, yesterday's cap named this file: *"NOW USES THE SHARED CONTROL … the surface converged. Delete the entry."* The declaration is **MOVED to a `converged` record, not deleted** — the withdrawn premise and its correction are evidence about how we work, and deleting the entry would erase that along with the divergence. **Four new §G probes proven RED first**, and the one that matters is **G4: the engine re-sorts by ONE key, so the model's capture-time tiebreak survives only because `Array.prototype.sort` is stable — the composition risk of the whole move, asserted rather than assumed.** ✏️ **And the probe had a blind spot I introduced and caught: its key regex was `[a-z]+`, so a column keyed `receipt_id` would have been skipped by the loop entirely and its missing `sortVal` would have passed silently.**

🔴 **TWO SESSIONS CORRECTED ME TODAY AND BOTH CORRECTIONS ARE IN THE CODE.** `trace-platform-e2` caught (a) a **net-new eslint error that was mine** — an unused `receiptSortKey` import left behind when the column moved to the model field; (b) my comment claiming the undated-row path was *"not live at 17 rows"* — **`receipts.date` is NULL on 1 of 37, on Test Dave's, the tenant David actually tests on.** *A claim about the tenant I happened to be looking at, stated as a claim about the table.* And a third session's `1f039b4` had already taken **#268** before my commit landed, so mine renumbered — **first-in-keeps-it**, and **`8c83574`'s message still reads `build(#268)` and is deliberately not rewritten**: the ledger row is the authority, and rewriting pushed history to fix a label is the worse trade. ⚠️ **The same stale read silently NO-OPPED my `built-inventory.md` header edit** — the line I anchored on had been rewritten underneath me. Caught by reading the result instead of trusting the write.

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE BOARD IS 0 OF 14 COVERED (13 `owed` · 1 `needs-test`)** — all nine covered cards flipped, because every one was written against a card stack that no longer exists (OP-14 cl.3); each keeps its 09-02 evidence verbatim. ⚠️ **Seven still settle from ONE print.** **The sharpest new step is CARD 12: sort Amount descending and confirm `$1,283.88` sits ABOVE `$920.13`** — sorted as text it does not, and the page looks completely normal. **(b)** 🔴 **TWO SHARED-CONTROL QUESTIONS ARE OWED DOC-FIRST AND WERE DELIBERATELY NOT DECIDED IN A SURFACE (R-74):** the expand toggle is **trailing and a chevron**, not the leading plus/minus you named — that is a `DataSheet.tsx` change for every consumer; and the engine's count pill **cannot name a server-side cap**, so at 100 rows it would say *"100 of 100"* about a tenant holding 236. **(c)** ⚠️ **THE COLUMN DEFAULTS ARE AN UNVERIFIED PREMISE AND ARE FLAGGED AS ONE IN THE FILE** — your set was relayed verbatim by a peer session whose prompt I have not read. One word flips it back. **(d)** 🔴 **`LINES` IS NOT BUILT AND IT IS YOUR CALL, NOT A GAP:** a line count needs `line_items` in `RECEIPTS_SELECT`, which this file's own invariant forbids — selecting it downgrades a STRUCTURAL guard (*the inputs are not in hand*) to a review-only one (*we have them and choose not to re-derive*). **INVOICE NUMBER** is blocked on another session's migration. **(e)** ⚠️ **One deliberate difference from the relayed set, and it is a standard-beats-prompt case:** it leads with RECEIPT DATE; this grid leads with VENDOR, because **G3 pins the leading IDENTIFIER column** and a date is not an identifier. **(f)** ⚠️ **23 bespoke surfaces remain undeclared and UNAUDITED**, and **`ui-standards.html` still renders 3 of the standard's 6 sections.**

### 2026-09-03 — THUNDER **THE PLATFORM'S ONE GRID ENGINE WAS LIVING INSIDE A SINGLE VERTICAL, AND NOTHING HAD EVER HELD IT THERE. #272.** 🔴 **THE HEADLINE IS THAT I ACCEPTED A CONSTRAINT INSTEAD OF MEASURING IT, AND DAVID DID NOT.** I had reported that `QboBooksReader` (in `packages/shared`) could not use `<DataSheet>` (in `packages/cultivar-os`) because shared never imports the app — true — and shipped a plain table. David's correction was that the question was never *how do I reach it*, it was **why does the platform's one grid engine live inside one vertical.** Measured: the engine's ENTIRE transitive closure is `react`, `lucide-react` and two sibling files that import **nothing**. No supabase, no business context, no permission hook, no router, no tile registry. ✏️ *The obstacle everyone had been reasoning around did not exist, and no one had checked in the two months since it was written down.*

**Type:** RECON → David ruled **OPTION 1** (move verbatim) → BUILD, on `main`. `020793b`. **BUILDER-COMPLETE** · `npm run verify` **exit 0, ZERO NET-NEW** (tsc 5/5 · eslint 244/244 · knip 10/12/15) · **71/71 files, 3640 assertions** · `build:cultivar` clean 7.96s · **NO migration · api/ 12/12 · NO new permission · ZERO behaviour change.** **§3 RETENTION: 1 archived verbatim (#266), 1 written — entries-in == entries-out, §3 back to 3** (archive **292 → 293**).

🔴 **THE TRIGGER HAD ALREADY FIRED AND NOBODY RE-READ IT — WHICH IS THE FINDING, NOT THE MOVE.** `DataSheet.tsx`'s own `PLACEMENT` header said: *"promote to @trace/shared only when a real second-vertical consumer appears."* That consumer existed. **It just wasn't a vertical — it was `QboBooksReader`, inside `shared` itself.** The condition assumed the second consumer would arrive from a sibling package, so when it arrived from the shared package the condition read as unmet and the default held. ✏️ **A trigger nobody re-reads is not a decision point, it is a decision that defaults silently** — [[R-26]]'s class, and the third component comment in a week asserting something its own repo contradicted (tech-debt **#61**, the `ReceiptsList` divergence, this).

🔴 **THREE THINGS BROKE AND NONE WAS FOUND BY READING.** ① `verify-unit-projection.mjs` hardcodes the registry path **twice** and died on ENOENT — **proven RED against the move before it was fixed** ([[R-33]]). ② `receiptsList.test.ts` **G9 asserted the engine's ADDRESS as a proxy for its INTENT** (*"uses the shared engine"*), so a pure move failed a probe about something else entirely; retargeted at `@trace/shared`, which is **strictly stronger — it now also catches a forked local copy**, and that was mutation-tested rather than assumed. ③ ⚠️ **A FIFTH FILE EXISTED THAT MY OWN RECON HAD NOT SEEN.** `dataSheetDisclosure.test.ts` landed from another session (`22bb55f`, G10) **after** I measured the directory and **before** I moved it; it reads the engine's source by absolute path and would have hard-failed. **My reported "four files" was one stale — caught because `ls` after the `git mv` disagreed with my own report, not because I re-checked.**

⚠️ **WHAT I DID NOT DO, EACH BY DAVID'S EXPLICIT SCOPE.** The divergence cap's `SCAN_ROOT` stays `packages/cultivar-os/src` — widening it re-baselines `undeclared_bespoke_surfaces: 23` into an unknown, and *"more unaudited surfaces, not fewer"* is a real decision he declined to bundle with a move whose whole virtue is that `verify` proves it. **But the one thing that genuinely breaks was done in the same commit:** the divergence cap keys `CARRIERS` on the **full path**, so `ui-control-standards.md:18-19` moved with the file — otherwise the grid engine is measured as a divergence from itself. **The `datasheet/` directory name is preserved deliberately**: `usesSharedGrid` matches on that path fragment, and renaming it would silently convert all 8 consumers into undeclared bespoke surfaces.

**FLAGGED FOR DAVID:** **(a)** ⚠️ **NO OWNER-TEST CARD MOVED, AND THAT IS AN ASSERTION I HAD TO EARN.** OP-14 cl.3 flips a card when a surface changes — so I proved no surface did: the rename-detected diff shows **37 changed lines in the engine, every one a comment**, and zero non-comment changes anywhere under `packages/` except 8 import lines, 3 manifest lines and 1 test path literal. **Filtered, not asserted.** **(b)** 🔴 **`QboBooksReader` IS STILL A PLAIN TABLE AND THAT IS NOW A CHOICE RATHER THAN A LIMIT** — the reach is real, the conversion is a separate build against G1–G7 with its own cards, and riding it along would have destroyed the one property that made this move safe. **Its false comment was corrected regardless, per your instruction.** **(c)** 🔴 **THE BOUNDARY IS ENFORCED BY NOTHING — tech-debt #156, filed at your instruction.** tsconfig, eslint, package.json, knip and vite were each checked and named: a shared file could import from cultivar-os today and **every check in `npm run verify` would pass.** ⚠️ Any cap must assert **import edges**, not the string `cultivar-os` — five legitimate shared test files read app source by path and a naive rule breaks them on day one. **(d)** 🔴 **`populate.ts:77` QUERIES `cultivar_plants` FROM INSIDE SHARED** — a runtime AC-1 leak, now in §1.5. **Population, not a sample: it is the only one.** **(e)** ⚠️ **`tokens.ts` HAS A COMPLETE PER-VERTICAL PALETTE AND ZERO IMPORTERS** — tech-debt #157, filed **against `shared`, not against `DataSheet`**, because the 35 literals were there first and the count is now **42 across 21 files**. The real blocker is unanswered: *how does a shared component learn which vertical it is rendering in.* **(f)** ⚠️ **THE LOCKFILE WAS OUT OF SYNC** the moment the manifest gained `react`/`lucide-react` — regenerated with `--package-lock-only` (3 insertions, no tree churn). **`npm ci` on Vercel would have failed**, and nothing in `npm run verify` would have said so. **(g)** ⚠️ **STILL OPEN:** `origin/assets` · tech-debt **#143**–**#145** · **#148**–**#157**.

### 2026-09-04 — THUNDER **A VENDOR HAD A CREATE PATH AND NO EDIT PATH, AND THE ONE THING EDITABLE WAS A JUDGEMENT SITTING INSIDE A LIST ROW. #273. R-83 / E7. TECH-DEBT #179 + #180 + #181.** 🔴 **THE HEADLINE IS THAT THE SESSION OPENED BY REFUSING TO BUILD, AND THE REFUSAL WAS THE VALUABLE PART.** David's prompt pre-empted a conflict — *"I am ruling that preferred moves into the modal. If a filed clause already answers where a per-record control lives, IT WINS and you tell me."* **Nothing did**, and that was proven in writing before a line was minted: the near-miss is the **E2 worked example** (*"Tier and Status are editable BOTH as roster grid cells AND as controls inside the customer form … That is CORRECT, not a compromise"*), which settles the COMMIT MODEL and never says where a control lives — it **permits both** rather than pinning either. ✏️ *A clause that looks like it answers your question, checked and ruled out with its own sentence, is worth more than the same conclusion reached by not looking.*

**Type:** RECON → David minted **E7** → BUILD, on `main`. **BUILDER-COMPLETE** · `npm run verify` **exit 0, ZERO NET-NEW** (tsc 5/5 · eslint 244/244 · knip 10/12/15) · **72/72 files, 3710 assertions** (was 71/71, 3640) · `vendorEdit` **70 probes · 9/9 mutants caught, 0 survived** · `build:cultivar` clean 6.90s · **ONE migration, NOT applied · api/ 12/12, nothing minted · NO new permission.** **§3 RETENTION: 1 archived verbatim (#267+#268, the oldest by ledger number), 1 written — entries-in == entries-out, §3 back to 3** (archive **293 → 294**).

🔴 **FOUR CAPS WENT RED BEFORE THEY WENT GREEN, NONE OF THEM PROMPTED, AND ONE OF THEM WAS THE SUITE CATCHING ITS OWN AUTHOR.** The divergence cap **DERIVES its clause list from the standard**, so minting E7 pushed `clauses_absent_from_board` to 11 against a baseline of 10 and D1 refused the commit — fixed by **RENDERING §4 (E1–E7) on `ui-standards.html`**, not by raising the baseline, which ratchets it **10 → 4** and puts the clause where a prompt-writer actually looks. The write-path cap flagged `VendorEditor` as a NEW writer. The zero-row cap flagged the update as `NEEDS_CHECK` **and was right to** — the rule had been extracted into a pure function it cannot see across a file boundary, so the comparison is now stated at the write as well. ✏️ **And the probe that exists to catch tech-debt #179 nearly missed it: its column regex was `[a-z_]+`, which cannot match `address_line1` — the very column the defect was about.** It reported 13 of 14 and I read the failure instead of the total.

🔴 **THE DEFECT THE BUILD FOUND, AND WHY NOTHING WE OWN COULD HAVE FOUND IT.** `VENDORS_SELECT` named **10 columns while its migration creates 14** — the four missing were the ADDRESS — under a comment that said *"ONE LIST, TWO READERS, AND THAT IS THE WHOLE POINT"*, **true about its intent and false about its effect**. A column with no reader and no writer is invisible to tsc, eslint, knip and every probe; the only thing that finds it is a build that needs the fourth column. **Tech-debt #179, filed as the instance on David's instruction rather than only fixed** — the class is *a declarative list that does not match what its migration creates*, and `vendors` is one table of many. ⚠️ **A consequence worth carrying: a DERIVED select is a plain `string`, so supabase-js loses row inference** — both call sites now say `.returns<VendorRow[]>()`. Deriving the list costs the inference; that is cheaper than the defect.

🔴 **AND A COMMENT DEFERRED A FIELD TO A SURFACE THAT CANNOT EDIT IT — [[R-26]], THIRD INSTANCE THIS WEEK.** `ReceiptKeeper` explained the invoice number's absence: *"Correcting a misread number belongs to the record's ONE edit surface, /receipts/:id (E1)."* **`/receipts/:id` does not edit `receipt_number`** — grepped, and absent from both `ReceiptDetail.tsx` and `receiptDetail.ts`. So the field was editable **nowhere** while a comment **citing a real clause** said otherwise, which is what made it persuasive. **E1 was therefore never in play**: adding the field creates no second edit surface because there was no first one. Tech-debt **#180** — *a deferral to a surface that was never built reads exactly like a deferral to one that was, and nothing catches the difference* (with #61 and #145, three this week).

**FLAGGED FOR DAVID:** **(a)** ⛔ **`20260904_receipts_receipt_number_original.sql` IS NOT APPLIED, AND CARDS 16–18 CANNOT PASS WITHOUT IT.** Capture does not break meanwhile — the save retries once without both document-number columns and shows an amber notice saying the number was read but not stored. **(b)** 🔴 **THE BOARD IS 0 OF 18 (17 `owed` · 1 `needs-test`), AND FIVE CARDS WERE FLIPPED `covered` → `owed`** — none had ever been proven, so nothing green was lost, but each described the inline row editor E7 removed; their prior wording is preserved inline as **WAS**. **The sharpest new step is CARD 13:** fill all eight fields on `bwi`, save, re-open — **any address field coming back blank is #179 returning.** **(c)** ⚠️ **THE NOTE LEFT THE LIST, BY YOUR RULING, AND IT COSTS SOMETHING REAL.** Lauren still sees it — read-only with its explanation, in the modal — but she opens the record to read it instead of scanning it off the list. The page header records that as a deliberate change in what one glance buys, and CARD 7's old text is kept because it is what the surface used to claim. **Say the word and it comes back as a read-only line under the chip.** **(d)** ⚠️ **THE OCR PROMPT NOW ASKS FOR VENDOR-SIDE CONTACT IN BOTH SHAPES AND IT HAS NEVER RUN AGAINST A REAL DOCUMENT** — no capture has happened since. CARD 2 is the first read, and *"a wrong-but-present address"* is a CARD 13 fix, not a failure. **(e)** 🔴 **THE DIVERGENCE CAP ABSOLVES ANY FILE THAT IMPORTS `sheetStyles`** — it lives in `DataSheet.tsx`, and `usesSharedGrid` matches the PATH, so a modal that merely centres itself correctly counts as using the shared GRID. Not a false green today; **check C1 would order a still-true declaration deleted** on the strength of a style import. Tech-debt **#181**, deliberately NOT fixed — it re-baselines an unknown, the same call you made on #272's `SCAN_ROOT`. **(f)** ⚠️ **STILL OPEN:** `origin/assets` · tech-debt **#143**–**#145** · **#148**–**#157** · **#179**–**#181**.
