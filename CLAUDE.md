...
# Last updated: 2026-08-23 (6) — anti-fabrication line shipped; 3 ARC tags; outbound ruling. See §3.
...
### 2026-08-23 (4) — THUNDER **THE STORY ARRIVED FOR THE DEFECT I FILED YESTERDAY, AND THE SEQUENCE IS THE RIGHT WAY ROUND.** #194 recorded — as unfixed code debt — that there is **no anti-fabrication instruction anywhere in the generator** while both prompts ask for posts that are *"warm, local, **specific**"* (`generate.ts:19` + `:106`); *specific with no facts to be specific about invites invented ones.* **That finding now has a written requirement behind it: *Truth in advertising — suggest facts, never censor, keep the record*.** A build spec citing it will not be re-deriving anything, which is what §9's story gate is for. The entry states three things nothing on the board stated before: **TRACE never ORIGINATES an unverified factual claim** — *"that is the platform putting a fabricated number in her mouth, on her own feed, under her name"* · **TRACE never CENSORS her** — the check FLAGS, it does not refuse · **three verdicts, not two**, because the middle one is where the value is: verified / **couldn't-substantiate-and-here-is-the-nearest-thing-that-does** / contradicted. Its own framing is the one this platform already runs on: *"the same discipline as CONFIRMED / DERIVED / UNKNOWN on cost — unverified is never silently false, exactly as unknown is never silently zero"* — **A9 arriving on the claim layer.** Plus the anti-scorecard clause (*"here's what changed", never "here's what you overrode"*) and a limit it states about itself rather than leaving to be discovered: **the record captures what LEFT TRACE, not what was published.** **DOCS ONLY — no app code, no schema, no migration, no cap; `stories.html` NOT edited; nothing deleted or retitled.** `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · GATE 0 **NOT APPLICABLE** · all `[TRACE:*]` ON.

**Type:** One addition + close-out. `user_stories.md` (87 → **88** stories). Docs: ledger **#195** · `RULINGS.md` (**NEW OWED row — retention + check scope**) · ⚡ ACTIVE STATUS · `built-inventory.md` · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 #192), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 STEP 5 — THE `NEEDS:` TAG NAMES THE WRONG HALF OF THE RIGHT PATTERN, AND THE RIGHT HALF IS ALREADY BUILT ON THIS VERY FEATURE.** The tag asks the audit record to reuse `receipts.accept_vs_edit`. **That column cannot carry a before/after and never did:** `20260612_receipts.sql:23-24` is a two-value `CHECK (accept_vs_edit IN ('accepted_as_is','edited'))`, computed by `detectAcceptVsEdit()` (`ReceiptKeeper.tsx:363-375`) as an **OR of five booleans collapsed to one string** — *a boolean in string clothing; it records THAT something changed, never WHAT.* The before/after on receipts is its **siblings** `line_items_original` + `amount_original` (`20260614_receipts_reconciliation.sql:31,36`), whose own header calls them *"the Tesla bit: we can always show what was read."* ✏️ **And the sharper fact, which nobody would find from the story's own tag: CAMPAIGNS ALREADY CARRY THE WHOLE PATTERN.** `business_voice_samples` stores `(business_id, platform, original_text, edited_text, source)` and is written at `campaigns.ts:171-177` with `original_text: post.copy_text` (the draft) and `edited_text: editedCopy.trim()` (what she changed it to) — **literally *"what was suggested, what she published, and the delta between them"***, and written at the COPY moment, **so the story's self-stated limit is already the mechanism's actual semantics rather than a caveat to add.** ⚠️ **TWO REAL GAPS, NAMED AND NOT BUILT — the instruction was to report, not extend:** **(1)** `campaigns.ts:168` writes a voice sample **ONLY when `wasEdited`**, so an accepted-as-is post writes **no row at all** — precisely the case `accept_vs_edit='accepted_as_is'` DOES record, and for this story **acceptance is a data point** (she agreed with the check), so voice-samples alone would silently drop it. **(2)** Neither mechanism carries **the VERDICT or its SOURCE**, nor a suggested replacement fact she took or ignored — that is the genuinely net-new `copy_suggestion_audit` piece, and it wants a **third column family, not a second mechanism.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE THING I DID NOT DO IS THE POINT, TWO SESSIONS RUNNING: the generator's system prompt is still untouched.** #194 flagged the missing anti-fabrication line as a one-line fix; this story is now its requirement; **making that change inside a docs-only task is exactly the drift the scope bar exists to catch, so it stays owed deliberately** rather than being tidied away in a session nobody would audit for code. **(b)** ✏️ **One premise in your own `NEEDS:` tag was checked and came back half-right — reported, not silently corrected, and the tag is left exactly as written.** The audit record should reuse a pattern; it is just not `accept_vs_edit`. **(c)** **RULINGS OWED gains one row:** retention (**and it is not housekeeping — D-51 already splits logs BY retention, so which of the two this is decides the answer**) plus whether the check runs on every generation or only on a supplied claim. **(d)** ✅ **PURE ADDITION, VERIFIED NOT ASSERTED: 1 addition (40 lines, 1 story) · 0 modifications · 0 deletions**, all **five** prior campaign entries byte-untouched by line-exact comparison, 87 → 88 stories, 88 parsing under `stories.html`'s **own extracted parser**. **(e)** **CLAUDE.md is 715 lines against its own ~600 budget** — unchanged in kind; the residual is the still-open §4 *"lean CLAUDE.md"* item.
