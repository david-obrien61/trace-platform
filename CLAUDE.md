...
# Last updated: 2026-09-03 — #261 records repaired: migration applied, 9 proven cards flipped, 7 rescued — see §3
...
### 2026-09-03 — THUNDER **THE RECORD WAS BEHIND THE WORK, IN FIVE PLACES AT ONCE — AND EVERY ONE OF THEM WAS WORK ALREADY DONE THAT NOBODY HAD WRITTEN DOWN. #261.** 🔴 **THE HEADLINE IS THAT THE GUARD WAS PROVEN BY BEING REFUSED.** David applied `20260902_receipt_line_edit_and_vendor_preference.sql` and the write-once trigger threw at him **twice, as `postgres`, from the SQL editor** — `ERROR: 42501: receipts.line_items_original is write-once: it is the record of what the OCR read`, `guard_receipt_snapshot_and_lines()` line 9. **It refuses everybody, superuser included** — not a UI check, not a policy a sufficiently-permissioned caller slips past. ✏️ **A refusal that names its own reason in plain words is better evidence than any passing test, and it is the negative control the guard could not have passed by accident** ([[R-33]]: a check that cannot disagree is not a check — this one disagreed, on demand, twice).

**Type:** Records. **RECORDS ONLY — ZERO changed lines under `packages/` and `api/`.** The one migration touched is **COMMENT-ONLY, proven by filtering the diff BOTH directions — 0 non-comment lines added AND 0 removed, not asserted** (§6 r1; the same precedent as `20260802_trialling_modules_are_live.sql:2` and #254's annotation of `20260827_history_orders.sql`). No build bar applies; `npm run verify` not run and not applicable. **§3 RETENTION: 1 archived verbatim (2026-09-02 #254), 1 written — entries-in == entries-out, §3 back to 3** (archive **280 → 281**).

**🔴 NINE PROVEN CARDS WERE READING `owed`, WHICH IS THE RENDERER'S FAILURE ONE LAYER UP.** `receipts-view` said **0 of 11 covered** while David had proved nine of them on 2026-09-02 against `7952cb1`. Flipped to **10 covered · 1 `needs-test`**, each card carrying its own evidence rather than a bare date — 17 rows newest-first with no inversions; both duplicate pairs at their own capture times, **unlabelled**; the six no-order rows reading *"No order recorded for this receipt."* and nothing else; orders 30 before and 30 after across three loads at 14:11, 16:18 and 16:52. **CARD 8 is `covered` WITH ITS FAILURE PRESERVED** — it failed, #257 fixed it (`listVisibleForStep('confirm')` plus a negative probe), **and the failure is not overwritten, because a card that failed and was fixed is a better record than one that reads green.** ⚠️ **Its re-proof on the fixed build is explicitly OWED.** **CARD 10 → `needs-test`** with the reason stated: the account does not exist.

📄 **AND THE THING NO CARD HAS EVER SAID: NINE OF THOSE WERE PROVEN FROM PRINTED PDFs.** **Cards 1, 2, 3, 4, 5, 7 and 11 all settle from ONE print of `/receipts`** — recorded on the board so nobody works it out again. **Zero of 370 cards across the corpus declares print-provability**; `DEVICE:` is on 100% of them and answers a different question. David settled seven cards off one print yesterday with no help from the corpus.

**🔴 SEVEN CARDS EXISTED WHERE NOBODY COULD RUN THEM — AND MY OWN AUDIT MISCOUNTED THEM AS TWO.** `docs/audits/offline-store-and-forward-recon-2026-08-23.md` held a fenced block of owner-test cards written 2026-08-23, **never at `docs/owner-tests/`, so never rendered and never swept.** Now a real board, moved verbatim, pointer and original left in place. The audit that found them said two: it matched line-anchored markers inside a line range and stopped at the first hits; corrected by counting `^### ` in the extracted block. ⚠️ **CARD 1 records a real airplane-mode proof David performed on 2026-08-23 that was never written down — which is why it was re-questioned and cost a recon.** It stays `owed`: **Thunder may never set `covered`** (OP-14), and a proof recovered from prose is a claim.

**⚠️ THE BOARD IS NAMED 24, HOLDS 26 ROWS, AND ITS NAME IS AN INTERFACE.** Counted mechanically: `0.1 · 1.1–1.5 · 2.1–2.3 · 3.1–3.7 · 4.1–4.3 · 5.1–5.7` = **26**, i.e. **25 L1–L5 capabilities under a name that says 24**; `CAPABILITY-PACKAGE-GROUNDTRUTH.md:9` says 25 and is one generation stale. **Both annotated, NEITHER renamed** — `status.html:261` parses `/^##\s*📋\s*24-CAPABILITY BOARD/i` and `:471` falls back to *"No … section found."*, so **renaming the heading to fix the count would silently blank the panel.** ✏️ **That is the exact format-vs-reader mismatch that left 19 of 20 owner-test boards unrendered for seven weeks: a heading a renderer parses is an interface, not a label.** The rename rides the reader build, where both sides move together.

**FLAGGED FOR DAVID:** **(a)** 🔴 **SECTIONS 3–5 ARE A BUILD, SCOPED AND NOT STARTED — you authorise.** The reader, the card standard and the print declarations are **ONE build**: a card format and its reader must agree, and doing them separately guarantees the next mismatch. **(b)** 🔴 **`receipt-detail` is 12 cards, 0 covered, and runnable for the first time.** **(c)** ⚠️ **`#255` IS NOT LOST — it is claimed on `thunder/qbo-review-test-mode` and #256's own Blocker cell says so.** A **placeholder row** is filed so the sequence reads continuous; **replace it wholesale at merge, do not merge the two** — its content is deliberately not copied in ([[R-62]], and a second copy drifts). That branch also holds **#260**, so I claimed **#261**. **(d)** ⚠️ **THE HAND-OFF TO `thunder/vendor-identity` IS FILED, NOT DELIVERED** — its step 2 is cleared and its chain can proceed, but **I did not apply from there and did not touch the branch** ([[R-62]]). The owning session was **unaddressable**: a peer name is not derivable from a worktree session id (#256 tested it). **(e)** ⚠️ **THREE SURFACES STILL HAVE NO CAPABILITY ID — and the offline board makes FOUR:** Receipt Keeper (both boards), `positions`, and now offline store-and-forward. **Eleven of 20 boards declare no `Capability:` line at all.** **(f)** ⚠️ **50 of 109 stories read `MAPS-TO: —`** (46%) — reported by ARC and STATUS, **no mapping invented**; 29 of the 50 also carry no inline `ARC:` tag, which I measured but did not diagnose. **(g)** ⚠️ **The three `inventory-*` banners were re-checked and are STILL TRUE** — those docs remain un-re-audited; #254's `12 of 12` correction holds, and `find api -name '*.ts'` measures **12** today. **(h)** ⚠️ **STILL OPEN AND UNTOUCHED:** `origin/assets` (remote-only, 67 days) · `docs/reference/` tracked by accident · tech-debt **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#141** · **#143**–**#145** · **#148**–**#151**.
