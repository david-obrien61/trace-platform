...
# Last updated: 2026-09-02 — #254 Stage 0 recon filed + the stale-declaration sweep; Stage 1 HELD on four rulings — see §3
...
### 2026-09-02 — THUNDER **THE DOC THAT WROTE THE WRONG PREMISE — CORRECTED FIRST, BECAUSE IT WOULD HAVE WRITTEN THE NEXT ONE TOO. #254. STAGE 0 FILED, STAGE 1 HELD ON FOUR RULINGS.** 🔴 **THE HEADLINE IS THAT THE BUILD PROMPT OPENED ON TWO FACTS THAT WERE TRUE ON 1 SEPTEMBER AND FALSE ON 2 SEPTEMBER, AND BOTH CAME OUT OF OUR OWN FILES.** It said `origin/main = 84cc695`, `thunder/receipts-view` **NOT MERGED**, **ten worktrees live**. Measured before writing anything: `main = 08fd5a1`, `0c066c8` an ancestor of `main`, merge `11f9401`, **two** worktrees, **25 branches already in `main`**. The source was `TRACE-SESSION-BOOTSTRAP.md:86-87` — **the file §10 opens FIRST every session** — still reading *"BUILDER-COMPLETE … NOT MERGED"* a day after the merge; the worktree count came from a §3 narrative line that was **true when written**. ✏️ **Neither was invented and neither was re-measured, which is the whole finding: a status line is a claim with a shelf life, and this one is read before any work begins.** [[R-26]], tech-debt #22's class.

**Type:** Recon + doc corrections. **RECON + DOCS ONLY — ZERO changed lines under `packages/` and `api/`, ZERO SQL lines under `supabase/`.** The one migration touched is **comment-only: 17 insertions, 0 deletions, every changed line prefixed `--`, proven by piping the diff through a non-comment filter rather than asserted.** No build bar applies. Files: **NEW `docs/decisions/2026-09-02-purchase-sale-split-recon.md`** · ledger **#254** · `TRACE-SESSION-BOOTSTRAP.md` (2 corrections + a measured branch/worktree block) · `docs/built-inventory.md` (4 corrections) · the three `inventory-*` docs (staleness banners; `inventory-functions.md` also corrected inline) · `supabase/migrations/20260827_history_orders.sql` (annotation) · `docs/RULINGS.md` (3 OWED rows) · DECISIONS-INDEX drift · this §3. **§3 RETENTION: 1 archived verbatim (2026-09-01 #249), 1 written — entries-in == entries-out, §3 back to 3** (archive **279 → 280**), and `verify-handoff-retention.mjs` re-run green.

**🔴 THE SWEEP FOUND ONE THING WORTH FAR MORE THAN THE DATES, AND IT WAS WRONG IN THE DIRECTION THAT COSTS A DEPLOY.** `docs/inventory-functions.md` — the doc whose own header calls it *"canonical source for what functions exist"* — claimed **`11 of 12 — ✅ 1 SLOT HEADROOM`**. Measured: `find api -name '*.ts' -type f` → **12. ZERO HEADROOM.** CLAUDE.md §6 r11 has said 12/12 since 2026-06-20, so **two canonical docs disagreed on the number and the stale one was the PERMISSIVE one** — it authorises minting function #13, and **#13 does not error: the whole deploy fails SILENTLY while Vercel keeps serving the last-good bundle.** That exact failure bit this project on 2026-06-20. ✏️ **STD-011's real cost is not the duplication, it is that the copy nobody maintains is the copy that says yes.** Corrected inline with all twelve slots named.

**🔴 THE SWEEP REPORTED ITS POPULATION AND HAD TWO NEGATIVE CONTROLS, DELIBERATELY.** Population: **9 live doc claims naming a `thunder/*` branch as a deliverable's home** — `handoff-archive` and §3 narrative excluded **by design**, because both are append-and-preserve accounts of what was true at the time and rewriting them would falsify a session's own record. **6 stale, 2 already correct, 1 superseded-in-place.** The two correct ones (`bootstrap:88` docnumber-matcher **MERGED**, `:89` #248 **MERGED**) are what prove the sweep could disagree — [[R-33]] applied to a docs pass. **All nine shas were re-verified with `git merge-base --is-ancestor <sha> main` before a single line was rewritten**, which caught that the board cited `ab617b2` — a real commit, in `main`, but a **mid-branch** one, not the tip (`0c066c8`).

**⚠️ §6 r1 WAS CHECKED, NOT WAIVED.** The instruction was to correct the stale warning in `20260827_history_orders.sql`, and §6 r1 says migrations are never edited. **Precedent settled it rather than judgement:** `20260802_trialling_modules_are_live.sql:2` already carries a post-hoc `🔴 SUPERSEDED` header on an applied migration — **the rule protects the SQL, and the SQL here is byte-identical.** Original text preserved per *"do not delete a claim that was once true"*. Measured live while annotating: status is `invoiced` on **29 of 30** LAWNS orders, i.e. the *"do NOT reach for 'invoiced'"* paragraph described **almost none of the table it governed**; ✅ D-52 is nonetheless clear because **0 of 39** lines carry a `business_inventory_id` — invariant (1) is holding alone, which the migration's own *"BOTH, not either"* did not anticipate.

**THE RECON ITSELF — FOUR PROMPT PREMISES CORRECTED, FILED AS PART 0 AND FIRST.** **(C1)** the merge state above. **(C2)** **the launcher never pinned the shape** — it pins `state:{from:'route'}`; `OCR_SHAPE` is a module constant `'invoice'`, identical for both doors, so there is **no per-door signal to recover, only one to introduce.** **(C3)** **`R-52` is taken** — the draft becomes **R-54**, unfiled. **(C4)** the 12-June design **assumed the opposite of what the prompt feared**: `CountOnceSeam.ts:32-38` and **D-5** both demote `receipt_id` *"from key to one high-confidence signal"* and name the failure mode as *"doubled, or fed twice … under a different id"* — **which is the two duplicate pairs sitting in the live data**, so narrowing `receipts` to purchases moves the design **toward** its stated intent. Written eleven weeks before the ruling. ✏️ **The most useful thing in the recon was written by someone else in June.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **STAGE 1 IS HELD ON FOUR RULINGS AND THREE ARE NOW ROWS IN THE `RULINGS.md` OWED QUEUE, NOT PROSE HERE** — that is the whole point of the queue and the reason four rulings evaporated in July. **The story is the binding one:** `ocr-doc-routing` is `STATUS: needs-input`, `MAPS-TO: —`, and does not contain the purchase/sale ruling; the §9 gate blocks the spec on your prose. **(b)** 🔴 **THE SHARPEST FINDING NEEDS YOUR RULING, NOT MINE: capturing a SALE requires four `costs:*` permissions and zero order permissions** — including the resource the manifest itself labels *"cost basis / unit cost. **Confidential**"* — while the order write gates on `customers:create` and **never checks `orders:create`**. **A manager who may take orders and may not see cost basis cannot capture a customer invoice, today, live.** **(c)** ⚠️ **R-54, not R-52, and it stays unfiled** — a board whose value is that every line is yours does not get a line written for you. **(d)** ⚠️ **Option B carries your authoritative-home clause into the spec:** during the overlap the evidence exists in two places, the spec must say which is authoritative, and new writes go to one only. **(e)** ⚠️ **`ocr.ts:342`'s second shape derivation is NOT fixed** — it rides the build that opens the file (§1.6 item 2), by your instruction. **(f)** ⚠️ **What actually gates Stage 1 is not the merge — it is that #252 is still owner-proof OWED, 11 cards, 0 covered.** **(g)** ⚠️ **Ledger #254 may collide with the QB owner-review session** — re-read at merge, keep both, renumber the later claim (#233). **(h)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs are **bannered, NOT re-audited** — the re-audit is its own task · `docs/reference/` tracked by accident · tech-debt **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#141** · **#143**–**#145**.
