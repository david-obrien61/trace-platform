...
# Last updated: 2026-07-31 — a permission string is chosen from what the code WRITES, not from what the surface is called (#172). See §3.
...
### 2026-07-30 — THUNDER A GATE ON A RETIRED STRING IS NOT A GATE, AND THIS ONE MADE THE CHECKOUT CUSTOMER SEARCH UNREACHABLE. `ScanOrder.tsx` asked `can('view_customers')` — a string RETIRED at #166 — so for every member the answer was **no**, the strip read "Add a customer", and the SEARCH BEFORE ADD path built the day before was **on a branch nobody could reach.** The route and the RLS policy had both already moved to `customers:read`. **Only the client gate had not, which is exactly why the two cards covering this string stayed green over a broken surface.** **ONE STRING. BUILDER-COMPLETE + DEPLOYED `1ae91a8`; 1 owner-test card OWED (board 0 of 43).** ZERO new api-fn (12/12), no schema.

**Type:** App code — **1 EDIT** (`packages/cultivar-os/src/pages/ScanOrder.tsx`: one gate + the three comments in the same file naming the same retired string). Docs: ledger **#170** · **NEW card `R-7b`** on `rbac-resource-action-full-surface-test.md` (board 42 → 43) · `built-inventory.md` (NEW checkout-customer-attach entry + line 4) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift line · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-28 RBAC-THROUGH-CALL-5 #163/#164), 1 written — entries-in == entries-out, §3 back to 3.** **NO new decision, NO new standard.** **BENCH: no trigger fires.**

**THE MECHANISM, because it is the reusable part.** `can()` (`BusinessProvider.tsx:693`) is a **literal array membership test**. The one function it composes with, `applyPermissionDependencies`, **FILTERS a set — it strips a string whose prerequisite is absent, and it never expands a legacy string into its successor.** There is no alias resolution anywhere on the client path. So a renamed string does not degrade; it **fails closed, silently, for everyone except the owner** — and `isOwner` short-circuits the gate at the front, which is why every session that looked at this screen as the owner saw it working perfectly.

**WHY THE COVERAGE DID NOT HELP.** R-7 proves the ROUTE admits a manager to `/customers`; R-8 proves it refuses without the string. Both pass. **Neither touches a client render gate**, and the surface they are named after is not the surface that broke. The pattern is worth stating plainly: **two green layers are what hid the third.** A7's counter-example list now reads nav tile · `assets:*` phantoms · this — and all three were found by a person using the app, none by a check.

**🔴 THE HONEST LINE, AND THE COUNT IS THE EVIDENCE: capP REPORTED 16 FINDINGS BEFORE THE FIX AND 16 AFTER.** The cap that owns this model never saw the defect. Its retired-string scan (P15) looks for strings that map to **no resource** — `view_dashboard`. `view_customers` maps to a **live successor**, so it is not retired, it is *legacy*, and **nothing in the toolchain reads legacy strings at a CLIENT gate at all.** That is **#164's standing class** — *every negative assertion scans for what the model USED TO declare* — narrowed one notch: **the client layer is not scanned in either direction.** The A7 client-gate sweep was already filed before this session; **it stays filed.** Scope was held to one string on purpose, and the cap that would have caught it is the thing that is actually owed.

**FLAGGED FOR DAVID:** **(a)** card **`R-7b`** — `/checkout/scan`, **as MANAGER `df7723be`, NEVER as owner**: the strip reads **"Look up or add a customer"** · tapping opens **lookup**, not create · a **surname** returns matches · picking one attaches and the tier badge appears. A blank list for a surname you know exists is a *different* failure — that is RLS, not this gate. **(b)** the **A7 client-gate sweep is the real fix** and is still owed; this commit is one instance of it. **(c)** nothing else from the filed list moved — the required-ness of first/last name, email off the checkout path, the person/organization toggle, tier + tax status on the add form, the two search implementations (A1), #81, #82, #83 are all exactly where they were.
