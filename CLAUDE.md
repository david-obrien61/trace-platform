...
**Type:** App code (2 files) + ONE gated migration (WRITTEN, NOT applied). **NO RLS policy / query-shape change this pass** — Step 1 makes the error VISIBLE; the read root-cause gets fixed in the NEXT prompt after David reports the surfaced error text (no guessing at RLS). `[TRACE:*]` STAYS ON (new ERROR emits added). Commit **`__PENDING__`** (pushed). Build clean (968kb bundle); changed files tsc-clean (only the 4 documented pre-existing errors — Confirmation/Orders/DeliveryRoute/SocialSetup); **verify-universals exit 0**. Close-out ledger row **#26**.
