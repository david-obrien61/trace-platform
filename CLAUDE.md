...
# Last updated: 2026-08-01 — ITEM 2: the tenant module seed + the per-module trial clock (#181). See §3.
...
### 2026-08-01 (3) — THUNDER THE RULING ASKED FOR A FIELD THAT ALREADY EXISTED, AND FINDING THAT OUT WAS THE WORK. David ruled that `MODULE_CATALOG` gains a field marking core vs paid. **It has had one since yesterday — `billing`, typed `core|add_on|unpriced`, already asserted by `verify-tile-fields`.** Minting a second would have been two representations of one fact in the file that spent this week paying that exact debt down. And once the existing field was read properly, **the mapping needed no new field at all**: `enabled = billing==='core'`, `start_trial = trial_days > 0` — the three-value `billing` collapses to one rule, and `unpriced` gets a row with NO clock because a trial is a countdown to a price decision and there is no price. **ITEM 2 COMPLETE; BUILDER-COMPLETE (`npm run verify` exit 0 zero net-new, `build:cultivar` clean, NEW `moduleSeed.test.ts` 88 assertions red-first ×2); `20260801c` GATED — David applies; cards 23–24 owed, board 0 of 24.** ZERO new api-fn (12/12), no new table, no policy change.

**Type:** Migration + app code + tooling — **NEW `supabase/migrations/20260801c_module_seed_and_trial_clock.sql`** (`start_module_trial` + `seed_business_modules`, 10-query V-block) · **NEW `packages/shared/src/business-logic/seedBusinessModules.ts`** · `tileRegistry.ts` (`moduleSeedRow`/`catalogSeedRows` + the MASTER_BRIEF reconciliation, in place) · `OwnerSignup.tsx` (**`moduleCatalog` REQUIRED in the config**) · `SignUp.tsx` · `AddBusiness.tsx` · `OnboardingWizard.tsx` · `moduleState.ts` (**bridge DELETED**) · `business-logic/index.ts` · **NEW `moduleSeed.test.ts`** · `verify-write-paths.mjs` (one declaration) · `write-paths-baseline.json`. Docs: ledger **#181 — and #180, which was MISSING** · **4 NEW RULINGS + 3 NEW OWED rows** · 2 NEW cards · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-31 (5) BUILD 2 #176), 1 written — entries-in == entries-out, §3 back to 3.**

**🔴 THE PERMISSION QUESTION WAS ARGUABLE, SO THE READ IS RECORDED BEFORE THE CHOICE — WHICH IS THE METHOD RULE, NOT A COURTESY.** Against `subscription:update`: a trial costs $0 on day one, and making a manager ask permission to TRY something is friction with no invoice behind it. For it, and taken: **a trial is a spend with a delay** — at day thirty it converts or goes dark, so starting the clock SCHEDULES a bill on a date the starter picked for an owner who may never have been asked; the clock is one-shot and never restarts; and decisively, **it lives in the same row as `enabled`, behind the same wall, for the reason 20260801's own header gives** — *a clock the person being billed can rewrite is not a clock.* Gating it more loosely would make the **timer writable by a strictly larger set than the enablement it governs**, and a wall whose weakest gate is on the timer is not a wall. **The cost is stated rather than hidden: a MANAGER cannot start a trial**, so *"let Lauren try Social for a month"* is one click by Terry. Reversing it is one string in two places.

**🔴 THE RED QUESTION — WHAT THE CLOCK ASSUMES WHEN THE SEED FAILS — AND THE PART THAT MAKES IT SERIOUS.** No row ⇒ no clock ⇒ no conversion date ⇒ **a customer who is never billed.** And the failure is invisible: **`useModules.ts:104` renders a MISSING row and a disabled row IDENTICALLY**, so an unseeded tenant looks exactly like a normal tenant with everything switched off, on every screen there is. That is D-9's shape sitting in the data layer. Built against it: the RPC **returns COUNTS, not a boolean** (`seeded+existing≠expected` is a SHORT SEED, warned with the numbers); it is **idempotent by construction**, so the repair path IS the create path; and **the onboarding call sits OUTSIDE the legacy-create branch** — deliberately unlike `seedPricingConfig`, which is noted at both sites — so **a signup-path failure self-heals when the owner finishes onboarding**. 🔴 **RESIDUAL NAMED, NOT CLAIMED: an owner who abandons onboarding with both calls failing has no rows and nothing notices.** That third home is the marketplace load (ITEM 3), and `20260801c` V6 is the standing detection query until it exists.

**THE REPAIR MECHANISM AND THE MONEY HAZARD ARE THE SAME CALL, WHICH IS WHY CARD 24 IS THE ONE THAT MATTERS.** A self-healing re-run that also restarted the clock would be a permanent free subscription, arriving through the feature built to make failure recoverable. So **a running clock is NEVER restarted** — `applied:true, was_already_running:true`, audited `no_change` with `restart_refused` — and it refuses even a caller who HOLDS `subscription:update`. Card 24 is *finish onboarding twice; every timestamp identical to the second.* **`created_at` was disqualified by DATA, not preference**: the 2026-06-04 `INSERT SELECT` PRESERVED timestamps from `nursery_modules` (`:54`), so a trial read off a live row **expired before it began** — #70's genesis shape.

**FLAGGED FOR DAVID:** **(a)** 🔴 **the marketplace's "what you already get" list has TWO rows, not five.** MASTER_BRIEF names five core items; only QR Checkout and QuickBooks carry a `module_key`. **Owner dashboard cannot have one** — it is the shell every tile renders into, and a row for it could be set `enabled:false`. Inventory/asset tracking and Customer records simply have none. Two ways out are written into the catalog and **neither is chosen**: mint no-cost core keys (⚠️ **fails `verify-tile-fields` assertion 2**, so it needs a cap change with its own probes) or render *included* from a separate list. **(b)** **Three OWED rows added:** that one · does changing `trial_days` retroactively move every running trial's end date (today: **yes**, and the catalog says 30 is *inherited, not ratified* — ruling 14 would silently expire tenants seeded more than fourteen days ago) · should a seeded CORE module read `active` on day one (`configured` seeds false for everything, so **this build has no visible effect on the grid**, which is honest and will otherwise read as a failed deploy). **(c)** ✏️ **ledger row #180 was MISSING** — the `20260801` migration claims it in its own header and no row existed. **I wrote it, marked as written by another session, with an instruction to correct rather than duplicate.** Found by grepping for the number, not by anything mechanical. **(d)** `moduleState.ts`'s **pre-migration bridge is DELETED**, as its own header promised — and it was not dead code: `isMissingFunction` also matched a generic `does not exist`, so a schema-cache miss would have routed a write around **both** authority gates into an upsert RLS now discards silently.
...
### 2026-08-01 — THUNDER THE COST OF A NOISY RATCHET WAS NEVER THE MINUTES; IT WAS THE HABIT THE TOOL WAS TRAINING. `verify-zero-row-writes` and `verify-field-lists` keyed their baselines on `path:line`, so any edit that shifted a line reported the same site as a NEW violation — **three times on 2026-07-31, each time re-baselined.** David's ruling is the entry: **a ratchet that cries wolf teaches reflexive re-baselining, and a reflexive re-baseline is how a real violation gets absorbed.** Three times the correct response was *"re-lock it."* **The fourth time there may be something in it, and it would be re-locked the same way.** **TECH-DEBT #78 CLOSED; BUILDER-COMPLETE (`npm run verify` exit 0); no owner-test card — tooling with no user-facing surface, recorded per OP-14's reason clause.** ZERO new api-fn (12/12), no schema.

**Type:** Tooling — **NEW `scripts/lib/siteKey.mjs`** (one implementation, both caps) · `verify-zero-row-writes.mjs` (+`--dump`, **6 new probes**) · `verify-field-lists.mjs` (+`--dump`, **3 new probes**) · both baselines re-keyed. Docs: ledger **#177** · **1 NEW RULING** · **tech-debt #78 CLOSED** · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-31 (3) THE A7 SWEEP #174), 1 written — entries-in == entries-out, §3 back to 3.** **BENCH: no trigger fires.**

**THE KEY IS `path::binding#table.verb`** (+ `@n` only within an identical triple) — **the shape `authority-grants-baseline.json` already used and which survived every edit of 2026-07-31.** Chosen after MEASURING rather than before: **all 167 candidate sites in the repo resolve a binding, ZERO failures**, so there is **no line-number fallback anywhere in the key**. The one stated fallback is `?` when the `.from('x')` sits outside the window — **exactly 1 site of 120**. Line numbers are still PRINTED for the human; a report you cannot navigate from is worse than a noisy one. They are simply no longer the IDENTITY.

**🔴 THE COUNT PROOF, required before any re-lock, and the part that could have gone silently wrong.** A re-key can drop a site — **which is the absorption the ruling describes, arriving through the remedy.** A new permanent `--dump` mode emits `key<TAB>tag` per site, so the proof is arithmetic: zero-row **85 → 85**, field-lists **35 → 35**, **distinct keys 85 and 35** (no collision quietly merging two sites), and the emitted line-tags reproduce the previous baselines **exactly — 0 unmatched in either direction.** `--dump` stays, so the next key change gets its proof for free. **CORRECTION, stated rather than absorbed: the counts were 85 and 35, not the 81 and 37 the prompt carried.**

**PROBES BOTH DIRECTIONS (STD-022), and the negative one is what keeps this from being a way to silence the cap:** a comment above a tracked site is NOT new (Z15/F13) · imports and blank lines are NOT new (Z16/F14) · **a genuinely new site still FAILS (Z17/F15)** · a second write on the same table in the same function is a distinct `@2` (Z18) · **renaming the enclosing function DOES re-key it (Z19)** — a real change of identity, stated in the helper header rather than implied away. **LIVE PROOF ON THE REAL REPO:** three comment lines inserted at the top of `Dashboard.tsx` — *the exact edit that produced two false NEWs twice yesterday* — and both caps report RATCHET CLEAN.

**🔴 TWO SELF-CAUGHT DEFECTS IN THE FIX, both before it was locked in.** **(1)** The first run keyed **38 of 120 sites as `::if#` or `::for#`** — the bare-method pattern matched control-flow keywords, so the key named the wrong thing and hid the function the site is actually in. Line-stable, and still wrong: **a key a human cannot read is a key that gets re-baselined without being read**, which is the exact habit this closes. Keywords skipped; counts re-proven afterwards, unchanged. **(2)** field-lists matches ON `.from('x')`, so a backward window from `m.index` sits BEFORE the table name and resolved `?` for **every** site — **caught by probes F13-F15 going 0-for-3**, fixed with `m.index + m[0].length`, with the asymmetry against the write cap commented at the line so it is not re-introduced.

**FLAGGED FOR DAVID:** **(a)** **the cards are yours and none are new** — **card 20 as STAFF and only as STAFF** (Contractors on `pricing_recipe:update`, Follow-Up on `customers:update`; staff hold neither and both tiles must still render — as owner it passes either way, which is exactly how it survived), and **card 19 beside card 12**: amber SOON where a red lock used to be, a Business insights panel that has never appeared on any session ever, and BEING BUILT next to a 🔒 NOT PERMITTED on one screen. **(b)** **two findings recorded because they are better evidence than the fixes:** until 2026-07-31 the **ONLY tiles ever rendering a red lock were the UNBUILT ones** — unheld tiles were filtered out of the grid entirely, so *"you are not allowed"* was shown exclusively about things nobody is allowed, because they do not exist. **The six-state model's whole thesis, inverted, in a grid, for eight days.** And `business_insights` carried `status:'planned'` from the day it was registered while rendering nothing on any session ever — **the fourth instance this week of a DECLARED STATE WITH NO READER** (after `<BeingBuilt>` unmounted, `view_costs` gating a dark metric, and `import_pricing`'s stale comment). **(c)** **#87 and #90 stay filed**; the requirement hook is still unbuilt.
