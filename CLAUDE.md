...
# Last updated: 2026-08-23 (7) — scoping: the count session (recon only, nothing built). See §3.
...
### 2026-08-23 (7) — THUNDER **SCOPING: THE COUNT SESSION. LAUREN'S QUESTION HAS AN ANSWER TODAY AND IT IS NEITHER OF THE TWO SHE OFFERED — THE APP STOPS AND MAKES HER ARBITRATE, TWICE, AND SHE FINISHES ON 5.** She counts 3, finds 6 more down the row, then 5 more, and asked whether TRACE updates the number or adds 3+6+5. **It does neither.** `InventoryCount.tsx:384-390` keys a within-session guard on (variety × size); the second entry never reaches the write — it opens a CONFLICT sheet reading *"3 first count → 6 now · The difference could be a sale, a miscount, or a tree that moved. Which count holds?"* with exactly two buttons, **replace or keep**, and **no third option**. Each replacement fires `count_reconcile_inventory` afresh, so her walk ends **`qty = 5`**, three `inventory_counts` rows, three immutable ledger rows, and a summary reading *"3 items counted"* for one block. 🔴 **THE PLATFORM IS ASSERTION-CORRECT AND WORKFLOW-WRONG:** the prompt's principle is genuinely enforced — `20260720:528-531` computes `v_delta := p_counted_qty − v_current` and SETs, **the delta is never accepted from the client**, and `:510-513` refuses a negative *"a count asserts physical truth"* — but it models three passes over one block as three disagreements about one number. **RECON + ESTIMATE ONLY. NOTHING BUILT** — no app code, no schema, no migration, no cap. ONE document: `docs/audits/count-session-multi-pass-recon-2026-08-23.md`. `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · GATE 0 **NOT APPLICABLE** (nothing deployed) · all `[TRACE:*]` ON.

**Type:** Scoping recon + close-out. Docs: ledger **#198** · `RULINGS.md` (**THREE NEW OWED rows** R-A/R-B/R-C) · ⚡ ACTIVE STATUS · `built-inventory.md` (5.1 / 4.2) · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 (4) #195), 1 written — entries-in == entries-out, §3 back to 3** (archive 228 → 229). ✏️ **CLAUDE.md read IN FULL (four chunks) and `docs/RULINGS.md` read IN FULL (145 lines, five chunks incl. the whole OWED table).**

**🔴 THE STORY GATE FIRES *CONFLICT*, AND IT IS THE HEADLINE — A WRITTEN STORY ALREADY ANSWERS LAUREN AND THE CODE CONTRADICTS IT.** `user_stories.md:424` — *"Inventory — the real spoken-count spec (Billy Bob + the messy walk)"*, **`STATUS: written`**, `MAPS-TO: 2.3` — lists as requirements **"running tallies"**, **"self-correction (last value wins)"**, and 🔴 **"location-spanning tallies (same variety here + in the greenhouse, SUMMED)."** **Her three clumps down one row are that third clause**, and the platform built the SECOND one and refuses the third. Per §9 this is STOP-and-surface, surfaced: the build needs **no new story**, it needs David to confirm the summation clause governs, and then the spec cites `:424`. ⚠️ **The adjacent story `:349` is NOT the parent and is named so nobody cites it** — *"scan the same tag, tap 45 gal, enter 12 — a SECOND stock row is born"* is the **different-size** case, and the platform already distinguishes the two correctly by `countKey`. **Only the same-size branch is wrong.**

**#67 AND #68 BOTH CONFIRMED STILL TRUE AGAINST THE CODE, with one mechanism sharpened and one made exact.** **#67:** the capture-time RPC is real (`InventoryCount.tsx:438-440`), and `blind_capture_mode` has **zero hits in any `.ts`/`.tsx`/`.sql`** — it exists only in `user_stories.md:392`'s `PIECES:`. ✏️ **But "residual 0 by construction" is mechanically off in a way that matters for the build:** the desk's `Counted` column is an **EMPTY input** (`InventoryReconcile.tsx:270-279`, `placeholder="—"`), so the residual is not cancelled — **the walk is simply never presented for review**, surviving only as `prior` (`:165-167`) used to open the ledger window. **#68:** per-lot across the catalog with **no session filter anywhere**, and its `status` clause is **exactly right, verified by grep** — three write sites, all in ONE file (`:230`/`:690`/`:701-703`); **`InventoryReconcile.tsx` never touches the sessions table**, so the second-writer hazard is one the build would **CREATE**, not one standing today.

**🔴 THE STRUCTURAL FINDING THAT SIZES THE WHOLE THING: THE DATA MODEL ALREADY PERMITS LAUREN'S THREE ENTRIES — WHAT REFUSES THEM IS THE CLIENT.** No unique constraint on `(session_id, inventory_id)` (`20260626:66-81`); `recordCount:680-683` is a plain INSERT; the conflict re-save already writes a second row. **So MINIMUM needs ZERO migrations.** Paired with the second: **blind capture needs NO new RPC** — it is the *absence* of the capture-time call, and `count_reconcile_inventory` already takes `p_source_id` (`20260720:496`) so a desk applier can link back with **no signature change**.

**THREE SCOPES — MINIMUM ~3 prompts / 0 migrations / 1 ruling · COHERENT ~9–10 all-in / 1–2 migrations / +2 rulings · COMPLETE ~12–13 all-in / 2–3 migrations / 3 rulings.** MINIMUM's honest residual is stated rather than buried: the count still applies itself (#67 untouched), and **"remove a line" only means remove-before-apply on this phone** — `sessionCounts` is React state (`:175`), so **a session resumed after a phone lock has forgotten her three passes.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **THREE NEW OWED RULINGS, and R-A blocks even MINIMUM: when three passes make one count, does the LEDGER get ONE row of 14 or THREE rows of 3/9/14?** Three rows put two assertions on an immutable log **that were never physically true at the moment they are dated** — #70's genesis shape arriving through a UI. **R-B:** where blind-capture mode lives (a session flag is a MIGRATION; *"per-session, default blind"* is a `NEEDS:` tag, not a ruling) — and the real subject is **the qty prefill at `:341-342`, which shows her the book number before she types hers: an anchoring bug or a feature, and nobody has said which.** **R-C:** does the desk applier write `status` or a new field — **#71 is the live precedent**, and `verify-write-paths` presents the debt either way. **(b)** 🔴 **FOUR LIVE FINDINGS OUTSIDE THE TEN QUESTIONS, recorded UNFIXED:** the conflict sheet's **"first count" label becomes FALSE after the first recount** (`prevQty` reads `sessionCounts` which `:564` overwrites — on her third pass it says *"first count 6"*; **§6 r18's class, in the sheet built to help a human choose between two numbers**); `item_count` inflates on any recount (`:691`); the DONE summary lists one lot three times; and **`'abandoned'` is a declared session state with NO writer anywhere** — harmless until reconcile filters by session, which is #68. **(c)** ✏️ **A CORRECTION ABOUT MY OWN METHOD, AND IT IS THE MORE USEFUL FINDING: my STEP 0 `git status` WENT STALE UNDER ME MID-RECON AND I ALMOST FILED IT AS A DEFECT.** I read HEAD `b3fa36f` with `generate.ts` uncommitted and drafted *"an uncommitted fix on `main` nothing will catch"*; by write-up the tree was clean at **`1e4a8aa`** — it had committed as **`c020560`** (ledger #197) **while this recon ran.** The work was IN FLIGHT, not abandoned. **That is `RULINGS.md:133`'s class — a claim about state sourced from a snapshot rather than the authority — arriving on GIT STATE instead of migration-apply state (#92's other face).** Caught by re-running `git log` at write-up; **cheap, and nearly not done.** ⚠️ Blast radius checked not assumed: `c020560` touches only `generate.ts`, so **no `file:line` in the recon is affected.** **(d)** ✅ **Q9 has a genuinely good half worth saying plainly:** the RPC path **is** protected — `FOR UPDATE` + `applied:false/lot_not_found` (`20260720:518-526`) **and `syncEngine.ts:210-214` actually reads the refusal.** The unchecked half is `syncEngine.ts:219`, already **baselined debt** (`zero-row-writes-baseline.json:82`); the one that bites is the `variant_group` backfill (`:456-463`), whose silent no-op sends the family UNKNOWN on the next scan. **(e)** **CLAUDE.md is 712 lines against its own ~600 budget** — it GREW this session despite the archive, which is the §4 residual OP-13 explicitly says N=3 alone does not close.

### 2026-08-23 (6) — THUNDER **THE LINE THAT WAS OWED THREE SESSIONS IS IN, AND IT WORKS BY CORRECTING THE WORD THAT CAUSED THE PROBLEM RATHER THAN BY SITTING NEXT TO IT.** I found this twice and refused to fix it twice, both times inside docs-only tasks, both times correctly — and it shipped in the session that was allowed to ship it. **The defect was never a missing sentence; it was that `SYSTEM_PROMPT` asked for posts that are *"warm, local, **specific**"* and supplied no facts to be specific about, which is an instruction to invent some.** So the addition reconciles that word instead of arriving beside it: ***"Specific means grounded in the facts you were given, never invented: no statistic, percentage, dollar figure, date, award, certification or comparative claim unless it appears in the supplied data. If a number would strengthen a post and you do not have one, write the sentence without it — a fabricated figure publishes under the name of the business, and the owner carries the liability."*** **ONE placement, in the SYSTEM prompt, because it governs BOTH prompts in that call** — `executeCapability('campaign_generate', {system, user})` sends them together and the user prompt is the other half that repeats *"specific"*. Story: *Truth in advertising — suggest facts, never censor, keep the record*. **ONE app-code change + three ARC tags + one ruling.** `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · api/ **12/12** · 🔴 **GATE 0 APPLIES and IS PROVEN — from the DEPLOYED BUNDLE, not the dashboard: `index-Nn14LRrJ.js` → `index-B4pdsrLq.js`, and the new bundle CONTAINS `c020560`.** ⚠️ That proves the DEPLOYMENT is live and the server function rode with it; **it cannot prove the model's behaviour — that is CARD 1 and only David can run it** · all `[TRACE:*]` ON.

**Type:** 1 app-code change + 2 doc items + close-out. Docs: ledger **#197** · `RULINGS.md` (**NEW RULED row + one OWED row half-closed**) · **NEW `docs/owner-tests/social-campaign-full-surface-test.md`** · ⚡ ACTIVE STATUS · `built-inventory.md` (both generators) · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 (3) #194), 1 written — entries-in == entries-out, §3 back to 3.** ✏️ **CLAUDE.md (707 lines, 4 chunks) and `docs/RULINGS.md` (145 lines, 5 chunks) were BOTH read IN FULL this session**, as instructed.

**🔴 THE FINDING THE FIX PRODUCED: THERE IS NO SINGLE PLACE THAT GOVERNS BOTH GENERATORS, AND THE SECOND ONE IS STILL OPEN.** `executeCapability` passes `opts.system` **straight through** (`ai/execute.ts:26-29`) — **no shared preamble seam exists**, so a platform-wide instruction has nowhere to live today. `packages/shared/src/social/generate.ts:47-52` — the WEEKLY generator, 4.3's feeder — still asks for *"Write warmly and **specifically** — reference what actually happened"* with no anti-fabrication clause. ✏️ **And the sharp part: that prompt ALREADY CARRIES THREE PROHIBITIONS** (*"Never mention prices. Never include customer last names, emails, or contact info."*) — **it is demonstrably a place where prohibitions live, so the absence is conspicuous rather than accidental.** **NOT FIXED: scope was ONE app-code change, and its prompt is a LIST where the campaign one is PROSE, so the wording is a judgement to make deliberately rather than a paste.** Recorded as **CARD 2 `needs-test`**.

**⚠️ THE HONEST LIMIT, AND THE TEST I DELIBERATELY DID NOT WRITE.** `npm run verify` **cannot test a prompt change** — no probe can assert that a model stopped inventing numbers. **A test asserting the instruction string is present would assert a CONFIGURATION and call it covered, which is STD-025's exact shape**, so none was written and **the reasoning is recorded in a comment above the constant** so nobody adds one later believing its absence was an oversight. The surface had **no owner-test board at all**, which was itself drift — **NEW board, GATE 0 + 2 cards, 0 of 2.** CARD 1 is the entire control: generate a week on Test Dave's Tree Nest **with the Context box EMPTY**, read every line, and **one invented number is a FAIL — not a rate.** **Until David runs it, the fix is a claim.**

**🔴 STEP 3 — THE OUTBOUND RULING, AND ITS WORDING WAS CORRECTED BEFORE FILING.** *"Nothing leaves the building on its own"* would have forbidden the **Blotato publish path, which exists and is correct** — keyed on an explicit `postId` from a button. **The rule is about INITIATION, not about whether TRACE is the mechanism**, and it is filed platform-wide. ✏️ **Its relation to *"surface, don't decide"* was CHECKED rather than assumed: SIBLING, not instance.** That one governs TRACE vs the owner's **JUDGEMENT** inside the product; this governs TRACE vs the owner's **VOICE AND REACH** toward third parties — and each is reachable while the other is violated (a system can surface-not-decide perfectly and still auto-send; it can never-initiate and still refuse to draft). **Two children of one parent.** **THE COUNT: SEVEN outbound-capable paths, ZERO that initiate without a human** — order confirmation on the Pay click · the silent-partner analysis behind a button needing a typed recipient (generation and sending are separate acts) · QBO invoice on Pay · QBO token refresh · Blotato (inert) · two `mailto:` where **TRACE is not even the mechanism** · SMS PIN reset (stubbed). 🔴 **And the structural fact under all of it: `vercel.json` has NO `crons` key — nothing fires on a schedule, so nothing COULD initiate today, which is exactly why filing this now is cheap.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **TWO EDGES THE RULING'S WORDING SHOULD BE READ AGAINST, both found by the sweep and neither invented: `submit.ts:954`'s owner leakage alert fires with NO human naming that moment — and is OUT OF SCOPE BY THE RULE'S OWN WORDS, because its recipient is `businesses.phone`/`.email`, INSIDE the business.** It is the path most likely to be misread as a violation later, which is why it is named. **The QBO token refresh is the other:** it reaches an external system unprompted, is not an act on anyone's behalf, and **the sentence as written does not say so.** **(b)** **The three ARC tags landed and were verified through the renderer, not assumed** — `arcTag` set, `sectionArc` null, resolved arc = the tag, **the story's own ARC winning over `## NEEDED` exactly as the header promises**; Unfiled drops 55 → 52. **(c)** ✅ **The second generator is the one thing I would fix next and did not** — one line, same class, and it is CARD 2 rather than a silent sweep-in. **(d)** **CLAUDE.md is 707 lines against its own ~600 budget.**

### 2026-08-23 (5) — THUNDER **LAUREN CORRECTED THE MODEL, AND THE CORRECTION IS THE WHOLE STORY: A BLOCK OF TREES GOES ROUND A LOOP, NOT DOWN A PIPELINE.** David had them repotting every six months; she said *"I had that backwards. We don't repot them every six months. It takes six to eight months to grow into their pots, and then they can live in their pots for say a year."* So: potted → **six to eight months UNDER PRODUCTION, not for sale at any price** → sellable, holding ~a year → **up a pot whether or not anyone bought it**, and the loop starts again one size bigger. 15→30→45→65→95 gal, eighteen to twenty months a rung, several years to the top. **Three stories added from that on-site (2026-08-22) and David's worked examples to her — the ladder is the PARENT and both others depend on its schedule, which is why they land together:** *The growing ladder — potted, waiting, ready, and up a size* · *What's in it when it sells — cost that accumulates over years* · *Tell me before I run out — reorder from what I actually use*. **Everything derives from one date and two numbers per rung** (*"a typed ready-for-purchase date would just be a second answer that can disagree with the first"* — STD-011 stated in the owner's terms), and **a blank interval means UNKNOWN and STOPS the schedule** — *"exactly as an unknown cost is never counted as zero"*, which is A9 arriving on the maturity clock. **DOCS ONLY — no app code, no schema, no migration, no cap; no lot-status value added; `stories.html` NOT edited; nothing deleted or retitled.** `npm run verify` exit 0 ZERO NET-NEW (5 / 247 / 10 / 12 / 15) · api/ **12/12** · **zero diff under `packages/`/`api/`/`supabase/`** · GATE 0 **NOT APPLICABLE** · all `[TRACE:*]` ON.

**Type:** Three additions + close-out. `user_stories.md` (88 → **91** stories). Docs: ledger **#196** · `RULINGS.md` (**THREE NEW OWED rows**) · ⚡ ACTIVE STATUS · `built-inventory.md` (5.1) · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 (2) #193), 1 written — entries-in == entries-out, §3 back to 3.** ✏️ **CLAUDE.md and `docs/RULINGS.md` were both read IN FULL this session** — four chunks and four chunks; last session I read only the changed regions and said so, and the honesty was right but the shortcut was not.

**🔴 STEP 5 — THE STORY'S CLAIM IS CONFIRMED, AND IT UNDERSTATES ITS OWN MECHANISM.** The entry says an under-production block *"would compute as available, because anything on hand and uncommitted computes as sellable."* True: `availableFrom(onHand, committed)` is `max(0, onHand − committed)` (`inventoryStates.ts:62-64`) — **two inputs, and maturity is not one of them** — and `checkSellable` (`:169-200`) takes exactly four (`onHand`/`committed`/`status`/`sellPrice`) with exactly three refusal reasons: `condition` · `no_price` · `none_available`. **There is no fourth, and none of the four inputs can express *not yet mature*.** **But it is not a fall-through — it is an ACTIVE ASSERTION:** `deriveStatus(onHand)` (`:141-143`) returns `'available'` for any qty > 0 and `DERIVED_STATUSES` is `['available','depleted']` (`:138`), so the platform **computes and writes `status:'available'`** onto forty trees that cannot leave for six months. **The prose is left exactly as David wrote it — it is correct, and the refinement belongs in the ledger, not in his sentence.**

**🔴 AND THE CHECK ANSWERED HALF OF THE STORY'S OWN `NEEDS:` QUESTION AT CODE LEVEL — WHICH IS EVIDENCE, NOT A RULING, SO IT IS FILED AND NOT ACTED ON.** The question is whether UNDER PRODUCTION is *a new value in the lot-status vocabulary* or *a derived state*. The only existing escape hatch is `MANUAL_CONDITION_STATUSES = ['damaged','returned','archived']` (`:131`) — and **a `status` value for UNDER PRODUCTION would be a THIRD author on the column tech-debt #71 already has two authors on.** The file says so about itself (`:133-137`): derived values are *"Never manually settable: the next qty write recomputes them"* — and **#71 records that D-42's qty-derive already reverts a manual `archived`.** So a status-modelled under-production would be **silently cleared by the next qty write**, while a clock derived from (potted date + rung maturation) cannot be. ✏️ One build-relevant detail, not a defect: the refusal order is condition → price → quantity (`:166-168`), so a condition-modelled under-production would refuse *before* the price check.

**FLAGGED FOR DAVID:** **(a)** ⚠️ **THE `MAPS-TO` CALL IS CLOSER THIS TIME AND I AM SAYING SO RATHER THAN REPEATING LAST WEEK'S ANSWER AS A FORMALITY.** **`5.1 Inventory management`** (`TRACE-SESSION-BOOTSTRAP.md:418`) is a **genuine PARTIAL parent for the ladder's STATE half** — `under_production_state` would live on `business_inventory` rows, 5.1's own table and grid, and the ladder changes what every availability read reports. **It is no parent at all for the SCHEDULE half:** the rotation date, the maturation clock and the up-pot schedule have no board row anywhere. **`2.3 Walk-and-count`** (`:407`) is NOT a parent — it is the capture loop, and the ladder is not a counting act. **Left `—` on all three per instruction; this one is a real judgement for you, not a default.** **(b)** ✏️ **ALL THREE ARE ARC-SHAPED AND ALL THREE RENDER "Unfiled".** Omitting `ARC:` is legal for a `## NEEDED` cross-cutting item and nothing is malformed — but the ladder reads as `asset-inventory-pmi`, the cost story as `cost-to-produce`, and the consumables story as `suggestion` (*"pattern-surfacing from owned data"* is that arc's own description). **Filed as written; your editorial call.** **(c)** 🔴 **THREE NEW OWED ROWS, and one of them is an outbound-action boundary worth reading before it is built:** the consumables story asserts *"TRACE prepares; Lauren sends. Nothing leaves the building on its own"* — **that is the story's assertion, not a ruling**, and it is exactly the kind of boundary that should be decided rather than inherited from a sentence. **(d)** **Joel owes the numbers** — container sizes in order, which are never sold, maturation + hold per rung — and the ladder cannot be built without them; a blank interval is UNKNOWN by design and stops the schedule. **(e)** ✅ **PURE ADDITION, VERIFIED: 3 additions (113 lines) · 0 modifications · 0 deletions**, all **six** prior campaign entries byte-untouched, 88 → 91 stories, 91 parsing under `stories.html`'s own extracted parser. **(f)** 🔴 **The generator's system prompt is STILL untouched — owed three sessions running**, deliberately, for the same reason each time. **(g)** **CLAUDE.md is 709 lines against its own ~600 budget.**
