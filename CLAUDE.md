...
# Last updated: 2026-08-29 (4) — the LAWNS discovery filed; a held push stopped saying FAILED (R-26) — see §3
...
### 2026-08-29 (4) — THUNDER **THE DISCOVERY IS FILED, THE HELD PUSH STOPS SAYING "FAILED", AND THE PATTERN BECOMES A RULING. R-26 FILED.** 🔴 **THE HEADLINE IS THAT THE MITIGATION WAS HALF-SHIPPED AND THE MISSING HALF WAS THE HONEST ONE.** `66d0aaf` landed the server guard — `pushQboInvoice` returns **409 `PUSH_HELD`** — and nothing on the client knew that code, so it fell to `else { qbStatus = 'failed' }` and rendered *"QuickBooks invoice FAILED — it was not created … fix the issue above, then push the invoice again from the order page."* **Wrong twice, and both are the reason the hold exists:** nothing failed — the order completed and is correct — and the instructed remedy is a re-push **the same guard holds again**. ✅ **`'held'` IS A FOURTH STATE, NEVER A REUSE OF `not_connected`** — that would tell an owner to reconnect a connected QuickBooks, and `Confirmation.tsx:110` already records that exact shape (*"wrong twice: QBO was connected"*). All four now render on both sides of the ownerView split; the customer sees the same neutral copy the other two non-success states already show. ✅ **THE DISCOVERY DOC IS FILED** — `docs/discovery/2026-08-27-29-lawns-discovery.md`, **638 lines**, pointer at §10 check 9, **two sections CORRECTED AT FILING and marked in place**. ✅ **R-26** — *a claim must name what was opened to produce it* — **eleven instances in three days.** ✅ **tech-debt #124** — the owed third policy query, run. **BUILDER-COMPLETE** `61d84af`→this commit · `npm run verify` **exit 0 ZERO NET-NEW** · **47/47 files · 1971 assertions** · **ZERO schema, ZERO migration** · api/ untouched · all `[TRACE:*]` ON.

**Type:** Fix + docs + close-out. Code: `api/orders/submit.ts` (409/PUSH_HELD branch) · `hooks/useSubmitOrder.ts` (`QbSyncStatus` + `'held'`) · `pages/Confirmation.tsx` (the held badge) — `848ede6`. Docs: **`docs/discovery/2026-08-27-29-lawns-discovery.md` (NEW, 638 lines)** · **`docs/RULINGS.md` R-26 FILED** · **`docs/tech-debt-log.md` #124 FILED** · ledger **#232** · CLAUDE.md §10 (**check 9 added, "all nine" → "all ten"**) · ⚡ ACTIVE STATUS (**4 new lines**) · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-29 #229, 9,093 chars), 1 written — entries-in == entries-out, §3 back to 3** (archive 257 → 258).

**🔴 THE FINDING WORTH MORE THAN THE WORK, AND IT IS ABOUT MY OWN EDIT RATHER THAN ANYONE'S CODE: I ASSERTED ON NINE OF TEN EDITS, AND THE ONE I DID NOT ASSERT ON IS THE ONE THAT SILENTLY FAILED.** My `push_held` change to `router.ts` used `isPushHeld` **without importing it** — the import insertion no-op'd because a concurrent worktree had rewritten the import block I was pattern-matching on, and that single `s.replace()` carried no assertion while every other edit in the same script did. **`esbuild` bundled it clean**, because esbuild does not type-check — the `api/` blind spot the verify chain names as tech-debt #77 — so it would have thrown `isPushHeld is not defined` on the live status endpoint. ✏️ **It was found only because I refused to re-baseline an unexplained `+2`**, chased it, and discovered the tree had changed underneath me. **The unexplained number was the thread; pulling it found a defect that had nothing to do with the number.**

**🔴 AND THE SECOND: A CONCURRENT WORKTREE HAD BEEN BUILDING IN MY WORKING DIRECTORY, AND THE RIGHT MOVE WAS TO STOP RATHER THAN COMMIT.** Since `bb622ff`, uncommitted: my `itemList.ts` generalised into `qboRead.ts` + `customerList.ts`, `QboItemListReader.tsx` renamed and doubled to 408 lines, a `/api/qbo/customers` route. **The `+2` eslint was theirs** — `QboBooksReader.tsx:171`, two unused args — and `npm run quality:baseline` would have locked someone else's in-progress violations into the ratchet **as accepted debt, under my name.** ✏️ **The reflexive re-baseline is the specific failure the ratchet exists to prevent**, and it is the third time in four sessions that refusing one surfaced something real. That session then committed the hold's server half at `66d0aaf` under an honest **"⚠️ NOT WRITTEN BY THIS SESSION"** header, which is the behaviour that made this reconstructable at all.

**FLAGGED FOR DAVID:** **(a)** 🔴 **THE HOLD IS UN-OWNER-PROVEN AND IT IS THE ONLY THING GUARDING A REAL COMPANY'S BOOKS. Order of operations matters:** set `QBO_PUSH_HOLD` to the LAWNS business id → **redeploy** (an env change does not take effect without one) → **confirm `push_held: true` on `/api/qbo/status` BEFORE trusting it** — the switch **fails open by design**, and that is only honest because it is readable. Then complete ONE checkout **on Test Dave's**: the screen must read *"Order saved — invoice NOT sent to QuickBooks (sending is paused)"*, **not "FAILED"**, and no invoice may appear in QuickBooks. **(b)** 🔴 **TWO CATALOG QUERIES ARE YOURS AND BOTH CHANGE WHAT GETS BUILT.** The `inventory_counts` / `inventory_count_sessions` policies — **the answer gets written INTO §7.3, and it cancels or confirms build #2**; and the full `pg_policies` sweep behind tech-debt #124. **I measured both from the MIGRATION CORPUS, which is not the catalog** — the limit that produced instances #2, #3 and #9 of the very pattern R-26 names. **(c)** 🔴 **tech-debt #124 IS THE MIRROR OF PASS 2 AND THAT IS WHY IT MATTERS:** that pass spent a whole build widening three surfaces that were too NARROW, while **eighteen tables sat too WIDE in the same schema at the same time** — `business_inventory` lets any active member including STAFF insert, update and delete. **We were measuring one direction of a two-directional problem.** Filed, not fixed, per your instruction. **(d)** ⚠️ **`#122` AND `#123` ARE CITED BY NUMBER ACROSS SEVERAL DOCUMENTS AND DO NOT EXIST IN `docs/tech-debt-log.md`** — the same drift as rows #85–#90, which is still open from #228. **(e)** ⚠️ **§11.3 OF THE DISCOVERY DOC — "decisions owed from David" — WAS TRUNCATED IN TRANSMISSION** mid-row. I marked the section INCOMPLETE and reconstructed a table from decisions named elsewhere in the document rather than inventing the original; **it is labelled as reconstructed and it is yours to restore.** **(f)** 🔴 **CLAUDE.md IS NOW EXACTLY 600 LINES — AT its ~600 budget, not under it.** The Session Starter check I added cost 10. The next addition crosses it, and the structural residual named in §4 (the §2 infra tables, ~155 lines that should be pointers) is still open. **(g)** ⚠️ **STILL OPEN FROM EARLIER PASSES:** `docs/inventory-functions.md` / `inventory-env.md` / `inventory-ai.md` stamped `2026-06-13` — and `inventory-env.md` **is instance #9** of R-26's own list; `TRACE-SESSION-BOOTSTRAP.md:54` still says *"No per-branch previews"*, **instance #6, uncorrected**; `docs/reference/` still untracked.

### 2026-08-29 (3) — THUNDER **READ: THE INVOICE HISTORY — THE THIRD ENTITY, AND THE ONE THE OTHER TWO WERE A DETOUR AROUND. R-25 FILED.** 🔴 **THE HEADLINE IS DAVID'S OWN, AND IT IS ABOUT SEQUENCING RATHER THAN CODE: *"THIS WAS THE OBJECTIVE FROM THE START AND I SCOPED IT OUT TWICE."*** An item row says a thing EXISTS; a customer row says a person EXISTS; **neither says what was SOLD.** The invoice carries the items, the quantities, the prices and the buyer on ONE record, which makes it the only place in LAWNS's books that can answer Terry's *"how many trees did we plant last year."* ✅ **`GET /api/qbo/invoices` RIDES THE SAME ROUTER, GATE, CLIENT, WALK AND COMPLETENESS REFUSAL** — three entities, one implementation with one word changed (§6 r8). **ZERO new Vercel functions (12/12) · ZERO writes to Intuit · ZERO schema · ZERO storage.** ✅ **THE DATE RANGE IS REPORTED FIRST, ABOVE EVERY OTHER NUMBER, AND THAT IS A RULE NOT A LAYOUT** — *"412 Shumard oaks"* is a different fact over ten years than over eight months. Min/max `TxnDate`, months spanned, by year, and a per-month curve that **emits the EMPTY months as zeros**, because the empty months ARE the seasonality answer. Dates are computed by **slicing the string, never `new Date()`** — `new Date('2025-01-01').getMonth()` is DECEMBER west of Greenwich, and a curve built that way moves every first-of-month back a month and looks entirely plausible. ✅ **NOT ONE INVOICE RECORD REACHES THE SCREEN AND THERE IS NO PREVIEW.** The customer read WITHHOLDS its records; this one **cannot produce them** — `QboInvoiceRow` has `customerId` and **no name field at all**, and free-text line descriptions are not carried, so the buyer's name is dropped **at the parse** rather than withheld at the render (R-24b/c). ✅ **WHETHER PLACEMENT IS DISCOUNTED IS MEASURED FROM THEIR HISTORY, NOT CHOSEN BY US (R-25a)** — per discount item, how many bases EQUALLED the invoice subtotal vs fell BELOW it, **with the excluded item NAMED**. ✅ **VOLUME IS A REFUSAL, NOT A SLOW READ (R-25b)** — above 10,000 invoices it stops before pulling a page and reports the real number. **BUILDER-COMPLETE** `24ae201`→this commit · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **47/47 files · 1971 assertions** (+1 file, +102) · **ZERO schema, ZERO migration** · api/ **12/12 untouched** · **GATE 0 + CARDS 12·13·14 (new) OWED — 0 of 14** · all `[TRACE:*]` ON.

**Type:** Build + close-out. Code: `packages/shared/src/quickbooks/invoiceList.ts` (**NEW** — `parseInvoiceList` · `summariseInvoices` · `DISCOUNT_ITEM_NAMES` · `BUNDLE_ITEM_NAMES`) · `invoiceList.test.ts` (**NEW, 83 probes**) · `qboRead.ts` (**`QBO_ENTITIES`** · **`QBO_ROUTE`** · **`QBO_WALK_CEILING` / `maxPagesFor` / `ceilingCheck`**) · `qboRead.test.ts` (+19) · `api/qbo/router.ts` (the ceiling refusal inside the SHARED walk · `handleInvoices` · `case 'invoices'`) · `QboBooksReader.tsx` (third button + the invoice summary) · `vercel.json`. **13 MUTANTS MEASURED, ALL CAUGHT — 2·1·3·3·1·1·3·1·5 · 2·3·1·2.** Docs: **`docs/RULINGS.md` R-25 FILED + R-23's and R-24's guard cells STRENGTHENED** · ledger **#231** · `built-inventory.md` (**new §4.1b + the #230 entry's "two entities" scope marked superseded at the site**) · owner-tests **CARDS 12·13·14 added, GATE 0 ③ gained this build's signal, board 0 of 14** · `user_stories.md` (**1 FALSE PREMISE corrected**) · DECISIONS-INDEX drift · ⚡ ACTIVE STATUS (**1 line added, 1 rewritten**) · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-28 (3) #228), 1 written — entries-in == entries-out, §3 back to 3** (archive 258 → 259).

**🔴 THE FINDING WORTH MORE THAN THE BUILD, AND IT IS THAT A SENTENCE ON THE STORY BOARD IS WHY DAVID SCOPED THIS OUT TWICE.** `user_stories.md` said the invoice read *"is only worth reading once the ItemRefs mean something"* — and that ordering is **backwards.** Reading the history is what TELLS us what the ItemRefs should mean: which items the trees actually sold as, how much installation is hiding inside a $0 bundle line, and what the discounts were computed on. **The read was the mapping's INPUT, not its reward.** ✏️ **The uncomfortable part is that the sentence worked exactly as written.** Two sessions in a row — mine included — treated items and customers as the prerequisites and the invoice history as the thing that comes after, and neither of us questioned it, because a written premise on a canonical board does not read as a claim, it reads as context. **This platform already has a name for the class** (§9's IN-CODE-NOT-ON-BOARD case, and #229's own *"a false premise the board did not know it held"*), but both prior instances were the board being **behind the code**. This one is different and worse: the board was not stale, it was **wrong about a judgement**, and being wrong about a judgement is invisible to every reconciliation gate we have — a date check cannot catch it, and nothing else reads prose. Corrected in this commit, with the correction stated rather than silently overwritten.

**🔴 AND A SECOND FINDING, WHICH IS THAT ADDING A THIRD ENTITY FOUND TWO COPIES OF ONE TERNARY, AND THE SECOND ONE WOULD HAVE FETCHED THE WRONG ENDPOINT.** `entity === 'Customer' ? 'customers' : 'items'` appeared in `rawCaptureFileName` **and again** in the client's URL builder. Under the first, an invoice capture files itself as **`qbo-items-…`** — a customer's billing history mislabelled as a product catalogue in a downloads folder. Under the second, pressing *Read invoice history* would have **called `/api/qbo/items`** and painted an item list under an invoice heading: a screen confidently answering a question nobody asked, with no error anywhere. **Both are now one exported `QBO_ROUTE` map keyed on the union**, so omitting an entity is a compile error and the file word and the route word cannot disagree. ✏️ The generalisable bit: a two-value ternary is **indistinguishable from a correct exhaustive map until the third value exists**, which means the defect is not written when the ternary is — it is written when somebody adds the third case and the ternary silently keeps working. A `Record<Union, T>` fails at that exact moment instead. I also made the read-only write-verb sweep **derive** its entity list from `QBO_ENTITIES` rather than a hand-typed array, for the same reason.

**FLAGGED FOR DAVID:** **(a)** 🔴 **READ THE DATE RANGE FIRST AND SAY THE TWO DATES OUT LOUD — that is the question this build exists to answer.** Does the history start at or near **2025-08-23** (the day 1,163 of their 1,936 customers arrived in one migration), or does it genuinely run further back? **Either answer is fine. Not knowing is the thing we were fixing.** CARD 12. **(b)** 🔴 **THE DISCOUNT TABLE'S LAST COLUMN IS THE ONE THAT DECIDES A BUILD.** If *Base = subtotal* dominates, placement is inside the discounted base in their practice. If *Base below* dominates and the **Excluded from the base** column names a placement item, it is not. **I did not pick a default; the table reports what their invoices did** (R-25a). CARD 13. **(c)** ⚠️ **THE SEVEN DISCOUNT NAMES ARE YOUR LIST, AND THE BUILD DOES NOT ASSUME IT IS COMPLETE.** A discount-shaped line outside it — Intuit's own discount line type, a negative amount, or "discount" in the name — shows up in an **amber box** as its own row. **If that box appears, read out what is in it**; it may be a discount item nobody remembered. **(d)** ⚠️ **THE `DIW`/`FDIW` TABLE COUNTS `$0` AND CARRYING-MONEY SEPARATELY, ON PURPOSE.** *"The $0 bundle items"* is a claim about their books; if **Carrying money** is anything but 0, the premise needs revisiting before the mapping pass uses it. **(e)** ⚠️ **THE CAPTURE FILE DOES HOLD CUSTOMER NAMES EVEN THOUGH THE SCREEN NEVER DOES** — it is Intuit's verbatim response. The amber warning now fires for the invoice read as well as the customer one, and **CARD 14 asks you to run `git status --short` and confirm it is NOT in the repo.** Same structural gap as before: a serverless function cannot write to your disk, so that check could not be made mechanical. Delete it when the mapping pass is done. **(f)** ⚠️ **THE 10,000 CEILING IS A HUMAN JUDGEMENT AND NOTHING CHECKS THE NUMBER.** A cap proves the refusal FIRES; no cap proves 10,000 is the right place for it. Almost certainly unreachable on LAWNS — if you ever see that box, it is the guard working. **(g)** 🔴 **THE OTHER WORKTREE'S PUSH-HOLD *CLIENT* HALF IS STILL SITTING UNCOMMITTED IN THIS TREE AND I DID NOT TOUCH IT.** `submit.ts`, `useSubmitOrder.ts` and `Confirmation.tsx` carry the UI for the `PUSH_HELD` state whose server half you committed as `66d0aaf`. Unlike last session, **my work does not depend on it**, so I committed by explicit path and left all three exactly as I found them. They are still uncommitted and still not mine. **(h)** ⚠️ **NOT TOUCHED, AS INSTRUCTED, AND EACH IS STILL TRUE:** the twelve literals · the silent `try/catch` at `router.ts:241-250` · the dead `shared/quickbooks/invoice.ts` + `customer.ts` barrel exports and their thirteenth dormant ItemRef literal. **(i)** ⚠️ **STORAGE IS STILL UNDECIDED AND THE QUESTION IS NOW THE LARGEST IT HAS BEEN** — whether TRACE may hold a customer's item chart, their 1,900-person customer list, **and now their complete billing history**. Nothing is persisted, on purpose, so no default was set in code. **(j)** ⚠️ **`docs/RULINGS.md` IS 193 LINES AND 193 KB, AND ITS OWN HEADER SAYS IT MUST STAY *"short enough that reading it is free."*** It is not free any more — reading it in full is ~50k tokens on every session load, and §10 gate 9 requires exactly that. This is tech-debt #59's disease in the one file the bootstrap says to read completely. Named, not fixed: trimming a rulings file is your call, not mine. **(k)** ⚠️ **`docs/inventory-functions.md` / `inventory-env.md` / `inventory-ai.md` still read `Last updated: 2026-06-13`. `docs/reference/` still untracked.** **(l)** **CLAUDE.md is under its ~600 budget.**

### 2026-08-29 (2) — THUNDER **READ, COMPLETE: THE ITEM LIST PAGINATES AND THE CUSTOMER LIST JOINS IT. R-24 FILED.** 🔴 **THE HEADLINE IS THAT LAST SESSION'S FLAG FIRED, AND THAT A FLAG IS NOT A GUARD.** #229 shipped `select * from Item` with no MAXRESULTS and wrote flag (c): *"one page only, and a truncated list would not announce itself."* It came back **`maxResults: 100, startPosition: 1`, exactly 100 rows, ids running past 1127** — truncated, silent, **and the only reason anybody knew is that David read the ids.** The flag predicted the defect precisely and could do nothing about it. ✅ **SO THE FIX IS A REFUSAL IN CODE, NOT A BETTER NOTE.** `select count(*)` runs **BEFORE** the loop; the loop pages at `MAXRESULTS 1000` with a 1-based `STARTPOSITION`; **retrieved ≠ expected returns `ok:false`/`INCOMPLETE` naming both numbers and the gap** — in BOTH directions, because an overshoot means rows moved under the walk. **An UNREADABLE count is ALSO not-complete** (`parseCount` returns `null`, never `0` — a `0` makes every later check pass trivially). The green agreeing line shows on every successful read too, because *a completeness claim nobody can see is a completeness claim nobody checks.* ✅ **`GET /api/qbo/customers` RIDES THE SAME ROUTER, GATE, CLIENT AND WALK** — `Item` and `Customer` are one implementation with one word changed (§6 r8). **ZERO new Vercel functions (12/12) · ZERO writes to Intuit · ZERO schema · ZERO storage.** ✅ **AND THE CUSTOMER READ IS DELIBERATELY NOT THE ITEM READ.** It is ~1,900 real people belonging to a customer's customers, so **the endpoint does not send the parsed records at all** — only counts, coverage, duplicate sizing and a **five-row** preview; the complete data reaches the browser once, as verbatim bodies written straight to a file. **BUILDER-COMPLETE** `a5a7fdb`→this commit · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **46/46 files · 1869 assertions** (+3 files, +127) · **ZERO schema, ZERO migration** · api/ **12/12 untouched** · **GATE 0 + CARDS 5·6 (re-worded, flipped) + 9·10·11 (new) OWED — 0 of 11** · all `[TRACE:*]` ON.

**Type:** Build + close-out. Code: `packages/shared/src/quickbooks/qboRead.ts` (**NEW** — `qboCountQuery` · `qboPageQuery` · `parseCount` · `parseRows` · `pageIsLast` · **`completeness`** · `rawCaptureFileName` · `classifyFailure`) · `qboRead.test.ts` (**NEW, 63 probes**) · `customerList.ts` (**NEW**) + `customerList.test.ts` (**NEW, 41 probes**) · `itemList.ts` (reduced to the item half; **NEW `summariseItems`**) · `itemList.test.ts` · `api/qbo/router.ts` (`openQboRead` · `qboQuery` · **`readAllPages`** · `completenessOrRefuse` · `handleCustomers` · `case 'customers'`) · `QboItemListReader.tsx` → **`QboBooksReader.tsx`** · `Settings.tsx` · `vercel.json`. **7 MUTANTS MEASURED, ALL CAUGHT — 6·2·2·1·1·3·1.** Docs: **`docs/RULINGS.md` R-24 FILED + R-23's guard cell RE-POINTED** · ledger **#230** · `built-inventory.md` (new §4.1 entry + the #229 entry's *"pagination deliberately not built"* clause superseded) · owner-tests **CARDS 5·6 re-worded and flipped, 9·10·11 added, board 0 of 11** · `user_stories.md` (`PIECES` + both built halves) · DECISIONS-INDEX drift · ⚡ ACTIVE STATUS (**2 lines rewritten/added**) · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-28 #225, 9,308 chars), 1 written — entries-in == entries-out, §3 back to 3** (archive 255 → 256).

**🔴 THE FINDING WORTH MORE THAN THE BUILD, AND IT IS ABOUT MY OWN CLOSE-OUT RATHER THAN THE CODE: I WROTE THE FLAG THAT CAUGHT THIS, AND WRITING IT WAS NOT ENOUGH.** Flag (c) of #229 named the exact failure mode — *a truncated list would not announce itself* — and shipped anyway, because "one page" was the scope bar and the flag felt like the honest way to ship under it. **It was honest and it was insufficient.** The defect it described then happened, and what caught it was not the flag: it was a human reading id numbers and noticing 1127 > 100. ✏️ **The through-line to this platform's own history is exact.** OP-15 exists because *"a rule filed where the actor isn't standing is a note, and notes don't act"* — and row 19B says the same. **A flag in a close-out is a note filed where nobody is standing.** The generalisable version, and it is the reason R-24's clause (a) is written as a refusal rather than a warning: **when a build knows the shape of the defect it is shipping, the choice is not "flag it or hide it" — it is "flag it or make it impossible", and flagging is only the right answer when the guard genuinely cannot be built in the same pass.** Here it could have been: the count query is one line and I did not consider it.

**🔴 AND A SECOND FINDING, SMALLER AND ENTIRELY MINE: I WROTE TWO ASSERTIONS I HAD NOT RUN, AND BOTH WERE WRONG.** The first suite run came back **RED on two probes** — an income-account ordering I had assumed and a negative-limit fallback I had assumed. Neither was a code defect I found; both were **claims I made about behaviour I had not executed.** One of the two turned out to point at something real (`(a.account ?? '')` sorts the empty string FIRST, so a tie floated *"Not set"* to the top of a breakdown whose whole job is naming real buckets — fixed in the code); the other was simply my test being wrong about a safe fallback (fixed in the test, and the fix is stated rather than quietly aligned). ✏️ **This is #228's finding recurring in the same seat one session later** — a number or a claim written from intuition and presented as a measurement. **Which is why the seven mutants in this build were RUN and their failure counts reported as measured (6·2·2·1·1·3·1) rather than described.**

**FLAGGED FOR DAVID:** **(a)** 🔴 **A SECOND, UNRELATED FEATURE WAS IN THIS WORKING TREE AND I DID NOT WRITE IT.** At 13:26 — six minutes after my session-open `git status` showed the tree clean — `pushHold.ts`, `pushHold.test.ts` (30 assertions, passing) and edits to `cultivar.ts` (+24 lines, a 409 `PUSH_HELD` guard) and `router.ts`'s `handleStatus` appeared. **You told me the other worktree is stopped and that I am priority.** I committed it **FIRST, on its own, attributed as not mine**, because my `router.ts` edit sits on top of it and committing my paths alone would have left HEAD importing an untracked module. **It is BUILDER-COMPLETE and entirely un-owner-proven by me — I have read it, I did not write it, and I cannot vouch for it.** **(b)** 🔴 **RUN THE ITEM READ FIRST AND TELL ME THE TWO NUMBERS.** The green line must say *"QuickBooks reports N and N were retrieved"*, **equal, with N well above 100.** If it still says 100, the pagination did not take and everything after that is fiction. **(c)** 🔴 **THE QUESTION THE WHOLE PASS EXISTS TO ANSWER IS NOW ON SCREEN IN A BOX: does item `Id 1` exist in LAWNS's company?** The first hundred rows did not contain it, and that was never evidence — it was a truncated page. If the box says it does **not** exist, the twelve literals are worse than mis-filing: QuickBooks **rejects** the push outright. **(d)** ⚠️ **THE CUSTOMER FILE IS ~1,900 REAL PEOPLE AND I CANNOT REPORT AN ABSOLUTE PATH** — a serverless function cannot write to your disk. The name is deterministic (`qbo-customers-<realm>-<ISO>.json`, `~/Downloads` unless you changed the default) and **CARD 11 asks you to run `git status --short` and confirm it is NOT in the repo.** That check is the part I could not make structural. Delete the file when the mapping pass is done with it. **(e)** ⚠️ **R-24's THIRD CLAUSE HAS NO CAP AND I AM SAYING SO RATHER THAN IMPLYING IT IS COVERED.** *Nothing personal in a log* is held by review and by CARD 10's signal line. A future `console.log` of a customer row would pass every check in this repo. **(f)** ⚠️ **STORAGE IS STILL UNDECIDED AND THE QUESTION GOT BIGGER.** Nothing is persisted, on purpose. Whether TRACE may hold a customer's chart of items was already yours; **it now also means their 1,900-person customer list**, which is a materially larger question and still nobody's default. **(g)** ⚠️ **A SYMBOL R-23's GUARD CELL CITED BY NAME NO LONGER EXISTS.** `QBO_ITEM_QUERY` is gone — queries are BUILT now — so I **re-pointed the guard in the same commit** at `qboRead.test.ts` §A, which asserts no generated query for **either entity at any page position** carries a write verb. Strictly stronger than what it replaces, but it is an edit to a filed ruling and you should know I made it. **(h)** ⚠️ **NOT TOUCHED, AS INSTRUCTED, AND EACH IS STILL TRUE:** the twelve literals · the silent `try/catch` at `router.ts:241-250` · the dead `shared/quickbooks/invoice.ts` + `customer.ts` barrel exports and their thirteenth dormant ItemRef literal. **(i)** ⚠️ **`docs/inventory-functions.md` / `inventory-env.md` / `inventory-ai.md` still read `Last updated: 2026-06-13`. `docs/reference/` still untracked.** **(j)** **CLAUDE.md is under its ~600 budget.**
