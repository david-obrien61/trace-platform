...
# Last updated: 2026-08-31 — #247 the fulfilment tap + the review ask; R-34 rules the copy — see §3
...
### 2026-08-31 (8) — THUNDER **THE STOP CAN FINALLY SAY IT HAPPENED — AND GOOGLE'S OWN POLICY REFUSED THE COPY I WAS TOLD TO SHIP. #247. TECH-DEBT #121 → PARTIAL.** 🔴 **THE HEADLINE IS THAT THE ONE INSTRUCTION IN THE PROMPT SAYING *"do not take my word for it"* IS THE ONE THAT PAID.** The brief named the guidance line and said the wording mattered: *"naming what was planted and who did it is what turns a rating into a review."* **It is prohibited, twice over.** Google Business Profile → Prohibited & restricted content → **Rating Manipulation**: *"nor should they request that specific content be included"*, and — separately — *"Merchants requesting that staff solicit reviews that include specific content, **including content that identifies a staff member**."* ✏️ **So the two rules the prompt called non-negotiable were right and INCOMPLETE: there is a THIRD, it is the least obvious, and it forbids the exact sentence the feature was designed around.** A neutral default ships, and `reviewCopyProblems` **REFUSES** such a line at the settings field rather than warning after save — the Save button goes dead and names both problems in words. ✅ **RULED SAME DAY, AND THE RULING WAS STRICTER THAN WHAT I SHIPPED — [[R-34]].** The copy is **"If you have a moment, we'd appreciate a review."** and nothing more, and **the rule is the CONSTRUCTION rather than the topic**: *"anything of the form 'it helps if you mention…' is the prohibited construction whatever follows it."* 🔴 **My first check keyed on the OBJECT** — it caught *"mention the crew"* and *"mention what we planted"* and would have **passed** *"it helps if you mention how quickly we arrived"*, which is the same prohibited act with a different noun. **Google's clause prohibits requesting that specific content be included and does not enumerate which content**, so the check now fires on the DIRECTIVE and never asks what is being directed. **And the two clauses are quoted VERBATIM beside the code that refuses**, on David's instruction and for his reason — *the prohibited wording reached the project from a research summary that recommended exactly what Google forbids*, i.e. it arrived looking like best practice, and the next one will too. **BUILDER-COMPLETE** `387d8aa` on **`thunder/fulfilled-tap`**, **not merged** · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **55/55 files · 2735 assertions** · **15/15 MUTANTS CAUGHT, MEASURED across two rounds** · `build:cultivar` clean · **api/ 12/12, nothing minted** · **no permission minted** · all `[TRACE:DELIVERY]` ON.

**Type:** Build + close-out. Code: `lib/deliveryFulfilment.ts` (**NEW, the pure core — every decision lives here because a render condition in a `.tsx` cannot be asserted, #134**) · `deliveryFulfilment.test.ts` (**NEW, 70 assertions**) · `components/delivery/ReviewAskSheet.tsx` (**NEW**) · `pages/DeliverySchedule.tsx` · `pages/Settings.tsx`. Schema: `20260831c` — **GATED, four nullable columns, UNAPPLIED**. Docs: ledger **#247** · **NEW owner-test board, 11 cards, 0 covered** · `user_stories.md` (**the review-ask story WRITTEN as the no-match case; the tap story's as-built half recorded**) · `built-inventory.md` · tech-debt **#121 → PARTIAL** · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-31 (5), the calendar entry), 1 written — entries-in == entries-out, §3 back to 3** (archive **274 → 275**, `grep -c '^### '`). ✅ Own worktree off `main`, with six others live.

**🔴 THE FINDING WORTH MORE THAN THE FEATURE IS THAT THE SEAM FOR THIS BUILD WAS WRITTEN FOUR DAYS AGO AND LEFT DORMANT — AND IT TOLD ME WHICH WORD TO USE.** `historyOrder.ts`'s `DELIVERY_COMPLETE` list already accepts `complete`/`completed`/`delivered`/`fulfilled`/`done`, and its own comment says why it exists: *"as of 2026-08-27 `deliveries.status` has exactly ONE value across every tenant… this list is currently unreachable. It is written anyway, with the likely vocabulary, so that the day a 'mark delivered' control ships the order status follows automatically instead of needing this rule rediscovered."* **This is that control.** ✏️ So the answer to the prompt's *"say what you did about `fulfilled` being the first other value the column has held"* is that **the word is new to the DATA, not to the CODE** — and probe **A3 reads that list out of the real source file** rather than copying it, because a copy would agree with itself forever while the two drifted apart in silence. **And no CHECK was added**, deliberately: `20260620_deliveries.sql`'s own header declares the no-CHECK convention for this column, `20260715` dropped one elsewhere for the reason `orderStatus.ts` records, and tech-debt **#91** is what the alternative costs. One vocabulary, one file, exactly as `ORDER_STATUSES`.

**🔴 AND THE SECOND: THE PAYWALL IS PROVEN BY A SIGNATURE, NOT BY A TEST.** The requirement was that the crew screen be byte-identical with the tile off — *"prove it"*. A test comparing two outputs would only check that a branch nobody wrote is still absent. **`crewStopModel` takes no module state, no config and no entitlement, so there is no input through which the two cases could differ**, and the probes assert the CONSTRUCTION (`length === 1`, and the function body mentions no module/config/entitlement/review). The day someone adds a config parameter it goes red. ⚠️ **The ask is consulted only AFTER the stop is already written**, which is both the honest moment (you cannot ask about a job that has not happened) and the reason the crew's own screen never has to know what the business pays for.

🔴 **AND THE DEFECT THE REWRITE SURFACED IS WORTH MORE THAN THE REWRITE: I ALMOST SHIPPED A CHECK THAT COULD NOT FIRE.** The new regex literals were written through a Python heredoc, which read every `\b` word-boundary as a **BACKSPACE escape** — so four patterns landed in the file carrying literal `0x08` bytes and **matched nothing at all**. They looked right in a terminal, because a backspace is invisible. **Caught by grepping for the byte, not by reading the code** — and the tests had not yet been extended, so a green run would have meant nothing. ✏️ **That is [[R-33]]'s own class — *a check that cannot disagree is not a check* — arriving inside the commit that cites R-33**, and the reason the 6/6 mutant round afterwards was not optional.

**FLAGGED FOR DAVID:** **(a)** ✅ **THE COPY IS RULED AND BUILT — nothing is blocked on it.** CARD 8 step ④ is the one to run: `It helps if you mention how quickly we arrived.` must be **refused**, and it names no staff and no product. A check that only caught the crew and the plants would be enforcing a word list rather than the clause. **(b)** 🔴 **`20260831c` IS GATED AND UNAPPLIED — SQL EDITOR, NOT THE TABLE EDITOR** (§6 r17; the migration carries its own privilege fingerprint query). Until you run it the delivery list still loads and every card honestly says the control is unavailable. **(c)** 🔴 **CARD 5 IS THE TEST THAT MATTERS: tile OFF, mark a stop done, and NOTHING about reviews may appear** — no greyed button, no upgrade copy, on a crew phone in a customer's garden. **CARD 3** is the first real measurement of stop duration; **CARD 4** proves an unmeasured stop reads nothing rather than `0 min`. **(d)** ⚠️ **TWO PREMISES IN THE PROMPT CORRECTED, both [[R-26]]'s shape.** *"The QR generator is in the repo with ZERO callers"* — **`generateQR` has THREE live callers** (Settings · Profile · MemberConsole), all rendering real DOM from a data URL, so the instruction was right and the reason was wrong; what has zero callers is `generatePlantQR` and both `qr/print.ts` functions, the `document.write` file you warned about. And `deliveries:update` is indeed in all three bundles — **the STAFF line's comment already reads *"see the stop, mark it done"***. **(e)** ⚠️ **THE STORY SAYS THE PLACEMENT IS OWED BY YOU** — route screen, calendar drill-in, or both, *"and the answer must not be both, built twice."* I mounted it **once**, in `DeliverySchedule`, which the calendar already renders as its day drill-in — so today's single mount serves both surfaces and `/deliveries` is a one-line addition when you rule. **(f)** ⚠️ **NOT BUILT, AS SCOPED:** the SMS follow-up (no provider; `customers.marketing_opt_in` exists and must be respected when it is) · the after-action note · contractor pay · material consumption · **and the reschedule half of the story, which is blocked on your `why` vocabulary** — so a day still cannot read back as *"six completed · one moved to the 5th"*. **(g)** 🔴 **SATURDAY'S SIX REAL LAWNS STOPS ARE YOURS TO MARK, NOT A BUILDER'S** — CARD 11 is `needs-test` for exactly that reason. **(h)** ⚠️ **THE LEDGER NUMBER `#247` MAY COLLIDE** — six other worktrees are live; re-read `CLOSE-OUT-LEDGER.md`, `tech-debt-log.md` and the §3 count at merge (#233's lesson). **(i)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs stamped `2026-06-13` · `docs/reference/` tracked by accident · `20260830c` unapplied · tech-debt **#124** · **#125** · **#126** · **#127** · **#132** · **#134** · **#135**.

### 2026-08-31 (7) — THUNDER **LAUREN'S DELIVERIES ARE IN THE APP — AND THE ENTRY IS THAT IT FAILED ALL 19 ROWS FIRST, PAST 87 GREEN ASSERTIONS THAT COULD NOT HAVE CAUGHT IT. #246. R-32 + R-33. TECH-DEBT #136 CLOSED BY RULING.** 🔴 **THE HEADLINE IS NOT THE FEATURE, IT IS THAT MY TEST DOUBLE WAS MORE FORGIVING THAN POSTGRES.** `Invoice.ShipDate` is Lauren's delivery date and it was already in QuickBooks; her calendar showed one stop against 22 future ship dates. The ingest reads them, parses the ship-to, resolves the customer and writes a stop. **It failed on ALL 19 rows live** with *"there is no unique or exclusion constraint matching the ON CONFLICT specification"* — I created the unique index **PARTIAL**, and Postgres infers a partial index only when `ON CONFLICT` repeats the predicate, which **PostgREST's column-list `onConflict` cannot express**. ✏️ **Not a costly tuning choice — unusable by its only caller, by construction, and I wrote the migration beside the upsert that could never name it.** ✅ **LIVE AND WORKING after `20260831b`: `0 customers created · 19 linked · 19 written`.** `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **54/54 files · 2642 assertions** (+149) · **14/14 MUTANTS CAUGHT** · api/ **12/12, nothing minted** · `main` **`83dc2c0`**, proven live from the artefact.

**Type:** Build + live fix + close-out. Code: **NEW `shipmentIngest.ts`** (pure: the parse, the plan, the customer verdict) · **NEW `deliveryIngestWriter.ts`** (the only new `deliveries` writer) · **NEW `QboDeliveryIngest.tsx`** · `customerUpsert.ts` (**`qb_customer_id` as the FIRST dedup key**, plus a narrow caller-resolved hatch so it stays the ONE customer write path) · `qbo/router.ts` (two branches, **no new function**) · two migrations, both APPLIED. Docs: ledger **#246** · tech-debt **#136 CLOSED · #137 · #138** · **R-32 + R-33** · **CLAUDE.md §6 r19** · owner-test board (**10 cards, 4 covered**) · `built-inventory.md` · ⚡ ACTIVE STATUS · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-31 (4)), 1 written — entries-in == entries-out, §3 back to 3** (archive **271 → 272**, `grep -c '^### 2026'`).

**🔴 THE FINDING WORTH MORE THAN THE FEATURE, AND DAVID NAMED IT AS A CLASS BEFORE I DID: THE THING ASSERTING WAS INCAPABLE OF DISAGREEING. THREE MECHANISMS IN A FORTNIGHT.** My stub stamped **any** `onConflict` string, so it was strictly more forgiving than the real system and **no test in that file COULD have caught the defect that shipped** — 87 green assertions, none of them able to fail this way. ✏️ *"A fake more forgiving than the real thing is a rubber stamp."* **① a check that could not fail** (`OWNER_ONLY_PENDING`, #73) · **② a seam tests could not reach** (the positions chooser — 25 assertions, 9/9 mutants, not one putting it on a screen; #242 / #134) · **③ a double that could not refuse** (this one). 🔴 **Filing these as three instances would have hidden the pattern**, which is why it is **[[R-33]]**, **CLAUDE.md §6 r19** (appended at 19, never inserted — r18's own lesson about breaking live citations) and **tech-debt #138**. ✅ **③ is fixed and proven BOTH ways:** the stub carries the index as data and refuses as Postgres does; `§K` reproduces the live failure by flipping one flag, **with a negative control** proving the dropped predicate is what fixes it; `§L` reads the REAL migration corpus from disk and fails if the predicate returns — **verified to bite by putting it back**. ⚠️ **A cap for the CLASS was deliberately NOT built: a checker that detects un-failable checks would itself be a check nobody has watched fail.** Per #174 the cheap next step is a COUNT.

**🔴 AND THE SECOND: DAVID OVERTURNED A BUILD I HAD FILED AS NECESSARY, ON EVIDENCE FROM OUTSIDE THIS BUILDING ([[R-32]], tech-debt #136 CLOSED).** I reasoned that because Cultivar now owns the delivery date, their invoices go stale whenever Lauren moves a stop, so a write-back is the necessary consequence — Ariel Thiry instance one. **The reasoning was sound and the conclusion was wrong: I treated a DIVERGENCE as a DEFECT.** Jobber syncs clients, products and services, timesheets, invoices, payments, refunds, tips and payouts — **one way, and the SCHEDULE is not in the list at all** (*"Jobber is the source of truth…"*). ✏️ **The invoice records what was BILLED; the app records what is SCHEDULED; they answer different questions and were never required to agree.** Lauren was only using `ShipDate` as a calendar because she had none. 🔴 **So the 2026-08-31 ingest was the LAST time we read a schedule out of QuickBooks.** This is the second time this week the customer's own world answered a question better than my derivation did — [[R-25]]'s argument, arriving from a competitor's docs instead of an invoice corpus.

**FLAGGED FOR DAVID:** **(a)** ✅ **NOTHING BLOCKING — both migrations applied and catalog-verified, the stops are in the app, `main` is `83dc2c0`.** **(b)** 🔴 **OWNER-PROOF IS 4 OF 10 AND THE GAP IS DELIBERATE — DO NOT READ THE CLEAN RUN AS COVERAGE.** David's instruction, and it is the right one: *do not mark a card covered because the run went well.* **CARD 3** is owed because **zero refusals appeared**, so the refusal path — the one guarding against a crew at the wrong house — **was never exercised**; it closes only on a deliberately malformed ship-to. **CARD 6** is owed because the second run was a **RECOVERY, not a repeat**: no run has yet started from 19 existing stops, so idempotency is asserted by test and unobserved live. **One more press of Ingest closes it — expect `0 written`.** **CARD 8** is the most important unproven one: `/orders` and available-to-sell were never checked, and *an argument is not an observation*. **(c)** ✅ **CARD 10 was ADDED AFTER THE FACT** for the recovery — a board holding only the tests somebody thought of in advance never learns. **(d)** ⚠️ **TWO THINGS IN THEIR DATA, REPORTED NOT FIXED:** **#3648.563 Dwight Gillespie is $0.00** with a real ship date and address (odd, not invalid — the ingest reads `ShipDate`, not the total); and **"Georgeown"** in Vicki Sosa's address goes in **exactly as typed** — silently correcting a customer's address is how a crew ends up somewhere nobody chose. **(e)** ⚠️ **FILED WHERE THE ItemRef BUILD WILL READ THEM, NOT HERE:** Jobber breaks on **sub-items / categorised items / non-`Sales` income accounts** and tells users to flatten their books — **LAWNS is exactly that shape (557 of 685 sub-items, 38 categories, five income accounts)**, and we are absorbing that complexity deliberately rather than asking Terry to destroy a structure he finds useful. Jobber's invoices also **sync once**. Both in the QB-invoice board + the discovery doc. **(f)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs stamped `2026-06-13` · `docs/reference/` tracked by accident · **#137** (`deliveries` has no field registry — #120's class on a second table) · **#73** · **#134** · **#124**–**#131**.

### 2026-08-31 (6) — THUNDER **BOTH BRANCHES MERGED, AND DAVID'S TWO ANSWERS TURNED ONE MISSING ROW INTO THREE MISSING POSITIONS. #245.** 🔴 **THE HEADLINE IS THAT BOTH FLAGGED QUESTIONS CAME BACK, AND *BOTH ANSWERS REFRAMED THE QUESTION RATHER THAN SETTLING IT.*** **(1) `INV-01` STAYS OUT OF THE CREW.** I flagged the workbook withholding *walk the lot and count stock* from the crew as odd beside the staff-count-walk work of #238/#67. David: **the crew set is DRIVERS; walking the lot to count is a YARD job.** So the two facts never conflicted — ✏️ **THE GAP IS A MISSING POSITION, NOT A MISSING ROW**, and adding `INV-01` to the drivers would have papered over a missing position with a wrong one. 🔴 **AND NO COUNT-BASED CHECK COULD HAVE CAUGHT THAT: the set would simply have held nine instead of eight, and F11 would have gone green on the new number.** **(2) `MON-10` GOES INTO THE BOOKKEEPER.** I reported it landing in no set at all; David ruled that *no set at all cannot be true — somebody pays vendors in every business*, and named the cause: **the workbook was doing two jobs at once, a position TEMPLATE and a LAWNS SNAPSHOT.** The row is added to the template **and the observation survives it** — *only the owner does it here* is on the workbook's own "Only the owner" tab with its consequence, and a template row does not get to delete an observation. **BUILDER-COMPLETE** · **MERGED: `044183a` → `c0810f4` (workbook) → `5c21182` (#245) → the calendar merge** · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **4/4 mutants caught, MEASURED** · ZERO schema · ZERO migration · api/ 12/12.

**Type:** Two merges + a build + close-out. Merge order was **David's instruction and it mattered**: `thunder/workbook-reconcile` (holding #243) FIRST, the answers carried in with it, then `thunder/calendar-window` (#244) with the counters re-read at merge. Code: `positionStartingPoints.ts` (**`MON-10` into the bookkeeper · the two flagged questions rewritten as ANSWERS · `MISSING_STARTING_POINTS` declared**) · `positions.test.ts` (**F13/F13b/F13c/F14/F14b/F15**, +9) · `startingPointChooser.test.ts` (K5's printed count 10 → 11). Docs: ledger **#245** · owner-tests · `built-inventory.md` · `user_stories.md` (**the gap story, filed**) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. 🔴 **§3 RETENTION — AND THE ARITHMETIC HAD TO BE REDONE AT THE MERGE, EXACTLY AS FLAGGED:** both branches independently archived the **SAME** entry (2026-08-31, the #240 positions entry), so the archive briefly held it **twice** and §3 held **four**. Resolved by removing the duplicate, merging its two provenance comments into one naming both sessions, and archiving **#242 and #241** as well. Across both merges: **3 archived verbatim (#240 once, #241, #242), 3 written (#243, #244, #245) — entries-in == entries-out, §3 back to 3** (archive **270 → 273**, counted with `grep -c '^### '`). ✏️ **This is the second occurrence of one-entry-archived-twice (08-30 (6)/(7) was the first), which makes it a pattern rather than an accident: two concurrent branches each hold a correct §3 count and their MERGE does not.**

**🔴 THE FINDING WORTH MORE THAN EITHER ANSWER IS THAT THE SETS ARE A SAMPLE, AND TODAY IS THE FIRST MEASUREMENT OF WHAT THE SAMPLE MISSED.** The header always said these five sets were drawn from watching one business. What it could not say is **which people that sample contained — and it contained the people David met.** Three positions at LAWNS were not among them and all three surfaced within hours: **the yard hand** (found by asking why the crew withheld `INV-01`), **on-site maintenance — CUTO, who lives on site, does the maintenance and the handy work, and does not speak English**, and **whatever customer two turns out to have**, named as unknown rather than guessed at. ⚠️ **This is evidence ABOUT the sets, not a defect IN them** — nothing in the workbook is wrong; it is incomplete in a way only a second look could show. 🔴 **And the declaration SELF-PRUNES** (`MISSING_STARTING_POINTS`, probe F13): an entry whose key has since been built is STALE and fails, so building the yard-hand set forces whoever builds it to come back and strike the line — the pattern `UNDELEGABLE_SUGGESTIONS` already uses, kept precisely so this does not rot into the unread noise `OWNER_ONLY_PENDING` became (#73).

**FLAGGED FOR DAVID:** **(a)** 🔴 **CARD 14 IS WHAT YOU RUN, AND THE NUMBER IS THE ASSERTION: back four weeks, Saturday 29 August, SEVEN STOPS, all seven by name.** GATE 0 first — the stamp at the foot of the screen must read the merge SHA, not `044183a`. **(b)** 🔴 **DO NOT ASSUME ONE LANGUAGE PER TENANT — carried into the code where it would be broken, not only into a note.** Cuto is the counter-example **inside one business**. ✅ **Measured today so the next reader does not have to: nothing in this repo assumes it** — there is no language or locale column on any business table, and the ONLY hardcoded locale in the positions feature is `positionDescription.ts`'s `toLocaleDateString('en-US', …)`, which sits at the **DOCUMENT** layer, i.e. exactly where a per-person choice will need to reach it. A tenant-level language setting would be the wrong shape and would have to be undone. ⚠️ **The two stories (a Spanish interface chosen BY THE PERSON on the invitation screen, and the on-site maintenance position) are yours to file; I have not written them.** **(c)** ✅ **#242's RENDER SEAM EARNED ITS KEEP WITHIN A DAY, AND THAT IS WORTH SAYING PLAINLY:** adding `MON-10` moved the bookkeeper button's printed count, and **K5 went red on it in seconds.** Before #242 nothing in this repository could see what a button PRINTS, only what a set CONTAINS — and those are different facts. **(d)** ⚠️ **THE THREE MISSING POSITIONS ARE FILED AS A GAP STORY AND NOT BUILT** — a sixth starting point is a build with its own owner-test, and inventing Cuto's set from a two-sentence description is the exact thing tech-debt #133 was filed about. **(e)** ⚠️ **STILL OPEN AND UNTOUCHED:** `docs/inventory-functions.md` / `inventory-env.md` / `inventory-ai.md` stamped `2026-06-13` · `docs/reference/` still tracked by accident · `20260830c` written and not applied · tech-debt **#124** · **#125** · **#126** · **#127** · **#132**'s owed `authority:baseline` · **#135**.
