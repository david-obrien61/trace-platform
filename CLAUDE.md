...
# Last updated: 2026-07-31 — a string is derived from the ACT, never from who needs to pass; the A7 client-gate class is now measured and capped (#174). See §3.
...
### 2026-07-31 — THUNDER THE PHASE 2 CONVERSION WAS LOSSY, AND THE ONLY THING THAT CAUGHT IT WAS AN OWNER-TEST CARD. A manager walk on `a191eb3` found the delivery-date field simply **absent** on `/checkout/customer` — present for the OWNER the day before, same transport, same flow. **The gate was `can('deliveries:create')`**, converted in Phase 2 from `isOwner || role === 'MANAGER'` — the very line capA's assertion 2 was written for. capA caught the SHAPE and the shape is now perfect; **what nobody checked is that the replacement string admits the same people the old predicate did.** MANAGER holds `deliveries:read` + `:update`, not `:create`. **And the string was wrong on the merits, which is the durable finding: this field writes `orders.delivery_date`, a COLUMN ON `orders` — it never touches the `deliveries` table at all.** DAVID RULED **`orders:update`**: *narrower and correct beats wider and convenient.* **ONE STRING. BUILDER-COMPLETE (`npm run verify` exit 0); 1 owner-test card OWED (board 0 of 15).** ZERO new api-fn (12/12), no schema.

**Type:** App code — **1 EDIT** (`packages/cultivar-os/src/pages/CustomerCapture.tsx` — one identifier + the comment block that explains it). Docs: ledger **#172** · **NEW ruling line** in `docs/RULINGS.md` + **2 NEW OWED rows** (#84, R-GRANTDIFF) · tech-debt **#84** · **NEW card 15** + card 6's fired FAIL clause recorded (board 14 → 15) · `built-inventory.md` (FIX 4 gate correction + line 4) · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift line · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-29 (2) ARCHITECTURAL STANDARD #169), 1 written — entries-in == entries-out, §3 back to 3.** **NO new numbered decision** (a David RULING, recorded in RULINGS.md). **BENCH: no trigger fires.**

**THE MECHANISM, because it is the reusable part.** The old predicate named MANAGER **explicitly**. The new one names a permission the manager does not hold. Both are authority checks; both are well-formed; **only one of them admits Lauren.** capA's three assertions are all about SHAPE — no `isOwner` in an authority position, no role-string compare, the SQL copy matches the manifest — and **not one of them looks at WHO the expression admits.** So a conversion can be perfectly compliant and still take a capability away from a real person. This is NOT #170's class: there the string was RETIRED and a scan for dead strings would find it. Here the string is **live, well-formed and wrong**, and nothing that scans strings can reach it.

**THE ONE-OFF DIFF IS FEASIBLE, AND IT WAS RUN RATHER THAN ESTIMATED.** `git show 123ad3a -U0` renders every conversion as a `-`/`+` pair on adjacent lines; the old predicate is fully recoverable and resolving it against the three bundles is arithmetic. **Five findings across the 15 sites, one of them known:** **(1) LOSSY ×1** — this fix. **(2) SILENT WIDENING ×2** — `DeliverySchedule:316` now admits the MANAGER to a customer-edit affordance; `ScanOrder:144` now admits **STAFF**, who hold `orders:create` by David's 2026-07-27 fulfilment ruling. Both look correct; **neither was recorded as a grant change.** **(3) ONE LEGACY STRING → DIFFERENT VERBS ×2** — `manage_orders` became `orders:update` at `OrderDetail` but `orders:create` at `ScanOrder`; `manage_settings` became `settings:update`, `settings:read` AND `team:read` at three different sites. Defensible per site, **invisible as a set.** **(4) NO OTHER LOSS** — the remaining 10 are `isOwner || can(X)` → `can(X)` where the owner set contains X. **(5)** see below.

**🔴 THE HONEST LINE, AND IT IS ABOUT WHAT ACTUALLY WORKS.** No cap moved. capP's count did not move for #170 either. **What caught this was CARD 6 of the authority board, whose FAIL clause reads *"a converted site took a string narrower than the one the server enforces"* — fired for the first time, on the site it was written about, during a manager walk.** OP-14's whole argument is that *"it works"* is a claim until someone drove it through the real UI; this is that argument paying for itself in one day. The card stays `owed` — **the walk that finds a defect is not the walk that proves the fix.** And finding (5): **`orders.delivery_date` has NO SERVER GATE AT ALL.** `submit.ts` proves authority for the discount, the tax exemption, an order edit and an order delete — and nothing for this field. **The client invented a wall the server never had, the exact inverse of Phase 2**, where the server was right and the client was stale. Deferred by David (demo days away, `submit.ts` is the money path) and **filed rather than left silent — an unenforced client gate is a fake pill by another name.**

**FLAGGED FOR DAVID:** **(a)** **card 15** — `/checkout/scan` → delivery+planting → `/checkout/customer`, **as MANAGER `df7723be`, NEVER as owner**: the **Delivery date** field renders with its helper, a date saves, the order shows on that day at `/deliveries`. **(b)** **card 6 re-walked WITH the delivery branch**; **card 1** closes on your mark — you hold the 09:53 OWNER screenshot (7.60% / $74.86 on `a191eb3`), and Thunder does not mark it. **(c)** **R-GRANTDIFF is on your desk** — is a standing before/after grant-set cap worth building? **If the answer is no, capA's output must SAY it cannot see this class**; a cap silent about its own blind spot is #164. **(d)** **#84** — does `orders.delivery_date` want a server gate, or does "may create an order" already imply "may set its fields"? Both readings are defensible, which is why it is a ruling and not a build. **(e)** the **2 silent widenings and 2 verb splits** above are unreviewed — they look right, nobody ruled on them.
