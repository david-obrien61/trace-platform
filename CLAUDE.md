...
# Last updated: 2026-07-23 — import-matcher defect fix: foreign item # no longer sources a match + size normalized before compare (ledger #150, BUILDER-COMPLETE, owner-prove owed). See §3.
...
### 2026-07-22 — THUNDER CLOSE-OUT: **the inventory ledger arc is OWNER-PROVEN end to end — DB layer + app layer + reconcile surface.** David proved it live; the evidence is the database, not assertion. **DOCS-ONLY.** Zero code, zero schema, zero migration.

**Type:** Docs only — **6 EDIT** (`docs/owner-tests/inventory-full-surface-test.md` · `docs/CLOSE-OUT-LEDGER.md` · `docs/built-inventory.md` · `TRACE-SESSION-BOOTSTRAP.md` · `docs/DECISIONS-INDEX.md` · `docs/tech-debt-log.md`) + this §3 + line 3. **§3 RETENTION: 1 archived verbatim (D-50 BUILD 2B), 1 written — entries-in == entries-out (193 → 194 archived), §3 back to 3.** **NO new decision, nothing built.** **BENCH: no trigger fires.**

**WHAT WAS PROVEN, IN THE DATABASE'S OWN NUMBERS.** **#144 D-52 order loop, both shapes:** a DELIVERY order (`897be269`) wrote `order_created` + `order_committed` at **16:49:15** with **delta 0** — on-hand UNTOUCHED, **no `sale` event at all** — then `order_fulfilled` + `sale −6` at **16:53:34**, the two at the same instant and **four minutes after checkout**. A WALK-IN wrote `sale −2` at **17:06:50, at checkout**, commit and fulfill collapsed. **One model, two spacings** — the D-42→D-52 relocation proven at the EVENT level, not inferred from a grid number. **#145 reconcile, all 9 cards:** both of the build spec's corrections held in the field — a **+5 receive** read residual **0** and was itemized *"5 adjusted by hand"* rather than becoming phantom shrinkage (correction A), and **−17** partitioned into `dead −4` + `loss −3` + `count_reconcile −10` landing on **46**, **not −24** (correction B — the seven invented dead plants did not happen). **V3(b) `qty == SUM(delta)` returned ZERO rows** with every new write counted. **#146 replay fix:** replay **46** == book **46**, banner gone. QBO **#437/#438** generated; the netting warning fired on self-pickup — **the Regina anchor, live.**

**⚠️ THE CENTRAL JUDGEMENT OF THIS CLOSE-OUT WAS WHAT *NOT* TO FLIP, AND IT WENT AGAINST THE PROMPT.** The instruction named the reconcile and inventory-states surfaces; the evidence did not cover all of their cards, and a third surface (`sellability`, 7 cards, shipped in `aca0b5d`) was not named at all. **#147 sellability keeps ALL 7 cards `owed`/`needs-test`** — the run proved the *derived number* (*"0 available (29 on hand, 57 committed)"* at review) but **not the CAP those cards assert** (Add disabled, stepper gone). **4 of 8 D-52 cards stay `owed`** — cancel-invents-no-stock (the sharpest: under the moved decrement a wrong cancel-restore **MANUFACTURES plants** into a log that cannot be retracted), the grid lock popover, the dashboard wording, the pre-D-52 remediation report. **Marking those covered would assert a proof nobody performed — the exact failure OP-14 clause 3 exists to prevent, and the exact shape of D-49's own suite blessing the defect it was written to catch.** Separately, **three reconcile cards rest on David's blanket "all cards green" rather than a cited figure**; they are flipped on his word and **labelled as such** in the board, the ledger and built-inventory, so the evidence gradient stays visible instead of being laundered into the same confidence as the measured ones. **Board: 17 → 32 of 70 covered (24% → 46%).**

**TWO DRIFTS FOUND WHILE RECONCILING, BOTH REAL.** **(1)** Ledger rows **#144–#147 all carried stale `*(this commit)*` / `*(pending commit)*` SHA placeholders** — the file's own Fabrication-check §1 class, recurring. Corrected to `e7471df` · `6245f27` · `679fb9a` · `aca0b5d`. **(2)** The **#146 regression card pinned a literal fixture (`replay 58`, `2 rows`) on lot `3ec53db3` — a lot the ATTRIBUTION-SPLIT card then legitimately moved to 46.** Left alone it would have manufactured a **false FAIL on its next run**. Rewritten to assert the **invariant** (replay == book, no banner) instead of a constant, with the reason recorded on the card. **A fixture that drifts by design must never be pinned to a number.**

**FLAGGED FOR DAVID:** **(a)** **NEW #71** — **one `status` column, two authors:** D-42's qty-derive **overwrites** D-52's tombstone, so a soft-deleted lot reads `depleted` (indistinguishable from sold-out) and a manual `archived` is silently reverted; the grid also still offers **Delete** on a tombstoned lot and surfaces the raw string **`already_deleted`**. Durable fix is lifecycle state in its own field — **a MIGRATION**; logged not fixed, per your instruction. **(b)** **NEW #72** — the `sale` row's **`reason` is NULL** while every neighbouring kind explains itself; carry the order number at the emit sites next time the order path is touched. **(c)** **#66 still OPEN** — anon QR cannot derive committed; `SECURITY DEFINER lot_available()` remains **PROPOSED, not built.** **(d)** **#67 blind capture is still your open ruling** — and it is why **cap 4.2 moved 🔴→🟡, not 🟢**: the arithmetic is proven, the count-then-review loop is not. **(e)** **INCOMING STOCK / RESTOCK is PARKED, not owed** — expected qty, supplier, expected date, receiving event; you are holding it pending real data on how LAWNS orders. Recorded as parked so it is not mistaken for a gap. **(f)** #56/#59/#60/#61/#62/#63/#64/#65/#68/#69/#70 unchanged.
