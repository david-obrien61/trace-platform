...
# Last updated: 2026-07-31 — a permission gates a CAPABILITY, not a FIELD; and the grant set is now baselined (#172/#173). See §3.
...
### 2026-07-30 (2) — THUNDER `owner_id` WAS THE AUTHORITY MECHANISM AT THREE LAYERS AND IT CANNOT EXPRESS THE FACT DAVID RULED ON FOUR DAYS EARLIER. An LLC has two owners. `businesses.owner_id` is single-valued. Lightning acknowledged the TWO OWNERS ruling on 2026-07-26 and then designed on `owner_id` anyway, and every `isOwner` check in the app inherited it from ONE LINE — `BusinessProvider.tsx:695`. **The symptom that proves it was never theoretical: `get_business_tax_rate` calls `has_permission`, which has no owner branch, so the OWNER read "Tax: not identified" while his own MANAGER read the rate** — because the manager's array was backfilled and the owner's was skipped on the theory that `owner_id` made it unnecessary. That theory was the defect. **FOUR PHASES, BUILDER-COMPLETE + DEPLOYED (`3d72b1d` · `123ad3a` · `df56f6a`); 3 migrations GATED; 14 owner-test cards OWED (board 0 of 14).** ZERO new api-fn (12/12).

**Type:** App code + tooling + 3 migrations + docs — **NEW `docs/RULINGS.md`** (Phase 4, shipped in Phase 1's commit) · **NEW cap `verify-authority-checks.mjs`** (capA, chained) · **NEW `SurfaceState.tsx`** (the six states) · **NEW owner-test board** (`authority-model-full-surface-test.md`, 14 cards) · 3 GATED migrations (`20260730a/b/c`) · ~20 EDIT. Docs: ledger **#171** · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift line · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-29 WRITE-PATH CAP #167/#168), 1 written — entries-in == entries-out, §3 back to 3.**

**PHASE ORDER WAS SAFETY-CRITICAL AND IT IS THE REUSABLE PART.** With the owner holding the strings first, removing `isOwner` breaks nothing. Reversed, David is locked out of his own platform. Same shape in the migrations: **a → b → c, and c must not run until b's V2 shows every owner at n = 52.**

**🔴 THE THING THE PLAN DID NOT CONTAIN, AND THE BUILD FOUND.** `/costs` and `/add-business` are gated on the **`owner-only` sentinel — which is NOT a manifest entry.** Deleting the short-circuit without noticing would have silently taken Cost-to-Produce and Add Business away from the owner, and nothing in the spec, the plan, or the 23-site count mentions it. It is now a STRING THE OWNER HOLDS — in the set, checked like the other 52, ungrantable to a manager. *"Owner-level access" is a permission somebody holds, not an identity somebody is.*

**capA WENT RED ON THE PRE-FIX TREE FIRST (STD-024) AND FOUND A SIXTEENTH SITE.** 15 expected, plus `Settings.tsx:355` — a **second team-member list living beside MemberConsole**, an A1 surface duplication nobody had counted. Declared (owner-as-target, not viewer-authority) and **flagged, not merged** — folding two team surfaces is its own build. **The second assertion is the one that earns the cap:** `CustomerCapture.tsx:109` was `isOwner || role === 'MANAGER'`, a raw role-string compare that never touched `can()` and **would have survived a cap that only looked for `isOwner`.**

**PHASE 3 — MEASURED BEFORE: ~30 refusal surfaces, 27 SILENT, 3 apologising AFTER a failed write, 0 pre-emptive.** `PermissionRoute` stopped redirecting: one file, 26 surfaces, and the URL now survives so the person can paste the link to whoever grants permissions. The MENU stopped hiding what you lack — *nobody asks for access to a thing they cannot see.* The three apologies became pre-emptive **while keeping the A8 post-write check**, because the flag is the client's belief and RLS is the authority.

**🔴 THE HONEST LINE.** **capB does not exist.** The 27→0 conversion was done BY HAND, so a 28th silent refusal can be added tomorrow and nothing fails — which is precisely the "an unenforced rule is a placeholder for a cap" ruling from the day before, still unpaid on this surface. **`<BeingBuilt>` is built and NOT MOUNTED** (card 12, `needs-test` with its reason). **Two baselines were re-locked for LINE-SHIFT ONLY** — the same sites displaced by comments — which means `verify-zero-row-writes` and `verify-field-lists` both report false NEWs on any comment edit above a tracked site; noted as a cap weakness, not fixed here. **GATE 0: `3d72b1d` and `df56f6a` both PROVEN present in the deployed bundle; `123ad3a` was superseded before I proved it and I am not claiming it.**

**FLAGGED FOR DAVID:** **(a)** **apply a → b → c, in order, and run b's V2 before deploying.** **(b)** the **14 cards** — card 1 is the tax symptom head-on, card 3 is TWO OWNERS proven end to end, cards 5–11 need the **MANAGER `df7723be`** session and are meaningless as the owner, card 8 is `DEVICE: phone`. **(c)** **capB is owed.** **(d)** `Settings.tsx:355`'s duplicate team list. **(e)** RULINGS.md's **OWED table has 8 questions on your desk** — E5, #75, R-STATUS, R-QBSTALE, #71, the OP-13 character amendment, #59, and #67/#68.
