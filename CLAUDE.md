...
# Last updated: 2026-09-02 — #259 vendor identity + the preferred vendor (Option C); R-65 corrected — see §3
...
### 2026-09-02 — THUNDER **THE BUILD I WAS AUTHORISED TO DO WAS ALREADY DONE, SO I VERIFIED IT INSTEAD — AND TWO DEFECTS SURVIVED ITS OWN 20/20. #258. R-65. TECH-DEBT #150 + #151.** 🔴 **THE HEADLINE IS THAT THE CENSUS DAVID ASKED FOR FIRST IS THE ONLY REASON THIS SESSION DID NOT BUILD A SECOND COPY OF SOMETHING THAT ALREADY EXISTED.** The prompt authorised the receipt detail view. `thunder/receipt-detail-view` `75d6e9d` was already committed and **pushed**, 41 minutes old, by session `00b23536`, delivering §1 upstream-fix, §2 RPC, §3 both sentences, §4 bwi and #252's failed CARD 8. **Reported and paused, per §0.** ✏️ **A build prompt is a claim about the repo like any other, and the cheapest thing to check.**

**Type:** Review + merge. **NOT A BUILD — no new capability; two defect fixes in SQL that has never run.** `main` **`32d5297`** (merge, `--no-ff`) + this commit. `npm run verify` **exit 0, ZERO NET-NEW** (5 / 244 / 10 / 12 / 15) · **60/60 files · 3137 assertions** (+7) · **22/22 MUTANTS against a green CONTROL** · `build:cultivar` exit 0 · **api/ 12/12, nothing minted · NO new permission · THE MIGRATION IS STILL GATED AND UNAPPLIED.** **§3 RETENTION: 1 archived verbatim (2026-09-01 #252), 1 written — entries-in == entries-out, §3 back to 3** (archive **281 → 282**).

**THE CENSUS, MEASURED:** `main` `7952cb1` · **4 worktrees** (not the ten a stale §3 line claimed) · **4 unmerged branches, every one pushed, 0 unpushed commits, no stashes, no dirty tree** · the only unowned line is **`origin/assets`**, remote-only since 2026-06-28. **`thunder/qbo-review-test-mode` is NOT orphaned** — session `a528e4bf` holds it.

🔴 **THE TWO DEFECTS, AND THEY ARE ONE SHAPE.** **F1 — the denial audit row could never exist.** The RPC INSERTed `receipt.line_edit_denied` and then `RAISE`d; with no enclosing `EXCEPTION` block the raise aborts the transaction and takes the INSERT with it. **CARD 8 step 4 asserted that row appears — it would have failed.** Probe **U6 asserted the STRING was in the file**, which it was, doing nothing. **F2 — the first edit of any existing receipt would log changes nobody made:** `->` returns SQL NULL for an ABSENT key and jsonb `null` for a PRESENT null, `IS DISTINCT FROM` calls those different, the form sends five keys and all 36 stored receipts carry two — so one correction on Sudderth records **1 real change and 9 phantom ones**. ✏️ **Both are R-33 / §6 r19: a check incapable of disagreeing. #257 caught seven mutants for exactly that reason an hour earlier and these two sat one layer further out** — F1 because a rolled-back statement is textually identical to a committed one, F2 because no probe executed the comparison at all. **M21 and M22 restore each defect verbatim and both go red.**

🔴 **AND I CONTRADICTED A FILED RULING, WHICH THE RULINGS GATE CAUGHT MID-WORK.** I applied both fixes on `thunder/receipt-detail-view` — **another session's branch** — before reading **R-62** (*"a session edits only branches it owns; one writer per branch"*), whose stated mechanism is **attribution**: #257's ledger row, its cards and its 20/20 would have belonged to a session that did not make the change. **Reverted their worktree to a pristine `75d6e9d`, merged it as its author wrote it, re-applied the fixes here as my own commit.** ✏️ **The gate worked because the ruling is READ every session; nothing mechanical could have stopped it, and R-62 says so itself.** ⚠️ **The merge conflict was resolved by DIRECTION, not by side:** main's `#256` and `R-62` rows were NEWER than the branch's copies, so *take theirs* would have reverted them — **proven by counting deleted lines in the resolution: ZERO** (#249's lesson).

**FLAGGED FOR DAVID:** **(a)** 🔴 **CARD 8 STEP 4 IS INVERTED AND IS NOW THE SHARPEST CARD ON THE BOARD** — it asserted a denial row appears; it now asserts `SELECT count(*) FROM audit_log WHERE action='receipt.line_edit_denied'` returns **0**, with the reason. The refusal itself is unaffected and hard. The gap is **tech-debt #150**. **(b)** 🔴 **[[R-65]] FILED — your ruling, the vendor store consolidates into `vendors`** — and the obstacle is measured rather than assumed: **`vendorKey()` strips corporate suffixes while `vendors_business_name_uidx` keys on `lower(btrim(name))`**, so *"Sudderth Brothers Contracting, Inc."* is ONE key and TWO rows. A backfill on the fold loses rows. **`vendors` also has no home for the billing-unit answer yet.** ⚠️ **CHEAPEST BEFORE EITHER MIGRATION IS APPLIED** — after that it is a data migration across two disagreeing folds (tech-debt **#151**). **Acceptance #8 is OWED by your ruling, not by a defect.** **(c)** ⚠️ **THE MIGRATION IS STILL UNAPPLIED — cards 6–11 cannot run until you apply it**, and un-applied the READ cards still pass, so a half-green board is not a code defect. **(d)** ⚠️ **The §2 sweep, with the reading #257 did not give:** 23 of 141 live policies omit `FOR`, **but 17 are `*_owner_all` and harmless by intent** — the shape that repeats `receipts_member_all` is the **six `*_member_all`** rows (`business_assets`, `business_discovery_profiles`, `business_pmi_schedule`, `business_inventory_ledger`, `inventory_count_sessions`, `inventory_counts`). Reported, none fixed. **(e)** ⚠️ **On CARD 2, `20.72` and `$35.00` will read as *"reader read 20.72 — the saved copy never carried it"* under an em-dash**, on all 36 existing receipts. Honest and correct — but that is what you will actually see. **(f)** ⚠️ **The #145 drill-in deep link is inert in live data** — `cost_objects.receipt_id` is 0 of 5 (**#144**), so it always falls back to the list. **(g)** ⚠️ **R-65 and #258 claimed at file time against `main` `32d5297`;** `thunder/vendor-identity` was live holding R-62 as its highest — **earliest claim keeps the number** if it files an R-65 of its own. **(h)** ⚠️ **STILL OPEN AND UNTOUCHED:** `origin/assets` (remote-only, 66 days) · the three `inventory-*` docs bannered not re-audited · `docs/reference/` tracked by accident · tech-debt **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#141** · **#143**–**#145** · **#148**–**#151**.


### 2026-09-02 — THUNDER **THE JUDGEMENT IGNITION CAPTURED AND THREW AWAY NOW HAS A READER — AND FOUR OF THE THINGS I HAD TO FIX WERE MY OWN CHECKS, NOT MY CODE. #259. R-65 CORRECTED.** 🔴 **THE HEADLINE IS THAT TERRY'S REASON FOR PAYING MORE IS THE ASSET, AND NOTHING IN THE DATA CONTAINS IT.** He buys the same tree from two vendors; one is cheaper and poorer, one dearer and better. Measured from LAWNS's own invoices, the same 100-gallon Eagleston Holly is **KBB $450 in April against Enchanted $725 in June, 61% apart** — so a preference derived from price would route to the cheaper vendor every time and be wrong every time. `vendors` + `vendor_aliases` + `receipts.vendor_id`, a pure resolver under D-47, a `/vendors` screen where **the note travels with the mark**, and resolve-or-create wired into capture. ⚠️ **Ignition had this exact field** — owner-set `priority`, labelled *"1=Highest"*, painted on a card — and across 94 files it has **zero sorts, zero comparisons, zero reads that change a decision**; the only vendor-picking code is `vendors[idx % vendors.length]`. Its design doc wrote the intent on **2026-05-07**, four months before [[R-54]] and nearly in its words: *"owner-set preference — could be relationship, price, proximity, or **any unstated reason**."*

**Type:** Build. **BUILDER-COMPLETE**, **NOT MERGED**, on `thunder/vendor-identity` (off `main` `7952cb1`, merged `62d3d34`). `npm run verify` **exit 0, ZERO NET-NEW** (5 / 244 / 10 / 12 / 15) · **62/62 files · 3241 assertions** · **19/19 MUTANTS CAUGHT against a green CONTROL** · `build:cultivar` exit 0 · **api/ 12/12, nothing minted · NO new permission string.** Code: **NEW `vendorIdentity.ts`** (the resolver, the write plan, both folds, the display decisions) · **NEW `vendorIdentity.test.ts`** · **NEW `vendorFoldAgreement.test.ts`** · **NEW `pages/Vendors.tsx`** · `ReceiptKeeper.tsx` (+the capture resolver) · `router.tsx` · `tileRegistry.ts` · `types.ts` (the phantom) · `verify-write-paths.mjs` (a declaration). **TWO MIGRATIONS, BOTH GATED AND UNAPPLIED.** Docs: ledger **#259** · **NEW owner-test board (12 cards, 0 covered, 1 `needs-test` with its reason)** · the story, written as-built · **R-65 corrected in its own row** · tech-debt **#178**. **§3 RETENTION: 1 archived verbatim (#254), 1 written — entries-in == entries-out, §3 back to 3** (archive **282 → 283**).

**🔴 THE RE-MEASURE CHANGED THE PREMISE, AND IT ONLY HAPPENED BECAUSE STAGE 0 HAD SAID PLAINLY THAT IT NEVER QUERIED THE DATABASE.** Live: **36 receipt rows across 3 tenants** (not 17 — Lauren is still uploading), **8 distinct vendor strings**, **norm() collapses 0, prefix-contained pairs 0.** ✏️ **So the duplicate-spelling problem this build exists for is NOT YET IN THE DATA** — `Sudderth Brothers`, `McGill`, the trailing-space `Top Notch ` are on PAPER invoices, not in `receipts`. It lands BEFORE the corruption, which is the only reason the unique indexes can land at all (contrast #58/#143, where the index cannot land until the live rows are settled). 🔴 **And the decisive one: the OCR emits 17 fields and NONE is a vendor email, phone, address or account number** — the contact fields it does emit are **customer-side**, because the prompt was written for sales invoices. Account numbers **0 of 35**; emails **1 of 35**, and that one is a `customer_email`. **The two signals the spec ranks strongest are unavailable from captured data today**, so D-47's one-signal answer (SURFACE, never bind) is not a design choice here, it is the only honest one.

**🔴 FOUR MUTANTS SURVIVED THE FIRST SWEEP AND ALL FOUR WERE THE PROBES.** A collision assertion that could not tell *"refused a collision"* from *"asked a near-match"* — both return NEED_CONFIRMATION with a null id, so deleting the collision branch changed nothing it could see. An empty-name assertion the code could not fail. A trigger assertion satisfied by two strings on **different lines** while the trigger was renamed to something that never fires. And two source-text probes that flagged the **prose explaining why price is never consulted** — that one went red **twice**, because stripping comments then caught a **runtime string** (`'Both are cheap'`). ✏️ ***"The word never appears"* was never the guarantee; *"no code path reads a cost-shaped field"* is** — and the probe now carries a control proving its pattern matches a real cost read, so it cannot pass by matching nothing.

**🔴 AND THE PLATFORM'S OWN CAPS CAUGHT THREE THINGS I HAD REASONED MY WAY PAST.** **(1) capA refused `role === 'OWNER'`** as an authority binding — correctly, and I had written a documented §6 r10 divergence arguing for it. The cap was right and the divergence was unnecessary: **`can('owner-only')` is the existing idiom**, and because an OWNER-**role** session resolves to `OWNER_LOCKED_SET` it covers Lauren without `owner_id` being consulted at all. **I converged instead of declaring past it.** **(2) The zero-row-writes cap found a real defect** — my preference UPDATE checked only `error`, and an RLS-filtered update returns **success with zero rows**, so a refused save would have reported "saved". **(3) The field-list cap** made me put the two `select` lists in one exported constant instead of two hand-written copies.

**🔴 THE SPEC'S OWN FRAMING WAS WRONG AND IT CHANGED THE CODE.** *"Lauren — a manager, not an owner"*: measured across 3 businesses / 8 member rows, **Lauren Bishop holds role=OWNER at LAWNS with a user_id that is NOT `businesses.owner_id`** — the only such row in the database, and exactly what David's 2026-08-28 ruling gave her. **My first draft tested `owner_id` alone and would have refused her on her own tenant**, reproducing `20260828_owner_role_carries_authority`'s defect on a fourth surface. ⚠️ **The only ACTIVE manager anywhere is `test obrien` at `f7ec5d67`** — joel joiner is MANAGER at LAWNS but `active=false` — so the manager-side cards cannot be run at LAWNS at all.

**FLAGGED FOR DAVID:** **(a)** 🔴 **[[R-65]]'s LOAD-BEARING EXAMPLE IS BACKWARDS, and the ruling itself stands.** It records `vendorKey()` folding *"Sudderth Brothers Contracting, Inc."* and *"Sudderth Brothers"* to ONE key. Measured: **`contracting` is not in `VENDOR_SUFFIXES`, so it yields TWO** — agreeing with the strict index rather than disagreeing with it. **It was MY fold that merged them**, by carrying `contracting` in a suffix list where it does not belong. The two folds did disagree, on **3 of 28 strings, and every disagreement was mine being wrong** (incl. `Co-op Gardens` → `-op gardens`, a trap `vendorKey.ts`'s own comment names). `looseVendorKey` **is now `vendorKey()`'s algorithm**, and `vendorFoldAgreement.test.ts` **fails the build if they ever diverge** — which is the mechanical guard R-65's own status cell records as not existing. **(b)** ⚠️ **NO RULING MINTED.** This implements **R-64**, already yours; minting an R-66 for the same decision would put a second line on a board whose value is that every line is yours. **(c)** 🔴 **TWO MIGRATIONS, BOTH UNAPPLIED, AND THE ORDER MATTERS** — `20260902b` **pre-flights and REFUSES by name** if `vendors` or `vendor_preferences` is missing. **(d)** ⚠️ **`20260902b` gives `vendor_preferences` a real `vendor_id`, a `vendor_preferences_resolved` view and a per-row `link_vendor_preference()` — but runs NO BACKFILL**, because both tables are empty and a backfill would be untested against the only data that matters. **The fold-keyed unique index is deliberately NOT re-keyed onto the nullable `vendor_id`**: Postgres treats NULLs as distinct, so that would silently un-guarantee *asked once*. **(e)** 🔴 **`types.ts` held FOUR phantom tables, not one.** Probed live against a negative control: `growth_goals`, `notification_log` and `ai_usage_log` are **ABSENT** too, all carrying `tenant_id`, none imported. **Only `Vendor` was removed** — the other three are flagged as **tech-debt #178**, because deleting types unrelated to vendors inside a vendor build is the scope creep that makes a diff unreviewable. **(f)** ⚠️ **CARD 5 and CARD 6 must run at `f7ec5d67` as `test obrien`**, per the framing correction above. **CARD 6 is `needs-test` with its reason** — a manager cannot be shown the control, so proving the refusal needs a direct write returning `42501`. **(g)** ⚠️ **STILL OPEN AND UNTOUCHED:** tech-debt **#143** (two duplicate pairs, $1,283.88 overstated) · **#144** (the count-once seam fed by nothing — so this build's id comparison is a correct mechanism on a path not yet travelled) · **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#141** · **#145** · the three `inventory-*` docs bannered but not re-audited.
