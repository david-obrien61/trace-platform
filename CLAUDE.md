...
**NEXT — David's go (report-before-flip ceremony honored):** step 6 flip read source (page `CostToProduce.tsx` + capture mirror select `cost_shape`/`cadence`/`recurring_amount`, feed pool, STOP counting `config.recurring` → no R2 double-count) → step 7 re-point write path (shape+nature+project pickers + substantiation; keep truncation-guard) → step 8 owner-prove. `[TRACE:*]` stays ON until owner-proven. Next layers NAMED not built: API connectors (via `cost_source`) + AI-classify (manual-first).
