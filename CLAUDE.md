...
# Last updated: 2026-08-31 — #243 positions: the starting sets are the workbook's, and my four errors were all authority I invented — see §3
...
### 2026-08-31 (4) — THUNDER **THE STARTING SETS ARE THE WORKBOOK'S NOW, AND MY FOUR WORST ERRORS WERE ALL AUTHORITY I INVENTED. #243. TECH-DEBT #133 CLOSED BY RECONCILIATION.** 🔴 **THE HEADLINE IS THAT I REPRODUCED THIS CAPABILITY'S FOUNDING FAILURE INSIDE THE FIX FOR IT.** David supplied the 2026-08-29 responsibility workbook. **75 of 88 rows agreed; 13 differed each way — and they are not randomly distributed.** Every one of my four worst rows was **authority the business does not grant**: `SEL-05` (set the discount tiers) and `SEL-15` (decide what the business sells and at what price) to the **sales manager**; `SEL-03` (quote off-list) and `SEL-04` (apply a discount) to the **external rep**. ✏️ *"Configuring permissions asks a harder question than the one they could not answer"* is the sentence this whole capability was built on — **and I answered a question nobody asked me, by reasoning about what a job title OUGHT to carry. A derivation over-grants, because a plausible title absorbs whatever sounds adjacent.** 🔴 **AND THE LINE THE WORKBOOK ACTUALLY DRAWS IS SHARPER THAN "LESS AUTHORITY": the sales manager KEEPS `SEL-03`/`SEL-04` and loses `SEL-05`/`SEL-15` — they OPERATE the pricing policy and do not SET it; the external rep carries no price discretion at all.** Three tiers of pricing trust, observed rather than reasoned, and none of it derivable from a title. **BUILDER-COMPLETE** `044183a`→`2852992` on **`thunder/workbook-reconcile`** · `npm run verify` **exit 0 ZERO NET-NEW** (5 / 245 / 10 / 12 / 15) · **52/52 files · 2422 assertions** (+3) · **6/6 MUTANTS CAUGHT on the declaration mechanism** · ZERO schema · ZERO migration · api/ **12/12** · all `[TRACE:POSITIONS]` ON.

**Type:** Reconciliation + close-out. Code: `positionStartingPoints.ts` (**five sets replaced with the workbook's membership**; the provenance header rewritten; **`UNDELEGABLE_SUGGESTIONS`** added) · `positions.test.ts` (**F3 replaced by F3/F3b/F3c + F12b**, the declaration and its self-pruning). Docs: tech-debt **#133 CLOSED** · ledger **#243** · owner-tests **CARD 9** gains two spot-checks and the two open business questions · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-31, the #240 positions build), 1 written — entries-in == entries-out, §3 back to 3** (archive **270 → 271**, `grep -c '^### '`). ⚠️ **On a branch, NOT merged — a deploy is David's call** (`thunder/workbook-reconcile`, `2852992`).

**🔴 THE SECOND FINDING IS THE MIRROR OF THE FIRST, AND IT IS THE MORE USEFUL HALF: WHAT THE WORKBOOK *ADDED* WAS REAL WORK WITH NO SOFTWARE GLAMOUR.** `PPL-09` **train new staff** — `capability: not_software`, **R-30's own headline example, and I had it in NO SET AT ALL**, having written R-30 into that very file. `SEL-13` **ask a customer for a review**, given to the **CREW** rather than the office, *because the person standing in front of the customer after the install is the one who asks.* `INV-03` **record a loss**, given to the driver who sees the damage. `PUR-07` **allocate freight**, to whoever receives the load. ✏️ **None of those is derivable from a job title — you only get them by watching.** So the asymmetry is clean and worth carrying: **my errors were authority I invented; the workbook's additions were unglamorous work I did not think to look for.** That is what a derivation and an observation are respectively bad and good at, and it is an argument for [[R-25]] beyond the invoice corpus it was ruled on.

**🔴 AND THE THIRD: THE WORKBOOK OVERTURNED A RULE OF MINE AND WAS RIGHT TO.** It puts `MON-07` (connect or manage the accounting system) in the **Bookkeeper** set — and `MON-07` is **UNDELEGABLE**. My blanket ban said *a suggestion the platform makes must not propose work the platform will then refuse.* **Both facts are true at once: at LAWNS the bookkeeper does that job, and the platform reserves the permission to the account holder** — and [[R-30]] already decided which one the document describes: **the JOB, not what the app covers.** ✏️ **Suppressing the row would have hidden a real mismatch between how the business runs and what the software permits, on the one screen built to surface exactly that** — and nothing is concealed by including it, because the picker marks it **"Cannot be delegated"** with the reason at the moment of ticking. The ban became a **SELF-PRUNING DECLARATION** in the pattern of `select-policy-declarations.json`: an undeclared undelegable row still fails the build, a declared one passes, **and a declaration whose row has left the set or is no longer undelegable is STALE AND FAILS** — so it cannot rot into the unread noise `OWNER_ONLY_PENDING` became (#73). ✅ **AND F11 EARNED ITS KEEP WITHIN MINUTES OF THE DATA LANDING:** transcribing the workbook I dropped `SEL-03` from the sales manager **and wrote a comment asserting the workbook excluded it** — a declaration false at the moment it was written, **[[R-26]]'s exact shape, from the author who has been counting them** — and the pinned count went red in seconds. **A cheap check that cannot be talked out of noticing is worth more than a careful reader.**

**FLAGGED FOR DAVID:** **(a)** ⚠️ **ON A BRANCH AND NOT MERGED — `thunder/workbook-reconcile` at `2852992`, one fast-forward from `main`.** 🔴 **This matters for your testing right now: the counts did NOT move (34/27/9/8/10), so CARD 9 reads identically — but WHAT EACH SET TICKS HAS CHANGED.** Running CARD 9 against live today tests my sets, not the workbook's. **(b)** 🔴 **TWO SPOT-CHECKS WORTH MORE THAN THE TOTALS.** Apply **Bookkeeper**: *"Connect or manage the accounting system"* must be ticked **AND marked "Cannot be delegated"** — **that pairing is the feature.** Apply **Crew member / driver**: *"Ask a customer for a review after a job"* must be in it. **(c)** 🔴 **TWO QUESTIONS ABOUT THE BUSINESS, FLAGGED RATHER THAN SETTLED IN CODE — and both are yours, not Lauren's:** the workbook gives the crew **no `INV-01`** (walk the lot and count stock), which **sits oddly beside #238/#67**, the whole staff-count-walk line of work — either the workbook is describing a business where the crew does not count, or counting has moved since. And **`MON-10` (pay a contractor or vendor) ends up in NO set at all** — nobody pays the vendors, which cannot be true. **(d)** ⚠️ **THE CAVEAT TRAVELS WITH THE DATA AND I DID NOT UPGRADE IT.** The workbook calls itself *"a DRAFT drawn from watching one business, not a measurement… do not treat agreement with it as proof."* That sentence is now in the file header, because **a draft that loses its own caveat becomes a fact somewhere downstream** — R-26's whole family. **#133 is closed by RECONCILIATION, which is a different and weaker thing than closed by measurement, and the row says so.** **(e)** ⚠️ **STILL OPEN AND UNTOUCHED:** the three `inventory-*` docs stamped `2026-06-13` · `docs/reference/` tracked by accident · `20260830c` unapplied · **#132** (the `authority:baseline` blessing, after CARD 7) · **#134** (a render condition inside a `.tsx` cannot be asserted — the CLASS, not the instance) · **#124** · **#125** · **#126** · **#127** · **#130** · **#131**.
