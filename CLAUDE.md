...
# Last updated: 2026-09-03 — #265 the standard outranks the prompt (G9 · the divergence cap) — see §3
...
### 2026-09-03 — THUNDER **THE VENDOR CHAIN IS APPLIED, AND THE COUNT IT WAS WRITTEN AGAINST HAD ALREADY MOVED. #263.** 🔴 **THE HEADLINE IS THAT CHECK (F) IS THE EVIDENCE AND THE WORD *applied* IS NOT.** David ran both remaining migrations and (F) returned **`unresolved 37 / total 37`** — **every stored receipt is UNCLASSIFIED**, so nothing was silently linked to a vendor. That is [[R-50]] holding **at the database**: the machine did not retro-classify one row. Also verified: `vendors` + `vendor_aliases` RLS on with **7 policies**; **`receipts.vendor` SURVIVES beside `vendor_id`**, so the captured string stays as evidence; both unique indexes fold on `lower(btrim(...))`; **owner-only enforced on BOTH verbs**, which a column GRANT could never do because owner and manager are both `authenticated`; and `vendor_preferences_one_per_vendor_kind_uidx` **SURVIVED** the consolidation, which is what keeps *asked once* structural rather than a UI promise.

**Type:** Records. **RECORDS ONLY — ZERO changed lines under `packages/` and `api/`.** The two migrations touched are **COMMENT-ONLY, proven by filtering the diff BOTH directions — 0 non-comment lines added AND 0 removed, not asserted** (§6 r1; the #261 precedent). `npm run verify` **exit 0, ZERO net-new** · **api/ 12/12, nothing minted · NO new permission.** ONE new read-only script, `scripts/measure-vendor-chain-applied.mjs`. **§3 RETENTION: 1 archived verbatim (2026-09-02 #257), 1 written — entries-in == entries-out, §3 back to 3** (archive **283 → 284**).

**⚠️ TWO CHECKS ARE RECORDED AS NOT-PASSED, AND THAT IS THE POINT OF WRITING THEM DOWN.** **Check (D)** — the cross-tenant refusal in `link_vendor_preference()` — **DID NOT RUN**: it needs a preference row in one business and a vendor row in another, and **all three tables are empty** (`vendors` 0 · `vendor_aliases` 0 · `vendor_preferences` 0, measured against a **negative control** proving a missing table errors `PGRST205`, so *0 rows* is a real read and not a failed one). David pasted the placeholder text literally and got `22P02` — **that is the placeholder, not the guard.** And the **optional pre-flight negative control was not reported either way**, so it is recorded **UNKNOWN**, not passed. ✏️ **[[R-33]]: a guard nobody has seen refuse is a claim, and the only honest cell for it is owed.**

🔴 **A BASELINE MOVED AND WAS STILL BEING QUOTED — R-26'S EXACT SHAPE, INSIDE OUR OWN CORPUS.** #259 measured **36 receipts across 3 tenants** and wrote it into a migration header, two owner-test boards, `built-inventory.md` and `user_stories.md`. (F) returned **37**. Found: **`TRACTOR SUPPLY CO` $8.65** on **`Test Dave's Tree Nest`**, captured **2026-09-02T21:56:12Z** by `dave_obrian@outlook.com`. Tenants now **Test Dave's 18 · LAWNS 17 (UNCHANGED) · Test David's new Business 2** — every LAWNS-scoped figure in the corpus is still correct. **Swept and DATED rather than back-edited**, because a snapshot that names its date is honest and one that does not is a claim. ✏️ **AND DAVID'S OWN HYPOTHESIS FOR THE ROW DOES NOT SURVIVE THE EVIDENCE:** it was **not** a CARD 6 test. **CARD 6 is an EDIT, not a capture** — it writes no `receipts` row — `audit_log` holds **0 rows since 2026-09-02**, so cards 6/7/8 have not been run at all, and the capture **predates the migration apply by a day**, when 6–11 were still blocked. An ordinary Receipt Keeper capture on his own test tenant.

**FLAGGED FOR DAVID:** **(a)** 🔴 **TWO RECORDS COULD NOT BE UPDATED FROM HERE AND ARE RELAYED, NOT SILENTLY SKIPPED ([[R-62]]):** the **`#259` ledger row** and the **12-card `docs/owner-tests/vendors-full-surface-test.md` board** exist ONLY on `thunder/vendor-identity`. `78e1c22` is untouched and its worktree clean. **You must relay to session `d967011d`.** **(b)** 🔴 **A HAZARD I CREATED AND WILL NAME RATHER THAN LET YOU FIND: annotating the two migration headers on `main` BROKE the byte-identity I reported yesterday.** The merge is now an **add/add CONFLICT**, not a silent auto-resolve. **The resolution is one line: take `main`'s copy of both `.sql` files** — comment-only superset, 0 SQL lines differ. **(c)** ⚠️ **[[R-65]] moved OPEN → PARTIAL** — the join landed and the two stores read as one, but **there are still two tables**; `vendor_key` and its unique index are deliberately kept, and the feared data migration is **not owed** because nothing is stored on either side yet. **(d)** ⚠️ **`receipt-detail` CARDS 6–11 are runnable since #261 and still unrun** — `audit_log`'s 0 rows since 2026-09-02 proves it. **(e)** ⚠️ **`thunder/vendor-identity` is UNBLOCKED to merge**; its remaining work is app-side (`Vendors.tsx`, the tile, the 12-card board), none of it schema. **(f)** ⚠️ **STILL OPEN AND UNTOUCHED:** `origin/assets` (remote-only, 67 days) · `docs/reference/` tracked by accident · tech-debt **#73** · **#124**–**#127** · **#132** · **#134** · **#135** · **#139**–**#141** · **#143**–**#145** · **#148**–**#151**.
