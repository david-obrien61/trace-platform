...
# Last updated: 2026-08-23 (8) — recon: can a test tenant be deleted? (no — see §3)
...
**Type:** Recon + close-out. Docs: ledger **#199** · `RULINGS.md` (**FIVE NEW OWED rows T-A…T-E**) · ⚡ ACTIVE STATUS · `built-inventory.md` · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-08-23 (5) #196), 1 written — entries-in == entries-out, §3 back to 3** (archive 229 → 230). ✏️ **CLAUDE.md read IN FULL and `docs/RULINGS.md` read IN FULL (149 lines, four chunks incl. the whole OWED table).**

**🔴 Q9 IS THE FINDING, AND IT ANSWERS "WHY DID WE RESET THE ENTIRE DB LAST TIME" IN ONE SENTENCE: THERE WAS NOTHING ELSE TO REACH FOR, AND THERE STILL ISN'T.** No tenant-delete path exists **anywhere** — 21 `from('businesses')` sites in `packages/*/src` + `api/`, **zero deletes**; no `delete_business` RPC, no script, no admin surface. The only teardown artifact is **`scripts/wipe-for-person-spine.sql`, a FULL-DATABASE NUKE** whose own header says *"⚠️ DESTRUCTIVE — this deletes ALL tenant data AND ALL auth users."* 🔴 **And it names its own replacement: *"the real LAWNS demo comes back … via **Build B (the per-tenant seed/reset function, queued — not built yet)**"* — 2026-06-25. Two months on, Build B was never built**, which is filed as ruling **T-C** rather than quietly added to the inventory as a gap. ✏️ Worth saying the nuke is *careful* — transactional, guards re-enabled in the same transaction, a `pg_trigger` proof at the end — **it is simply scoped to the whole database, with no `WHERE` anywhere.**

**⚠️ THE METHOD IS THE POINT, AND IT CUT AGAINST GIVING A CONFIDENT ANSWER.** STD-021 says a database claim comes from the catalog or is not made — and here the repo is **structurally blind to the tables that matter most: NINE core tenant tables have NO `CREATE TABLE` migration at all** (`orders`·`order_items`·`customers`·`cost_objects`·`plant_events`·`cultivar_plants`·`order_addons`·`addons`·`social_drafts` — tech-debt **#39** landing directly on this task). Four of them have **no FK to `businesses` declared anywhere in the repo**, which is exactly Q3's orphan class. Second proof the repo lies: **`opportunity_items` appears with BOTH `CASCADE` and `NO ACTION`** across different migrations. **So every FK claim in the document is labelled a HYPOTHESIS and the Q1–Q8 queries are handed to David unrun.** ✏️ **I did not work around the boundary:** reading `.env.local` was **denied by the sandbox**, and rather than route around it I recorded that the recon is catalog-unverified and said so on every claim. **The queries themselves are REUSED, not invented — `wipe-for-person-spine.sql` STEP A already holds proven-run versions of the FK-graph, guard-list and `business_id`-table queries (§6 r8).**

**FLAGGED FOR DAVID:** **(a)** 🔴 **FIVE RULINGS OWED, and T-B is the one that decides the rest: MAY AN APPEND-ONLY GUARD EVER BE DISABLED TO REMOVE A TENANT?** The 2026-07-20 ruling says a ledger row is immutable *including against `postgres`*; the wipe script **did** disable every guard once, explicitly labelled a one-time teardown *"never a reusable normal-path move."* **If the guard is absolute, hard delete is off the table by construction and T-A answers itself.** **(b)** 🔴 **Q7 IS A REAL ORPHAN FINDING NOBODY HAD NAMED: `storage.objects` carries the business id as the FIRST PATH SEGMENT OF THE OBJECT NAME, not as an FK** (`20260613:36-47`). No cascade can reach it — and because the policy resolves through `businesses`/`business_members`, **deleting the row makes the receipt files orphaned AND permanently unreachable through the app, for everyone.** The wipe script calls these *"harmless orphans"* — **true in a full nuke, false per-tenant. Files must go BEFORE the row or they cannot go at all.** **(c)** ✅ **The recommendation is TOMBSTONE, and it is not a new opinion — it is the reasoning David already used to rule out the campaign delete** (*a deleted thing destroys its own history*). Here it is stronger, because the platform has already encoded that belief in two triggers. ⚠️ **Its cost is stated rather than buried: every read listing businesses must filter `deleted_at IS NULL`, and a tombstoned tenant still showing somewhere is a worse bug than a slow cleanup — one column and N read sites, and the N is the work.** **(d)** ✏️ **FOR WEDNESDAY, THE ANSWER IS THE CHEAP ONE: create the throwaway tenant, walk the provisioning path, and LEAVE IT.** It is the honest state, it costs nothing, and it is **the only option needing no ruling and no code before Wednesday** (T-D). **(e)** ✅ **Q11's guard set, since none of it exists today: a NAME CHECK IN THE SAME STATEMENT is the strongest of the three** — it fails on the typo itself rather than its consequences — plus a dry-run count read by a human and a `RETURNING id, name`. ⚠️ **The only protection standing right now is accidental: the guards and the `NO ACTION` FKs would abort a mis-pasted delete against `f7ec5d67` too. That is luck by good design, not a control.** **(f)** 🔴 **The git re-run at write-up was CLEAN this time** — HEAD still `f0e195f`, tree carrying only the new doc; last session's stale-snapshot catch did not recur, and I checked rather than assumed. **(g)** **CLAUDE.md is 712 lines against its own ~600 budget** — the §4 residual OP-13 says N=3 alone does not close.
