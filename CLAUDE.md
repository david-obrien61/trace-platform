...
# Last updated: 2026-07-31 — a planned tile renders BECAUSE it is planned, not because someone holds its string (#176). See §3.
...
### 2026-07-31 (5) — THUNDER `planned` WAS REACHABLE EXACTLY FOR THE PEOPLE WHO DID NOT NEED TO BE TOLD THE FEATURE WAS COMING. `useModules.ts:88` filtered `.filter(t => can(t.required_permission))` and read `status === 'planned'` **after** it — so a planned tile rendered only for a session that **already held its string**. **The inverse of the state's purpose**, and invisible from the owner's chair, where every string is held. **BUILD 2 COMPLETE; BUILDER-COMPLETE (`npm run verify` exit 0, 60 capA probes); cards 19–20 owed and card 12 flipped `needs-test` → `owed`, board 0 of 20.** ZERO new api-fn (12/12), no schema.

**Type:** App code + tooling — `useModules.ts` (the filter) · `Tile.tsx` (**NEW `TileState` `'planned'`**) · `Dashboard.tsx` (the readout branch — **`<BeingBuilt>` mounted**) · `verify-authority-checks.mjs` (**assertion 6**, 5 → **6 assertions**, 51 → **60 probes**) · 2 baselines re-locked for line-shift. Docs: ledger **#176** · **1 NEW RULING** + BUILD 1's finding recorded above the mint + **#89 CLOSED** · **2 NEW cards** + card 12 rewritten · `built-inventory.md` · ⚡ ACTIVE STATUS · DECISIONS-INDEX drift · this §3 + line 3. **§3 RETENTION: 1 archived verbatim (2026-07-31 (2) capA ASSERTION 4 #173), 1 written — entries-in == entries-out, §3 back to 3.**

**THE SECOND HALF, and it is why the tile that started this whole thread still rendered nothing.** `dashboardTiles()` filters `kind !== 'readout'`, so `business_insights` **never entered `useModules` at all** and never could — and `Dashboard.tsx` had **no render block for it whatsoever**. It has carried `status:'planned'` since the day it was registered and rendered **NOTHING, on any session, ever.** Fixing only the grid filter would have left it exactly as it was. A `readoutPlanned()` branch now renders **`<BeingBuilt>` — mounted for the first time**, having been written in Phase 3 (#171) and left unused for eight days, which is precisely what card 12's `needs-test` reason said out loud. That card is now runnable.

**🔴 `planned` IS A NEW TILE STATE, AND THE OLD MAPPING WAS THE SIX-STATE DEFECT IN A GRID.** `useModules` mapped `status:'planned'` → `'locked'`, and `Tile.tsx` renders `locked` as **a greyscale square with a RED LOCK** — which says *"you are not allowed"* about a thing **nobody** is allowed, because it does not exist. **Collapsing them makes a roadmap item look like a snub and a snub look like a roadmap item** — the six-state ruling's own words, and the grid had been doing it all along. `TileState` gains `'planned'`: amber **SOON**, its own colour at reduced strength rather than greyscale, inert like `locked` so every interaction guard reads both. **What the fix reveals is worth stating: until today the ONLY tiles ever rendering a red lock were the UNBUILT ones**, because unheld tiles were filtered out of the grid entirely.

**capA ASSERTION 6 IS DELIBERATELY HALF AN ASSERTION.** ASSERTED: a tile gating on a `planned` permission must itself be `status:'planned'`. **NOT ASSERTED: the converse** — a planned SURFACE may legitimately gate on a LIVE string, and **7 of the 8 planned tiles do exactly that** (`online_shop`→`settings:read`, `contractor_tiers`→`pricing_recipe:update`, …). Asserting it would fail the build on seven correct rows. The cap **prints those seven every run**, so the exemption is visible rather than implicit, and **probe T4 proves the converse PASSES** rather than leaving it to a comment. **STD-024 red-first against the REAL registry, not a fixture:** flipping `business_insights` to `status:'live'` makes it report the row by name and exit 1; restored, exit 0.

**ONE SOURCE PER SURFACE, held mechanically rather than by intention.** The readout branch reads **`tileByKey(key)?.status`** and never the manifest. Roles page reads the manifest; dashboard reads the tile; **`maintenance:override` is the proof they must stay separate — a `planned` permission with NO TILE AT ALL.** Assertion 6 is the only join between them and it fires only when both exist and disagree.

**🔴 THE HONEST TAX: tech-debt #78 FIRED FOR THE THIRD TIME TODAY.** Two baselines re-locked again for pure LINE SHIFT — comments I added moved two Dashboard mutation sites down eleven lines and `zero-row-writes` reported them as NEW. **Three occurrences in one day is a per-edit tax, not an anecdote**, and the fix is the shape this session's own new baseline already uses (`file::binding`). Also unchanged and still owed: **the requirement hook is not built** — `onPlannedSelect` remains a declared seam.

**FLAGGED FOR DAVID:** **(a)** **card 20 is the only card that proves the fix, and it must be run as STAFF** — as the owner it passes either way, which is exactly how this survived. **(b)** **card 19** — amber `SOON` tiles, not red locks, plus a **Business insights "coming soon"** panel that has never appeared before. **(c)** **card 12 is live at last** and worth running beside 19: BEING BUILT next to a 🔒 NOT PERMITTED on one screen. **(d)** **#90 stays filed** — `team:*` removal is still blocked on the applied-migration reconciliation. **(e)** **#78 wants a decision** after three hits in a day.
