...
**NEXT — David's go:** step 7 re-point write path (Settings recurring entry → `cost_objects` + shape/nature/project pickers + substantiation; keep truncation-guard) → step 8 owner-prove the live tile through the UI under RLS. `[TRACE:*]` stays ON until owner-proven. Next layers NAMED not built: API connectors (via `cost_source`) + AI-classify (manual-first).
