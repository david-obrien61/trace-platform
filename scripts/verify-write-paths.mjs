...
const ALLOWED_DIVERGENCE = {
  ...
  // (`insertShipToSite` / `retireShipToSite`). All three contact tables have ONE writer FILE among the
  // app's `.from()` writes — `contactRecord.test.ts` §G asserts that set in both directions.
  // ✏️ SAME DAY, after merging #342: the one all-or-nothing undo (`undo_import_run`, now carrying
  // 20260916d) is the second path on each list, by design. Declared, not baselined.
  'customer_phones': {
    reason: 'The contact lists have ONE app writer, contactWriter.ts, and ONE other path: the import UNDO, itemImportWriter.ts calling the undo_import_run RPC, which since 20260916d deletes the run\'s TAGGED contact rows before its customers inside the same transaction (customer_addresses is ON DELETE RESTRICT, so it must). The undo is not a writer of contact details — it removes a run whole, all-or-nothing, and refuses on any untagged (hand-added) row. Folding it into contactWriter would split one transaction into PostgREST calls, which is the defect 20260916c exists to remove (ledger #335/#342).',
    paths: ['packages/shared/src/business-logic/contactWriter.ts',
