...
const MUTANTS = [
  ...
  { id: 'L1', target: PAGE, suite: PAGE_SUITE,
    why: '🔴 the species roll-up comes back — 29 rows by variety between the mix and the posts, pushing the bulk onto page 2',
    from: '              <div style={S.row} className="ll-row">\n                <span style={S.big}>{LOAD_LIST_COPY.treesLine(model.treeCount, model.stopCount)}</span>',
    to:   '              {model.trees.map(t => <div key={t.name}>{t.name} × {t.quantity}</div>)}\n              <div style={S.row} className="ll-row">\n                <span style={S.big}>{LOAD_LIST_COPY.treesLine(model.treeCount, model.stopCount)}</span>' },
  { id: 'L2', target: PAGE, suite: PAGE_SUITE,
    why: 'the day\'s trees line loses its stop count — "29 trees" with nothing to check the stops against',
    from: '{LOAD_LIST_COPY.treesLine(model.treeCount, model.stopCount)}',
    to:   '{`${model.treeCount} trees`}' },
  { id: 'L3', target: LIB,
    why: 'the one-line count drops the stops',
    from: "    `${trees} tree${trees === 1 ? '' : 's'} across ${stops} stop${stops === 1 ? '' : 's'}`,",
    to:   "    `${trees} tree${trees === 1 ? '' : 's'}`," },
