...
const MUTANTS = [
  ...
  { id: 'A3b', target: LIB,
    why: '🔴🔴 THE BUBBLER GOES BACK TO ONE PER TREE — David, 2026-09-18: *"not on every tree, only those specified"*. Saturday would print 27 again instead of none, and the crew would load 27 manufactured parts nobody bought.',
    from: '    bubblers: stops.reduce((n, st) => n + st.bubblers, 0),',
    to:   '    bubblers: stops.reduce((n, st) => n + st.treeCount, 0),' },
  { id: 'A3c', target: LIB,
    why: '🔴 the billed bubbler line stops being the count — a specified 3 reads as 0 and the crew arrives without them',
    from: "      bubblers: items.filter(i => i.kind === 'bubbler').reduce((n, i) => n + i.quantity, 0) * settings.ops.bubblersPerTree,",
    to:   '      bubblers: 0,' },
  { id: 'A3d', target: LIB,
    why: '🔴🔴 WATER MONITOR KITS STOP COUNTING THE INSTALLED TREES — every tree LAWNS plants needs one, and the sheet would say none',
    from: '      waterMonitors: (s.installs ? sums.treeCount : 0)',
    to:   '      waterMonitors: (false ? sums.treeCount : 0)' },
  { id: 'A3e', target: LIB,
    why: '🔴 a BILLED water monitor kit stops adding on top — a customer who bought kits for their own planting gets none',
    from: "        + items.filter(i => i.kind === 'water_monitor').reduce((n, i) => n + i.quantity, 0),",
    to:   '        + 0,' },
  { id: 'A4', target: LIB,
  ...
  { id: 'B4', target: LIB,
    ...
    from: "      return { ...base, kind: 'other_goods', reason: `Sold by ${r.unit}",
    to:   "      return { ...base, kind: 'tree', reason: `Sold by ${r.unit}" },
  { id: 'B5', target: LIB,
  ...
  { id: 'C14', target: LIB,
    why: '🔴🔴 GOODS STOP PRINTING — a customer\'s fertiliser, tarp or stake kit is dropped with the fees, and the crew arrives without it (David, 2026-09-17: anything physical prints)',
    from: "      return { ...base, kind: 'other_goods', reason: `Sold by ${r.unit}",
    to:   "      return { ...base, kind: 'not_loaded', reason: `Sold by ${r.unit}" },
  { id: 'C16', target: LIB,
    why: '🔴 a stop is treated as an install when the order does not say so — every delivered tree would be given a water monitor kit',
    from: '      installs: !!s.installs,',
    to:   '      installs: true,' },
  { id: 'C11', target: LIB,
  ...
  { id: 'P3', target: PAGE, suite: PAGE_SUITE,
    ...
    to:   '            {[].map(r => (' },
  { id: 'P8', target: PAGE, suite: PAGE_SUITE,
    why: '🔴 the page stops rendering "Also on the truck" — the model reads the goods and the sheet drops them, which is the same omission one layer out',
    from: "            {model.otherGoods.length > 0 ? (",
    to:   '            {false ? (' },
  ...
  { id: 'L1', target: PAGE, suite: PAGE_SUITE,
    why: '🔴🔴 the PULL list is gone — the yard pulls by variety and the sheet no longer says what to pull (ledger #358)',
    from: '              <h2 style={S.h2}>{LOAD_LIST_COPY.pullHeading(model.treeCount, model.stopCount)}</h2>',
    to:   '              <h2 style={S.h2}>{`${model.treeCount} trees`}</h2>' },
  { id: 'L2', target: PAGE, suite: PAGE_SUITE,
    why: '🔴 the pull list moves BELOW the stops — pulled after staging, which is not the order the work happens in',
    from: '              {model.trees.map(t => (\n                <div key={`pull|${t.name}|${t.rungLabel}`}',
    to:   '              {[].map((t: any) => (\n                <div key={`pull|${t.name}|${t.rungLabel}`}' },
  { id: 'L3', target: LIB,
    why: 'the pull heading drops its stop count — no way to check the pull against the day',
    from: "    `Trees to pull — ${trees} across ${stops} stop${stops === 1 ? '' : 's'}`,",
    to:   "    `Trees to pull — ${trees}`," },
  { id: 'L4', target: PAGE, suite: PAGE_SUITE,
    why: '🔴 the rope moves off page 1 — below the stops, where the person loading the bulk never turns',
    from: '              <div style={S.row} className="ll-row">\n                <span style={S.big}>{model.ropeFeet} ft rope</span>\n                <span style={S.note}>{LOAD_LIST_COPY.ropeRule(model.valuesUsed.ropeFeetPerTPost)}</span>\n              </div>\n',
    to:   '' },
