...
const MUTANTS = [
  ...
  { id: 'A1', target: LIB,
    why: '🔴 THE COST MODEL\'S 0.7 MIX RATIO RETURNS — every load on every day is ~30% short of mix and the page states the shortfall as a fact (tech-debt #291)',
    from: '  mixRatioOfContainerVolume: 1.0,',
    to:   '  mixRatioOfContainerVolume: 0.7,' },
  { id: 'A2', target: LIB,
  ...
  { id: 'A6', target: LIB,
    why: '⚠️ a mulch line returns to the model — materials that are never bought, which is exactly what the cost model gets wrong (tech-debt #290)',
  ...
  { id: 'C2b', target: LIB,
    ...
    from: '  const mixGallons = trees.reduce((n, t) => n + t.gallons * t.quantity * BOM_RULES.mixRatioOfContainerVolume, 0);\n  const tPosts = trees.reduce((n, t) => n + t.tPosts * t.quantity, 0);\n  const unresolved',
    to:   '  const mixGallons = trees.reduce((n, t) => n + (t.gallons > 95 ? 0 : t.gallons) * t.quantity * BOM_RULES.mixRatioOfContainerVolume, 0);\n  const tPosts = trees.reduce((n, t) => n + t.tPosts * t.quantity, 0);\n  const unresolved' },
  { id: 'C3', target: LIB,
  ...
  { id: 'D2', target: LIB,
    ...
    to:   '  const treeCount = trees.length;\n  const mixGallons = trees.reduce((n, t) => n + t.gallons * BOM_RULES.mixRatioOfContainerVolume, 0);\n  const tPosts = trees.reduce((n, t) => n + t.tPosts, 0);' },
  { id: 'D3', target: LIB,
