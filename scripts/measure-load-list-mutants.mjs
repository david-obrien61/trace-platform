...
const MUTANTS = [
  ...
  { id: 'C3', target: LIB,
    ...
    from: '    totalsAreFloors: unresolved.length > 0 || unreadStops > 0 || noVolumeTrees.length > 0,',
    to:   '    totalsAreFloors: false,' },
  { id: 'C3b', target: LIB,
    why: '🔴 an unreadable STOP stops raising the floor',
    from: '    totalsAreFloors: unresolved.length > 0 || unreadStops > 0 || noVolumeTrees.length > 0,',
    to:   '    totalsAreFloors: unresolved.length > 0 || noVolumeTrees.length > 0,' },
  { id: 'C3c', target: LIB,
    why: '🔴 a size with NO VOLUME stops raising the floor — its mix is missing and the total reads complete',
    from: '    totalsAreFloors: unresolved.length > 0 || unreadStops > 0 || noVolumeTrees.length > 0,',
    to:   '    totalsAreFloors: unresolved.length > 0 || unreadStops > 0,' },
  { id: 'C3d', target: LIB,
  ...
  { id: 'C7', target: LIB,
    ...
    to:   '    // not counted' },
  { id: 'C8', target: LIB,
    why: '🔴 the "no container size" lines are filtered away (R-144)',
