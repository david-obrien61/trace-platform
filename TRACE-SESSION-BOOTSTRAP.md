...
>
> **Last updated:** 2026-09-14 — **#323** the two assertions built: **`verify-main-ancestry`** (#280 ① — local `main` never ahead of `origin/main`; proven red with a real commit) and **`verify-id-citations` clause E** (#294a — a ledger row may not exceed the header; red-first naming the four predicted, then repaired). **#280 stays 🟡 PARTIAL — ② unchanged.** ⚠️ A live `#323` collision is another session's to move. Prior: 2026-09-14 — **#321 OWNER-PROVEN** (David's live run: *"the foot of the screen reads · prod"*); **`production-stamp` board created to receive the flip — it did not exist**, an OP-14 gap in #321; CARD 1 `covered`, 1 of 5. Prior: 2026-09-14 — **#321** the **production stamp**, MERGED and **READ LIVE**: `built Sep 14, 12:28p · 15fe4f2 · prod` on `cultivar-os.app`; GATE 0 on all 40 boards requires `prod`; tech-debt **#280 → 🟡 PARTIAL (observable, NOT asserted)** and **① sharpened after this session's own unpushed merge**; **#294a** separated with a **proposed** row-shape check. Prior: 2026-09-14 — **#321** the **production stamp**: the deployed bundle now says WHERE it is, not only which code — `built <time> · <sha> · prod`, or an amber `PREVIEW <branch>`; **GATE 0 on all 40 boards now requires `prod`**; tech-debt **#280 → 🟡 PARTIAL (observable, NOT asserted)**; **owner-proof owed — the stamp cannot prove itself**. Prior: 2026-09-14 — **#320** a close-out ledger row with **no CLAUDE.md §3 entry** is now an **INCOMPLETE CLOSE-OUT**, asserted by `verify-handoff-retention` check 5; **fifteen rows had none** (they never entered — they did not scroll out), declared not backfilled in `handoff-entry-declarations.json`, a list that prunes itself both ways; **§3 entries trimmed to headline + `Type:` + state + `FLAGGED FOR DAVID`**, the narrative moved verbatim to the ledger row; CLAUDE.md **678 → 651 lines, −14KB**; **N=3 unchanged**. Prior: 2026-09-12 — **#314** tech-debt **#286** RESOLVED: the sweep's two populations split, inheritance decided per id at the merge-base, `main` made a rival, proven by making it fail from `main`. Prior: **#312** six owner-test cards flipped `covered` from David's live runs (campaign 1/2/4 · ship-to 1/3 — `customer_addresses` **APPLIED**); the ship-to CARD 3 leak clause split (tech-debt #279); tech-debt **#286** — the id sweep reports a taken id as free from `main`. Prior: **#303** the ship-to address book (`customer_addresses`) — D-41's L2 hook taken up; migration written, NOT applied; no string minted; no backfill (tech-debt #279) · **#301** one stop, three screens — schedule, route and order render the same stop; ship-to editable; checkout stop carries its order · **#300** the Google review link is on Business Profile; the ask cannot run until Follow-Up can be turned on (#270) · **#299** recon: `orders.install_date` is never set; `business_inventory.location` exists and is empty; no warranty code exists (R-143 · tech-debt #267–#269) · **#298** zone walk hosted at `/tools/zone-walk.html`; its import target does not exist (#266) · **#297** PMI: the asset table was renamed, not dropped — no second table written; `20260529_pmi_shared` retired (R-139); generator documented; R-140…R-142 · **#296** R-121 settled: no OWNER-role member can remove their own permissions · **#295** 17 rulings filed (R-122…R-138); 🔴 R-125 *every action audited* conflicts with D-51 · **#294** handoff transcribed; **R-121** owners cannot remove their own permissions; migration report (`20260905` needed now, #253) · **#293** a transport service must say who transports (R-120; the editor's required check could never fire) · **#292** which migrations are in the database is derivable (`--catalog`: NOT_APPLIED 3, all known; `nursery_modules` found dropped by hand) · **#290** the audit redirect is scoped (triggers for coverage, RPCs for refusals; `audit_log` has no reader) · **#289** the 49 raw `owner_id` policies triaged, both migrations APPLIED · **#288** the permission test becomes literal · **#287** the tax-rate wipe. ⚠️ **THIS LINE IS A POINTER, NEVER A SUMMARY (STD-011 / OP-13).** 🔴 **IT READ `2026-08-24` UNTIL 2026-09-10 — SEVENTEEN DAYS AND AT LEAST FOUR SESSIONS BEHIND, ON THE FIRST DOCUMENT EVERY SESSION OPENS.** Corrected by the #290 session, which did not create it: a stale date on the canonical status front-page is a freshness claim nobody earned, and **leaving it because it predates you is how it reached seventeen days.** The prior header's prose is preserved verbatim in `docs/handoff-archive.md` under the #290 provenance comment. ⚠️ The per-item statuses below are what the §9 gate actually maintains; if an item's line disagrees with `built-inventory.md`, the inventory wins and the line is DRIFT (PRIOR HEADER: **#207 — 🔧 BUILD: THE SWALLOWED STORAGE FAILURE. A LOCAL WRITE THAT DID NOT PERSIST STOPS REPORTING SUCCESS.** 🔴 **`store.ts` discarded a quota / disabled-storage exception, so `enqueue()` returned normally over a queue that had not grown — under a banner reading *"counts are saved on this phone and will sync when you're back in signal."*** 🔴 **AND THE HALF THE RECON DID NOT HAVE, MEASURED BY COUNTING DATABASE CALLS: THE ONLINE PATH WAS WORSE — it returned `applied`, the strongest claim the engine can make, with `drain applied:0 failed:0 remaining:0` and the database called ZERO times.** Success inferred from an ABSENCE (A9/D-9 on the write path; R-12's class on the client side of the wire). ✅ **`save()` STILL NEVER THROWS — the exception is RETURNED** as `StoreWriteResult`, R-11's union discipline applied to the write side, with `quota` and `unavailable` kept distinct because they need different actions. ✅ **AN UP-FRONT WRITE-THEN-READ-BACK PROBE warns BEFORE the walk** — *the difference between losing one entry and losing an afternoon* — and reads back deliberately, because a Private tab can ACCEPT `setItem` and persist nothing. ✅ **AFTER: offline → `failed` naming STORAGE; online → `applied` with the database called ONCE, so the claim is TRUE.** ⚠️ **IndexedDB NOT taken — OWED as S-A, with its data answer written down.** **GATE 0 PROVEN BY WATCHING THE TRANSITION `680b00d` → `4056de8`.** NO schema, NO migration, NO cap.) — PRIOR: 2026-08-23 (15) (**#206 — 🔧 BUILD: THE SCANNER STOPS BLAMING THE TAG FOR A DEAD ZONE — R-11's FIRST INSTANCE.** 🔴 **`stockLineResolver`'s three reads discarded the Supabase `error` and substituted `(rows ?? [])`, so a network failure returned the BYTE-IDENTICAL value a genuinely absent tag returns — and the app told Lauren, by name, in a modal, to *check the tag*.** ✅ **THE FIX IS A TYPE, NOT A DISCIPLINE: new `ReadResult<T>` = `{ok:true;value}` | `{ok:false;error}` — `.value` is unreachable without handling the failure, so `tsc` refuses the careless caller.** **PROVEN BY THE CHANGE: the signature flip produced 36 NET-NEW `tsc` ERRORS, one per site that had been ignoring a failure, across ALL 5 call sites in 3 files** — the thing a returnable `{data,error}` demonstrably cannot do. ✅ **`isConnectivityError` MOVED (not copied) to `utils/supabaseError` — one predicate, two consumers (§6 r8).** 🔴 ***"Check the tag"* can no longer appear on a connectivity failure, on any of the three screens** — checkout gets its own `'unreachable'` phase, the count sheet swaps its heading and **keeps typed entry open**, the QR profile stops saying *Plant not found*. ⚠️ **AND BEYOND COPY: resolve-before-create now REFUSES on a failed read instead of falling through to *genuine NEW variety*** — an unreadable catalog looked identical to an empty one and the answer was *create it*. ✅ **The undocumented 24-hour QR read cache is DECLARED, not removed.** **GATE 0 PROVEN FROM THE DEPLOYED BUNDLE (`3d0944b`) — new copy present AND old copy survived.** ⚠️ **R-11's 30-site build STAYS OWED.** NO schema, NO migration, NO cap.)
