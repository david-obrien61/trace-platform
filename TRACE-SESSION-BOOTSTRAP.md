...
- ⚠️ **NOT BUILT AND SAID SO:** **there is no screen** (nor has #277 one — checked, not assumed; the board drives both from the console) · **no undo route** — `customers:delete` is an UNMINTABLE verb (R2/A3) and `orders.customer_id`'s `ON DELETE` rule is live-only schema (#39), so R2's FK question is unanswerable from the repo (**tech-debt #197**) · **no merge** — the 72 are flagged and shown, never merged · **terms and discounts deliberately skipped** (`SalesTermRef` is on 2 of 1,946).
- 🔴 **OPEN, DAVID'S CALL:** ① **wire the undo or rule the import one-way** — until then **import onto Test Dave's, not LAWNS** · ② the **story gate is open**, same gap as #277 · ③ **`customer_type` is a stated rule, not a QuickBooks fact** — ~559 of 1,946 land as organizations and only David's eye can check it.
