...
- 🔴 **BEFORE CLAIMING ANY ID: `npm run verify:id-sweep`, AND RUN IT FROM A BRANCH — NOT FROM `main`.** The
  ...
  **Then RESERVE AND PUSH before building** (R-149) — the sweep does not close the race and says so.
  ...
  - ✏️ **THE CACHED NUMBERS THAT USED TO SIT HERE WERE DELETED 2026-09-12 (ledger #316, David's ruling — option
    (a), closing tech-debt #289).** They were wrong three times in one day by three different mechanisms — stale
    by a merge that consumed an id, stale by a manual survey that could not see far enough, stale by nothing at
    all but an hour passing on a busy tree — **and every correction was itself wrong within the hour.** David:
    ***"A number that is usually right is worse than no number, because it gets trusted."*** The line already
    said RUN THE SWEEP; the number was the convenient half, so it was the half that got read.
...
- 🟡 **BUILDER-COMPLETE on `main` · api/ 12/12 · no permission string minted · no column added to `customers` or `deliveries` · 12 owner-test cards, 0 COVERED** → `docs/owner-tests/customer-addresses-full-surface-test.md`
