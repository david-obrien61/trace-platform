...
- 🟡 **#390 + #391 (2026-09-23) — THE GROW LADDER · BUILDER-COMPLETE · ⛔ 4 MIGRATIONS WRITTEN AND HELD, NOTHING APPLIED, NOTHING MERGED** · `container_ladder` gains `grow_months`/`hold_months` (`20260923h`) and `sellability`/`sellability_because` (`20260923k`) · NEW append-only `production_rung_dates` — one row per entry or correction, current is the latest, no UPDATE/DELETE policy for anyone (`20260923j`) · `business_operations_config_history` written by a TRIGGER so it cannot drift (`20260923l`) · **Sellable from** on `/inventory/uppot` in FOUR states (date · *not sold at this size* · *UNKNOWN — set GROW on the X rung* · *no uppot window set*) · `commitPlan` now writes `grow_months` + `sales_per_month`, live since `20260905` and never written · NEW `npm run migration:slot` (reads worktree working directories — where untracked migrations hide from `verify:id-sweep`) · rulings [[R-176]]–[[R-179]] · apply order: **h → j → k → l**, then the step-0 file · branches `feat/grow-ladder-rungs` → `feat/grow-ladder-dates` (stacked) · owed: uppot CARDS 32–36 → `docs/CLOSE-OUT-LEDGER.md` #390, #391
