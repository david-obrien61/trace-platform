...
>
> **Last updated:** 2026-09-26 — **#422 BUILT WITH CAI.** David's own GENERAL business, to be served at `builtwithcai.app`. 🔴 **NO MIGRATION IS OWED AND NONE WAS WRITTEN: `business_type` is unconstrained text (LIVE), so `'general'` was already legal — and `/add-business` ALREADY creates a general business** (`AddBusiness.tsx:15`). **The tenant is made by signing up, not by SQL.** ✅ **Serving the new domain needs NO CODE — the app never skins by hostname** (the one `location.hostname` read is a WebAuthn RP id). 🔴 **But a `general` business gets the NURSERY dashboard — exactly ONE registry tile is non-`general`** (tech-debt #367), so Andrew sees *Plants* and *QR Checkout* on Monday. DNS/Vercel/auth URLs are DAVID'S — see ⚡ ACTIVE STATUS.
> **Last updated:** 2026-09-23 — **#390 + #391 THE GROW LADDER.** GROW and HOLD per rung; sellability per rung (slips/4-inch/plugs never sold, 3/5 gal rarely-but-sellable); a POTTING DATE LAWNS enters and corrects itself, append-only with a visible history; every operations-setting change dated by a trigger. 🔴 **The graduation date had been COMPUTED on every batch since #276 and NEVER RENDERED** — *Sellable from* now shows four states. NEW: `npm run migration:slot` (it did not exist; four prompts cited it — and it caught a live slot collision within minutes). 🔴 **4 migrations WRITTEN AND HELD** — `20260923h` (#390) then `20260924a` → `20260924b` → `20260924c` (#391); step-0 data file HELD pending Joel confirming the window. Two branches, NOT merged. Boards: uppot 0 of 36 — see ⚡ ACTIVE STATUS.
> **Last updated:** 2026-09-23 — **#370** mix planning: the MRP engine — install mix = 2.0 × container volume ([[R-155]], no shrink), uppot = 1.0 × the pot's gallons grossed up by shrink ([[R-181]]), lead time 2 days, all tenant config under Settings → Operations → Mix planning. **MERGED and LIVE on `2559f93`.** 🔴 **ENGINE ONLY — `mixRequirement` has ZERO callers, by David's ruling that mix planning is below the go-live bar; there is nothing to click.** ✅ **`20260922d` IS APPLIED** (proven live 2026-09-24; it was recorded as held for two days and was not). — see ⚡ ACTIVE STATUS.
> **Last updated:** 2026-09-24 — **#393** every on-hand figure at the till says where it came from: `qty_basis` + `_at` + `_because`, **1,079 of 1,079 LAWNS lots `placeholder`, 0 `counted`** (the three count rows are a TEST run). `20260923m` APPLIED, **MERGED `007f02d5`, LIVE (`007f02d`)**. ⚠️ **It describes the number; it does not fix it** — the purchases-minus-sales derivation still has no inputs. Board `checkout-item-entry` CARD 9 — see ⚡ ACTIVE STATUS.
> **Last updated:** 2026-09-23 — **#386** basic checkout: install is priced by the tree's CONTAINER SIZE from a per-rung, tenant-editable price; a rung with no price reads *"not set"* and asks for a typed amount with a reason, never $0; tech-debt **#251** fixed so every staff/flat transport row is offered. 🔴 **3 migrations WRITTEN AND HELD, nothing applied, nothing merged** — `20260923e` → `20260923f` → `20260923g`, then the step-0 data file. Board `checkout-install-price` 0 of 14 — see ⚡ ACTIVE STATUS.
> **Last updated:** 2026-09-17 — **#347** the crew day link (Saturday pilot, not merged) · **#343** the container ladder merged and live — see ⚡ ACTIVE STATUS.
> **Last updated:** 2026-09-17 — **#350** the load list prints an allow-list (not merged). Previous: **#343** the container ladder is the one source for sizes (not merged; apply its migration first). Previous: **#345** the writer registry. Earlier: **#341** the preview read asks QuickBooks for inactive records and five transaction types — see ⚡ ACTIVE STATUS.
- 🟡 **BUILT WITH CAI — THE HOME TENANT, DOCS COMPLETE AND EVERY LIVE STEP OWED TO DAVID, ledger #422 · tech-debt #367 · #368** · 🔴 **NOTHING TO APPLY — there is no SQL:** `businesses.business_type` is `text NOT NULL DEFAULT 'nursery'` with **no CHECK and no enum** (LIVE 2026-09-26), and `AddBusiness.tsx:15` already mounts the shared signup with `businessType: 'general'`, so the tenant is created at **`/add-business`** with the **address left blank** (`OwnerSignup.tsx:282` writes it only when non-blank, and **nothing in the app can ever edit it** — tech-debt #368). ✅ **`builtwithcai.app` needs NO app code** — the one `location.hostname` read in the whole surface is a WebAuthn RP id (`OwnerSignup.tsx:570`); the skin follows the signed-in tenant. 🔴 **DNS IS A REPLACE, NOT AN ADD:** two live GoDaddy **parking** A records serving HTTP 200, nameservers `ns61/62.domaincontrol.com` (**the Cloudflare transfer did not happen**) → apex `A → 216.198.79.1`. 🔴 **AND THE SUPABASE ALLOW LIST DOES NOT CONTAIN `cultivar-os.app` AT ALL** — only `cultivar-os.vercel.app`; a repair listed beside the new domain. 🔴 **A `general` business gets the NURSERY dashboard** — one non-`general` tile in the registry (tech-debt #367), David rules the per-tile list. ⚠️ **Jon cannot self-onboard as general today** (`SignUp.tsx:29` hardcodes `'nursery'`, no type input anywhere). Board `builtwithcai-home` **0 of 5 covered**; report `~/Desktop/DAY-2026-09-26/BWC-SETUP.md`.
- 🟢 **ON-HAND PROVENANCE AT THE TILL — MERGED AND LIVE (`007f02d`), ledger #393, [[R-176]]** · every on-hand figure in item entry renders through `describeOnHand` and reads `"<qty> · <label>"`, **never a bare number** · 🔴 **1,079 of 1,079 LAWNS lots are `placeholder` and 0 are `counted`** — the three `inventory_counts` rows are David's own 3½-minute test (the same lot twice, 27s apart) and seeding them as `counted` was the red-team's catch · **four states, and `unknown` ≠ `placeholder`**: `unknown` means the SELECT did not ask · ⚠️ **it DESCRIBES the number, it does not fix it** — the derivation has no inputs (0 purchase-kind ledger rows; 3,924 of 3,925 order lines unlinked to a lot) · ⚠️ **the till wording is David's open call** and the live constant stands until he rules · owner-proof OWED, board `checkout-item-entry` CARD 9 · pointer: `packages/cultivar-os/src/lib/onHandProvenance.ts`
- 🟡 **A BUILD RECORDS WHAT IT ACTUALLY MOVED — BUILDER-COMPLETE, MIGRATION HELD, ledger #410** · `20260925f_build_run_records_what_it_moved.sql` **WRITTEN, NOT APPLIED — it is the whole fix** · 🔴 **`qty` AND `delta` are BOTH `integer NOT NULL`, so a 2.5 yd batch put 3 yards on the books while the RPC reported `"made": 2.5`** — half a yard invented every batch, in the direction that hides a shortage · 🔴 **worse on the consume side: a component clamped at zero moved the row by 10 and wrote −25 to the ledger** — a consumption that never happened · ✅ **nothing corrupted today: LIVE 2026-09-25 there are 0 `build_runs`, 0 `build` ledger rows on every tenant and `record_build_run` has NO CALLER anywhere** (incl. `origin/feat/recipe-surfaces` — comments only), so it fires on the first MADE IT tap · ⚠️ granularity NOT claimed fixed — `qty` stays integer, the RPC now REPORTS the rounding and names the remedy (gallon base, #409) · 26 probes red-first, V1–V4 verbatim, 2 mutants CAUGHT and 2 declared EQUIVALENT (§6 r19) · board `recipe-builder` CARDS 19–20, both `owed` · pointer: `supabase/migrations/20260925f_build_run_records_what_it_moved.sql` · harness `scripts/sql-harness/build-run-rounding-410.pglite.mjs`
- 🟡 **ONE ITEM, HELD IN ONE UNIT, SOLD IN SEVERAL — BUILDER-COMPLETE, MIGRATION HELD, ledger #409** · `20260925g_item_sale_units.sql` **WRITTEN, NOT APPLIED** · NEW `item_sale_units` keyed on **`qb_item_id`** (wipe class **CONFIG — survives the reload**, no FK to `business_inventory` on purpose) + one-level chain trigger + 4 policies (read on membership, write on `inventory:update`) · NEW pure reader `packages/shared/src/inventory/saleUnits.ts`, **35 probes**, and a missing conversion **REFUSES** rather than falling back to 1:1 · 🔴 **LIVE 2026-09-25: `sku` is NULL on all ten mix rows (the prompt's `SFCM1`/`FCMB15` SKUs do not exist), there are TEN rows in TWO families not five, and all ten are `qty_basis = 'placeholder'`** — nothing merges them · 🔴 **BLOCKED ON ONE DAVID DECISION: which row holds the pile and what is in it** (morning file, recommended default) · **table ships EMPTY** — conversions proposed from product names, written by a person · no UI, no `api/` change, no permission string, no new config key · board: inventory card `needs-test` **with its reason** · pointer: `supabase/migrations/20260925g_item_sale_units.sql` · harness `scripts/sql-harness/item-sale-units-409.pglite.mjs`
- 🟢 **TWO DOUBLE-BOOKED MIGRATION SLOTS FIXED, AND INSTALL MATERIALS NOW ONLY ON INSTALL STOPS — ledger #416 · tech-debt #364 RESOLVED** · 🔴 **APPLY ORDER IS NOW `20260925f` → `g` → `h` → `i`; `20260925a/b/c/d` ARE SUPERSEDED NAMES** (slot `c` had THREE claimants, `d` had TWO — *"apply 20260925c"* named two files, Rule 11b's defect) · mine moved because all four were **measured UNAPPLIED live** · ✏️ the `d` claimant is NOT applied either (`delivery_rings` absent live) · ② `buildLoadList` gates mix/T-posts/rope/fence-posts on `s.installs`; **#415 had built the predicate and wired it to `waterMonitors` alone**, and the day total needed its own tally because it is recomputed over every item · ⚠️ **the TREES are untouched** · 🔴 **27 assertions went red: every fixture omitted `installs`, so the suite was proving the mix arithmetic on accidental delivery stops** — helper now defaults to an install, §DO adds 10 probes · pointer: `packages/cultivar-os/src/lib/loadList.ts`
- 🟡 **NEED IT BEFORE YOU NEED IT — THE PLANNED BATCHES, BUILDER-COMPLETE, NO CALLER, ledger #414** · NEW pure `packages/shared/src/costing/mixSchedule.ts`, **29 probes, NO migration** · `planMixBatches` carries stock forward day by day and plans WHOLE batches dated `leadTimeDays` earlier; sentence = *"NEED MIX — stock low: N batches by <day>"* · 🔴 **does NOT compute demand — `mixRequirement` (#370) already does, and its answers are the input (§6 r8)** · 🔴 **REFUSES when the on-hand is not a COUNT, which is every LAWNS mix row today** — and the screen sentence IS the refusal · 🔴 **an unreadable day breaks the chain: every later day projects `null`, never 0** · several shortfalls → several batches; a past due date is flagged and said · ⚠️ **ENGINE ONLY — no caller, no screen, no draft writer; blocked on the pile being counted once** · pointer: `packages/shared/src/costing/mixSchedule.ts`
- 🟢 **MIX PLANNING — THE MRP ENGINE, MERGED AND LIVE (`2559f93`), ledger #370, [[R-155]] · [[R-181]]** · `net = gross − on hand`; install 2.0 × container (no shrink), uppot 1.0 × pot gallons ÷ (1 − shrink); non-consuming lines counted and set aside · 🔴 **ENGINE ONLY, `mixRequirement` has ZERO callers — below the go-live bar by David's ruling, so there is NOTHING TO CLICK and 0 owner-test cards is correct, not owed** · ✏️ **2026-09-25: DAVID'S OVERNIGHT PROMPT RAISES THIS ABOVE THE BAR** — yard production P7 asks for the shortage surfaced on the Operations calendar and on Lauren's schedule as a DRAFT work order. **The engine is not to be rebuilt (§6 r8): `mixRequirement` and `mixLeadTimeDays` already exist and P7 is the wiring** · one predicate `isCounted` whose default is NOT counted (a count is proven by a DATE) · ✅ **`20260923m` APPLIED 2026-09-24 and #393 MERGED (`007f02d5`) — `qty_basis` is populated on 1,079 of 1,079 LAWNS lots, all `placeholder`**, so the stored-basis switch is available now (call `describeOnHand`, and NAME `qty_basis, qty_basis_at, qty_basis_because` in the SELECT or every row reads the fourth state `unknown`) · ✅ **`20260922d` APPLIED — RLS on, 4 policies, 0 rows, proven live 2026-09-24; my "written and held" was wrong for two days** · pointer: `packages/shared/src/costing/mixPlanning.ts`
- 🟡 **THE INSTALL KIT IS CONFIGURABLE — BUILDER-COMPLETE, MIGRATION HELD, NOT WIRED, ledger #411 · tech-debt #364** · `20260925h_install_kit_components.sql` **WRITTEN, NOT APPLIED** · component → inventory item (by `qb_item_id`) + one of six rules (*per_tree · per_container_gallon · per_t_post · per_rung · only_if_ordered · only_if_marked*), `per_rung` carrying NO factor by CHECK (the count lives on the rung — STD-011) · NEW `packages/shared/src/inventory/installKit.ts` (33 probes) + equivalence suite (18) · 🔴 **`loadList.ts` IS UNTOUCHED AND THE ARITHMETIC IS NOT REBUILT — it already computes every install quantity ([[R-155]], #343); the missing piece was the component → ITEM LINK** · 🔴 **an unlinked component is REPORTED, never skipped**; an `unknown` deer-fence state gives null not 0 · 🔴 **THE EQUIVALENCE SUITE CAUGHT THE SHEET, NOT THE KIT — tech-debt #364: the load list computes planting mix, T-posts and rope for DELIVERY-ONLY stops, 26 of 63 LAWNS stops (LIVE 2026-09-25)** — David's ruling, filed not fixed · **BLOCKED ON TWO DAVID ANSWERS: #364, and which product each component is** (the seed cannot be written without it) · board: inventory card `needs-test` with its reason · pointer: `supabase/migrations/20260925h_install_kit_components.sql`
- 🟡 **YARD WORK ORDERS + A BUILD THAT TIMES ITSELF — BUILDER-COMPLETE, MIGRATION HELD, NO SURFACE, ledger #413 · tech-debt #365** · `20260925i_production_work_orders.sql` **WRITTEN, NOT APPLIED** · `production_work_orders` + `_lines` + 8 policies + kind trigger + `work_order_apply(uuid)`; `build_runs` gains `started_at`/`finished_at` · NEW `packages/shared/src/costing/buildTiming.ts` (17 probes) — *"measured over N batches"*, **never an estimate** · 🔴 **named `production_*` by applying [[R-168]]** (not a bare `work_orders`, not a vertical `yard_*`) and **FK to `delivery_teams` — `teams` is GONE, verified live** · 🔴 **Done adds no stock logic: it calls `record_build_run`** · 🔴 **idempotent by line** (a second tap applies 0, skips 1) and **a partial failure stays `in_progress`** · 🔴 **tech-debt #365: `build_runs` had NO writer at all, so its emptiness was never evidence** — the work order is now its first writer; `build_run_components` and the cost columns still have none · **NO UI — nothing calls it** · board `work-orders` 1 card `needs-test` with its reason · pointer: `supabase/migrations/20260925i_production_work_orders.sql`
- 🟢 **MIX PLANNING — THE MRP ENGINE, MERGED AND LIVE (`2559f93`), ledger #370, [[R-155]] · [[R-181]]** · `net = gross − on hand`; install 2.0 × container (no shrink), uppot 1.0 × pot gallons ÷ (1 − shrink); non-consuming lines counted and set aside · 🔴 **ENGINE ONLY, `mixRequirement` has ZERO callers — below the go-live bar by David's ruling, so there is NOTHING TO CLICK and 0 owner-test cards is correct, not owed** · one predicate `isCounted` whose default is NOT counted (a count is proven by a DATE) · ✅ **`20260923m` APPLIED 2026-09-24 and #393 MERGED (`007f02d5`) — `qty_basis` is populated on 1,079 of 1,079 LAWNS lots, all `placeholder`**, so the stored-basis switch is available now (call `describeOnHand`, and NAME `qty_basis, qty_basis_at, qty_basis_because` in the SELECT or every row reads the fourth state `unknown`) · ✅ **`20260922d` APPLIED — RLS on, 4 policies, 0 rows, proven live 2026-09-24; my "written and held" was wrong for two days** · pointer: `packages/shared/src/costing/mixPlanning.ts`
- 🟡 **ROUTE THIS TEAM — IT NOW ROUTES, AND A CREW THAT CANNOT BE ROUTED SAYS WHY, ledger #405, §6 r24** · 🔴 **David, 2026-09-24: *"it DOES NOTHING when pressed"*** · **cause: `buildRoute` had exactly ONE caller — the "Route N Stops" button — so the schedule's per-crew button navigated and stopped** · 🔴 **second half, and the worse one: the gate returned `null` on an empty selection, so a crew with nothing routable showed a GREYED BUTTON AND NO WORDS** — an absence read as an answer · **FIX: `teamRouteProblem` extracted PURE (`packages/cultivar-os/src/lib/teamRouteGate.ts`) so the "why not" is probed, not inspected — 11 probes · 3/3 mutants, the original defect caught by 6** · plus one effect that routes once per `(date, team)` on arrival with `?team=`, refusing while a refusal is on screen · ⚠️ **three theories measured FALSE against live LAWNS first** — not an empty selection, not a query-key mismatch, not a missing `team_id` · ⚠️ **Lauren's workaround (route the day, untick the other crew) ran TWO weekends** · ✅ **MERGED `origin/main` `6b177f67`, LIVE — `version.json` read twice 60s apart, `sha 6b177f6 · env production`** · **route board 0 of 10; CARDS 9 and 10 `owed`** · **DEPLOYED, owner-proof owed** · ⛔ **piece 3 (crew link per crew) still PARKED, `20260921d` unapplied**
...
### ✅ MERGED 2026-09-17 (`6bdcf15`) — #342 (`fix/rehearsal-never-writes-the-record`, 2026-09-16) — **DURING TESTING NOTHING WRITES THE RECORD**
...
### ✅ MERGED 2026-09-17 (`6bdcf15`) — #335 (`feat/contact-record`, 2026-09-15 · amended + rebased 2026-09-16)
...
- 🟡 **BUILDER-COMPLETE, OWNER-PROVE OWED** — books-read board CARDS 19 · 20 (plus 1 · 2 · 7 reworded).
  Item/Customer queries now ask for **inactive** records (count and page). **Five transaction types** are read.
  `minorversion=75`. The newer custom fields are requested. **Retired records stay out of the import and
  the list-describing findings.** api/ 12/12 · no migration · verify exit 0 · 14/14 mutants caught.
- 🔴 **Step ② (a live pull) is a press on the merged build** — no local credential could run it.
  Diff (projection): `docs/recon/2026-09-16-inactive-items-diff.md`.

### 🟢 PUSHED, NOT MERGED — #336 (`fix/332-ledger-row`, 2026-09-15) — **`main` GOES GREEN**

- ✅ **THE MISSING `#332` CLOSE-OUT ROW IS FILED, NOT WITHDRAWN.** BUILDER-COMPLETE, **no owner-proof
  owed — there is no surface to drive** (OP-14: a claim, not an omission). DOCS ONLY: no app code,
  no migration, no schema, no policy, no permission string, api/ 12/12, nothing added to `verify`.
  🔴 **`verify:id-citations` clause D was failing on a pristine `origin/main`** — `ecfb376`'s subject
  claimed close-out **#332** and the ledger had no row and no reservation for it. **The `&&` chain
  HALTS there**, so `verify:id-sweep`, `verify:main-ancestry`, `verify:migration-apply-state` and all
  111 test files were **unreached** — neither passing nor failing.
  ✅ **`npm run verify` now exit 0, ZERO NET-NEW, 111/111 files, 5,959 assertions — the first run
  since `ecfb376` landed that reached the end of the chain.**
  ✏️ **FILED rather than withdrawn under [[R-148]] clause (3) because the work is real and is on
  `main`** (`889b740` closed tech-debt #253 across six homes; `ecfb376` corrected #329's stale BAR
  cell and made the one-fact-many-homes recon close its own loop). **Withdrawal would have recorded
  that nothing happened in front of two commits where it did.**
  - ⚠️ **THE ROW IS MINE AND THE WORK IS NOT.** It records what those commits and their own corrected
    documents say; it does not claim a close-out I performed.
  - 🔴 **THREE STALE CLAIMS CORRECTED IN PLACE IN THE SAME PASS, WHICH IS `#332`'S OWN RULE:**
    `open-questions.md` said `20260911b` was *"written, not applied"* three days after it was applied
    · its address-book story citation pointed at `:1630` where the row is at **`:1862`** · **this
    file said #299 was *hard-blocked on #253* and the short list was *seven*.** *Writing the true
    value somewhere else is not a correction; it is a third home.*
  - 🔴 **29 OLDER SUBJECT CLAIMS STILL HAVE NO LEDGER ROW** (`#79 · #81 · #82 · #121 · #141 …`),
    baselined and named on every run. **Deliberately not filed** — 29 retroactive rows written by
    someone who did none of the work is a worse artefact than the gap. **David's call.**
  - 🟡 **NOTHING CATCHES A SESSION THAT CLOSES OUT WITHOUT FILING ITS ROW, AT THE MOMENT IT HAPPENS.**
    Clause D catches it on the next run on someone else's branch — which cost a red `main` blocking
  ...
  **② RING DIAMETER IS A TOTAL √ FUNCTION OF CONTAINER GALLONS**, through 15 gal → 5 ft and
  ...
  - 🔴 **#299 IS A BUILD, NOT A FIX — RE-SCOPED 2026-09-15 ON DAVID'S CORRECTION.** There is no
    ...
    **So: BUILD it here, reading `business_operations_config`, no mulch line, the ring as R-156's
    ...
    ✅ **UNBLOCKED 2026-09-15 — THE HARD BLOCKER WAS FALSE AND IS STRUCK (ledger #336 correcting in
    place; the proof is `889b740` on `main`).** This read *"🔴 Hard-blocked on #253 — `20260905_
    production_planning` is unapplied, so `business_operations_config` is absent on every tenant.
    Apply-or-retire gates this."* 🔴 **It is applied.** David changed values in Settings → Operations
    on `c99a4c5` and they **persisted across a reload** — a value that survives a reload was written
    to and read back from `business_operations_config` under real RLS — and
    `docs/recon/2026-09-12-one-fact-many-homes.md` had carried the catalog read for all three tables
    (`rls=true`, four policies each) **since 2026-09-12**. **Tech-debt #253 is RESOLVED.**
    ⚠️ **Still genuinely unproven and it is a CARD, not a blocker: nobody has driven an Uppot plan
    COMMIT end to end.** Unproven is not broken.
...
- 🔴 **BEFORE CLAIMING ANY ID: `npm run verify:id-sweep`.** The command is the answer; **no next-free
  ...
  - ✏️ **THE CACHED NUMBERS THAT USED TO SIT HERE WERE DELETED 2026-09-12 (ledger #316, David's ruling — option
    (a), closing tech-debt #289).** The line once read *"next free is #310 / tech-debt #286 / R-150"* — and
    🔴 **all three were already taken by the time anyone read it**: `#310` by `feat/channel-vocabulary`,
    `R-150` by ledger #308, tech-debt `#286` by ledger #312. Wrong three times in one day by three different
    mechanisms — stale by a merge that consumed an id, stale by a manual survey that could not see far enough,
    stale by nothing at all but an hour passing on a busy tree — **and every correction was itself wrong within
    the hour.** David: ***"A number that is usually right is worse than no number, because it gets trusted."***
    The line already said RUN THE SWEEP; the number was the convenient half, so it was the half that got read.
    ✅ **Ledgers #314 and #316 reached deletion INDEPENDENTLY and David ruled it** — the one-line merge decision
    #314 raised is answered, and both sides of this merge had already deleted the numbers.
...
- ✅ **3 of 12 COVERED — David, 2026-09-12, build `efc02f8` (ledger #312): CARDS 1, 2 and 4.** The stop-gate is closed by the person it gates — `'cancelled'` IS in the live CHECK, so **R-146 does not become a migration**; both member policies are live in the catalog (#241's class clean); and **both** zero-post `arbor day` rows read *"No posts yet"*, which is the original lie measured dead on the only pair that could prove discrimination rather than a string. **NEXT: CARD 3** — `pre-merge`, read-only, and its SQL needs no repair (both recorded defects were fixed in `efc02f8` itself). **The remaining NINE are owed** (8 `owed` · 1 `needs-test` — CARD 12 needs Lauren's login) → `docs/owner-tests/campaign-lifecycle-full-surface-test.md`. ✏️ **This line used to read *"0 of 12 … CARD 1 IS A STOP-GATE AND RUNS FIRST"*, and the gate it described has now fired clean** — kept only as the reason CARD 1 existed: tech-debt **#91** is a campaign-table inline CHECK whose live definition disagreed with its migration, which is why a repo file was not taken as proof. Its own board, per David; the 2-card social board is untouched.
...
### ✅ THE SWEEP'S OWN BLIND SPOT IS CLOSED — TWO POPULATIONS, INHERITANCE PER ID (2026-09-12, ledger #314, tech-debt #286 RESOLVED)

- 🔴 **THE GATE BUILT TO PREVENT COLLISIONS WAS CAUSING THEM.** One day old, run from `main`, `verify-id-sweep` printed **`NEXT FREE: #310`** in green while `origin` held `reserve(#310)`, `reserve(#311)` and a filed `#311` row — and it was also behind on tech-debt and rulings. **ONE filtered population was answering TWO questions that need opposite ones.**
- ✅ **FIXED — and the fix is NOT "delete the filter".** The old exclusion was right about inheritance and wrong about grain: **inheritance is a property of an ID, not of a BRANCH.** Every ref is now swept for both questions; an overlapping claim counts as inherited only if it was already claimed at **`merge-base(HEAD, ref)`**. `selectPopulations()` takes **no lineage predicate at all**.
- ✅ **`main` IS A RIVAL NOW TOO, and that was the same defect one layer over** — this tree's claims used to be filtered by *"is it on main right now?"*, which is true only while your base IS main's tip. **Measured live the same hour:** this branch held tech-debt `#286`, `main` MOVED, another session filed a **different** `#286` on it, and the cap could not see it. Inheritance from `main` is now decided at the merge-base like every other ref.
- ✅ **PROVEN BY MAKING IT FAIL (§6 r19 · R-33):** one tree standing on `main`, one injected claim of `#310`. **OLD → exit 0, green, and four wrong numbers. NEW → exit 1, naming `origin/feat/channel-vocabulary`.** Same tree, same id.
- 🔴 **AND THE NEGATIVE CONTROL CAUGHT A REAL BUG IN THE FIX** — the merge-base read counted filed rows but **not RESERVATIONS**, so an inherited reservation read as a collision: the exact false positive the deleted filter existed to prevent. Both sides now call one `fileClaims()`. **Found by running the probe, not by reasoning about it.**
- ✅ **The highest id NAMES ITS HOLDER** (`held by: origin/feat/channel-vocabulary`), and a collision names **every** holder, flagging when one is `origin/main`. **9/9 mutants caught, five of them POPULATION mutants** (#182's prescription).
- ⚠️ **The cap compares CLAIMS, never TIMES, and never moves an id** — R-148 clause (4) needs a human.
- ✅ **TODAY'S THREE COLLISIONS ALL RESOLVED THEMSELVES, each by a session applying clause (4) against itself:** `#310` → zone-walk renumbered to `#311` · `R-150` → channel-vocabulary moved to `R-152`/`R-153` · tech-debt `#286` → ledger #308 moved to `#289`. **The rule works when sessions can SEE the collision; the sweep is what makes it visible.**

### ✅ "PUSHED" IS NO LONGER TAKEN ON TRUST, AND A LEDGER ROW CANNOT SILENTLY DROP A CELL (2026-09-14, ledger #323)

- ✅ **`npm run verify:main-ancestry` — tech-debt #280 ①.** Local `main` must not be **AHEAD** of `origin/main`; and a close-out row claiming *"MERGED TO `main`"* must cite at least one commit that IS an ancestor of it.
- 🔴 **THE INCIDENT IT WAS BUILT FROM IS OURS: 2026-09-14, #320 was reported merged while `origin/main` had not moved.** Not `git status`, not the tests, not `npm run verify`, not even the §3-entry check built that same session caught it. **Proven red with a REAL commit on local `main`**, then restored.
- ⚠️ **Clause A asserts `main` ONLY** — an unpushed feature branch is often correct mid-build (R-149). **Clause B is "at least one", not "all": "all" reports 8 rebase artefacts and none is a defect** (#73 — a cap red on arrival is one people switch off).
- ✅ **`verify-id-citations` CLAUSE E — tech-debt #294a.** No close-out row may have MORE cells than the header. Counts **unescaped** pipes, derives the width, and prints **the text GFM is discarding**.
- 🔴 **RED-FIRST ON THE REAL CORPUS, NAMING EXACTLY THE FOUR PREDICTED** (`#279 · #299 · #311 · #317`). Three were stray pipes in inline code; **`#311` was a genuine EXTRA COLUMN, so its cells were MISALIGNED and its `Blocker` had fallen off**. **All 77 rows now fit.**
- 🔴 **#280 ② IS UNCHANGED — #280 STAYS 🟡 PARTIAL.** Nothing here reads Vercel.
- 🔴 **LIVE `#323` COLLISION, NOT MINE TO MOVE** — `origin/docs/worktree-rule-and-282-close` claimed it 1m19s later; R-148 clause (4) says they renumber. **`npm run verify` stays red until they do.**

### 🟢 THE PRODUCTION STAMP — THE BUNDLE SAYS *WHERE* IT IS (2026-09-14, ledger #321) · **OWNER-PROVEN**

- 🔴 **A PREVIEW AND A PRODUCTION DEPLOY OF THE SAME COMMIT WERE INDISTINGUISHABLE IN THE APP.** Same SHA, same stamp, GATE 0 passed on both — and **#303 was recorded complete *"with only Preview deploys"***. Right code, wrong target.
- ✅ **THE STAMP NOW READS `built <time> · <sha> · prod`** — or an amber **`PREVIEW <branch>`** / **`prod⚠ <branch>`** (production built from a branch) / **`env?`** (target unknown) / quiet **`local`**.
- ✅ **#280 SAID ② WAS *"not checkable from the repo — nothing we own reads Vercel"*. STILL TRUE — AND NOTHING HAS TO:** Vercel sets the vars at **BUILD** time, so the answer is baked into the artefact and read **where David is standing**.
- ✅ **GATE 0 ON ALL 40 BOARDS: the last token must read `prod`. If the chip is amber, STOP.**
- 🔴 **AN UNRECOGNISED `VERCEL_ENV` IS NEVER COERCED TO `production`** — it shows a loud `env?`. A stamp that guesses *prod* is the silent false green it exists to prevent.
- ⚠️ **PRODUCTION IS STAMPED POSITIVELY — a deliberate deviation from the standard env badge (§6 r16)**, which shows nothing in production. A blank production is indistinguishable from a bundle built before the feature (A9).
- ✅ **55 assertions both directions · 2 mutants against the real source, both caught · the define proven to reach the bundle by two real `vite build` runs** (preview → `PREVIEW` + the branch; production → `production`/`main`, zero stale preview strings).
- 🔴 **tech-debt #280 → 🟡 PARTIAL, NOT RESOLVED. ② is OBSERVABLE; NO CAP ASSERTS IT, and the close-out gates still accept *pushed*.** ① is satisfied for #320 by the merge, also unasserted.
- 🟢 **OWNER-PROVEN 2026-09-14 — DAVID'S LIVE RUN on `cultivar-os.app`: *"the foot of the screen reads · prod"*.** `production-stamp` board **CARD 1 `covered`, 1 of 5**.
- 🔴 **THE BOARD HAD TO BE CREATED TO RECEIVE THE FLIP** — #321 changed a surface on every screen and made **no card for the stamp itself**, while writing the GATE 0 clause into 40 boards. **An OP-14 gap in the build tightening OP-14's neighbour**, found only because David reported a run and there was nothing to flip.
- ✏️ **Thunder's bundle reading is kept and is NOT the proof** — `jJ("production","main")` → `{label:"prod", loud:false}` traced through the shipped minified code. Evidence about the artefact, not a person at a screen.
- ⚠️ **CARDS 2–4 are `owed` and CARD 5 is `needs-test`** — the preview chip, the promoted-branch chip, the stamp surviving a broken screen, and the old-bundle contrast that can no longer be run.
- 🔴 **#280 ① IS STILL OWED AND IS THREE LINES** — `git fetch` then `merge-base --is-ancestor <sha> origin/main` (**`origin/main`, NEVER local `main`**). This session reported #320 merged while `origin/main` had not moved, and **no gate we own caught it**.

### ✅ A LEDGER ROW WITH NO §3 ENTRY IS AN INCOMPLETE CLOSE-OUT — ASSERTED (2026-09-14, ledger #320)

- 🔴 **FIFTEEN CLOSE-OUT ROWS HAD NO CLAUDE.md §3 ENTRY AT ALL — #253 · #255 · #256 · #264 · #270 · #271 · #291 · #302 · #304 · #308 · #310 · #311 · #313 · #316 · #317.** **They did not scroll out at N=3; they never entered.**
- 🔴 **AND THE GAP IS SILENT IN A WAY A LOST ENTRY IS NOT.** §3b's clause — *a close-out that adds a `FLAGGED FOR DAVID` item adds a line to `docs/open-questions.md` too* — fires from the §3 write. **A session with no entry never fires it either, so its flagged items reach NEITHER surface, and the absence looks exactly like a session that had nothing to flag.**
- 🔴 **THE POINTED ONE IS #302 — the row that BUILT `open-questions.md`, to catch what §3 drops at N=3, is itself a row §3 never recorded.**
- ✅ **GUARD: `npm run verify:handoff-retention` check 5.** Every close-out row must be claimed by an entry's **headline bold run**, or be **declared in `handoff-entry-declarations.json` with a reason**. ⚠️ **A MENTION IS NOT AN ENTRY** — `#310`/`#311` appear inside `#314`'s and `#312`'s entries, which is what made them read as covered. `TECH-DEBT #NNN` is not a ledger claim; a `⏳ RESERVED` row owes none; a combined entry (*"#250 + #251"*) claims both.
- ✅ **THE DECLARATION LIST PRUNES ITSELF BOTH WAYS** — an id with no row, or one that has since gained an entry, is **STALE and FAILS**. That is tech-debt **#73**'s lesson: a gap list that only grows stops being read.
- ✅ **10 probes both directions + THREE red-first proofs against the real tree**, and **the gate caught this very build** — #320's row was filed before its entry existed and the check refused it by name.
- 🔴 **§3 ENTRIES ARE NOW TRIMMED: headline + `**Type:**` + state + `FLAGGED FOR DAVID`, NOTHING ELSE.** The proof narrative lives in the **CLOSE-OUT LEDGER ROW**, which is permanent and which nothing has to cut. **§3's value is being LOADED, not being stored.**
- ✅ **NOTHING WAS DELETED — every paragraph was measured before it moved.** 11 paragraphs (10.3KB) the rows did not carry were appended **verbatim**; 5 the rows already carried were dropped after a per-sentence check. **CLAUDE.md 678 → 651 lines, −14KB. N=3 UNCHANGED.**
- ⚠️ **DECLARED, NOT BACKFILLED — David's to overrule.** Backfilling means composing a handoff for sessions nobody was in and dating it as though written then. **The fifteen sessions' flagged items are lost and that already happened**; the declaration says so rather than papering it.
...
- ✅ **ONE detector: `packages/shared/src/hooks/useDevice.ts`.** `useBreakpoint()` VIEWPORT · `useInput()` INPUT · `useContainer()` CONTAINER. `useIsMobile`, `detectMobile`, `useIsNarrow` and the **user-agent regex** are DELETED; `deviceDetector.test.ts` §C/§D derive the population from the corpus and fail the build on a third detector or a new UA reader.
- ✅ **The numbers live once** — `design-system/tokens.ts` `breakpoints` (compact 0 / medium 640 / wide 1024, Tailwind's `sm`/`lg`, already the tile grid's own numbers) — **and `TileGrid` INTERPOLATES them**, so CSS and JS share the constants literally. No pixel moved.
- 🔴 **PLATFORM HAS NO DETECTOR, DELIBERATELY** — the only mechanism is a self-declared UA (iPadOS reports itself as a Mac). David's 2026-08-23 ruling (*declared, not accidental: `phone`|`desktop`|`either`*) is still **OPEN**; this pass mints none of its vocabulary.
- 🔴 **CONTAINER returns `browser` and says why the other two are unreachable** — no wrap sets the handshake, and the app ships **no web-app manifest**, so `standalone` cannot fire. The seam is built; it is not a claim that the wrap works.
- ⚠️ **ONE DELIBERATE BEHAVIOUR CHANGE — board CARD 2, David's to override in one word.** The Operations calendar switches to arrows below **1024px**, not 767px: the old number cited *"§6 r7, the tile grid"* and **the tile grid has never used 768**, while 767 contradicted the intent three lines above the control (*"arrows … on a phone OR THE TABLET IN THE YARD"*).
- ✅ **CARD 1 COVERED — David, 2026-09-12, build `13d64aa`. Board 1 of 7.** **NEXT: CARD 2**, the only card that asks you to DECIDE rather than observe (arrows at tablet width; one-word revert).
- ✅ **§6 r7 REWRITTEN TO DESCRIBE (ledger #307, tech-debt #283 closed).** David's ruling: the grid was **designed phone-first and carried over to desktop; the 768 dates from the phone design and was never revisited when it moved** — a third shape, and the worse one, because **a rule that was once true reads exactly like a rule that is true.** r7 now states the real 4/6/8 ladder, records the origin, and says **it describes rather than decides**. 🔴 **No pixel of `TileGrid` moved; the desktop grid layout is David's separate decision.**
- 🔴 **NEW tech-debt #284 — THE ID-CLAIM RULE EXISTS AND WAS NEVER MINTED.** `RULINGS.md:272` + the OWED row, unnumbered since 2026-09-02, **disqualifying itself in its own text**; its only home is now **grepped, not read**; and **no script checks ledger-row ids** (`verify-id-citations` is bounded to the tech-debt log). **Six collisions in 24 hours**, incl. `#304` claimed by two sessions **4m35s apart**. **Two mechanisms PROPOSED, neither built — David's call.** — was #281, renumbered at collision; **§6 r7 itself** says the tile grid is *"desktop/tablet only (768px+)"*; it breaks at 640/1024 and renders **4 columns on a phone**. **David rules: stale prose, or unimplemented intent?** It decides what the dashboard does on a phone, and the next mobile pass reads that rule.
- 🟡 **STORY GATE = IN-CODE-NOT-ON-THE-BOARD** — an as-built line was written (`user_stories.md` → PLATFORM STANDARD CAPABILITIES → *Device vocabulary*); **the BEHAVIOURAL mobile story (Lauren running delivery day twice) is OWED TO DAVID.**

### 🟡 THE SHIP-TO ADDRESS BOOK — `customer_addresses`, D-41's L2 HOOK, TAKEN UP (2026-09-11, ledger #303 · cards flipped #312)

- ✅ **MIGRATION APPLIED 2026-09-12 (David, board CARD 1) AND CATALOG-VERIFIED (CARD 3, all six clauses).** RLS on · 14 columns incl. `line2` · exactly three policies naming `customers:*` and **none naming `owner_id`** · two UNIQUE partial indexes · 0 rows · no `shipping_*` leak. **CARD 2 is now permanently unrunnable** — its pre-apply window cannot be re-entered without dropping a live table.
- 🟡 **THE TABLE IS PROVEN, THE CODE PATH IS NOT — 2 of 12 cards covered** → `docs/owner-tests/customer-addresses-full-surface-test.md`. **NEXT: CARD 4 on the delivery SCHEDULE** (it FAILED once on `fe24e68`; the fix is on `fix/stop-site-offer-unmount` and `/orders/:id` is the screen that was never broken). api/ 12/12 · no permission string · no column added to `customers` or `deliveries`.
- ✏️ **CARD 3's LEAK CLAUSE WAS SPLIT 2026-09-12 (David's instruction, ledger #312):** it forbade `shipping_*` **and** `deliveries.address_line2` behind one `OR`. The first is D-41's permanent redline; the second is **the column the `BillAddr.Line2` fix needs** and an OPEN decision (tech-debt **#279**), so the board would have failed a correct migration. The redline stays a gate; the other half is a note owned by #279.
- ✅ **The invariant holds and is proved by ACT, not inspection:** the order still SNAPSHOTS the chosen address onto the delivery row; the book is a SOURCE for the picker, never a pointer a stop resolves. Board **CARD 11** is the live proof.
- ✅ **`customers:read`/`:create`/`:update` REUSED** — a saved site is a field of the customer relationship, not a capability (tech-debt #84 · R-22). No raw `owner_id` policy (`20260910b` took 49→12). No DELETE policy — retiring is an UPDATE (R-133).
- ✏️ **A LIVE DEFECT CLOSED ON THE WAY:** a delivery address typed at checkout for a customer who already had one on file **never reached the stop** — fill-never-clobber kept the old address and the stop was read off it. The truck went to the billing address.
- 🔴 **NO BACKFILL FROM HISTORY, and it is true in the code** — the importer does not read `BillAddr.Line2` (#254), so seeding would make AGAVE's four spellings of one yard permanent. A corpus probe fails the build if anything seeds the table.
- 🟡 **STORY GATE = NO MATCH — a story is OWED TO DAVID.** `user_stories.md:1630` still carries this table as `STATUS: scoped-out`; this build inverts that row, which is his flip to make.
- 🟡 Tech-debt **#279** — `line2` is created and no surface writes it, by decision. #267's shape, filed so it cannot rot.
...
- 🔴 **THE SHORT LIST IS SIX — ✏️ CORRECTED 2026-09-15 (ledger #336): IT READ *SEVEN* AND ITS FIRST ITEM WAS FALSE.** ~~apply-or-retire `20260905_production_planning` (#253)~~ ✅ **RESOLVED — it IS applied**, proven by David's own reload test on `c99a4c5` and by the catalog read sitting in `docs/recon/2026-09-12-one-fact-many-homes.md` since 2026-09-12. **THE REMAINING SIX BLOCK WORK RATHER THAN SLOW IT:** who may see the equipment list without its cost (#262, Joel sees nothing today) · how Follow-Up gets turned on (#270, the review ask cannot fire on any tenant) · LAWNS's three service rows (yours to write, blocked on #251) · the 8-hour rule's actual words · the yard-worker permission set · the coarse-split A-or-B.
...
### 🟡 THE GOOGLE REVIEW ASK — THE LINK IS ON BUSINESS PROFILE; FOLLOW-UP STILL HAS NO TURN ON (2026-09-11, ledger #300)

- 🟡 **BUILDER-COMPLETE `f7c0e00` on `main` · no migration · api/ 12/12 · 17 owner-test cards, 0 COVERED** → `docs/owner-tests/delivery-fulfilment-full-surface-test.md` · **RUN CARD 13 FIRST** (read-only SQL)
- ✅ Link field on Business Profile (same store) · per-table Save with read-back · a stop marked done after its own date never prompts
- 🔴 **#270 — the ask cannot run on any tenant:** `followup_engine` is `planned`, `/subscription` shows no Turn on. David's call: clock, by hand, or core.

### 🟡 INSTALL DATE + INVENTORY LOCATION — RECON, TWO DECISIONS OWED (2026-09-11, ledger #299)

- 🟡 **`orders.install_date` — never set, never read, never wired** (0 of 76, no writer in any commit) → tech-debt #267 · recommendation: retire it once David confirms the warranty clock reads the planting stop's `completed_at` (R-143)
- 🔴 **No warranty code exists.** `deliveries.completed_at` is 0 of 57, and 7 of 19 QuickBooks-ingested stops are installs typed as delivery (tech-debt #268)
- 🟡 **`business_inventory.location` already exists and is empty on every tenant**; no zone table anywhere · recommendation: build the zone record, retire `location`, let the walk data pick column vs join → `docs/decisions/2026-09-11-install-date-and-inventory-location-recon.md`
- ⚠️ **Zones have no story on the board** — one is owed before any zone build

### 🟡 ZONE WALK — A CAPTURE TOOL SERVED FROM A URL; ITS EXPORT HAS NO HOME YET (2026-09-11, ledger #298)

- 🟡 **HOSTED:** `https://cultivar-os.app/tools/zone-walk.html` · source `packages/cultivar-os/public/tools/zone-walk.html` · **not app code, writes nothing to the database** → built-inventory *Field Capture Tools*
- 🔴 **#266 — the export names `business_inventory.zone` and irrigation zone records; neither exists (measured).** The shape is David's call before any import.
- ⚠️ **Try Export on the walker's phone before zone 1**; same phone, same Safari, Export each day.
- ⚠️ **Root `status.html` / `owner-tests.html` / `ui-standards.html` are not served by Vercel** — those URLs return the app shell.

### 🔴 PMI — THE ASSET TABLE WAS RENAMED, NOT DROPPED; THE GENERATOR IS LIVE; JOEL SEES A PADLOCK (2026-09-11, ledger #297)

- 🟡 **DOCS + ONE MIGRATION RETIRED IN PLACE on `main` · nothing applied · no app code · api/ 12/12 · no permission string · 10 owner-test cards, 0 COVERED** → `docs/owner-tests/pmi-full-surface-test.md`
- 🔴 **THE PROMPT'S PREMISE WAS FALSE, MEASURED:** `business_assets` → `cost_objects` (2026-06-15); both `asset_id` FKs point there; the 3 "orphan" schedules resolve (Mahindra 4025 · Craftsman compressor · Kubota L4802HST). **No `pmi_assets`-based table written** — it would be a second store for one fact.
- ✅ **R-139:** `20260529_pmi_shared` retired in place — reason in its header, every statement commented, `--catalog` reads NOTHING_TO_APPLY.
- 🔴 **#262 — Joel (LAWNS MANAGER) holds `pmi:*` and not `costs:read`: `/pmi` is a padlock with nothing behind it**, and its copy describes a schedule that is not there. OWED to David in RULINGS.md; blocks the yard-worker set.
- 🔴 **#261 — `/api/pmi/suggest` checks no login;** every call is billed.
- ⚠️ **FILED:** R-140 usage recurrence (10 of 27 tasks) · R-141 interval on the task · R-142 duty generates task · #263 · #264 (`Kubota Hours` $45, linked to nothing) · #265 (NO SCHEDULE beside 8 tasks). Capability **5.2 🟢 → 🟡**.
- 🔴 **RUN CARD 1 FIRST** — one paste in the SQL editor, read-only; it is the measurement this build rests on.
...
- ⚠️ **PENDING DATA TASK, DAVID'S — LAWNS's THREE service rows** (TC trip charge · Tailgate Delivery · self-collect). 🔴 **Blocked on #251:** checkout offers ONE staff/flat row, so trip charge + tailgate would hide one.
- ⚠️ **FILED:** **#251** (one staff delivery per shape) · **#252** (`price_type`/`price_unit`, Q2's answer) · **#218 PARTIAL**

### 🔴 LAUREN CAN DO HER WORK — THE 49 RAW `owner_id` POLICIES, TRIAGED (2026-09-10, ledger #289)

- 🟡 **BUILDER-COMPLETE on `main` (`8db220b`, pushed) · ✅ BOTH MIGRATIONS APPLIED 2026-09-10 (re-measured off the catalog: `lauren_refusals 3→0`, `dead_permission_policies 31→0`, `surviving_dropped 10→0`, **`joel_refusals` HELD at 12**, raw-`owner_id` policies **49 → 12**) · 12 owner-test cards, 0 COVERED** · `verify` exit 0 ZERO NET-NEW, unpiped · **91/91 files, 4976 assertions** · **16 mutants, 16 caught** · NEW cap **capR2** with an 11-probe `--self-test` · **api/ 12/12 · TWO new permission strings, backfilled in the same transaction** → `docs/owner-tests/owner-id-policy-repoint-full-surface-test.md`
...
- ⚠️ **FILED:** **#237** (`get_my_permissions` carries the postgres default ACL — a convention gap, **not** a hole; measured) · **#238** (the client computes **58** while arrays hold **57** — the `owner-only` sentinel exists in no array; ~~blocked on David's self-removal ruling~~ ✅ **ruled 2026-09-11, R-121 — every OWNER-role member**) · **#239** (8 of the 10 undocumented policies still uncaptured) · **#240** (`SET LOCAL role authenticated` **has never worked here**) · **#241** (🔴 the dual-RLS cap encodes the model this triage retires **and keeps passing**, because `tableHasOwnerPolicy` is a corpus grep with no drop-tracking).
- 🔴 **CARD 1 IS NOW `needs-test` — IT EXISTED TO BE WATCHED FAILING AND THE APPLY LANDED FIRST.** Only Thunder's pre-measurement stands as evidence the check could fail. **CARD 9 (Lauren edits the install price in the UI) is the one that closes this**; nothing measured tonight is a person. ⚠️ *Superseded instruction, preserved:* **RUN CARD 1 *BEFORE* YOU APPLY** — it is the acceptance query reading RED, i.e. you watching the check fail. Then apply `20260910b` (**SQL editor, never the table editor**; §0 refuses unless the literal merge is in), then **CARD 2**. **If `joel_refusals` comes back 0, stop.**
...
- 🟡 **#390 + #391 (2026-09-23) — THE GROW LADDER · BUILDER-COMPLETE · ⛔ 4 MIGRATIONS WRITTEN AND HELD, NOTHING APPLIED, NOTHING MERGED** · `container_ladder` gains `grow_months`/`hold_months` (`20260923h`) and `sellability`/`sellability_because` (`20260924b`) · NEW append-only `production_rung_dates` — one row per entry or correction, current is the latest, no UPDATE/DELETE policy for anyone (`20260924a`) · `business_operations_config_history` written by a TRIGGER so it cannot drift (`20260924c`) · **Sellable from** on `/inventory/uppot` in FOUR states (date · *not sold at this size* · *UNKNOWN — set GROW on the X rung* · *no uppot window set*) · `commitPlan` now writes `grow_months` + `sales_per_month`, live since `20260905` and never written · NEW `npm run migration:slot` (reads worktree working directories — where untracked migrations hide from `verify:id-sweep`) · rulings [[R-182]]–[[R-185]] · apply order: **h → j → k → l**, then the step-0 file · branches `feat/grow-ladder-rungs` → `feat/grow-ladder-dates` (stacked) · owed: uppot CARDS 32–36 → `docs/CLOSE-OUT-LEDGER.md` #390, #391
...
- 🟡 **#343 — THE LADDER IS THE ONE SOURCE FOR SIZES · ✅ MERGED `56107ee` 2026-09-17, MIGRATION APPLIED (V1–V4), LIVE · OWNER-PROVEN OWED: load-list CARD 19 on Saturday 2026-09-19 (7 yd, was 3.5)** · ⛔ **APPLY `20260916_container_ladder_install_t_posts.sql` BEFORE MERGING** · T-posts on each size · mix **2.0** + rope + bubblers + deer-fence posts in Settings → Operations → Planting materials · load list / uppot / count / import preview all read the ladder · **Settings → Container sizes** · Saturday 08-29 now **5 yd** of mix · owed: load-list CARDS 1–3, 5, 13–18 · uppot CARDS 28–30 · count-chips card · catalogue CARD 42 · open: tech-debt **#309** (staff read), **#310** (checkout size list) → `docs/CLOSE-OUT-LEDGER.md` #343

- 🟡 **BUILDER-COMPLETE · CARDS 22–27, 0 COVERED** · ✅ **MIGRATION `supabase/migrations/20260914_container_ladder.sql` APPLIED 2026-09-16 by David — V1–V4 run and passing (12 columns · RLS + three policies r/a/w, no delete · nine LAWNS rungs · duplicate label refused); catalog check APPLIED — CARDS 22–27 UNBLOCKED** (✏️ corrected by ledger #343; this said NOT APPLIED after it had run) · `api/` 12/12 · **NO new permission string** (`settings:update` reused) · **60 assertions · 23/23 mutants caught, 0 survived** · → `docs/owner-tests/uppot-planning-full-surface-test.md`
...
- 🔴 **THE 7-GALLON HYPOTHETICAL HAD ALREADY HAPPENED — 21 live rows, in NONE of the four size lists.** Two in `constants.ts` with zero importers; **the only live one is a HAND COPY in `api/orders/submit.ts:13`**, which knows nothing of 65 or 200 (51 more rows). The list was forked before anybody proposed one home.
- 🔴 **A RUNG'S NUMERIC KEYS ARE DERIVED, NEVER DECLARED.** `3/5 gal` claims **both 3 and 5** (R-71 ③ — Terry: the difference is only pot height); `95/100` claims both. Nobody types out `15`/`#15`/`15G`.
- 🔴 **THE PARSER RUNG ORDER CHANGED AND THE CORPUS WAS RE-RUN: 565 rows unchanged, EXACTLY 3 changed.** `20 inch box` read as **length** while `24 box` read as **container** — the same pot, one word apart, in two different unit KINDS. ⚠️ **Those 3 rows' stored projection is now STALE and owes `npm run units:backfill`** — the parser moved, not the size, so the trigger cannot have caught it.
- 🔴 **NOTHING-ON-HAND IS NOW THE REASON GIVEN**, above every size reason. ⚠️ **This reversed a written test assertion; the old expectation is recorded, not deleted.**
- ⚠️ **NOT BUILT AND SAID SO: no screen EDITS the ladder — adding a rung today is SQL.** The 121 off-ladder rows are **LISTED, not resolved** (one decision per size, David's). No rung sets a handling figure yet.
- ✏️ **CORRECTS tech-debt #253 AND this board's own migration gate:** the three production-planning tables **EXIST**, full shape, **0 rows** — both said otherwise for three days.

### 🔴 UPPOT PLANNING — THE SPLIT SHIPS, AND THE HOLD IS THE PLAN (2026-09-05, ledger #276)

- 🟡 **BUILDER-COMPLETE · 21 owner-test cards, 0 COVERED** (19 `owed` · 2 `needs-test`) · ✏️ **MIGRATION `20260905_production_planning.sql` IS APPLIED — CORRECTED 2026-09-14 (#326), MEASURED: all three tables exist, 0 rows. CARDS 8–17 ARE NOT BLOCKED** (this line said they were for three days) · `api/` 12/12 · NO new permission string · `npm run verify` exit 0 zero net-new · **76/76 files, 4022 assertions** · **40/40 mutants caught** · → `docs/owner-tests/uppot-planning-full-surface-test.md`
...
| 🟡 | **5.2** Equipment PMI | live — **proven-in-Ignition, already extracted** · ✏️ **2026-09-11 (#297): 🟢 → 🟡.** The generator has written 3 real schedules, but its accept path was never owner-proven in Cultivar (0 of 10 cards), the endpoint checks no login (#261), and **LAWNS's manager is shown a locked list** (#262) | `shared/modules/PMI.tsx` · `docs/owner-tests/pmi-full-surface-test.md` |
