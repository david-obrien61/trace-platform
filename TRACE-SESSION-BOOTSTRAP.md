...
### 🔴 FINDINGS REPORT v1 — THE UNION WAS 72, NOT 52, AND THE PDF STOPPED NAMING PEOPLE (2026-09-08, ledger #284)

- 🟡 **BUILDER-COMPLETE · `4966601` on `thunder/findings-report` — NOT MERGED, NOTHING DEPLOYED · 11 owner-test cards, 0 COVERED** · `verify` exit 0 ZERO NET-NEW · **93/93 files, 5,062 assertions** (was 87/87, 4,821) · **109 mutants, 109 caught, 0 survived, 0 never applied** · `build:cultivar` clean · **api/ 12/12, nothing minted · NO new permission string.** → `docs/owner-tests/quickbooks-books-read-full-surface-test.md` CARDs 19–29
- 🔴 **`max(byEmail, byPhone)` IS NOT A UNION AND CANNOT BE MADE INTO ONE FROM TWO COUNTS.** The retired rule reasoned correctly that ADDING double-counts, then took the LARGER — which silently discards every record one axis found and the other did not. The rule now reads the customer ROWS the browser already holds (parsed from `capture`, as the invoices already were — **no endpoint**, R-117) and unions **three** axes: email · phone · **exact name**. [STATED: the real union is **72, not 52**.]
- 🔴 **THE PDF NAMES NOT ONE CUSTOMER, AND THE SCREEN IT SENDS HER TO CAN NOW ANSWER THE QUESTION (R-116).** The report carries the COUNT and *"the records are on your Customers screen… fix them in QuickBooks and read your books again"*. **The dependency shipped with it:** `/customers` marks duplicates, sorts them to the top and filters to them, **derived at read time, never stored** — new clause **G12**, minted doc-first, which invalidated both live divergence declarations and **failed the build by name** until Vendors and UppotPlan re-answered it.
- 🔴 **FOUR RULE IDS RETIRED RATHER THAN REDEFINED (R-118).** Repeated invoice numbers were an **OBSERVATION** — all 22 groups carry different totals and Lauren confirmed the bookkeeper renumbered; the successor compares **customer · date · lines · total, all four** and [STATED] finds **one pair**, not 44 invoices. **639 of the 888 undated invoices are COLLECTIONS worth $722,526** and correctly carry no date; only **249** are gaps. Contact data is re-worded as a CAPABILITY leading with **1,828 reachable**.
- 🔴 **THE 33 UNREADABLE SIZES ARE 15.** A discount has no size, and neither does a trip charge or a refund — **18 unfixable rows are why "33 → 13" could never be shown**, because the count can never reach zero. The other 18 are named as not-applicable with the reason. ⚠️ The catalogue import screen still says 33 and **neither number should be edited to match the other**.
- 🔴 **49 WARRANTY GIVEAWAYS ACROSS 29 INVOICES, RECORDED FOUR WAYS — COUNTED, AND VALUED AT COST OR NOT AT ALL.** A cost total is stated only when every counted line's item publishes a purchase cost; otherwise the money is refused and the reason named. The retail figure lives in a field called `retailTotalNotToBeQuoted`.
- ⚠️ **TWO PROMPT PREMISES CORRECTED.** ① *"maxDuration ceiling is 60s"* contradicts **§6 r11**, which David verified the same morning: Hobby is **300s / 300s with fluid compute on**, and 60s is a figure that block already lists as WRONG. Nothing was changed to match it. ② *"`personNamesMatch` finds 6 pairs"* — it is **exact token-set equality**, so `Nicholas`/`Nicolas`, `Zach`/`Zack`, `Rebeca`/`Rebecca` and `Turnstile`/`Turnstyle` are **NOT** matched; the limit is declared, probed as behaviour, on the owner-test card as *expected*, and filed as **#224** for David's ruling.
- 🔴 **`20260908_books_report_runs.sql` IS WRITTEN AND UNAPPLIED (#227)** — two append-only tables, RLS `business_id`-scoped, **no UPDATE or DELETE policy at all**. Until David applies it, every insert is refused by a table that does not exist, which through PostgREST is **zero rows with `error: null`**; the screen says so in red and the report still opens. **CARD 29 is the 33 → 13 proof and cannot pass before it.**
- ✏️ **EVERY FIGURE ABOVE IS [STATED], NOT MEASURED (#223) — AND THE REASON FIRST GIVEN WAS FALSE, WHICH IS THIS BUILD'S OWN [[R-26]].** They come from a **QuickBooks capture file on David's machine**, outside this repo by construction (R-23 c); a Postgres key reaches Intuit not at all. The original line read *"`SUPABASE_SERVICE_KEY` is EMPTY in both env files"* — **inherited verbatim from #283 and restated in six documents without one `ls`.** It is empty in the two ROOT files and **populated, 219 characters, in `packages/cultivar-os/.env.local`** — the file every script reads — and it **authenticates** (read-only probe: `businesses` returned rows; negative control failed correctly). **The code hardcodes none of the figures.**
- ⚠️ **FILED:** **#223** (every figure is stated) · **#224** (the four one-letter pairs, and the fuzzy-match ruling that is David's) · **#225** (`booksFindings.ts` is 1,350 lines) · **#226** (duplicate mutant ids) · **#227** (the unapplied migration).
...
- ✏️ **CORRECTED 2026-09-08 (#284): THE KEY IS NOT EMPTY AND THIS LINE'S PREMISE WAS WRONG — SO THIS IS NOW MEASURABLE AND IS STILL UNMEASURED.** `packages/cultivar-os/.env.local` holds a working 219-character key (probed read-only; `businesses` returned rows). **What remains true is only the second half:** everything about live `service_offerings`, including the $125 placement price, is **STATED, not measured**.
