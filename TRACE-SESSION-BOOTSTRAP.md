...
>
> **Last updated:** 2026-09-15 — **#337** the catalogue-import undo refuses BEFORE it deletes anything (GATE 2: a lot with ledger history is undeletable, so the undo used to delete the customers and then throw). ⚠️ **It was live on LAWNS WITHOUT the seed — one test order did it.** Plus **CARD 17** for selling on an OWNER-role login, left `owed`. See ⚡ ACTIVE STATUS.
...
### 🛡️ PUSHED, NOT MERGED — #337 (`fix/undo-refuses-before-deleting`, 2026-09-15)

- 🔴 **THE CATALOGUE-IMPORT UNDO NOW REFUSES *BEFORE* IT DELETES ANYTHING.** BUILDER-COMPLETE,
  owner-proof **OWED** (board **CARD 12b**, LAWNS, read-only in effect). No migration, no schema,
  no permission string, api/ 12/12. `npm run verify` exit 0, **111/111 files · 5,984 assertions**,
  **mutants 10/10 caught · 0 survived · 0 no-build.**
  🔴 **THE DEFECT: a lot with `business_inventory_ledger` history is UNDELETABLE** (`ON DELETE SET
  NULL` is an UPDATE; the ledger's append-only trigger refuses it), and `undoItemImport` deletes
  `customers` FIRST and `business_inventory` second, each its own transaction. **So it removed the
  customers, hit the ledger, and threw — a half-wiped tenant — and reported `customersDeleted: 0`
  for rows that were already gone**, because the catch returns zeroed counts.
  ⚠️ **IT ARRIVED WITHOUT THE OPENING-STOCK SEED: one test order — `6a60a0ca`, 2026-09-09,
  `order_kind = test`, $1,875 — against one imported lot did it. The seed would do it 647 times.**
  - 🔴 **DAVID'S RULING IS OWED ON WHAT THE UNDO MEANS** — (a) the seed ends the rehearsal ·
    (b) tombstone instead of delete · (c) exempt the cascade (tech-debt **#79**, which also blocks
    the OP-12 reference-environment teardown). **The build made the refusal honest and picked none.**
  - 🔴 **"IMPORT, LOOK, WIPE AND RELOAD AS MANY TIMES AS IT TAKES" IS OVER ON LAWNS.** Board
    **CARD 10** can no longer PASS there — the wipe is correctly refused. **CARD 5 and CARD 10
    flipped `covered` → `owed`** (OP-14 cl.3); CARD 5 is a cheap re-run, CARD 10 is not.
  - ⚠️ **A SECOND REFUSAL GATE 2 DOES NOT CATCH:** `20260905_production_planning`'s
    `ON DELETE RESTRICT` lands in the catch, still after the customer delete. Named, not folded in.
  - ✏️ **A FALSE COMMENT OF 48 DAYS DELETED** — the file claimed all four FKs were `SET NULL`.
    True of three. [[R-26]] in the file a builder reads before deciding the undo is safe.

### 🟢 PUSHED, NOT MERGED — CARD 17 on `owner-role-authority` (same branch, #337)

- 🟡 **SHE CAN SELL — an OWNER-role member who is not `businesses.owner_id` rings up an order end to
  end.** **`owed`, deliberately, on David's ruling.** Order `6a60a0ca` proves **access** and
  **server-side enforcement** — `tax_exempt_by = 790b31d2`, Lauren's uid, a resale exemption she
  applied — **and proves nothing about a screen.** Nobody watched the control render or the total
  re-compute. 🔴 **46 boards were swept and no existing card covered this**; it is
  `authority-model` CARD 10's positive twin. **Only David's live run flips it.**
