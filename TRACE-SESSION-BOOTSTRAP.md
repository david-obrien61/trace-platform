...
- 🟡 **Inventory datasheet v1 — 5.1 reconcile (editable grid)** (ledger #75, `c2d34bf`) · BUILDER-COMPLETE, owner-proof owed (David, **desktop**) · DEMO · `/inventory` read-only list → editable/sortable/filterable/column-hideable desktop grid; inline edit **UUID-keyed + business_id-scoped** (`sku` display-only, Case 1/3); manual `unit_cost`→CONFIRMED (overridable) + Add-Item aligned; dup-size CASE-5 amber flag reuses `findDuplicateSizeGroups` via new zero-dep leaf `discovery/dupSize.ts` (no AI-SDK in client bundle); NO schema/migration/RLS (inherits view_costs gate); `[TRACE:invsheet]` ON; `npm run verify` exit 0 zero NET-NEW · reuse: `OperatingCosts` inline-edit pattern · deps: none · → `BusinessInventory.tsx`, built-inventory 5.1, ledger #75
- 🟡 **Asset-capture tool v1 — snap/import → Vision value → real `cost_objects` ASSET row** (ledger #76, uncommitted) · BUILDER-COMPLETE, owner-proof owed (David: apply migration + PAT-prove, then phone) · DEMO/arc-8 · TWO DOORS (camera + MULTI-import) → ONE pipeline: compress ALWAYS → OCR **`shape:'asset'`** seam (ZERO new Vercel fns, 12/12 held) → real `cost_objects` ASSET row via shared `SyncEngine`; no-signal → compressed blob held in NET-NEW shared IndexedDB `assets/assetBlobStore.ts` (#57 sync is string-only) + drained on reconnect; NEW `estimated_value` @ESTIMATED, owner inline-edit → CONFIRMED, never 0; NOT-NULL name honest placeholder; GATED migration `20260701_cost_objects_estimated_value.sql` (two-field model — David applies as postgres, schema-verification gate OWED); DEPLOY-WINDOW SAFE (missing-column → `forget`+re-insert; `/assets` SELECT untouched — post-migration fast-follow); `/assets/capture` view_costs-gated + Capture button on `/assets`; AC-1 (category vocab as VALUE)/AC-2/3; `origin/assets` recommended RETIRED (rebuild-not-refactor); `[TRACE:ASSET]` ON; `npm run verify` exit 0 zero NET-NEW, build clean 2242 modules · reuse: `resizeAndCompressImage`(100%), OCR seam(100%), `SyncEngine`(100%), `CATEGORY_OPTS` · deps: David applies `20260701` migration + PAT-prove (EV1–EV5) + phone owner-prove (live+import, no-signal hold→drain, edit→CONFIRMED, shows in `/assets` + PMI-eligible) · → `AssetCapture.tsx`, `utils/assetCapture.ts`, `shared/src/assets/`, built-inventory ledger #76, close-out #76
