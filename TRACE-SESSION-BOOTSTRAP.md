...
- 🟡 **Customer-zero rhythm logger** (ledger #63, `8839502`) · BUILDER-COMPLETE, phone owner-proof owed · TOOLING (data instrument on customer-zero David — gather the day's rhythm to design the north-star TIMING LAYER) · `?rhythm=1` 🟢 panel (bottom-LEFT, beside 🐞): foreground `watchPosition` (point on >10m move + 60s heartbeat while still) + voice/text narration stamped with location + shape (buy/task/inventory-check/project) + thread → localStorage ring (survives reload) → Share/JSON/TXT export · **FOREGROUND-ONLY** (web can't track a locked phone; background = future native, flagged not faked); data LOCAL until exported; David-only, not tenant-wired · deps: none (mobile tab) · → `packages/shared/src/rhythm/rhythmBuffer.ts`, `RhythmLogger.tsx`
