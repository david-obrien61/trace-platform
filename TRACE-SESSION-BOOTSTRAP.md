...
- 🟡 **BUILDER-COMPLETE · 22 owner-test cards, 0 COVERED** (20 `owed` · 2 `needs-test`) · ⛔ **THREE MIGRATIONS NOT APPLIED** — `20260906_inventory_import_run_provenance.sql` · `20260906b_customers_import_run.sql` · `20260906c_qb_identity_unique_indexes.sql` · **api/ 12/12, nothing minted · NO new permission string** · `npm run verify` **exit 0, ZERO NET-NEW** · **79/79 files, 4254 assertions** · **40 mutants, 40 caught, 0 survived** → `docs/owner-tests/qb-catalogue-import-full-surface-test.md`
...
- 🔴 **OPEN, DAVID'S CALL:** ① the **story gate is open** and I did not close it by inventing one — `user_stories.md` has no heading for importing a product list · ② **is `50lb` the SIZE of a bag, or is the size `each`?** — the answer decides whether #193 is a parser change or a data-model question.
