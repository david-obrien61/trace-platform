...
- 🔴 **BEFORE CLAIMING ANY ID: `npm run verify:id-sweep`** — next free is **#315** / tech-debt **#290** / **R-154** ⚠️ *(corrected by the merge of ledger #308, which consumed **R-150** and **tech-debt #285** — see tech-debt #289: this line is an ANSWER, not a claim, so nothing re-derives it and the sweep cannot see it go stale)*. **Then RESERVE AND PUSH before building** (R-149). The sweep does not close the race and says so.
