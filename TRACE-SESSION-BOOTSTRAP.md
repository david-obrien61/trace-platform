...
| 🟡 | **2.3** Walk-and-count inventory | LOOP BUILT (scan→qty→save→next→complete, `InventoryCount`+`QrScanner`/jsQR) + now OFFLINE-CAPABLE (ledger #57 — shared `packages/shared/src/sync/`: dead-zone Save queues + drains on reconnect, identity-stamp, double-count surfacing), BUILDER-COMPLETE, migration gated + phone owner-proof owed; OCR-intake sibling still NEXT | ledger #54 · #57 · `walk-and-count-inventory-verify-first.md` |
