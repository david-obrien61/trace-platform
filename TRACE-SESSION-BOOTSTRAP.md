...
- 🟡 **Customer-zero rhythm logger** (ledger #63, `8839502`) · BUILDER-COMPLETE + **PARTIAL owner-proof 2026-06-29** (clean ~40-min attended drive-home segment → PROVES rhythm legible WHEN captured); full-day run owed · TOOLING (data instrument on customer-zero David — gather the day's rhythm to design the north-star TIMING LAYER, see `NORTH-STAR.md` §5) · `?rhythm=1` 🟢 panel (bottom-LEFT, beside 🐞): foreground `watchPosition` + voice/text narration stamped with location + shape (buy/task/inventory-check/project) + thread → localStorage ring → Share/JSON/TXT export · **FINDING: ATTENDED-ONLY** (foreground/screen-on; the larger ~2h loss that day was the app LOGGING DAVID OUT on a network hiccup — `BusinessProvider.tsx:237` `getUser()` — NOT screen-lock; passive all-day capture = NATIVE app) · → `packages/shared/src/rhythm/rhythmBuffer.ts`, `RhythmLogger.tsx`
- 🔴 **Offline-session bug — ELEVATED to priority** (was "fix before PWA wrap") · proven painful in real field use this session (logs David out mid-trip; a low-signal greenhouse count could log Lauren out mid-count → threatens the demo) · DEMO-FLOOR · root: `BusinessProvider.tsx:237` `getUser()` network call · **= option A of the open A/B next-build decision** (A session/offline-login fix [Lightning's lean] vs B count-side size-picker [ledger #62]) · → `docs/CLOSE-OUT-LEDGER.md` GENUINELY OPEN
...
- **FIELD FINDINGS (master bank #64, recorded — see CLOSE-OUT-LEDGER GENUINELY OPEN):** ⚪ **size-variant SEQUENCING LANDMINE** — migration `20260628` NOT applied; do NOT populate per-size rows live until the count-side size-picker exists (L4 `InventoryCount.tsx:263` AMBIGUOUS→UNKNOWN otherwise; the L5 NEED_CLARIFICATION seam) · ⚪ **A/B next-build** (A offline-login fix vs B size-picker, David decides) · ⚪ **reconciliation rhythm is PER-CATEGORY** (seasonal fast/markdown · trees slow/appreciate · tropicals inverse — domain ontology §3, confirm vs clean transcript) · ⚪ **buyer-per-category** (inventory cognition distributed across humans by category — may scope roles + the count tool) · ⚪ **no-system grower** = the ~88% case (count builds the catalog — user_stories asset-inventory-pmi).
