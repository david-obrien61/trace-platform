...
|---|---|---|---|---|
| 2026-07-31 | **A permission string is chosen from what the code WRITES, not from what the surface is called.** The checkout delivery-date field writes `orders.delivery_date`; it never touches the `deliveries` table, so it gates on `orders:update`. **Narrower and correct beats wider and convenient** — granting a manager `deliveries:create` to fix a field on a different table would also hand her the ReceiptKeeper delivery INSERT she was never owed. | IMPLEMENTED (one string, `CustomerCapture.tsx:139`) | — **owed: capA cannot see this class.** capA asserts SHAPE (no identity token in an authority position) and the shape was perfect throughout. **Nothing compares who-passed-BEFORE to who-passes-AFTER.** The one-off 15-site diff is DONE (ledger #172); the standing cap is not built | ledger #172 · tech-debt #84 |
...
| **#84** — does `orders.delivery_date` want a SERVER gate on `orders:update`, matching the client? `submit.ts` gates discount, tax_exempt, order-update and order-delete — **nothing guards this field on the create path**, so the client invented a wall the server never had. Deferred by David 2026-07-31 (demo is days away and `submit.ts` is the money path), **recorded rather than left silent: an unenforced client gate is a fake pill by another name.** | nothing today — the field is client-gated only | tech-debt #84 · ledger #172 |
