...
| G2 | **Reachable horizontal scrollbar** — the h-scrollbar lives on a viewport-bounded box, reachable WITHOUT scrolling past every row. 🔴 **AMENDED 2026-09-07: THE BOUND MUST SURVIVE ANYTHING RENDERED ABOVE THE BOX INSIDE ITS OWN CARD.** A fixed `maxHeight: calc(100vh - N)` is bounded but not robust: it cannot know about a banner, a notice or a second filter row added later, so each of those pushes the box's bottom edge — and its h-scrollbar — below the fold, restoring the exact defect this clause exists to prevent. **The card is a flex column with the bound; the box takes the remainder (`flex:1; min-height:0`).** Then chrome added above it SHRINKS the box instead of displacing it, and there is no magic number to re-tune. ⚠️ *Found live: the "needs a look" banner (R-101) made the inventory grid taller and the h-scrollbar dropped off screen — and the columns past the fold were **price, size and variant group**, the exact three the banner tells the owner to edit. A flag naming a fix she cannot reach.* | Bounded scroll container (all grid libraries scroll the grid, not the page). | The defect that started this: a wide 111-row grid forced a scroll to the very bottom to find the h-scrollbar. |
...
| G10 | 🔴 **A ROW DISCLOSURE OPENS FROM A *LEADING* TOGGLE, AND THE ROW ITSELF IS A CLICK TARGET.** Where a grid renders per-row expansion, the toggle is the FIRST column — before the frozen identifier, in its own reserved track (G3/§6 r14) — and a click anywhere on the row that is not itself an interactive control toggles it. ⚠️ **TWO EXCLUSIONS, BOTH BINDING:** ① the row is clickable ONLY on a grid that HAS an expansion — a grid without one must not acquire a mystery click target; ② the handler MUST ignore clicks originating in an interactive descendant (`input`, `button`, `a`, `select`, `label`, `[role=button]`), or it swallows inline edit (**G8**) and in-cell links on the six editable consumers. | Excel's grouping +/− in the left gutter · the Supabase table editor's leading `>` · AG-Grid master/detail · every file explorer and mail thread list. | **A trailing toggle is invisible exactly when it is most needed:** on a wide grid it sits past the horizontal fold, so the control that reveals the row's detail is the one control you must scroll to find — while the eye looks for a disclosure where the hierarchy begins, at the START of the row. And a 15px chevron is a small target for a pointer: the ROW is the affordance people actually aim at, which is why every list that has ever done this makes the row clickable. |

| G11 | 🔴 **COLUMN ORDER IS ACTIONS · NAME · DATA.** The per-row action track comes FIRST, the record's identifier column SECOND, and everything else after — on every grid, with no per-consumer choice. Exactly ONE column declares `identifier: true`; the actions track is pinned immediately before it, in its own reserved track (G3/§6 r14). **The only things permitted ahead of the actions track are non-data gutters** — G10's disclosure toggle and a per-row flag glyph — because a mark is neither an action nor data. Nothing else may precede it, and a grid must not order its columns by hand. | Every operations console that carries per-row actions puts them at the row's start or its end and puts them there on EVERY list — Gmail, Jira, Linear, the Supabase table editor, AG-Grid's pinned action column. The invariant is not WHICH end; it is that one screen does not re-choose it per list. | 🔴 **Four grids had three different shapes and nothing said they were wrong, because the G-clauses cover sort, filter, freeze and the read-only mark and say NOTHING about column order.** /inventory rendered `ACTIONS · NAME`, /customers rendered `NAME · ACTIONS`, and the difference was not a decision anyone made — it fell out of where each config happened to put its frozen run. **The cost is muscle memory: the same button lives in a different place on each screen, so the hand has to re-find it every time.** Ruled by David 2026-09-07. ⚠️ *And it exposed a silent G3 failure it was not looking for: /inventory's `needs a look` column, inserted between the flag and the Name column, BROKE the leading frozen run — so `Name` carried `frozen: true`, was not pinned, and nobody could see the difference until they scrolled right.* |
| G12 | 🔴 **A DERIVED ROW MARK SORTS ITS FLAGGED ROWS TO THE TOP BY DEFAULT, ORDERED BY WHAT THE MARK IS WORTH — AND IT IS THE ONE CLAUSE PERMITTED TO OVERRIDE G9.** Where a grid carries a derived "needs a look" mark (`rowFlag` + a mark column), the grid's DEFAULT sort is that column, descending, with the mark's own weight as the sort value — money at stake on a price collision, cluster size on a duplicate — and every unflagged row sorts below every flagged one. It must also be **filterable to the flagged rows alone**, and the mark must be **DERIVED at read time, never stored**. G9 governs every grid that has no such mark, and governs this one the moment the flags are cleared. | Every operations console that computes an exception surfaces the exceptions first and lets you filter to them — a bank's reconciliation queue, a linter's error list, Jira's "assigned to me". The mark is not decoration; it is a worklist. | 🔴 **Two grids were already doing this and NO CLAUSE NAMED IT, so it read as a per-consumer choice rather than as the standard it is.** `/inventory` sorts on `needs` under R-101, and `/customers` now sorts on its duplicate mark under David's 2026-09-08 names-on-paper ruling — the dependency that ruling ships with, because the printed books report says *"review your customers in Cultivar"* and a mark on row 900 of 1,953 is a mark nobody will ever reach. ⚠️ **The DERIVED half is load-bearing and is the reason this is not simply "sort by a column":** a stored flag stays red after the owner fixes the record, so the worklist never empties and stops being believed. Ruled by David 2026-09-08. |
...
🔴 **G12 IS NEW (2026-09-08) AND IT IS MET ON EXACTLY TWO SURFACES — the two that provoked it.** `/inventory` (`defaultSortKey="needs"`, `rowFlag` on the shape collisions, derived by `findShapeCollisions` at read time) and `/customers` (`defaultSortKey="dup"`, `rowFlag` + an `extraFilter` on the duplicate mark, derived by `findDuplicateParties` at read time). ⚠️ **EVERY OTHER GRID IS UNAUDITED AGAINST G12 — not found wanting, not looked at**, which is the same honest wording this board carries for G9 and E7, and for the same reason: a clause minted today has not swept the corpus it now binds. ⚠️ **AND IT IS A NAMED EXCEPTION TO G9, NOT A CONTRADICTION OF IT** — G9's own text reserves the ground (*"unless a clause here names another"*), and until today nothing did, so two grids were diverging from a ruling with nothing on paper saying they were allowed to.

🔴 **G10 IS NEW (2026-09-03, ledger #270) AND IS A KNOWN RED AT THE MOMENT IT IS WRITTEN.** `DataSheet.tsx` renders the expand toggle **TRAILING** (`{renderExpand && <th style={S.th}></th>}`) and the row is not a click target, so **both halves of G10 are unmet on both consumers** — `ReceiptsList` and `BusinessInventory`. It is minted HERE FIRST, before the widget and before either surface, under this document's own enforcement order (*"① this doc is updated → ② then the shared display widget is updated, once → ③ surfaces use the widget"*) — which is the R-74 order, and the reason the clause exists before the code rather than after it. ⚠️ **The declaration for `Vendors.tsx` was re-answered for G10 in the same commit**: it is a bespoke surface with no per-row expansion, so the clause is vacuous there — recorded as such rather than left to fail the cap.
...
| E7 | 🔴 **A CONTROL THAT CHANGES ONE RECORD LIVES WHERE THAT RECORD IS OPENED, NOT ON THE ROW. The row carries a READ-ONLY MARK of the result.** **The unit of work decides:** a cell edited in a list of many stays a cell (**G8**, and the E2 worked example below); a DECISION MADE ABOUT A RECORD — one the owner reasons about, that carries a reason with it, that a second person will later read to understand why — belongs where the record is opened. ⚠️ **TWO CONSTRAINTS TRAVEL WITH THE MARK, BOTH BINDING:** ① it must carry what distinguishes THIS row and never restate what the header already said (**§5 clause 4** — otherwise it is a second representation of one fact, and the redundant copy is the one that drifts); ② it must **NOT read as clickable** (**G8**) — a mark that looks like a control and is not is a dead affordance, which is the defect this clause would otherwise trade for the one it fixes. | Every records UI that separates a list from a record: the row shows state, the record's own surface holds the controls that change it. | 🔴 **A per-record control on the row forces the row to carry an editor, and an editor on a row is a second edit surface the moment the record gets a real one** — which is E1's defect arriving through the list rather than through a second dialog. It also makes the row's height a function of its edit state, so opening one record moves every row beneath it. **The worked case:** `/vendors` put *"Why is this vendor preferred?"* and *"Save as preferred"* inside the row (`Vendors.tsx:271`/`:283`) while the vendor record had **no edit surface at all** — so the ONE judgement the screen exists to record was the ONE thing editable, and the other eleven columns had no writer. Ruled by David 2026-09-04. |
...
🔴 **E7 IS NEW (2026-09-04, ledger #273) AND IT IS MET ON EXACTLY ONE SURFACE — the one that provoked it.** `/vendors` moved the preference control and its note into `VendorEditor` (a modal over the opened record) and left the row a read-only `PREFERRED` chip: no `onClick`, no `cursor:pointer`, `aria-hidden` on the glyph, and the word carries what only that row knows (§5 cl.4). ⚠️ **EVERY OTHER RECORD LIST IS UNAUDITED AGAINST E7** — not found wanting, not looked at. That is the same honest wording the board carries for G9, and for the same reason: a clause minted today has not swept the corpus it now binds.
