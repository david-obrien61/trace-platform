...
- **Step 1 — schema delta WRITTEN + GATED** (`supabase/migrations/20260617_cost_objects_shape_nature_source.sql`, append-only, NOT yet applied — David applies manually). Adds: `cost_shape text NOT NULL DEFAULT 'ONE_TIME' CHECK(6 values)` · `cadence text CHECK(5 values, nullable)` · `recurring_amount numeric(10,2)` (DISTINCT from `acquisition_cost` — never overload capex onto recurring) · `cost_nature text NOT NULL DEFAULT 'CAPEX' CHECK(CAPEX|COGS|OPEX)` · `cost_source text NOT NULL DEFAULT 'MANUAL'` (provenance SEAM for the connect-via-API layer; intentionally NO CHECK so sources grow without a migration) · `node_type` CHECK WIDENED with `'COST'`. NOT NULL defaults are TRUE for every pre-existing row (all ASSET/ONE_TIME/CAPEX/MANUAL — no row mis-tagged). Shapes 4-6 carry columns (amortize term/start, increment_size, scales_with) DEFERRED honest-debt (no writer yet). **⚠️ SCHEMA-VERIFICATION GATE PENDING:** `scripts/verify-cost-objects.mjs` extended with catalog checks (J)-(P) but NOT YET RUN — PAT was revoked; David runs `SUPABASE_PAT=… node scripts/verify-cost-objects.mjs` AFTER applying. Schema is BUILDER-WRITTEN, NOT catalog-proven this session.
...
- **HELD for David (fresh confirm before proceeding):** step 4 backfill (recurring config lines → `cost_objects` rows: nature OPEX, source MANUAL, lossless, UNKNOWN→UNKNOWN never $0) → step 5 verify flat total matches the step-0 anchor → step 6 flip read source → step 7 re-point write path (shape + nature + project pickers + substantiation; preserve truncation-guard intent) → step 8 owner-prove. **Labor DEFERRED** (rate×hours stays in config this pass, flagged not dropped — R3). Next layers named, NOT built: API connectors (provenance via `cost_source`) + AI-assisted classification (manual-first works without it).
...
**Status:** ✅ BUILDER-COMPLETE — 2026-06-16 (THUNDER · D-10). **NOT yet OWNER-PROVEN** (two-tenant owner-proof owed — see below).
**Files:**
