...
- **HELD for David:** step 7 re-point write path (Settings recurring entry → `cost_objects` rows + shape + nature + project pickers + substantiation; preserve truncation-guard intent) → step 8 owner-prove the live tile through the UI under RLS. **Labor DEFERRED** (rate×hours stays in config this pass, flagged not dropped — R3). Next layers named, NOT built: API connectors (provenance via `cost_source`) + AI-assisted classification (manual-first works without it).
