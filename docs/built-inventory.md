...
- **ASSET-CAPTURE TOOL v1 — two-door capture → Vision value → real `cost_objects` ASSET row (2026-07-01, ledger #76, BUILDER-COMPLETE; migration GATED + owner-proof owed):** the general-layer capture→value→manage primitive that turns the asset LIST (read/edit only) into a DOER. **Two doors → one pipeline:** in-app camera (`capture="environment"`) + MULTI-select device import → compress ALWAYS (reuse `packages/cultivar-os/src/utils/imageCompression.ts`, both doors) → Vision via the OCR **`shape:'asset'`** seam (NEW `assetPrompt(categoryHint?)` in `packages/cultivar-os/api/receipts/ocr.ts` → `{name,category,estimated_value,confidence}`; **ZERO new Vercel functions** — 12/12 held) → a REAL `cost_objects` ASSET row (`node_type='ASSET'`, never notes-JSON) written through the shared `SyncEngine`. **No-signal path:** the COMPRESSED blob is held in a NET-NEW shared IndexedDB store `packages/shared/src/assets/assetBlobStore.ts` (the #57 sync slice is string-only, can't hold blobs; tenant-scoped, in-memory fallback) + drained on reconnect (Vision+insert then) via `drainPendingAssets`. **NEW `estimated_value`** (what-it's-worth: net-worth/insurance — DISTINCT from `acquisition_cost` = what-was-paid) lands @ESTIMATED; owner inline-edit → CONFIRMED (`promoteAssetValue`); value NEVER 0 (Surface Honesty); NOT-NULL `name` gets an honest "Unidentified asset (from photo)" placeholder. **Files:** orchestrator `packages/cultivar-os/src/utils/assetCapture.ts` (`captureAsset` / `promoteAssetValue` / `drainPendingAssets`); page `packages/cultivar-os/src/pages/AssetCapture.tsx` (`/assets/capture`, `view_costs`-gated, offline + "N to sync" chips); `/assets` gains a **Capture** button (its datasheet SELECT deliberately UNTOUCHED — adding `estimated_value` there would 400 the live page pre-migration → post-migration fast-follow). **GATED migration** `supabase/migrations/20260701_cost_objects_estimated_value.sql` — adds `estimated_value numeric(10,2)` + `estimated_value_confidence` CHECK(CONFIRMED|DERIVED|ESTIMATED|UNKNOWN), both nullable/additive/lossless (EV1–EV5 embedded); **DAVID APPLIES AS postgres, schema-verification gate OWED.** **DEPLOY-WINDOW SAFE:** on a missing-column reject the insert `forget`s + re-inserts WITHOUT the value cols (mirrors the deliveries `service_type` fallback). AC-1 held (category vocab passed as a VALUE via `CATEGORY_OPTS` — no vertical noun in the seam); AC-2/3 business_id-scoped writes; PMI/service-log downstream unchanged (job ends at the `cost_objects` row). Andrew's `origin/assets` branch (1520-line `AssetManager.jsx` + localhost:8000 Python pipeline + notes-JSON metadata) reviewed + recommended **RETIRED** — this is a rebuild-fresh on the settled spine, not a refactor. `[TRACE:ASSET]` ON by default (standing owner hold). `npm run verify` exit 0 zero NET-NEW (tsc 10 / eslint 266 / knip 10·14·15); `build:cultivar` clean 2242 modules (+4 files). **Owner-proof owed (David):** apply the migration + PAT-prove EV1–EV5 + revoke; then on phone — capture live AND import a batch → both produce `cost_objects` ASSET rows; a no-signal capture holds then drains on reconnect; edit a value → CONFIRMED; the asset shows in the `/assets` datasheet + is PMI-eligible. **Fast-follow (post-migration):** surface `estimated_value` inline-edit on the `/assets` datasheet; optional photo persistence to storage (`photo_url` null in v1).
