...
**Core-1 (2026-06-15) — BUILDER-COMPLETE, awaiting apply:** `business_assets` → `cost_objects` rename-in-place + node/edge schema. ⚠️ David must apply `supabase/migrations/20260615_cost_objects_rename_and_node_schema.sql` then run `SUPABASE_PAT=… node scripts/verify-cost-objects.mjs` (catalog gate). See "Core-1 — Cost-Object Node Model" subsection below.  
...
**Core-1 — Cost-Object Node Model (2026-06-15, BUILDER-COMPLETE, awaiting David's apply + catalog gate):**
