...
- **Endpoint (`api/deliveries/create.ts`):** writes `service_type`. **Migration-window-resilient:** on a missing-column error (42703/PGRST204) it retries WITHOUT service_type so delivery creation never breaks before the column is applied (honest debt, logged — column now applied; fallback can be removed next touch).
