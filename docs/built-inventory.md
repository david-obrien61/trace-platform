...
- **Write path (B2):** "Schedule delivery" destination in the router is now a live checkbox. On confirm, `doSave` (`ReceiptKeeper.tsx`) resolves the customer ONCE (`resolvedCustomerId`) and passes that exact id to new endpoint `api/deliveries/create.ts` (+ cultivar impl, service-key) — **no double-create** (the delivery endpoint never calls findOrCreateCustomer). Scheduling implies a customer: the two checkboxes are coupled (schedule→on forces add-customer; add-customer→off clears schedule). Delivery uses ship-to (falls back to bill-to) address + the ISO delivery_date.
...
- **DIAGNOSIS (the owner's "Schedule delivery — coming" on phone):** NOT an unwired front-end — the toggle was already functional in `253cf49` (`ReceiptKeeper.tsx:988-1003`, no "coming" badge; `doSave` calls `/api/deliveries/create`). The phone hit a **stale Vercel bundle** predating 253cf49. A fresh deploy of `main` resolves it; this commit (634b990) forces that redeploy.
- **Schema (GATED, NOT applied):** `supabase/migrations/20260620_deliveries_service_type.sql` — `ALTER TABLE deliveries ADD COLUMN service_type text` (nullable, NO CHECK per AC-4). Append-only, no other table touched. Catalog gate `verify-deliveries.mjs` (G) — OWED after David applies + PAT. Apply AFTER the base deliveries migration (already applied).
...
- **Endpoint (`api/deliveries/create.ts`):** writes `service_type`. **Migration-window-resilient:** on a missing-column error (42703/PGRST204) it retries WITHOUT service_type so delivery creation never breaks before the column is applied (honest debt, logged; remove fallback once (G) is green).
- **Day view:** `DeliverySchedule.tsx` shows a service_type badge (planting=blue, delivery_only=green).
- **Owner-proof owed (on phone, after deploy + migration apply):** snap the Marcus Webb invoice → "Schedule delivery" is LIVE (no "coming") → service type shows inferred + correctable → confirm → Marcus Webb under Jun 25, 2026 with the service_type badge → "Route this day" plots.
