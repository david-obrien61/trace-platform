...
- **Role-config UI (visibility axis)** — ⬜ PLANNED — **NEXT Role Machine rung** (reads `allTiles()`/`registryPermissions()` from the now-built registry). Create/clone custom roles + tune own system roles; three-tier storage; factory-reset = override-delete; system roles locked. Cultivar-native rebuild of the Ignition console. (D-010, D-011)
...
- **Persistent identity header** — ✅ **BUILDER-COMPLETE 2026-06-22** (verify-universals **cap #1 FAIL → PASS**; OWNER-PROVEN owed-after-deploy). Shared `<AppHeader>` (`packages/shared/src/components/AppHeader.tsx`) — a sticky strip showing active business name + signed-in email + role badge (OWNER/MANAGER/STAFF, the 3 Cultivar runs today; inherits the 5-role model for free when Role Machine lands because it reads the resolved role, not a hardcoded list). Mounted ONCE via `AppLayout` (`packages/cultivar-os/src/components/layout/AppLayout.tsx`) wrapping the PrivateRoute routes (`router.tsx`) — one mount, every authenticated page, not per-page. Identity comes from the canonical BusinessProvider context (extended to expose `userEmail` + display-ready `role`); the header NEVER fetches on its own. `[TRACE:HEADER]` ON. Cultivar-native rebuild of the Ignition ShopBanner/IDENTITY MATRIX strip (shape ported, not code).
