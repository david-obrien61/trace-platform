...
### Role-Based Financial Permissions — v1 hardening (RLS wall OWNER-PROVEN · Phase 3 render layer owed-after-deploy) — updated 2026-06-22

**What:** The role wall over financial data, decided in [docs/decisions/2026-06-21-role-financial-permissions.md](decisions/2026-06-21-role-financial-permissions.md) (recon: `data/grower-scan/role-enforcement-ground-truth.md`). Closes the verified gap that role enforcement was RENDER-ONLY. **Four permissions** (view_wages, view_pricing_config, view_costs, view_margin — view_margin REQUIRES view_costs), **role defaults** (Owner=all four; Manager=costs+margin; Staff/Tech=none), DEFAULT-DENY, SINGLE CHOKEPOINT, RESTRICTIVE-FIRST/EXPAND-ONLY, PROOF-BAR = low-role read refused at the NETWORK RESPONSE (not render-absence). Enforcement by data class: **HARD data-layer wall (RLS)** for wages + pricing config AND cost/inventory; **render/route gates + dashboard query-shaping** as the convenience layer; checkout charge made server-authoritative.
**Status — phased, proof-gated:**
- **Phase 1 — FOUNDATION ✅ BUILDER-COMPLETE + GATE 1 PROVEN (25/0 service-key):** four permissions in one shared module + role bundles; `can()` chokepoint in BusinessProvider (`view_margin⊆view_costs`, `[TRACE:PERM]` ON); all 6 live `business_members` backfilled additively (nothing removed, owner unaffected, default-deny). Commits `99a76e3` (decision), `00dc641`.
- **Phase 2 — HARD WALL ✅ BUILDER-COMPLETE + GATE 2 PROVEN:** child-table split — `labor_resource_wages` (view_wages) + `business_pricing_config` (view_pricing_config), role-aware RLS; values COPIED into the gated children + CLEARED from the member-readable home; all reads/writes via a migration-window-resilient seam. **Catalog gate 16/0** (`verify-financial-wall.mjs`, PAT, since revoked) + **low-role RLS 6/0** (`verify-financial-wall-rls-auto.mjs`: no-perm member refused, perms granted → access appears). Migration `20260621_financial_wall_phase2.sql` applied. Commits `00dc641`, `3ef3101`.
- **Phase 3 ✅ BUILDER-COMPLETE (commit `8918fe8`):** Part A (view_costs) — `PermissionRoute` route-gates `/costs,/inventory,/assets,/operating-costs,/pmi`; dashboard selects qty-only + hides the inventory-value tile without view_costs. Part B — checkout charge server-authoritative (`api/orders/submit.ts` refetches unit_cost; client-POSTed price never trusted; tamper PROVEN 3/0, `verify-checkout-tamper.mjs`). Checkout customer DISPLAY unchanged (unit_cost is the customer's price on that surface — out of the view_costs wall; see Part C).
- **Canonical RLS (David, applied this session):** `is_active_member()` primitive + member-RLS standardization + md_self leak fix (commit `07dc88a`, `20260622_is_active_member_canonical_rls.sql`); `has_permission()` + compose-gate the 7 cost/wage tables on `is_active_member() AND has_permission()`, plus QB OAuth secret relocation to owner-only `business_accounting_secrets` (closes a member-readable QB-token leak) (commit `e81ba6e`, `20260622_oauth_secrets_relocation_and_cost_wall.sql` + `quickbooks/secrets.ts`).
**GATE 3 — proof split (recorded exactly):**
- **RLS COST WALL — OWNER-PROVEN** via HAR capture, real Staff session (`trace_staff_only`, user `9d0d2123-…-657aaa96c298`, isOwner:false, effectivePermissions Array(3), NO view_costs), 2026-06-22 ~17:06–17:09Z. Every cost-bearing read returned **HTTP 200 with body `[]`** (RLS row-filter refusal): `cost_objects` (both select variants), `business_inventory?select=name,unit_cost,cost_confidence`, `business_pricing_config?select=config`. Zero cost/margin/pricing values reached the Staff client. Composed gate `is_active_member(X) AND has_permission(X,'view_costs')` is live and refusing. Artifact: `cultivar-os_vercel_app.har`.
- **TAMPER DEFEAT — PROVEN 3/0** (builder): client-claimed unit_price ignored; server refetches unit_cost from `cultivar_plants → business_inventory`.
- **Phase 3 Part A render layer (route-gate redirect + dashboard qty-only shaping) — BUILDER-COMPLETE, OWNER-PROOF OWED-AFTER-DEPLOY.** The HAR predates the Phase 3 deploy (cost requests fired from the `/` dashboard referer unredirected; dashboard inventory query still went out as `business_inventory?select=qty,unit_cost`, the old shape). Nothing leaked (RLS caught it, `[]`), but the route gate + qty-only shaping aren't demonstrated by that capture. **Re-capture Staff HAR post-deploy** — expect: no cost request from the dashboard, dashboard inventory query `select=qty` only, and direct nav to `/costs|/inventory|/assets|/operating-costs|/pmi` redirects to `/dashboard`. That capture flips Part A → OWNER-PROVEN.
**Part C — checkout cost-as-price = grower-import gap (record + link, NOT new debt):** on checkout, `business_inventory.unit_cost` is shown/charged as the customer PRICE (`PlantHero`, `CartReview`, `Confirmation`, `order_items.unit_price`). This overload is the **second symptom of the known `sell_price`/`price_basis` schema gap** — same cure as grower import, not standalone debt. The fix (add `sell_price` + `price_basis`; rewire checkout to display/charge `sell_price` while `unit_cost` stays private cost) RIDES the grower-import schema build. See [docs/decisions/2026-06-21-grower-import-and-mobile-roles.md](decisions/2026-06-21-grower-import-and-mobile-roles.md) + `data/grower-scan/inventory-fit-map.md`. No `sell_price` column built here.
**Type:** capability (security/auth) | **Vertical:** cross-vertical (shared spine), proven on cultivar.
**Location:** perms `packages/shared/src/auth/financialPermissions.ts`; chokepoint `BusinessProvider.tsx` (`can`/`useCan`); role bundles `cultivar-os/src/auth/roles.ts`; wall seam `business-logic/financialDataAccess.ts` (→ `CostToProduceSettings.tsx`, `pages/CostToProduce.tsx`); route gate `cultivar-os/src/components/layout/PermissionRoute.tsx` + `router.tsx`; dashboard gate `pages/Dashboard.tsx`; checkout `api/orders/submit.ts`; QB secrets `quickbooks/secrets.ts`; migrations `20260621_financial_wall_phase2.sql`, `20260622_is_active_member_canonical_rls.sql`, `20260622_oauth_secrets_relocation_and_cost_wall.sql`; scripts `backfill-financial-permissions.mjs`, `verify-financial-permissions.mjs`, `verify-financial-wall.mjs`, `verify-financial-wall-rls.mjs`, `verify-financial-wall-rls-auto.mjs`, `verify-checkout-tamper.mjs`.
