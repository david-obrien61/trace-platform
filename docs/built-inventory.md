...
- **HELD for David (report-before-flip ceremony):** step 6 flip read source (page `CostToProduce.tsx` + capture mirror select `cost_shape`/`cadence`/`recurring_amount` and stop counting `config.recurring` to avoid R2 double-count) → step 7 re-point write path (shape + nature + project pickers + substantiation; preserve truncation-guard intent) → step 8 owner-prove. **Labor DEFERRED** (rate×hours stays in config this pass, flagged not dropped — R3). Next layers named, NOT built: API connectors (provenance via `cost_source`) + AI-assisted classification (manual-first works without it).
