...
**FIRST-RUN MATRIX (the audit result):** Ignition #1 PASS · #2–5 SKIP. Cultivar #1 **FAIL** (no persistent identity header mounted — `PrivateRoute` renders a bare `<Outlet/>`, `BusinessProvider` is context-only, `NavBar.tsx` unmounted/title-only; each page renders an ad-hoc header) · #2 PASS (9 financial policies gated) · #3 PASS (12 member-scoped tables dual-RLS; 9 owner-only tables KNOWN-GAP) · #4 PASS (14 member policies canonical; `md_self` documented exception) · #5 PASS. Gate exits **1** until Cultivar #1 is addressed. NOT auto-chained into `build:*` (would silently break the build given the known FAIL) — run as its own gate.  
...
**Status:** ✅ READ-WALL OWNER-PROVEN (Staff HAR: cost reads return 200 [], RLS row-filter; tamper 3/0; migrations live). ⚠️ WRITE-WALL NOT PROVEN — known open instances: (1) operating-cost "+cost" save writes operating-cost with no view-permission gate; (2) costDiscovery cost-apply endpoint (service-key bypass). Gate-3b write-wall owed. Enforces the D-009 moat on READS only today. Staff HAR detail: `trace_staff_only`, `isOwner:false`, `effectivePermissions` Array(3), no `view_costs`; every cost read (`cost_objects`, `business_inventory…unit_cost`, `business_pricing_config`) returned `200 []`. Artifact: `cultivar-os_vercel_app.har`. **Type:** capability (RLS/security) | **Vertical:** shared.
