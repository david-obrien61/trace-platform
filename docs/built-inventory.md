...
**Embedded Maps JS map — 2026-07-01 (`bfb202a` pushed to main, ledger #78, BUILDER-COMPLETE, app-code-only, owner-proof owed):** the built route now renders an INTERACTIVE map above the URL-handoff card — numbered pins in route order + a green polyline, not just a link. RECON-first resolved the gating detail: stack is **Vite**, client reads env via `import.meta.env?.VITE_*`, so David's unprefixed `GOOGLE_MAPS_API_KEY` is invisible to the browser → correct name is **`VITE_GOOGLE_MAPS_API_KEY`** (David renames in Vercel + redeploys + enables Geocoding API). BUILT in `DeliveryRoute.tsx`: singleton client-side Maps JS **script loader** (`loadGoogleMaps` — one `<script>`, **ZERO new Vercel functions**, api/ stays 12/12) reading `import.meta.env.VITE_GOOGLE_MAPS_API_KEY`; new `RouteMap` component geocodes the origin anchor (⌂) + selected stops → drops **numbered classic `Marker` pins in route order** + polyline, fits bounds. Structured `routeStops`/`routeOrigin` state set alongside `routeUrl` in `buildRoute()` (map + URL share one ordered source). **Graceful fallback (mandatory, held):** no key → map skipped; load/geocode failure → self-degrades to a muted overlay; the "Open in Google Maps" card is ALWAYS present + working (no regression). Standard-by-value: classic `Marker` over AdvancedMarkerElement (needs mapId + marker lib) for v1 — divergence in the component header. Deferred: Address Validation, stop-order optimization, embedded turn-by-turn. `[TRACE:MAP]` on geocode+render (STD-003, ON until owner-proven). `npm run verify` exit 0 zero NET-NEW (tsc 10 / eslint 262 / knip 10·14·15); `build:cultivar` clean. **OWNER-PROVEN owed (David, desktop, AFTER the Vercel env rename+redeploy+Geocoding-enable):** build a route → map renders with pins in order; blank the key → map gone, card still works.
...
**Status:** ✅ Built — TypeScript (BUILDER-COMPLETE, owner-proof owed — ledger #81, uncommitted, David commits)
...
**Owner-proof owed (David):** `/customers` shows the roster (all OCR + hand-added customers); inline-edit a name/phone → persists → reflects on that customer's deliveries via the join; sort/search/hide-columns work; "Edit customer →" from a delivery card lands on the roster; a Staff session cannot reach `/customers`.
