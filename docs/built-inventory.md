...
- **HELD — Step 8 OWNER-PROOF (David, through the live UI under RLS):** the whole loop, logged in as the owner (anon key, real RLS) — see the step-8 checklist in the 2026-06-17 handoff. **Labor DEFERRED** (rate×hours stays in config this pass, flagged not dropped — R3). Next layers named, NOT built: API connectors (provenance via `cost_source`) + AI-assisted classification (manual-first works without it).
