...
### Role-Based Financial Permissions — v1 hardening (Phase 1 SHIPPED · Phase 2 BUILDER-COMPLETE) — updated 2026-06-21

**What:** The role wall over financial data, decided in [docs/decisions/2026-06-21-role-financial-permissions.md](decisions/2026-06-21-role-financial-permissions.md) (recon: `data/grower-scan/role-enforcement-ground-truth.md`). Closes the verified gap that role enforcement was RENDER-ONLY (`business_members.permissions` read into context but consulted by no RLS/API). **Four permissions** (view_wages, view_pricing_config, view_costs, view_margin — view_margin REQUIRES view_costs), **role defaults** (Owner=all four; Manager=costs+margin; Staff/Tech=none), DEFAULT-DENY, SINGLE CHOKEPOINT, RESTRICTIVE-FIRST/EXPAND-ONLY, PROOF-BAR = low-role direct query refused at the NETWORK RESPONSE (not render-absence). Enforcement by data class: **HARD data-layer wall** (child-table split + role-aware RLS) for wages + pricing config; **response shaping** for costs + margin (Phase 3); **render gates** the top convenience layer (Phase 4).
**Status — phased, proof-gated:**
- **Phase 1 — FOUNDATION ✅ BUILDER-COMPLETE + GATE 1 PROVEN (25/0 service-key), live (enforcement OFF):** four permissions minted in one shared module + role bundles; `can()` chokepoint wired into BusinessProvider (`view_margin⊆view_costs` enforced, `[TRACE:PERM]` ON); all 6 live `business_members` backfilled (additive union — nothing removed, owner byte-for-byte unaffected, default-deny correct). No gate reads the new perms yet, so behavior is unchanged.
- **Phase 2 — HARD WALL ✅ BUILDER-COMPLETE (build clean, tsc-clean); GATE 2 OWED (David applies):** child-table split — `labor_resource_wages` (view_wages) + `business_pricing_config` (view_pricing_config), role-aware RLS reading `permissions ? '…'`; migration COPIES the values in + CLEARS the member-readable copies. All reads/writes route through a migration-window-resilient seam. Migration is GATED/UNAPPLIED → schema-verification gate + Gate-2 network proof owed once David deploys code → applies migration (+PAT) → runs `verify-financial-wall.mjs` (catalog) + `verify-financial-wall-rls.mjs` (low-role refused).
- **Phase 3 (cost/margin response shaping, checkout-first) + Phase 4 (render gates):** pending, gated on Gate 2.
**Mechanism choice (reported):** child-table split over masking-view — Postgres RLS gates rows not columns, and a masking view needs role-wide `REVOKE` that would block the owner's own client-side writes + collide with the 12-fn ceiling (#41). **Type:** capability (security/auth) | **Vertical:** cross-vertical (shared spine), proven on cultivar.
**Location:** perms `packages/shared/src/auth/financialPermissions.ts` (+ `auth/index.ts`); chokepoint `packages/shared/src/context/BusinessProvider.tsx` (`can`/`useCan`, exported via `context/index.ts`, aliased in cultivar `NurseryProvider.tsx`); role bundles `packages/cultivar-os/src/auth/roles.ts`; wall seam `packages/shared/src/business-logic/financialDataAccess.ts` (+ `business-logic/index.ts`), consumed by `CostToProduceSettings.tsx` + `pages/CostToProduce.tsx`; migration `supabase/migrations/20260621_financial_wall_phase2.sql`; scripts `backfill-financial-permissions.mjs`, `verify-financial-permissions.mjs`, `verify-financial-wall.mjs`, `verify-financial-wall-rls.mjs`.
