...
- 🔴 **OPEN — APPLY `supabase/migrations/20260916_container_ladder_install_t_posts.sql`, THEN MERGE — NEVER THE OTHER WAY ROUND.** The ladder read asks for the new column; merged first, every ladder read fails. SQL editor, `postgres`, then V1–V4. Owner: the migration file.
- 🔴 **OPEN — WHICH SIZES ARE "LARGE" FOR THE LEAKAGE FLAG?** `submit.ts` `LARGE_CONTAINERS` is the last hardcoded size list; it misses 65 and 200 gal and names a 60 nobody sells. Options: sizes that take install T-posts · a flag on each size · (a volume threshold is ruled out). Owner: tech-debt #310.
- 🟡 **OPEN — MAY STAFF READ THE FOUR PLANTING FIGURES?** They sit behind `settings:read`; a staff login's load list prints the standard figures and says so. Nothing wrong today — no business has saved one. Owner: tech-debt #309.
...
- 🟡 **OPEN — WHICH STOPS NEED DEER FENCE?** Nothing stored says, so every stop with trees prints as UNRESOLVED for fence. The in-total arithmetic is built for the day a stop can say so. Owner: `loadList.ts` `LoadStopInput.deerFence`.
