...
**Last updated: 2026-09-14** (ledger #322 — the import preview's two field checks; previously #320 — the #320 block added, and 🔴 **the register's own blind spot is named**: this file is fed by CLAUDE.md §3's *FLAGGED FOR DAVID*, and **fifteen close-out rows never wrote a §3 entry at all**, so whatever they flagged reached neither surface. #302 — the row that created this file — is one of the fifteen. Now asserted by `verify:handoff-retention` check 5; ledger #318 — the #318 block; #314 · #312 blocks; the counts below are still measured 2026-09-11 and say so) — created at David's request (ledger #302).
...
- ⚠️ **NOT A QUESTION, A HAZARD TO EXPECT:** #321 sits UNCOMMITTED in the primary checkout and both it and #322 archive #314 from §3, so **they will conflict in `handoff-archive.md`.** Tech-debt #282.
