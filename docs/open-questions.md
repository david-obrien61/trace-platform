...
**#380 — the applied migration's file, and which V-block did not prove what it claimed (merged? no — docs branch)**
- ⚠️ **OPEN — `#361` IS TWO DIFFERENT ITEMS AND ONLY ONE IS CLOSED.** tech-debt #361 (the NOT APPLIED header, closed here) and ledger #361 (the starting number / 43 real goods, still open above). Their lines sit ~30 apart in this file. Different registers, same number. Owner: ledger #380.
- ⚠️ **OPEN — NOTHING MECHANICAL CAN CONFIRM AN APPLY.** The corrected header is a true claim because David reported his V-block results, not because anything read the database. §6 r22's own guard says the same of itself. [[R-111]]: *a migration label is not evidence.* Owner: ledger #380.
- 🔴 **OPEN — A V-BLOCK THAT REFUSES FOR THE WRONG REASON IS INDISTINGUISHABLE FROM ONE THAT PASSES.** `20260921`'s V3 refused with the WIPE GUARD's message, not the CHECK's, so the identity CHECK was never reached; it is proven only by the corrected probe in `20260921c`'s footer. **This corpus writes many V-blocks and nothing checks which guard fired.** The corrected-probe pattern is the shape that catches it — whether it becomes a convention is David's call. Owner: ledger #380.
