...
- 🔴 **BLOCKING — apply three migrations in order, then the step-0 data.** `20260923` (ladder price + LAWNS's seed) → `20260923b` (price source) → `20260923c` (the MANAGER's `order_discount:apply`) → `docs/decisions/2026-09-23-step0-lawns-transport-rows.sql`. **Nothing below can be run until these land.** ⚠️ `20260923b` V5 is **expected to ERROR** (`23514`); that error is the pass. Owner: ledger #386.
...
- ⚠️ **`20260923c` IS A NEAR-TWIN OF DAVID'S OWN UNCOMMITTED `20260922e`** (ledger #382, `costs:read`). Same two rows, independently idempotent, either order — but the V-block string COUNTS depend on which runs first, and both files say so. Owner: ledger #382 / #386.
