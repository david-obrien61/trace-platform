...
> ✅ **Drift watch (2026-07-20 · D-50 LAYER 1 OWNER-PROVEN — the movement ledger DB layer + the `audit_log` first writer; ledger #140; NO new decision; close-out DOCS-ONLY):** No drift — this **BUILDS D-50**, it does not reinterpret it. Abided by **D-50** itself (same-transaction emission is now structural, not conventional — the qty write and the ledger INSERT are one plpgsql transaction, so "replay vs on-hand must be ZERO **by construction**" is a measured fact: 126/126 lots, `qty == SUM(delta)`, zero drift), **§6 r10 / STD-011** (the immutability guard REUSES the proven `audit_log` pattern verbatim in shape rather than spelling a second variant; the ledger insert is ONE shared `emit_inventory_movement` seam, not six copies), **AC-1** (`business_inventory_ledger` — business_ layer, no vertical noun), **AC-2/AC-3** (RLS `owner_all` OR `member_all`, business_id-scoped, no `USING(true)`; RPC actor gate refuses cross-tenant actors — proven live), **AC-4** (`kind` carries NO CHECK so the value set grows without a migration), **D-9** (the re-scan bar SKIPS **and REPORTS** protected lots rather than silently passing over them; a refusal writes no ledger row because nothing moved), **OP-15** (STEP ZERO — the migration's apply-state was confirmed at postgres before any V-check was read as evidence), and **OP-4/OP-11** (Thunder set BUILDER-COMPLETE; **David's live V1–V6 run** flipped it to OWNER-PROVEN and triggered this reconcile — Thunder never marked its own cards `covered`). **TWO DEVIATIONS FROM THE BUILD PROMPT, both surfaced not silent:** (1) actor validation uses a new `is_member_of(biz, uid)` on the **PASSED** uid — the specified `is_active_member()` reads `auth.uid()`, NULL under the service key, so the literal instruction **would have refused every sale**; (2) a later prompt specified a corrective migration for an owner-refusal that turned out **not to be a bug** — the owner branch already existed, so the fix would have been a **no-op**; a live read-only probe showed a cross-tenant test uid and a nonexistent lot. **Zero lines was the correct fix**, and it was reported rather than shipped. **LAYER 2 (app rewiring) OWED and marked 🟡 everywhere** — the DB layer is proven, the app is not rewired, and no surface claims otherwise. **Row 19B is PARTIALLY closed** — vault filled, `role.factory_reset` still owed; recorded as a split rather than a tick.
>
