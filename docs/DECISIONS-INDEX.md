...
> ✅ **Drift watch (2026-07-20 · OWNER-GATED DEV SURFACES + always-visible version stamp; ledger #142; NO new decision):** No drift — this **IMPLEMENTS OP-15 and the 2026-07-20 typed-activation recon**, and reinterprets neither. Abided by **§6 r8 SEMANTIC-DUP** (the whole build is that rule applied: `?debug=1` and `?rhythm=1` were one OPERATION written twice with two sticky keys and two `xEnabled()` readers free to drift — now ONE shared gate), **AC-2/AC-3** (dev state is keyed to the signed-in identity and purged on user OR role change; nothing carries across a person or a permission level), **AC-1/AC-4** (the gate and the header are vertical-neutral — `AppHeader` takes an injected `devSurfaces` prop exactly as it already takes `onSignOut`, so the SHARED header never decides authority and no vertical noun enters shared code), **§6 r11 FUNCTION THRIFT** (zero api-fn — 12/12 untouched), **§6 r15/r16 UI STANDARDS** (the toggles are `role="menuitemcheckbox"` + `aria-checked` with 48px targets and a visible On/Off state — a menu row that reported nothing back would be the dead-affordance case), **D-9 Surface Honesty** (the stamp says `dev` rather than inventing a SHA; a non-owner sees the Developer block **absent**, not disabled — a greyed control still discloses that the capability exists), and **OP-14** (five cards shipped WITH the surfaces, all set `owed`; Thunder set none `covered`). **THE ONE JUDGEMENT CALL, AND IT CUTS AGAINST THE OBVIOUS READ:** the version stamp is deliberately **NOT** behind the new owner gate, even though everything else in this build moved behind it. Gating it would mean **a deploy broken enough to break the menu could hide its own tell** — the exact failure tech-debt #60 exists to catch, reintroduced by the very build that hardened everything around it. **A tell must not depend on the thing it is testing.** **SCOPE FLAGGED, not silently accepted:** the rhythm logger records David’s physical location and voice notes, so an OWNER gate is WIDER than its real audience (Lauren would see it) — the narrower gate needs a platform-dev role that does not exist yet.
>
