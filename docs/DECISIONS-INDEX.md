...
**Last updated:** 2026-07-16 (**OP-13 (proposed) — RETENTION OVER TRIMMING (ledger #134); DOCS-ONLY, no owner-prove owed.** CLAUDE.md is loaded EVERY session, so its size is a tax paid before any work begins: trimmed to 746, measured **907** — **+100 in ONE session**, +44 more on a build whose write-back was deliberately minimal. **Trimming to 600 buys ~5 sessions — the close-out protocol manufactures bloat faster than any trim removes it**, so the deliverable is a trim PLUS a retention rule that makes it self-maintaining (**AC-4** applied to the handoff: settle once, encode as a variable, stop re-deciding). RULE: **§3 holds N=3 entries; overflow moves VERBATIM to the archive before the new entry is written; a close-out that skips it is INCOMPLETE** (same force as the built-inventory / close-out-ledger / ⚡ ACTIVE STATUS gates). Second clause: **the line-3 header is a POINTER, never a summary** — it was a ~600-word block restating §3 (**STD-011** duplicate) and, being ONE physical line, **cost ~1,400 tokens/session while hiding from the line-count metric being used to measure the problem.** First application same session: **14 entries → 12 archived (SHA-verified byte-identical, `diff` empty) + 2 kept + 1 new = 3**; archive 167 → 179; **nothing summarized, nothing lost.** **⚠️ HONEST MISS: 907 → 736 (−171), still 136 OVER the ~600 budget — N was NOT tuned to hit the number** (N is David's call; a miss is a finding). §3 is now 91 of 736 lines; the residual is structural (§2 infra tables, §6 rules, §9 instructions) = the still-open §4 item *"Lean CLAUDE.md to rules + state + pointers only,"* of which this ticks two sub-items. **The gate stops the growth; it does not close the budget.** Flagged not built: `GEMINI.md` **does not exist** (a header instruction points at a dead file); the ledger/bootstrap/this-index headers carry the same duplicate-header disease at lower cost. PRIOR: **D-49 (proposed) — THE COUNT FILLS THE STUB (ledger #133); a scraped stub is a VARIETY PLACEHOLDER, not a stock line. ONE invariant: any path that mints a size-sibling leaves the family picker-ready BY CONSTRUCTION. Scar: counting a variety made it PERMANENTLY UNSCANNABLE — the promote created a 2nd row beside a size-less scraped parent (114→118) and the re-scan then went UNKNOWN; 103 of 112 scraped rows were landmines. #55 made the branch reachable for the first time. ENFORCES D-34, REFINES D-45, is D-46's #126 rule reaching the second sibling-minting path. Closes tech-debt #57, records #56. BUILDER-COMPLETE, owner-proof owed.** PRIOR: **D-48 (proposed) — A SERVICE PRICE OVERRIDE IS A DISCOUNT (ledger #131); the fix for the QBO `6070` rejection; supersedes D-39's "leakage ≠ discount" service branch.** An owner override wrote `netTotal 1000` against `retailTotal 1575` leaving `discountAmt 0` — **the line contradicted itself**, and QuickBooks was the FIRST check anywhere in the chain that multiplied rate × qty: `6070 "Amount is not equal to UnitPrice * Qty"`. Silent because every TRACE surface rendered a correct-looking total — **no TRACE surface checks that a LINE is internally consistent**; every check looked at aggregates ACROSS lines while the defect lived WITHIN one. **An invariant only verified in a migration is not enforced** (D-43's verify passed only because no override existed yet). FIX: the retail baseline is PRESERVED and the concession rides `discountAmt` → the invariant holds, QBO gets a line that multiplies (225 × 7 = 1575) + the 575 on the SAME negative-Amount mechanism as the tier discount (ONE `discountLine()`, STD-011); the invariant now THROWS at compute time; reason REQUIRED at UI + server + spine (STD-013 via D-40's shape — **no migration, the column existed**); `discountTotal` restricted to GOODS-only + NEW `serviceAdjustmentTotal` (STD-011 — two facts, two representations); an upward override is a negative discount (surcharge), ALLOWED; QBO failures now report honestly (D-9 — the type was `success|pending`, so there was **no failed state to render**). Moves no money. 138/138 tests (RED-first). PRIOR: **D-47 (proposed) — QBO CUSTOMER IDENTITY: the three-way rule (ledger #130); proposes STD-019; closes tech-debt #53.** The email-only matcher bound TRACE "TERRENCE OBRIEN" to QBO 81 "Andrew O'Brien" — **nine real invoices cross-billed over two months**, silent because every TRACE surface showed the correct customer. Root asymmetry: QBO enforces `DisplayName` UNIQUE and does NOT enforce unique email; we matched on the one field it permits to collide. Recon answered the readback question first (**no readback exists** → the matcher is load-bearing → fix, don't delete). Now: query by email AND DisplayName; ONE shared rule — concur→LINK, **email-YES/name-NO→CREATE** (never link), name-YES/email-no→SURFACE (409), neither→CREATE; **stored links verified before billing** (drift→refuse); collision guard; 6240 surfaces; the junk email-as-DisplayName fallback retired. One reasoned divergence surfaced: the #61 `nameTokenSet` splits on apostrophes so person-name matching got its own function reusing the same equality engine (plant key untouched; its apostrophe blind spot flagged as latent). PRIOR: **DROP `orders_status_check` (ledger #129) — NO new decision; implements AC-1 + STD-011 + D-37.** A pre-D-37 payment-lifecycle CHECK (`pending|invoiced|paid|cancelled`) rejected the code's fulfillment vocabulary (`confirmed`/`fulfilled`) → 500 on two of four order-status buttons, undetected because every order was stuck at `pending`; ground truth was David's Vercel function log (the first "unconstrained text" read was WRONG — it trusted the stale live-schema-map because the service key pulled empty). ONE gated migration DROPs the constraint (status is a string VALUE, no CHECK — AC-1; `ORDER_STATUSES` in code is canonical — STD-011; invoiced/paid are QBO's — D-37); ZERO data migration; sibling `orders_transport_method_check` flagged (tech-debt #52). Also corrected ledger #128 to committed+deployed (`aafcfca`) — its display fixes were live all along, the "old block" was a stale cached bundle (STD-017-inside-STD-017). PRIOR: **POST-CHECKOUT DISPLAY PARITY (ledger #128) — NO new decision; implements D-39/D-40/D-43 + STD-017 + STD-012 persistence clause + STD-011.** David's full-surface owner test (CLV-20260715-2764, contractor 10% + tax-exempt) found the money correct on all five surfaces but order-detail + QBO DRIFTED from the D-39/D-40 display structure Review/Confirm render. Six display fixes in one pass, all from the D-43 STORED breakdown + D-40 stored taxStatus (no surface recomputes): order-detail totals brought onto the canonical structure via a NEW shared `OrderTotals` presenter ("Add-ons" aggregate deleted); tax-exemption reason now renders on order-detail + QBO preview via the shared `describeTaxLine` (real QBO push already did); Confirm no longer drops $0 service lines; ONE "$0.00" zero convention; order-detail sub-label shows SIZE not the internal DISC- SKU; addons page notes the tier discount applies at checkout. ONE totals presenter + ONE tax presenter, no per-surface fork (STD-011); historical/null orders net-only (D-9). ZERO migration, ZERO api-fn (12/12); verify exit 0 zero net-new (eslint 248 locked). PRIOR: **D-46 OWNER-PROVE FOLLOW-ON (ledger #127) — NO new decision; implements D-46 + STD-011.** Three defects found live-testing D-46, fixed in one pass: (1) SKU edit didn't persist (edit-mode controlled input made `saveText`'s equality guard short-circuit → typed value lost; fixed by making edit-mode inputs uncontrolled — also fixed size/variant_group/location/notes); (2) "+ Add size" now pre-fills the sibling SKU as parent-SKU + size-suffix via NEW shared `deriveSiblingSku`/`skuSizeSuffix` next to `variantGroupSlug` (STD-011; DISC-1001 + "45 gal" → DISC-1001-45G; parent untouched; blank when parent has none — D-9; uniqueness enforced); (3) engine-level LEFT-PINNED row actions (new `rowActions` prop on shared `<DataSheet>`, inventory + customers). ZERO migration, ZERO api-fn (12/12); verify exit 0 zero net-new (eslint 248 locked). PRIOR: **D-46 (proposed) COMPLETE INVENTORY CRUD — BUILDER-COMPLETE, owner-proof owed.** The manual-entry half of inventory (D-45 was the scan half): ONE `InventoryEditor` (mode create|edit, mirroring the #119 CustomerPartyEditor — flat Add-Item form retired), "+ Add size" from a row (inherits name + variant_group, auto-groups the parent if null → size-picker fires by construction), group-aware rename (renames all size-siblings), reference-aware DELETE (order_items/cultivar_plants ref → SOFT `status='archived'`; unsold → HARD). Closes the entry-completeness gap (Create ×3, Update partial, Delete missing). Coexists with D-45 via the SHARED `variantGroupSlug` (extracted from InventoryCount's local slugify — STD-011). NEW STD-018 proposed (a capability ships its full entry surface). ZERO migration, ZERO new api-fn (12/12); verify exit 0 zero net-new (eslint 249→248). Home: `docs/decisions/2026-07-14-complete-inventory-crud-D46.md`. Ledger row #126. PRIOR: **D-45 (proposed) COUNT-COMMIT PROMOTES size+qty → variant_group-keyed business_inventory (create-or-update) — BUILDER-COMPLETE, owner-proof owed.** The broken capture→persist→read seam: the walk-and-count loop captured size+qty into inventory_counts ONLY, never promoting to business_inventory (the store the order picker + grid READ), and variant_group (the size-picker gate) was READ but never WRITTEN → null → picker inert. FIX (ONE file InventoryCount.tsx): every resolve builds a variety context; the review sheet captures WHICH SIZE (existing-size chips + free-text, QR is per-variety) + qty; commitCount PROMOTES create-or-update by (variety × size), backfilling variant_group so the picker fires; new size = new row (sell_price null = needs-price, D-9). KEY = existing sibling group ?? scanned QR slug ?? slugify(typed name) (populate product-slug convention → scrape/count converge). RESOLVE-BEFORE-CREATE on the typed no-QR path reuses resolveStockLine/#61 token-set (§6 r8). STD-011 (canonical on-hand) · AC-1/AC-3 · §6 r11 (no api-fn, 12/12). Closes the count-size-persist bug (buildspec item 4, never shipped) + the variant_group orphan-read. Gated DATA backfill docs/decisions/2026-07-14-variant-group-backfill.sql. ZERO migration; verify exit 0 zero net-new. Home: `docs/decisions/2026-07-14-count-promote-D45.md`. Ledger row #124. PRIOR: **D-44 (proposed) CUSTOMER DETAIL PAGE (`/customers/:id`) with order history — BUILDER-COMPLETE, owner-proof owed.** The per-customer drill-in David asked for ("where do I view all of john smith's orders?"); no such view existed (`/customers` was roster + edit modal only). New page: header → 4 LIVE-computed stat cards (Orders count incl. cancelled · Lifetime value + Avg EXCLUDING cancelled · Last order) → order-history list (rows link to `/orders/:id`) → stubbed insights strip (D-9, the AI-advisory hook). Reuses the Orders roster query (`orders.customer_id` populated by submit.ts); NO stored lifetime_value. Editing via the EXISTING `CustomerPartyEditor` (STD-011). ZERO migration, ZERO new api-fn (12/12); verify exit 0 zero net-new. Home: `docs/decisions/2026-07-14-customer-detail-page-order-history.md`. Ledger row #122. PRIOR: **D-43 (proposed) ORDER PERSISTS ITS OWN LINE BREAKDOWN (frozen-at-charge show-the-work) — BUILDER-COMPLETE, migration APPLIED +
...
|---|---|---|
| **DECISIONS.md** | The living decision ledger — **D-1…D-33** (product/architecture decisions) + **OP-1…OP-13** (operating principles). The single home for anything not frozen. | [docs/DECISIONS.md](DECISIONS.md) |
...
## 5. OPERATING PRINCIPLES (how we work) — OP-1…OP-13
...
| OP-13 | Retention over trimming — CLAUDE.md §3 holds N=3 entries (overflow moves verbatim to the archive every close); the header is a pointer, never a summary |
