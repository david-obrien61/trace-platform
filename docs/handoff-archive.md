...
<!-- ARCHIVED 2026-07-23 by the OP-13 §3 retention gate (N=3): moved VERBATIM from CLAUDE.md §3 to make room
     for the 2026-07-23 CSV catalog import entry. Not summarized, not condensed. entries-in == entries-out (1 == 1). -->
...
<!-- ARCHIVED 2026-07-22 by the OP-13 §3 retention gate (N=3): moved VERBATIM from CLAUDE.md §3 to make room
     for the 2026-07-22 GATE FLIP close-out entry. Not summarized, not condensed. entries-in == entries-out (1 == 1).
     NOTE: the build this entry describes (D-50 BUILD 2B, ledger #145) was OWNER-PROVEN on 2026-07-22, the day after
     it was written — all 9 cards green. This entry is preserved as written; the proof is recorded in
     docs/CLOSE-OUT-LEDGER.md #145 and docs/owner-tests/inventory-full-surface-test.md. -->
...
<!-- Archived 2026-07-22 by the SELLABLE-PREDICATE close-out (§3 RETENTION N=3, OP-13) — verbatim, not summarized. -->
...
<!-- Archived 2026-07-22 by the 2B DEFECT FIX close-out (§3 RETENTION N=3, OP-13) — verbatim, not summarized. -->
...
<!-- Archived 2026-07-21 by the D-50 BUILD 2B (reconcile screen) close-out (§3 RETENTION N=3, OP-13) — verbatim, not summarized. -->

### 2026-07-20 — THUNDER OWNER-GATED DEV SURFACES + the stamp made ALWAYS-VISIBLE: the typed flags are gone, and **the panels no longer mount before login** — a leak closed STRUCTURALLY, not with a conditional. BUILDER-COMPLETE; owner-prove OWED (5 cards); **tech-debt #60 still NOT closed.** Zero api-fn (12/12), no schema, no migration.

**Type:** App code — **3 NEW** (`packages/shared/src/devtools/devSurfaces.ts` + `index.ts` — THE one gate · `packages/cultivar-os/src/components/VersionStamp.tsx`) + **7 EDIT** (`App.tsx` · `components/layout/AppLayout.tsx` · `DebugPanel.tsx` · `RhythmLogger.tsx` · `shared/src/components/AppHeader.tsx` · `vite.config.ts` · `vite-env.d.ts`). Docs: ledger #142 · `built-inventory.md` (2 NEW entries + 1 rewritten + line 4) · owner-test board (**GATE 0 ① rewritten + 4 NEW cards**) · `tech-debt-log.md` #60 · `DECISIONS-INDEX.md` (drift + line 7) · `TRACE-SESSION-BOOTSTRAP.md` ⚡ · this §3 + line 3. Also commits the **2026-07-20 typed-activation recon** this build implements. **§3 RETENTION: 1 archived verbatim (2026-07-17 OP-14), 1 written — entries-in == entries-out, §3 back to 3.** **NO new decision.** **BENCH: no trigger fires** (no payments/PII-export/webhooks/AI-calls/invoices/compliance-actions — roster matched, reported per §6 r10). **§1.6:** applicable — **1 STORY** (implements the recon + OP-15, no new user story), **4 CRUD-PERMISSIONS** (owner-only, enforced by mount location AND by the toggle, not by hiding a control), **5 UI** (48px targets, visible On/Off state, no dead affordance), **6 AC-1/3/4** (vertical-neutral; identity-scoped; injected prop), **7 FUNCTION THRIFT** (zero api-fn), **8 REUSE** (this build IS §6 r8), **9 TRACE STAYS ON** (`[TRACE:DEVGATE]` new, ON by birth), **11 OWNER-TEST** (5 cards, all `owed`).

**THE LEAK, AND WHY THE FIX IS STRUCTURAL.** Yesterday's recon found `DebugPanel` and `RhythmLogger` mounted in `App.tsx` as siblings of `<AppRouter/>` — **outside every auth gate**. So `?debug=1` worked **pre-login, on `/plant/:tagId`, the page a customer opens by scanning a tag in the lot** — and that panel prints tenant ids and emails by its own footer's admission. The fix was not to add an `isOwner &&`. The panels **moved into `AppLayout`**, which renders only inside `<PrivateRoute>`: **there is now no code path that mounts them for a signed-out visitor.** A conditional can be re-bypassed by the next person who edits it; a mount location cannot be bypassed without deleting the router. The owner-only toggle is then a *second* gate on top. **Neither is load-bearing alone — that is the point.**

**ONE GATE, NOT TWO (§6 r8, the rule this build is an instance of).** `?debug=1` and `?rhythm=1` were the same OPERATION written twice: two `xEnabled()` readers, two sticky localStorage keys, two floating buttons deliberately placed on opposite corners so they wouldn't collide. Now: `packages/shared/src/devtools` — `useDevSurface` / `toggleDevSurface` / `bindDevSurfaceIdentity` / `clearDevSurfaces`. **The typed flags and the raw `traceDebug`/`traceRhythm` keys are DELETED** (grep-confirmed: the only remaining mentions are comments saying *don't bring these back*). State is keyed to **(userEmail + role)** and purged when either changes — the old keys were raw localStorage with **no session coupling**, so they survived logout *and* role change, and a device left in debug stayed in debug for whoever signed in next.

**⚠️ THE JUDGEMENT CALL — AND IT CUTS AGAINST EVERYTHING ELSE IN THIS BUILD.** The version stamp is deliberately **NOT** behind the new owner gate, though every other dev surface moved behind it. #141 had put the SHA in the DebugPanel footer — which was correct *then*, and became **wrong the moment this build owner-gated that panel**: a deploy broken enough to break the menu would have **hidden its own tell**, which is precisely the failure tech-debt #60 exists to catch, reintroduced by the build hardening everything around it. So the stamp moved OUT to an always-visible `built <date> · <sha>` line — every screen, every user, outside every gate, `pointer-events:none` so it can never swallow a tap — and was **removed from the panel** (STD-011: one home; two surfaces printing one fact drift, and a drifted version stamp asserts a build you are not running). **A tell must not depend on the thing it is testing.**

**TWO THINGS THE PROMPT ASSUMED THAT WERE NOT TRUE — checked, not inherited.** (1) It said the build "builds on the SHA/**BUILD** defines already live"; only `__COMMIT_SHA__` existed — **`__BUILD_TIME__` did not, and was added here.** (2) I nearly shipped a real bug: the context documents `role` as *"null for owners"* (BusinessProvider.tsx:137) and my gate treats a null role as signed-out — which would have made the gate **fail for owners, the only users it serves.** Reading further showed line 137 belongs to a *different* (membership) interface; the **context** `role` at line 165 is display-ready `'OWNER' | 'MANAGER' | 'STAFF'`. **The correct action was zero lines of defensive code** — but the answer came from the source, not from the comment that happened to be nearest.

**A CAPABILITY DELIBERATELY NOT LOST.** Owner-gating the *viewer* does not stop the *capture*: `installCapture()` still runs in `main.tsx` **before React**, so the pre-login trail — including the customer QR page — is still **recorded**. David signs in to read it. Worth stating because "we gated debug" reads like "we lost pre-login debugging," and we didn't.

**PROOFS.** `npm run verify` exit 0, **zero net-new** (tsc 5/5 · eslint 248 · knip 10/14/15). `build:cultivar` exit 0 **both branches** — with a SHA the bundle carries the 7-char prefix; without, the literal `"dev"`. Deploy `4eebb08` **confirmed live by fetching the bundle** (`index-DIrngk_f.js`) — and the poll again caught the stale window: two probes served the old bundle before the third flipped. **Structural claim verified, not asserted:** only `AppLayout` imports the panels, and `AppLayout` sits inside `<PrivateRoute>` (router.tsx:109-110).

**FLAGGED FOR DAVID:** **(a)** the 5 owner-test cards — **card ② is the leak card** (signed out on `/plant/SCV-0031`, `?debug=1` and `?rhythm=1` must do nothing). **(b)** tech-debt **#60 closes on your live match**, not before. **(c) SCOPE, flagged not accepted:** the rhythm logger records **your** location and voice notes, so an OWNER gate is **wider than its real audience** — Lauren would see it. The narrower gate needs a platform-dev role that does not exist. **(d)** the recon's other findings are **untouched and still open** — `/discovery/inspect` is still unauthenticated, and its `send` branch still takes an arbitrary recipient and body with no caller gate. **That is a backend fix; this build did not address it.** **(e)** #56/#59/#61/#62/#63 unchanged.


<!-- Archived 2026-07-20 by the D-50 Layer 2A-2 close-out (§3 RETENTION N=3, OP-13) — verbatim, not summarized. -->


<!-- Archived 2026-07-21 by the D-52 build close-out (ledger #144) — §3 RETENTION N=3 (OP-13).
     Moved VERBATIM from CLAUDE.md §3, not summarized. entries-in == entries-out: 1 archived, 1 written. -->

### 2026-07-20 — THUNDER SHA STAMP: the deployed bundle now STATES ITS OWN COMMIT — OP-15's mechanical form, built in 2 lines of app code. BUILDER-COMPLETE; **owner-prove OWED and tech-debt #60 deliberately NOT closed.** Zero api-fn (12/12), zero schema, zero migration.

**Type:** App code — 3 files touched, **2 functional lines**: `packages/cultivar-os/vite.config.ts` (NEW `define` block injecting `__COMMIT_SHA__`) · `packages/cultivar-os/src/components/DebugPanel.tsx` (footer renders `SHA: {__COMMIT_SHA__}` + widget-header OUTPUTS updated per §9 gate 8) · `packages/cultivar-os/src/vite-env.d.ts` (`declare const __COMMIT_SHA__: string`). Docs: `docs/tech-debt-log.md` #60 · `docs/built-inventory.md` (DebugPanel entry + line 4) · `docs/CLOSE-OUT-LEDGER.md` row #141 · `docs/owner-tests/inventory-full-surface-test.md` (**GATE 0 ① rewritten** + NEW card) · `docs/DECISIONS-INDEX.md` (OP-15 row + drift line) · `TRACE-SESSION-BOOTSTRAP.md` ⚡ · this §3 + line 3. **§3 RETENTION: 1 entry archived verbatim (2026-07-17 OP-15), 1 written — entries-in == entries-out, §3 back to 3.** **NO new decision** (this IMPLEMENTS OP-15). **BENCH: no trigger fires** (no payments/PII/webhooks/AI-calls/invoices/compliance-actions — roster matched, reported per §6 r10). **§1.6:** most N/A and stated as such; applicable — **7 FUNCTION THRIFT** (a build-time string substitution mints ZERO api-fn; 12/12 untouched — the ceiling was never approached), **9 TRACE STAYS ON** (nothing commented out; the stamp is itself a permanent deploy-provenance signal), **11 OWNER-TEST** (card shipped WITH the surface, set `owed`).

**THE BUILD, exactly.** `define: { __COMMIT_SHA__: JSON.stringify((process.env.VERCEL_GIT_COMMIT_SHA || 'dev').slice(0,7)) }` → footer `SHA: {__COMMIT_SHA__}` → the global declared for tsc in `vite-env.d.ts`. **Placement (a) — the DebugPanel footer, per the recon's two options and my lean:** it is where David already stands during an owner-prove, and it keeps Lauren's demo surface clean. (b), an always-visible corner stamp, is a one-line move if David rules otherwise. **Name collision checked, not assumed** — `__COMMIT_SHA__` appears nowhere in the repo; `src/vite-env.d.ts` was already inside tsconfig's `include`, so the declaration needed no config change.

**PROVEN AT THE BUNDLE, BOTH BRANCHES — because "the define is right" is a claim until you grep the artifact.** Built with `VERCEL_GIT_COMMIT_SHA=abc1234deadbeef` → the emitted JS contains **`abc1234`** and **`deadbeef` is ABSENT (count 0)** — the `slice(0,7)` truncation is real and the full SHA does not leak into a client bundle. Built with **no** env var → the bundle carries the literal **`"dev"`**. `npm run verify` **exit 0, tsc 5/5 — zero net-new**, which was the one real risk this build carried: an undeclared build-time global is exactly the shape tsc rejects, and the `declare const` held. `build:cultivar` exit 0 both times.

**⚠️ WHY tech-debt #60 IS STILL 🟡 AND I DID NOT CLOSE IT — the point of the whole build, applied to the build itself.** The prompt's close-out said flip #60 to RESOLVED *"only after David's live match"*, and that condition has not happened. **A stamp nobody has read on the real phone is a claim** — and a stamp that is subtly WRONG (stale, wrong var, wrong slice) would not fail loudly; it would **silently un-verify every future GATE 0**, which is precisely the failure class #60 exists to catch. Closing it on a builder's bundle-grep would be the builder grading their own homework (OP-4's two bars) on the very row written because an artifact could not be trusted to say what it was. So: **BUILDER-COMPLETE stated plainly, #60 held OPEN, the owner-test card set `owed` — Thunder never sets `covered`.**

**GATE 0 CHANGED SHAPE, and that is the deliverable — not the two lines.** The board's step ① was *"in the Vercel dashboard, find the deployment for the SHA under test."* It now reads: **open `?debug=1` → 🐞 → the footer's 7-char SHA must MATCH the pushed commit** — with `dev` and *blank* called out as distinct, meaningful failures (a local build reached the deploy · a **pre-stamp bundle**, which is itself proof of old code — *the stamp catches its own absence*). **The card asserts a MATCH against `git log -1`, deliberately NOT a literal SHA frozen in the doc** — a hardcoded value goes stale on the very next push, which is the exact rot OP-15 was written to prevent. **OP-15 is NOT retired by this and I did not narrow it:** the panel proves only what the BROWSER holds, so the hard-refresh step stands and the dashboard remains the fallback when the panel is unreachable. The discipline rule and the mechanism now back each other.

**THE THROUGH-LINE THIS CLOSES (2026-07-17's own finding, one layer at a time).** *The artifact does not carry its own provenance* — `business_inventory` rows could not say who changed them (**closed by D-50's ledger, 2026-07-20**) · a doctrine corpus could not say what was in it (**closed by the capture-index sweep**) · **and a deployed bundle could not say what it was — closed here.** Three layers, one disease, three mechanisms.

**FLAGGED FOR DAVID:** **(a)** the live match — `?debug=1` → 🐞 → footer SHA vs `git log -1 --format=%h`; that one glance closes #60. **(b)** placement (b), the always-visible corner stamp, remains your call — one line. **(c)** the stamp is **cultivar-os only**; `trace-app` and `ignition-os` have their own vite configs and no stamp — deliberate scope, named not swept. **(d)** tech-debt **#56** / **#59** / **#61** / **#62** / **#63** remain open, unchanged.

### 2026-07-20 — THUNDER D-50 LAYER 1 ✅ OWNER-PROVEN (David, live at postgres, V1–V6 all green) — the movement ledger EXISTS, is DB-IMMUTABLE, and `audit_log` RECEIVED ITS FIRST APPLICATION-AUTHORED ROW after 27 days empty. Close-out is DOCS-ONLY: zero app code, zero new schema, zero migration, zero api-fn (12/12).

**Type:** Close-out write-back — 6 EDIT (`docs/built-inventory.md` · `docs/CLOSE-OUT-LEDGER.md` row #140 NEW + row **19B updated** · `TRACE-SESSION-BOOTSTRAP.md` ⚡ ACTIVE STATUS 🔴→🟡 · `docs/owner-tests/inventory-full-surface-test.md` NEW `SURFACE: movement-ledger` (6 cards, V1–V6, `covered` by David's live run) · `docs/DECISIONS-INDEX.md` D-50 row + drift line · this §3 + line 3) + 1 ARCHIVE (§3 overflow). **No `.ts`/`.tsx` opened to edit.** `npm run verify` NOT run — the gate is code-scoped and no code changed. **NO new decision** (D-50 was already ACCEPTED; this proves Layer 1 of it). **BENCH: no trigger fires** (docs only — roster matched, reported per §6 r10). **§1.6:** most N/A and stated as such; applicable — **1 STORY** (D-50's story, `user_stories.md`, cited not re-derived), **11 OWNER-TEST** (this write-back IS the OP-14 gate firing — David's live run flips the cards; Thunder never sets `covered`).

**OWNER-PROVEN — 2026-07-20, David, live at postgres on `bgobkjcopcxusjsetfob`, migration `20260720_inventory_movement_ledger.sql` APPLIED.** All six checks green, none asserted: **V1** schema exact + FKs (`business_id` CASCADE, `inventory_id` **SET NULL** — history outlives the row). **V2** immutability REAL: INSERT ok, **UPDATE and DELETE both rejected** by `reject_inventory_ledger_mutation` — *and the trigger fires even for `postgres`*, which is the whole point (it defends against our own future code, not just against users). **V3(a)** 126 lots / 126 `opening_balance` rows, exactly one each; **V3(b)** `qty == SUM(delta)`, **zero drift** — D-50's disagreement #1 ("replay vs on-hand must be ZERO **by construction**") is now a measured fact, not a hope. **V4** RLS `owner_all` OR `member_all`, FOR ALL (so SELECT is covered — the missing-SELECT trap that bit this platform three times), no `USING(true)`. **V5a** owner adjust 37→35 with a ledger row `delta -2`; **F3** a non-owner **MANAGER** passed 35→34 `delta -1`; **V5b** forgery (all-zero uuid) RAISED; a cross-tenant actor also correctly refused. **V6** soft-delete: the lot **SURVIVES as a tombstone** (status flipped, qty 0 — no `DELETE FROM` anywhere), a `delete_tombstone` ledger row `delta -44`, **AND** an `audit_log` row `inventory.delete` / `success` / actor `95c1b2e9`.

**⚠️ ROW 19B IS PARTIALLY CLOSED — AND THE PART THAT IS STILL OPEN IS THE PART THE RECON RANKED FIRST.** `audit_log` has its first application-authored row. It was created 2026-06-23, owner-proven 2026-06-24, and sat **EMPTY for 27 days** — immutable, correct, catalog-proven, and never written to, because no writer was ever built. That is closed. **But the writer the spine recon named FIRST — `deleteTenantRole` → `role.factory_reset` — is STILL OWED.** This build filled the vault with `inventory.delete`, which was the writer D-50 needed, not the writer 19B specified. Recording the distinction rather than letting "19B: audit_log first writer LANDED" read as if the whole row closed — **the vault being non-empty is not the same fact as the governance writer existing**, and conflating them is exactly how a row rots while looking done.

**⚠️ THE MID-SESSION FINDING THAT WAS *NOT* A BUG — and did not become a migration.** V5a first failed: `assert_movement_actor` refused the uid David passed. The prompt diagnosed it as a membership-only check missing the owner branch and specified a corrective migration. **Reading the code first showed the owner branch was already there** — `is_member_of` is `EXISTS(businesses WHERE owner_id = p_user_id) OR EXISTS(business_members … active)`, already the exact shape prescribed, so the "fix" would have been **logically identical to what was live: a no-op migration**, and F1 would have failed again. Settled by querying live read-only: business `f7ec5d67` "Test Dave's Tree Nest" is owned by **`95c1b2e9`**; the uid under test (`98f4e56b`) owns a **different** business (`ed2e5933` "Test Lawns Tree Farm") and has no member row on `f7ec5d67`; the cited lot `a00a477e…` **exists in no tenant at all**. **The gate was refusing a genuine cross-tenant actor — AC-3 working.** Re-run with the right owner: passed. **The lesson is the one this platform keeps relearning from the other side: a red test is a claim too.** Verify the diagnosis against the code before shipping the fix, or you ship ceremony and stay blocked. Nothing was committed for this; the correct action was zero lines.

**LAYER 2 IS OWED AND NOTHING PRETENDS OTHERWISE.** The DB layer is live and proven; **the app still writes qty the old way.** All ten app-side callers (count SET, desk grid, editor, populate/clear) are UNCHANGED and do NOT yet route through the RPCs — so today a count still overwrites rather than reconciles, and the recon's live warning (`InventoryCount.tsx:612-614` — *"count record SKIPPED … inventory already updated"*) still stands. The ⚡ line is **🟡, not 🟢**, for exactly that reason, and the count-loop UI owner-test cards stay **`owed`** — the SET→reconcile rewrite has not shipped.

**FLAGGED FOR DAVID:** **(a)** the `role.factory_reset` writer (19B's named first writer) remains owed. **(b)** Layer 2 — rewire the ten callers; the RPCs were built backward-compatible so this can be staged. **(c)** the V2 probe row is permanent and intentional — cleaning it up would need exactly the power the table denies. **(d)** tech-debt **#56** / **#59** / **#60**'s SHA-stamp remain open, unchanged.
