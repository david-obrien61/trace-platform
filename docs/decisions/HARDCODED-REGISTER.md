...
## Owning capability: **positions** — the business-context proposal (🟡 1 OPEN · 1 cleared)
...
|---|---|---|---|---|---|---|
| P2 | [`pages/Positions.tsx`](../../packages/cultivar-os/src/pages/Positions.tsx) — the three context `placeholder=` strings | ~~`"grows and sells shade trees on forty acres in Leander"`~~ + two more LAWNS-shaped examples, shipped in `4fce9f9` (#240) | 🔴 **AN OWNER READ IT AS HIS OWN DATA, WHICH IS THE EVIDENCE THAT THIS CLASS IS NOT COSMETIC.** A sentence shaped exactly like LAWNS, sitting in LAWNS's own field, carrying **forty acres** when everything measured this week says **twenty** — a number that was never anybody's data. A placeholder is only visible when the field is EMPTY, so a placeholder that reads like a value tells an owner their blank field is filled: **A9/D-9 inverted** (absent is not empty, running backwards), and it defeats the proposal card directly beneath it | all three read **"For example: …"**; the tenant identity and the unsupported number are gone. The proposal beside them says "forty varieties **of tree**", so two unqualified forties cannot sit in one card | MED | **✅ CLEARED** (#242) |
...
## Owning capability: **container sizes / Settings** — Lauren's price sheet, shown for comparison (🟡 1 OPEN)

Filed by the build that created it, in the same commit (ledger #386) — §1.6 gate 2: a build introduces no new tenant literal without registering it, and *"a flag on a gap board ages; a hardcoded fake ships."*

| id | file:line | what it is | why it is debt | now | sev | status |
|---|---|---|---|---|---|---|
| C1 | [`components/settings/ContainerSizesSettings.tsx`](../../packages/cultivar-os/src/components/settings/ContainerSizesSettings.tsx) → `SHEET_PRICE` | `{ '15 gal': 150, '30 gal': 300, '45 gal': 450, '65 gal': 600, '95/100': 900 }` — Lauren's 2026-09-23 install price sheet, rendered beside the rung's own price with the gap and its direction named | one tenant's price list in code that every tenant's Settings screen runs. It is keyed by RUNG LABEL, so it shows for any tenant whose ladder uses the same labels — which is a coincidence waiting to happen, not a design | **it is a COMPARISON, never a price.** Nothing reads it to charge anybody: no checkout path, no migration, no seed. The seeded prices come from LAWNS's own billed medians and live in `container_ladder.install_price`. It exists only so David's ruling (b) can be carried out — *"with Lauren's sheet shown BESIDE each rung and the gap named, for her to confirm or change"* | LOW (renders text; charges nothing) | 🟡 **OPEN — tech-debt #363. THE EXIT CONDITION IS NAMED AND NEAR:** it comes out the moment Lauren has confirmed or changed each of the five rungs, because the comparison has then done its job and a stale sheet is worse than none. If David wants it to survive that, the durable form is a per-tenant `reference_price` column on the rung, not this map |

⚠️ **THIS CAPS THE CONTAINER-SIZES CAPABILITY AT AMBER** (§6 r12) — which costs nothing today: its owner-test board is already amber with cards owed.
