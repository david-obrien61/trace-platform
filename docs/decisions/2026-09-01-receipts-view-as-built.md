...
**Ledger #252.** Branch `thunder/receipts-view`, off `main` at `84cc695`. BUILDER-COMPLETE;
...
| Derive a document type on the stored rows | [[R-50]]. No column exists to hold one; retro-classifying from the vendor name works on these 17 rows and is not a rule. |
