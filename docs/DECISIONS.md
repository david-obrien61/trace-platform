...
**Date captured:** 2026-07-16 · **Status:** PROPOSED — awaiting David's ratification. Number **CONFIRMED, not guessed**: OP-1…OP-13 exist in BOTH `DECISIONS.md` headings and the `DECISIONS-INDEX` §5 table ⇒ OP-14 is next. First applied the session it was written (ledger #136): the inventory standing test + `owner-tests.html` shipped together, and the first thing the board reported about its own author's work was **19% proven**.
