...
- **Losses / shrinkage** — the `'lost'` `plant_events` type is the live signal. NOTE: a bare `losses` table appears in the legacy live-table list (CLAUDE.md §2) but has **no migration and no code reference** — treat `plant_events['lost']` as the real source, not the orphan `losses` table, until proven otherwise. **✅ PROVEN AND SETTLED 2026-07-28 (ledger #162) — the "until proven otherwise" is discharged.** A named-corpus sweep (`packages/{cultivar-os,shared,ignition-os,trace-app}/src`, `packages/*/api`, `api/`, `supabase/migrations/`, `scripts/`, plus repo-wide) found **zero** reads or writes of `losses` — no page, no route, no permission string, no client call. The table is the pre-`businesses` generation (it keys on `nursery_id`, not `business_id`) and is **DROPPED** by GATED `supabase/migrations/20260727d_drop_losses_and_nurseries.sql`. This line is the reason that drop is cleanup rather than the deletion of a planned feature's shape: **the successor was chosen here first, three weeks before the table was removed.**
